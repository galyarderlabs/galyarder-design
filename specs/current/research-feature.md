...
system prompt. The agent invokes a stable GD command when current external
...
the GD research command, then asks the agent to summarize findings with
...
agent runtime
        |
        | calls "$GD_NODE_BIN" "$GD_BIN" research search ...
        v
...
```bash
"$GD_NODE_BIN" "$GD_BIN" research search --query "<search query>" --max-sources 5
```

```powershell
& $env:GD_NODE_BIN $env:GD_BIN research search --query "<search query>" --max-sources 5
```
...
If the GD command fails because Tavily is not configured or unavailable, the
...
- Manual smoke: start `pnpm tools-dev run web --daemon-port 17456 --web-port
  ...
  agent calls the GD command first, JSON output is valid, a Markdown report is
...
the agent-callable `/search` + `gd research search` path and removed daemon
