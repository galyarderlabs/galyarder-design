...
const namespace = process.env.GD_PACKAGED_E2E_NAMESPACE ?? 'release-beta';
...
const pnpmCommand = process.env.GD_E2E_PNPM_COMMAND ?? 'pnpm';
...
const shouldRunPackagedMacSmoke = process.platform === 'darwin' && process.env.GD_PACKAGED_E2E_MAC === '1';
const macDescribe = shouldRunPackagedMacSmoke ? describe : describe.skip;
const shouldRunDesktopMacSmoke = process.platform === 'darwin' && process.env.GD_DESKTOP_SMOKE === '1';
...
macDescribe('packaged mac runtime smoke', () => {
  ...
  test('installs, starts, inspects, stops, and uninstalls the built mac artifact', async () => {
    ...
    try {
      ...
      expect(inspect.status?.url).toMatch(/^(gd:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);

      const value = assertHealthEvalValue(inspect.eval?.value);
      expect(value.href).toMatch(/^(gd:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);
