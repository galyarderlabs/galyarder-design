...
describe('live artifact tool CLI environment', () => {
  ...
  async function makeArtifactInputFiles() {
    const root = await mkdtemp(path.join(tmpdir(), 'gd-live-artifact-cli-'));
    tempRoots.push(root);
  ...
  it('reads GD_DAEMON_URL and GD_TOOL_TOKEN from the injected environment', async () => {
  ...
  it('fails before making a request when the injected environment is missing', async () => {
    ...
    expect(stderrOutput.join('')).toContain('GD_DAEMON_URL is required');
  });

  it('requires GD_TOOL_TOKEN from the injected environment', async () => {
    process.env.GD_DAEMON_URL = 'http://127.0.0.1:7456';
    delete process.env.GD_TOOL_TOKEN;
    ...
    expect(result.exitCode).toBe(1);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(stderrOutput.join('')).toContain('GD_TOOL_TOKEN is required');
  });
});
