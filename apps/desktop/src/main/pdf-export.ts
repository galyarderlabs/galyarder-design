 ...
 * `gd:print-pdf` IPC handler reaching for `webContents.print()`, which
...
function injectPrintStylesheet(doc: string, css: string): string {
  const tag = `<style data-gd-desktop-pdf>${css}</style>`;
...
export async function waitForPrintReadyHandshake(webContents: Electron.WebContents, nonce: string): Promise<void> {
  // The parent wrapper document caches 'GD_PRINT_READY' in
    ...
    `(function() {
      ...
      return new Promise(function(resolve) {
        window.addEventListener('message', function handler(event) {
          if (event.data && event.data.type === 'GD_PRINT_READY' && event.data.nonce === '${nonce}') {
