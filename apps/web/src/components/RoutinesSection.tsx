...
/** Run status badge variant for DS Badge primitive. */
  ...
  if (runs === null) {
    return (
      <div className="routines-history-empty">
        <Spinner size="sm" aria-label="Loading runs" />
      </div>
    );
  }
  if (runs.length === 0) {
      ...
      {runs.map((r) => {
        ...
        return (
          <li key={r.id} className="routines-history-row">
            <Badge variant={runStatusBadgeVariant(r.status)} size="sm">
              {r.status}
            </Badge>
            ...
            >
              Open project
              <Icon name="ChevronRight" size={16} strokeWidth={1.5} />
            </button>
            {failureReason ? (
...
/**
 * Automations section rendered as a `<section aria-labelledby>` block per
 * Req 21.1. Entries render through Card rows with Badge status and primary
 * Button actions (Req 21.2). Per-section error isolation with retry (Req 21.5).
 * EmptyState when zero entries (Req 21.6). Switch primitive controls the
 * enabled/disabled toggle per automation row.
 */
export function RoutinesSection({ onClose }: RoutinesSectionProps) {
  ...
  const headingId = 'automations-rail-heading';
  ...
  return (
    <section
      className="automations-rail-section"
      aria-labelledby={headingId}
    >
      <div className="automations-rail-section-head">
        <h2 id={headingId} className="automations-rail-section-title">
          Automations
        </h2>
        {!showForm ? (
          <Button
            variant="primary"
            size="sm"
            leadingIcon={<Icon name="Plus" size={16} strokeWidth={1.5} />}
            onClick={() => {
              setForm(emptyForm());
              setShowForm(true);
            }}
      ...
      {/* Per-section error state with retry (Req 21.5) */}
      {error ? (
        <div className="automations-rail-error">
      ...
      {showForm ? (
        <Card elevation="resting" padding="md" className="routines-form-card">
          <form onSubmit={submit} className="routines-form">
            <label className="routines-field">
              <span>Name</span>
              <input
                required
      ...
      {loading ? (
        <div className="automations-rail-loading" aria-live="polite">
          <Spinner size="sm" aria-label="Loading automations" />
        </div>
      ) : routines.length === 0 ? (
        /* Zero entries — EmptyState per Req 21.6 */
        <EmptyState
          icon={<Icon name="Clock" size={24} strokeWidth={1.5} />}
          title="No automations yet"
          description="Schedule an unattended agent run to run on a recurring basis."
          action={
            <Button
              variant="primary"
              size="sm"
        ...
        /* ScrollArea wraps the list so the section scrolls independently (Req 21.1) */
        <ScrollArea className="automations-rail-scroll" ariaLabel="Automations list">
          <ul className="automations-rail-list">
            {routines.map((r) => {
              const targetLabel =
                r.target.mode === 'reuse'
              ...
              const lastRunBadgeVariant = r.lastRun
                ? runStatusBadgeVariant(r.lastRun.status)
                : 'neutral';
              ...
              return (
                <li key={r.id}>
                  <Card
                    elevation="resting"
                    padding="sm"
                    className={`automations-rail-row${r.enabled ? '' : ' is-disabled'}`}
                  >
                    <div className="automations-rail-row-main">
                      <div className="automations-rail-row-info">
                        <span className="automations-rail-row-name">
                          {r.name}
                          {!r.enabled ? (
                            <Badge variant="neutral" size="sm" className="automations-rail-paused-badge">
                              paused
                            </Badge>
                          ) : null}
                        </span>
                        <span className="automations-rail-row-schedule">
                          {describeSchedule(r.schedule, r.nextRunAt)}
                        </span>
                        <span className="automations-rail-row-meta">
                          ...
                          {r.lastRun ? (
                            <>
                              ...
                              <span>
                                last:{' '}
                                <Badge variant={lastRunBadgeVariant} size="sm">
                                  {r.lastRun.status}
                                </Badge>
                        ...
                        {failureReason ? (
                          <Banner variant="danger" className="automations-rail-row-error">
                            {failureReason}
                          </Banner>
                        ) : null}
                      </div>

                      {/* Primary action — Run now (Req 21.2) */}
                      <div className="automations-rail-row-actions">
                        <Button
                          variant="primary"
                          size="sm"
                          loading={isBusy && busyId === r.id}
                          disabled={isBusy}
                          onClick={() => runNow(r.id)}
                        >
                          Run now
                        ...
                        {/* Switch replaces Pause/Resume button — ds/Switch primitive (Req 21.2) */}
                        <Switch
                          checked={r.enabled}
                          disabled={isBusy}
                          aria-label={r.enabled ? 'Pause automation' : 'Resume automation'}
                    ...
                    {isExpanded ? (
                      <div className="automations-rail-row-history">
