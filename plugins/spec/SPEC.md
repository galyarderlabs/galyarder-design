# Galyarder Design Plugin Spec

Language: English

This spec is the compact contract for portable Galyarder Design plugins. The canonical product spec remains `docs/plugins-spec.md`; this document is optimized for contributors and external coding agents.
...
## 2. Enriched Galyarder Design Plugin

Add `galyarder-design.json` when the plugin should appear in Galyarder Design as a marketplace card or starter:
...
`galyarder-design.json` points at the skill and declares the product surface:
...
| --- | --- | --- | --- |
| `import` | Bring external sources into GD | `figma-migration` or `code-migration` | `figma-extract`, `code-import`, `design-extract`, `token-map`, `rewrite-plan` |
...
- `specVersion` is the Galyarder Design plugin spec version that this manifest follows. Use the current spec kit value (`1.0.0`) unless the schema moves.
...
- `od.pipeline.stages[].atoms[]` should use known first-party atoms unless the plugin clearly targets a future GD release.
...
2. Confirm `galyarder-design.json` includes `specVersion` and a bumped plugin `version` when behavior changed.
3. Run `pnpm guard`.
4. Run `pnpm --filter @galyarder-design/plugin-runtime typecheck`.
5. If available, run `gd plugin validate ./path/to/plugin`.
