import { PluginManifestSchema, type PluginManifest } from '@galyarder-design/contracts';
...
//     the plugin actually declared (when gd.connectors are present).
...
export function validateSafe(manifest: PluginManifest): ValidateResult {
  ...
  if (gd) {
    const stages = gd.pipeline?.stages ?? [];
    ...
    for (const ref of gd.connectors?.required ?? []) declaredConnectorIds.add(ref.id);
    for (const ref of gd.connectors?.optional ?? []) declaredConnectorIds.add(ref.id);

    const declaredMcpNames = new Set<string>();
    for (const mcp of gd.context?.mcp ?? []) {
      if (typeof mcp.name === 'string') declaredMcpNames.add(mcp.name);
    }

    for (const surface of gd.genui?.surfaces ?? []) {
      ...
      if (oauth.route === 'connector') {
        if (!oauth.connectorId) {
          ...
          errors.push(`genui.surfaces[${surface.id}]: oauth.connectorId='${oauth.connectorId}' is not in gd.connectors.required/optional`);
        }
        ...
        if (!oauth.mcpServerId) {
          ...
          errors.push(`genui.surfaces[${surface.id}]: oauth.mcpServerId='${oauth.mcpServerId}' is not declared in gd.context.mcp`);
        }
      }
    }
  }
