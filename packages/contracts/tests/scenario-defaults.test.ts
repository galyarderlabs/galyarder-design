...
describe('defaultScenarioPluginIdForKind', () => {
  it('maps every supported ProjectKind to a bundled scenario id', () => {
    const expected: Record<string, string> = {
      ...
      // generic gd-new-generation router. See scenario-defaults.ts.
      prototype: 'example-web-prototype',
      deck:      'example-simple-deck',
      template:  'gd-new-generation',
      image:     'gd-media-generation',
      video:     'gd-media-generation',
      audio:     'gd-media-generation',
      other:     'gd-new-generation',
    };
  ...
  it('exposes the hidden free-form Home fallback plugin separately from kind defaults', () => {
    expect(DEFAULT_UNSELECTED_SCENARIO_PLUGIN_ID).toBe('gd-default');
    expect(DEFAULT_SCENARIO_PLUGIN_BY_KIND.other).toBe('gd-new-generation');
  });
});

describe('defaultScenarioPluginIdForTaskKind', () => {
  it('maps every taskKind to the matching scenario plugin', () => {
    expect(defaultScenarioPluginIdForTaskKind('new-generation')).toBe('gd-new-generation');
    expect(defaultScenarioPluginIdForTaskKind('figma-migration')).toBe('gd-figma-migration');
    expect(defaultScenarioPluginIdForTaskKind('code-migration')).toBe('gd-code-migration');
    expect(defaultScenarioPluginIdForTaskKind('tune-collab')).toBe('gd-tune-collab');
    expect(DEFAULT_SCENARIO_PLUGIN_BY_TASK_KIND['new-generation']).toBe('gd-new-generation');
  });
