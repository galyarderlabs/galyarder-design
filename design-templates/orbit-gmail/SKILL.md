...
od:
  ...
  design_system:
    requires: false
  example_prompt: >-
    Generate today's Open Orbit Gmail briefing. Gmail is my only connected connector — pull yesterday's mail and render it as the opened Orbit Daily Digest email inside Gmail's reading view.
---
...
- Same digest-body sections in the same order: greeting → summary
  strip → requires attention → noteworthy → for information only → digest footer.
...
2. **Email chrome** (`<main class="digest-wrap"> <div class="email-chrome">`)
   ...
   b. **Email subject area** — `<h1 class="email-subject">` with the
      digest subject (e.g. `☀ Your 6 important emails from yesterday — Open Orbit
   ...
   d. **Digest body** (`<div class="digest-body">`):
      - greeting paragraph
      - summary strip — 3 numeric cells (urgent / noteworthy / for information only)
      - section **🔴 Requires Attention** — cards with `action-btn primary`
      - section **🟡 Noteworthy** — cards with `action-btn ghost`
      - section **⚪ For Information Only** — cards
