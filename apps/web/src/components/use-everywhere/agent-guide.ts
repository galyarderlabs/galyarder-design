...
// everything it needs to install Galyarder Design, expose it as MCP, and
...
export interface AgentGuideOptions {
  ...
  daemonUrl?: string;
  /**
   * Optional `gd` binary path / hint. When provided we mention it in the
   * setup checklist so the agent knows whether to run `gd …` directly or
   * spawn the packaged binary.
   */
  cliHint?: string;
  /** Optional Galyarder Design version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Galyarder Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Galyarder Design wired into your workflow. Galyarder Design is a local ' +
      'privileged daemon (`gd`) plus a Skills + Design-Systems registry that exposes ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Galyarder Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Galyarder Design app (packaged).');
  lines.push('');
  lines.push('2. Detect available agent CLIs and confirm `gd` is on PATH:');
  lines.push('');
  lines.push('   ```bash');
  lines.push('   gd doctor');
  lines.push('   gd status --json');
  lines.push('   ```');
  if (options.cliHint) {
    lines.push('');
    lines.push(`   The user reported \`gd\` at: \`${options.cliHint}\``);
  }
  lines.push('');
  ...
  lines.push('   gd skills list --json');
  lines.push('   ```');
  lines.push('');
