...
This deployment ships Galyarder Design as a single Alpine-based runtime image. The
...
```bash
cd deploy
GALYARDER_DESIGN_IMAGE=ghcr.io/galyarderlabs/galyarder-design:latest docker compose pull
GALYARDER_DESIGN_IMAGE=ghcr.io/galyarderlabs/galyarder-design:latest docker compose up -d --no-build
```

Defaults:

- Host port: `127.0.0.1:7456` (`GALYARDER_DESIGN_PORT=8080` to publish on `127.0.0.1:8080`)
- Runtime data volume: `galyarder_design_data` mounted at `/app/.od`
- Node heap cap: `--max-old-space-size=192`
- Compose memory cap: `384m` (`GALYARDER_DESIGN_MEM_LIMIT=256m` to override)
...
proxy, set `GALYARDER_DESIGN_ALLOWED_ORIGINS` to the browser origins that should be
allowed to call `/api`:

```bash
GALYARDER_DESIGN_ALLOWED_ORIGINS=https://od.example.com,http://203.0.113.10:7456 docker compose up -d --no-build
```

Pin a specific published image with a digest instead of the mutable `latest` tag:

```bash
GALYARDER_DESIGN_IMAGE=ghcr.io/galyarderlabs/galyarder-design@sha256:<digest> docker compose up -d --no-build
```
...
deploy/scripts/publish-images.sh --image docker.io/your-user/galyarder-design:0.1.0
```

The script defaults to:

- `ghcr.io/galyarderlabs/galyarder-design:<tag>`
