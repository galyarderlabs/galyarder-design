...
import type { ContextItem, InstalledPluginRecord } from '@galyarder-design/contracts';
...
describe('applyPlugin', () => {
  ...
  it('resolves localized use-case queries at apply time', () => {
    ...
    // i18n locale fixture; the CJK string is the zh-CN translation that the
    // resolver must return when locale='zh-CN'. KEPT verbatim because the
    // assertion below pins the exact resolved value.
    ...
    // i18n locale fixture; CJK is the zh-CN translation that must round-trip through the resolver.
  ...
  it('does not require a registry roots argument (no FS access at apply time)', () => {
    ...
    const expectedDataDir = path.resolve(process.env.GD_DATA_DIR ?? path.join(process.cwd(), '.gd'));
