...
This is the part of the #DeFigma argument that needs care. The canvas may be the wrong unit of work for an agent-native system, but the clarity people got from the canvas is still valuable. Galyarder Design has to move that clarity into the artifact model, not throw it away.

## Galyarder Design already has the right primitives

The reason this proposal fits Galyarder Design is that the project is already built around explicit contracts. A skill is a `SKILL.md` file. A design system is a `DESIGN.md` file. A plugin adds an `galyarder-design.json` sidecar. The mechanics are covered in [31 skills, 72 systems: how the Galyarder Design library works](/blog/31-skills-72-systems-how-the-library-works/), and the product argument is in [Why we built Galyarder Design as a skill layer, not a product](/blog/why-we-built-galyarder-design-as-a-skill-layer/).
...
[Contribute a skill](https://github.com/galyarderlabs/galyarder-design/tree/main/skills).

## Related reading

- [Why we built Galyarder Design as a skill layer, not a product](/blog/why-we-built-galyarder-design-as-a-skill-layer/) — the product shape that makes public artifact contracts possible
- [31 skills, 72 systems: how the Galyarder Design library works](/blog/31-skills-72-systems-how-the-library-works/) — the current file-driven primitives underneath the proposal
