...
import { requestJsonIpc } from "@galyarder-design/sidecar";
...
/**
 * Resolve the daemon HTTP base URL for `gd` client commands.
 *
 * Spawn order: explicit `--daemon-url` flag, `GD_DAEMON_URL` env, then
 * a STATUS roundtrip to the concrete sidecar IPC endpoint supplied by
 * the lifecycle owner in `GD_SIDECAR_IPC_PATH`. Falls back to the
 * legacy default for direct `gd` launches that do not run as a sidecar.
 */
export async function resolveDaemonUrl(
  ...
  const envUrl = env.GD_DAEMON_URL;
