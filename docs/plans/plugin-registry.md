# Galyarder Design Plugin Registry — Plan (living)

> **One sentence:** Turn the existing `galyarder-design-marketplace.json` federation
> into a real npm-/clawhub-/skills.sh-style **registry**: GitHub repo as the v1
> storage backend, `gd` CLI as the canonical client, official site as one
> rendered consumer, and the whole thing pluggable so a third party can stand
> up their own Galyarder Design plugin source with one config line.
...
- [ ] **R1. CLI is the canonical client.** Every UI action and every website
  feature must be expressible as one `gd` subcommand. UI / site are renderers.
...
- [ ] **R3. `SKILL.md` floor stays portable.** A plugin published to OD's
  ...
  Code / Cursor / Codex / Gemini CLI / OpenClaw / Hermes. `galyarder-design.json`
...
```text
Plugin source repo
  galyarder-design.json includes plugin.repo
        |
...
Plugin artifact
  GitHub ref, GitHub Release .tgz, HTTPS archive, or local folder
        ...
        v
Registry index
  v1: galyarder-design/plugin-registry or this repo
      community/**/galyarder-design.json
      generated galyarder-design-marketplace.json
  future: DatabaseRegistryBackend
        |
        | od marketplace search / gd plugin install
        v
Installed plugin
...
- **galyarder-design.dev/plugins** is the public renderer of the official and
...
Default community registry
  ...
  -> ~/.galyarder-design/plugins/<plugin-id> -> Installed -> agent
```
...
root (`~/.galyarder-design/plugins/<plugin-id>` by default). Daemon startup reloads
...
the user describe the plugin, writes `SKILL.md` and `galyarder-design.json`, adds
...
entries plus a generated `galyarder-design-marketplace.json`. The JSON is what
daemon/CLI/UI fetch; the source entries are what humans review in PRs. This can
start in the main Galyarder Design repo, but the code path must still be expressed as
`RegistryBackend` so moving to `galyarder-design/plugin-registry` or a database later
...
A dedicated public repo — proposed **`galyarder-design/plugin-registry`** — owns the
...
├── plugins/
│   └── <vendor>/<plugin-name>/
│       ├── manifest.json              ← latest copy of galyarder-design.json
│       ├── versions/
...
├── schema/
│   └── galyarder-design.marketplace.v1.json
├── .github/workflows/
...
**Tarball storage:** plugins ship as `.tgz` (per existing `gd plugin pack`).
...
need a special layout if `gd plugin validate` and `gd plugin pack` pass. The
manifest must include `plugin.repo` in `galyarder-design.json`, pointing to the
...
`gd plugin publish` end-to-end:

1. Run `gd plugin pack` → produces `<vendor>-<name>-<version>.tgz` + manifest digest.
2. Read or create a GitHub release on the plugin's **own** source repo,
   upload the tarball, capture release asset URL + SHA-256.
3. `gh auth status` to confirm login (no token persisted by `gd`).
4. Fork (or reuse fork of) `galyarder-design/plugin-registry` via `gh repo fork`.
...
9. On merge, `publish-index.yml` regenerates `marketplace.json` and pushes
   ...
   JSON, while `https://galyarder-design.dev/plugins/` renders the public browser
...
preserve them for `gd plugin info <name>@<v>`.
...
`gd plugin upgrade [--policy latest|pinned]` re-resolves against lockfile and
applies new versions atomically (no in-place mutation; new version directory,
swap symlink, rollback on failure).

### 1.5 Login model

- `gh` is a first-class dependency of `gd` registry workflows. Installing
  `gd` should ensure `gh` is present when the platform channel can bootstrap
  it; otherwise the installer fails with exact remediation.
- `gd plugin login` wraps `gh auth login` with Galyarder Design copy, scopes, and
  host guidance. `gd plugin whoami` wraps `gh auth status` plus `gh api user`.
- `gd plugin logout` may wrap `gh auth logout`, but only after explicit
...
- **Official site (galyarder-design.dev/plugins)** — static, SSG against
  repo-owned `plugins/registry/*/galyarder-design-marketplace.json` sources. Browse,
  ...
  canonical SEO pages. `galyarder-design.dev/marketplace` can be kept as an alias
...
- The packaged daemon seeds built-in `official` and `community` registry
  sources from `plugins/registry/*/galyarder-design-marketplace.json`. `official` is
...
- Bundled official plugins now carry `sourceMarketplaceId=official` and
  `sourceMarketplaceEntryName=galyarder-design/<plugin-id>`, so they are modeled as
  preinstalled official registry entries while keeping offline first-run bytes
  in the runtime image.
- `gd plugin login` and `gd plugin whoami` now delegate to `gh`, and
  registry publishing now has three paths: `--to galyarder-design` produces the
...
- `od marketplace plugins`, `od marketplace doctor`, `od marketplace login`,
  versioned `gd plugin install`, policy-aware `gd plugin upgrade`, marketplace
  `gd plugin info`, and `gd plugin yank` are implemented as headless surfaces
...
- `apps/landing-page` now exposes the public SEO renderer at `/plugins/` plus
  ...
  so galyarder-design.dev can show the ecosystem without calling daemon APIs.
...
- Registry evaluation cases now live in
  ...
  The first covered set locks raw `galyarder-design-marketplace.json` source input,
...
- [x] **P1.1 New CLI subcommands.**
  ...
  - `gd plugin install <name>@<version|range>`
  - `gd plugin upgrade [--policy latest|pinned]`
  - `gd plugin yank <name>@<version> --reason "..."`
  - `gd plugin info <name> [--version <v>] [--json]`
  - `gd plugin login` -> wraps `gh auth login` with OD-specific host/scope guidance.
  - `gd plugin whoami` -> wraps `gh auth status` plus `gh api user`.
...
- [x] **P1.2 GitHub backend module.** `apps/daemon/src/registry/github-backend.ts`
  implements `RegistryBackend` against `galyarder-design/plugin-registry`. Uses
...
- [x] **P1.3 Publish orchestrator, first mutation-capable slice.**
  ...
  PR payloads; `gd plugin publish --to marketplace-json --catalog <path>`
...
- [x] **P1.4 Lockfile.** `.od/od-plugin-lock.json` records resolved
  `name@version` + integrity + `sourceMarketplaceId`. `gd plugin install`
  honors lock on second run; `gd plugin upgrade` rewrites it.
...
- [x] **P2.4 Agent-assisted Create plugin flow.** The `Create plugin` action
  ...
  writes `SKILL.md`/`galyarder-design.json`, validates, installs a local test copy,
  packs, checks `gh` login/whoami, and publishes by opening a GitHub registry
  PR through `gd plugin publish`. Current slice upgrades the product prompt,
...
- [ ] **P2.5 Plugin detail drawer.** Provenance line, permissions, capability
  ...
  `gd plugin install <name>@<version>`).
...
- [x] **P3.1 Stand up `galyarder-design/plugin-registry` repo shape.** Schema, validation
...
- [x] **P3.2 Static site renderer.** `apps/landing-page` now
  statically generates `galyarder-design.dev/plugins` and per-plugin detail routes
  from `plugins/registry/*/galyarder-design-marketplace.json` plus bundled official
  manifests, with SEO metadata, search JSON, and `od://` detail links.
- [x] **P3.3 Submission guide.** `docs/publishing-a-plugin.md` + zh-CN. The
  guide must be runnable end-to-end with `gd plugin init` →
  `gd plugin pack` → `gd plugin publish`, no manual JSON editing.
- [x] **P3.4 Self-host kit.** `docs/self-hosting-a-registry.md` — copy the
  ...
  exit option exists.
- [x] **P3.5 `gd plugin publish --to marketplace-json`.** Lets third-party
  catalog owners accept submissions from their own users using the same CLI by
  writing/upserting their own static `galyarder-design-marketplace.json`.
...
- [x] **P4.1 DB-backed RegistryBackend.** Same interface, SQLite or Postgres.
  Validates R2.
- [x] **P4.2 Search index.** Static `galyarder-design.dev/plugins/search.json`
...
2. **Plugin source-of-truth repo.** The source repo can be any shape that
   survives `gd plugin validate` and `gd plugin pack`. Registry publish
   requires a `plugin.repo` field in `galyarder-design.json` pointing to source.
...
- [ ] `R1`–`R6` invariants each have a regression test that fails when
  violated.
- [ ] A third party can fork `galyarder-design/plugin-registry`, change two
  config values, run one workflow, and have a working GD plugin source at
  their own URL — verified with an e2e fixture catalog.
- [ ] Every UI action in `PluginsView.tsx` Sources/Available tabs is
  expressible as one `gd` CLI invocation, verified by a script that drives
  both surfaces against the same fixture.
- [ ] One real third-party publisher (target: a Claude-skills-community
  contributor) has published a plugin via `gd plugin publish` end-to-end
