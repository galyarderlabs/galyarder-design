...
// pure renderer in @galyarder-design/contracts then assembles the stage
...
import { renderActiveStageBlock } from '@galyarder-design/contracts';
...
beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-atom-bodies-'));
  ...
  await writeFile(path.join(atomA, 'galyarder-design.json'), SAMPLE_MANIFEST('discovery-question-form'));
  await writeFile(path.join(atomA, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form', 'Ask the user about audience.'));
  await writeFile(path.join(atomB, 'galyarder-design.json'), SAMPLE_MANIFEST('todo-write'));
