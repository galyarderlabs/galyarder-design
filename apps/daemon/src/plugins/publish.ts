// Phase 4 / spec §14.1 — `gd plugin publish --to <catalog>` PR-template launcher.
//
...
//   - galyarder-design            → galyarderlabs/galyarder-design (plugins/community/<plugin-name>/).
...
export function buildPublishLink(args: {
  ...
  switch (args.catalog) {
    ...
    case 'galyarder-design': {
      const bodyWithRegistry = [
        body,
        '',
        '## Galyarder Design registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/galyarder-design.json`',
        '- Generated index: `plugins/registry/community/galyarder-design-marketplace.json`',
        '- Required checks: `gd plugin validate`, `gd plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
      const url = newIssueUrl('galyarderlabs/galyarder-design', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'galyarderlabs/galyarder-design',
        url,
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'galyarder-design-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `galyarder-design.json` sidecar (additive Galyarder Design metadata).');
  lines.push('');
  lines.push('Generated by `gd plugin publish` — see https://galyarder-design.ai/docs/plugins-spec.md.');
  return lines.join('\n');
}
