...
// ("remember:" in English; "记住" / "我喜欢" — Chinese for "remember" /
// "I like" — in CJK input). For everything else — implicit
...
function envKeyFor(provider) {
  ...
  if (provider === 'senseaudio') {
    return (
      process.env.GD_SENSEAUDIO_API_KEY?.trim()
...
//
// The `GD_MEMORY_MODEL` env continues to override the model name across
...
async function pickProvider(projectRoot, dataDir, chatAgentId, chatProvider, chatModel) {
  ...
  const envOverrideModel = (process.env.GD_MEMORY_MODEL || '').trim();
  ...
  if (chatProtocol) {
    const localCliProvider = localCliProviderFor(
      normalizedChatAgentId,
      chatProtocol,
      process.env.GD_MEMORY_MODEL || chatModel,
    );
    if (localCliProvider) return localCliProvider;
...
async function writeLocalCliPromptAttachment(agentId, prompt) {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), `gd-memory-${agentId}-`));
