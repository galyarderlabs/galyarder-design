...
export function PluginCard({
  ...
  const hasQuery = Boolean(record.manifest?.gd?.useCase?.query);
          ...
          {onShareAction ? (
            ...
            >
              ...
              <button
                type="button"
                className="plugins-home__action plugins-home__action--secondary plugins-home__action--compact"
                onClick={() => onShareAction(record, 'contribute-galyarder-design')}
                disabled={pendingAny || shareBusy}
                aria-busy={sharePendingAction === 'contribute-galyarder-design' ? 'true' : undefined}
                aria-label={`Contribute ${title} to Galyarder Design`}
                title="Contribute plugin to Galyarder Design with a pull request"
                data-testid={`plugins-home-contribute-galyarder-design-${record.id}`}
              >
                <Icon
                  name={sharePendingAction === 'contribute-galyarder-design' ? 'spinner' : 'share'}
                  size={12}
                />
                <span>{sharePendingAction === 'contribute-galyarder-design' ? 'Starting…' : 'Contribute'}</span>
              </button>
            </div>
          ) : null}
        </div>
