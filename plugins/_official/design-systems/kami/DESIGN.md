...
kami compresses into one sentence: **warm parchment canvas, ink-blue accent, serif carries hierarchy, no cool grays, no hard shadows.** It is not a UI framework — it is a constraint system for the page, designed to keep deliverables stable, clear, and unmistakably *printed*. The name "kami" is the Japanese word for paper (the same character is shared with Chinese), and the system is co-designed across English, Simplified Chinese, and Japanese typesetting from the ground up, not retrofitted.
