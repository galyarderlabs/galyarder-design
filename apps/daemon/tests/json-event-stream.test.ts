...
test('cursor stream emits partial text once and usage events', () => {
  ...
  handler.feed(
    ...
    JSON.stringify({
      type: 'assistant',
      timestamp_ms: 1,
      message: { role: 'assistant', content: [{ type: 'text', text: 'GD' }] },
    }) +
    '\n' +
    JSON.stringify({
      type: 'assistant',
    ...
    JSON.stringify({
      type: 'assistant',
      message: { role: 'assistant', content: [{ type: 'text', text: 'GD_OK' }] },
    }) +
    '\n' +
    JSON.stringify({
      type: 'result',
...
test('codex json stream preserves line boundaries between assistant message items', () => {
  ...
  // UTF-8 / CJK fixture; the '一' char (3 UTF-8 bytes) verifies non-ASCII bytes survive line concatenation.
  handler.feed(
    JSON.stringify({ type: 'turn.started' }) + '\n' +
    JSON.stringify({
      type: 'item.completed',
    ...
    JSON.stringify({
      type: 'item.completed',
      // UTF-8 / CJK fixture: '一' must round-trip across line boundaries.
  ...
  // UTF-8 / CJK assertion; verifies the CJK char survives the codex stream concat.
...
test('codex json stream does not duplicate existing assistant message newlines', () => {
  ...
  // UTF-8 / CJK fixture; the '一' char verifies non-ASCII bytes are preserved when collapsing newlines.
  handler.feed(
    JSON.stringify({
      type: 'item.completed',
      item: { id: 'item-1', type: 'agent_message', text: 'English: one\n' },
    ...
    JSON.stringify({
      type: 'item.completed',
      // UTF-8 / CJK fixture: '一' must round-trip without duplicating newlines.
      item: { id: 'item-2', type: 'agent_message', text: 'Chinese: 一' },
    }) +
    '\n' +
    JSON.stringify({
  ...
  // UTF-8 / CJK assertion; verifies the CJK char survives the newline-dedupe.
  assert.equal(text, 'English: one\nChinese: 一\nEnglish: two');
});
