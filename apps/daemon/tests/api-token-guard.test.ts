...
//
// Two halves:
//   1. The daemon refuses to start with GD_BIND_HOST=0.0.0.0 when no
//      GD_API_TOKEN is set.
//   2. When GD_API_TOKEN is set, every /api/* request from a non-loopback
//      peer must carry `Authorization: Bearer <GD_API_TOKEN>`. The
...
const PREVIOUS_TOKEN = process.env.GD_API_TOKEN;
const PREVIOUS_HOST  = process.env.GD_BIND_HOST;
...
afterEach(async () => {
  ...
  server = undefined;
  shutdown = undefined;
  if (PREVIOUS_TOKEN === undefined) delete process.env.GD_API_TOKEN;
  else process.env.GD_API_TOKEN = PREVIOUS_TOKEN;
  if (PREVIOUS_HOST === undefined) delete process.env.GD_BIND_HOST;
  else process.env.GD_BIND_HOST = PREVIOUS_HOST;
});

describe('bound-API-token guard', () => {
  it('refuses to start with GD_BIND_HOST=0.0.0.0 when GD_API_TOKEN is unset', async () => {
    delete process.env.GD_API_TOKEN;
    await expect(startServer({ port: 0, host: '0.0.0.0', returnServer: true }))
      .rejects.toThrow(/GD_API_TOKEN/);
  });

  it('starts on a public host when GD_API_TOKEN is set', async () => {
    process.env.GD_API_TOKEN = 'test-token-abc';
...
describe('bearer middleware', () => {
  beforeEach(async () => {
    process.env.GD_API_TOKEN = 'secret-test-token';
    const started = (await startServer({ port: 0, host: '127.0.0.1', returnServer: true })) as {
      url: string;
      server: http.Server;
      shutdown?: () => Promise<void> | void;
