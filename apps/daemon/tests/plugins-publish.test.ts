// Phase 4 / spec §14.1 — `gd plugin publish` URL builder unit test.
//
...
const META = {
  pluginId:          'galyarder-design/sample-plugin',
  pluginVersion:     '1.0.0',
  pluginTitle:       'Sample Plugin',
  pluginDescription: 'A fixture for the publish flow.',
  repoUrl:           'https://github.com/galyarder-design/sample-plugin',
};
...
describe('buildPublishLink', () => {
  ...
  it('builds a github-issue URL for anthropics/skills with title + body', () => {
    ...
    expect(link.prBody).toContain('https://github.com/galyarder-design/sample-plugin');
  });
  ...
  it('points at skills.sh + the npx skills add command (no PR form there)', () => {
    ...
    expect(link.prBody).toContain('npx skills add galyarder-design/sample-plugin');
  });

  it('builds an Galyarder Design registry submission URL', () => {
    // The dedicated `galyarder-design/plugin-registry` repo per
    // docs/plans/plugin-registry.md §1.2 is the long-term target; until that
    // operational launch step happens, submissions land in `galyarderlabs/galyarder-design`
    // (plugins/community/<plugin-name>/), keeping contribution where stars and
    // PR traffic already are.
    const link = buildPublishLink({ catalog: 'galyarder-design', meta: META });
    expect(link.catalogLabel).toBe('galyarderlabs/galyarder-design');
    expect(link.url).toMatch(/^https:\/\/github\.com\/galyarderlabs\/galyarder-design\/issues\/new\?/);
    expect(link.prBody).toContain('plugins/community/<plugin-name>/galyarder-design.json');
    expect(link.prBody).toContain('plugins/registry/community/galyarder-design-marketplace.json');
  });
...
describe('upsertMarketplaceJsonEntry', () => {
  it('adds a namespaced plugin entry with a reproducible github source', () => {
    ...
    expect(outcome.entry).toMatchObject({
      name: 'galyarder-design/sample-plugin',
      source: 'github:galyarder-design/sample-plugin',
      version: '1.0.0',
      title: 'Sample Plugin',
      publisher: {
  ...
  it('updates existing entries and preserves unrelated catalog metadata', () => {
    const outcome = upsertMarketplaceJsonEntry({
      ...
      manifest: {
        ...
        plugins: [
          {
            name: 'galyarder-design/sample-plugin',
            source: 'github:galyarder-design/sample-plugin@old',
            version: '0.9.0',
            tags: ['kept'],
          },
        ],
      ...
      meta: {
        ...META,
        pluginVersion: '1.1.0',
        repoUrl: 'https://github.com/galyarder-design/sample-plugin/tree/main/plugins/sample',
      },
    });
    ...
    expect(outcome.manifest.plugins[0]).toMatchObject({
      name: 'galyarder-design/sample-plugin',
      source: 'github:galyarder-design/sample-plugin@main/plugins/sample',
      version: '1.1.0',
      tags: ['kept'],
    });
  });
