...
import { SIDECAR_ENV } from '@galyarder-design/sidecar-proto';
...
describe('GET /api/mcp/install-info', () => {
  ...
  beforeAll(
    () =>
      new Promise<void>((resolveBoot) => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-mcp-info-'));
  ...
  afterEach(() => {
    ...
    delete process.env.GD_BIND_HOST;
  });
  ...
  it('non-sidecar launch bakes --daemon-url so custom ports keep working', async () => {
    ...
    // Direct `gd` launches have no IPC socket; the snippet bakes the
    // URL so the spawned `gd mcp` reaches the right port without any
    // discovery.
    expect(body.args).toEqual([cliPath, 'mcp', '--daemon-url', `http://127.0.0.1:${port}`]);
    // env always carries GD_DATA_DIR (issue #848); no sidecar keys in
    // a non-sidecar launch.
    expect(body.env).toEqual({ GD_DATA_DIR: dataDir });
  ...
  it('pins GD_DATA_DIR in the env so IDE-spawned MCP processes write to the daemon data dir (issue #848)', async () => {
    const { port } = nonSidecar;
    const res = await fetch(`http://127.0.0.1:${port}/api/mcp/install-info`);
    const body = await readInstallInfo(res);
    expect(body.env).toBeDefined();
    expect(body.env.GD_DATA_DIR).toBe(dataDir);
  });
  ...
  it('sidecar launch omits --daemon-url and emits the concrete IPC path with GD_DATA_DIR', async () => {
    const { port, server } = await startHarness(
      cliPath,
      {
        [SIDECAR_ENV.IPC_PATH]: '/tmp/galyarder-design/ipc/default/daemon.sock',
      },
      dataDir,
    );
    try {
  ...
  it('sidecar non-default endpoint still propagates only the concrete IPC path', async () => {
    const { port, server } = await startHarness(
      cliPath,
      {
        [SIDECAR_ENV.IPC_PATH]: '/tmp/galyarder-design/ipc/foo/daemon.sock',
      },
      dataDir,
    );
    try {
  ...
  it('sidecar with custom IPC base does not propagate namespace or base hints', async () => {
    const { port, server } = await startHarness(
      cliPath,
      {
        [SIDECAR_ENV.IPC_PATH]: '/var/run/galyarder-design/foo/daemon.sock',
        [SIDECAR_ENV.NAMESPACE]: 'foo',
        [SIDECAR_ENV.IPC_BASE]: '/var/run/galyarder-design',
      },
      dataDir,
    );
    try {
