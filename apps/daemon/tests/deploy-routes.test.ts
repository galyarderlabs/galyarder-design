...
describe('deploy provider routes', () => {
  ...
  it('dispatches deploy config reads and writes by providerId', async () => {
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-config-'));
  ...
  it('lists Cloudflare Pages zones for saved account credentials', async () => {
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-zones-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
    try {
      const saveResp = await fetch(`${baseUrl}/api/deploy/config`, {
  ...
  it('dispatches deploy preflight by providerId', async () => {
    const dataDir = process.env.GD_DATA_DIR;
    if (!dataDir) throw new Error('GD_DATA_DIR is required for daemon route tests');
  ...
  it('derives Cloudflare Pages project names from the Galyarder Design project', async () => {
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-auto-project-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
    const projectId = 'cf-route-123456';
    const expectedPagesProject = 'gd-ai-cf-route-123';
    try {
      const createProjectResp = await fetch(`${baseUrl}/api/projects`, {
        ...
        body: JSON.stringify({
          id: projectId,
          // UTF-8 / CJK fixture; verifies safeDnsLabel strips non-ASCII so the slug becomes just 'ai'.
          name: 'AI 生图网站',
          skillId: null,
          designSystemId: null,
        }),
  ...
  it('rejects invalid Cloudflare custom-domain selection before Pages deploy', async () => {
    const dataDir = process.env.GD_DATA_DIR;
    if (!dataDir) throw new Error('GD_DATA_DIR is required for daemon route tests');
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-invalid-domain-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
  ...
  it('refreshes Cloudflare Pages custom-domain API status during check-link', async () => {
    const dataDir = process.env.GD_DATA_DIR;
    if (!dataDir) throw new Error('GD_DATA_DIR is required for daemon route tests');
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-domain-check-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
  ...
  it('keeps Vercel deploy payload free of Cloudflare custom-domain fields', async () => {
    const dataDir = process.env.GD_DATA_DIR;
    if (!dataDir) throw new Error('GD_DATA_DIR is required for daemon route tests');
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-route-vercel-payload-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
