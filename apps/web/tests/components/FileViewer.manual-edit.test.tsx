...
describe('FileViewer manual edit regressions', () => {
    ...
    try {
      ...
      const { rerender } = render(
        <FileViewer projectId="project-1" projectKind="prototype" file={first}
          liveHtml='<!doctype html><html><body><main data-gd-id="hero">Hero</main></body></html>'
        />,
      );
      ...
      rerender(
        <FileViewer projectId="project-1" projectKind="prototype" file={second}
          liveHtml='<!doctype html><html><body><main data-gd-id="second">Second</main></body></html>'
        />,
      );
  ...
  it('clears loaded source immediately on file switch without liveHtml before manual edit can save', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response('<!doctype html><html><body><main data-gd-id="hero">First</main></body></html>', { status: 200 });
    });
    vi.stubGlobal('fetch', fetchMock);
    try {
      const first = htmlPreviewFile();
      ...
      secondResolve(new Response('<!doctype html><html><body><main data-gd-id="second">Second</main></body></html>', { status: 200 }));
    } finally {
      vi.useRealTimers();
    }
  });
