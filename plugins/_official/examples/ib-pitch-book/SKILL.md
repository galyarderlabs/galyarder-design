...
repackaged as an Galyarder Design `deck` skill.
...
Follow Galyarder Design's deck export path for the active session (HTML / PDF /
PPTX per daemon capabilities).

## Relationship to Galyarder Design financial skills
...
original adaptation for Galyarder Design.
