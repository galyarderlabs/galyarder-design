...
function deriveValidation(paths: string[]): ValidationCommand[] {
  ...
  if (touched("apps/web/")) {
    add("pnpm --filter @galyarder-design/web typecheck", "apps/web changed");
    add("pnpm --filter @galyarder-design/web test", "apps/web changed");
    add("pnpm --filter @galyarder-design/web build", "apps/web changed");
  }
  if (touched("apps/daemon/")) {
    add("pnpm --filter @galyarder-design/daemon typecheck", "apps/daemon changed");
    add("pnpm --filter @galyarder-design/daemon test", "apps/daemon changed");
    add("pnpm --filter @galyarder-design/daemon build", "apps/daemon changed");
  }
  if (touched("apps/desktop/")) {
    add("pnpm --filter @galyarder-design/desktop typecheck", "apps/desktop changed");
    add("pnpm --filter @galyarder-design/desktop build", "apps/desktop changed");
  }
  if (touched("apps/packaged/")) {
    add("pnpm --filter @galyarder-design/packaged typecheck", "apps/packaged changed");
    add("pnpm --filter @galyarder-design/packaged build", "apps/packaged changed");
  }
  if (touched("packages/contracts/")) {
    add("pnpm --filter @galyarder-design/contracts typecheck", "packages/contracts changed");
  }
  if (touched("packages/sidecar-proto/")) {
    add("pnpm --filter @galyarder-design/sidecar-proto typecheck", "sidecar-proto changed");
    add("pnpm --filter @galyarder-design/sidecar-proto test", "sidecar-proto changed");
  }
  if (touched("packages/sidecar/")) {
    add("pnpm --filter @galyarder-design/sidecar typecheck", "packages/sidecar changed");
    add("pnpm --filter @galyarder-design/sidecar test", "packages/sidecar changed");
  }
  if (touched("packages/platform/")) {
    add("pnpm --filter @galyarder-design/platform typecheck", "packages/platform changed");
    add("pnpm --filter @galyarder-design/platform test", "packages/platform changed");
  }
  if (touched("tools/dev/")) {
    add("pnpm --filter @galyarder-design/tools-dev typecheck", "tools/dev changed");
    add("pnpm --filter @galyarder-design/tools-dev build", "tools/dev changed");
  }
  if (touched("tools/pack/")) {
    add("pnpm --filter @galyarder-design/tools-pack typecheck", "tools/pack changed");
    add("pnpm --filter @galyarder-design/tools-pack build", "tools/pack changed");
  }
  if (touched("tools/pr/")) {
    add("pnpm --filter @galyarder-design/tools-pr typecheck", "tools/pr changed");
    add("pnpm --filter @galyarder-design/tools-pr build", "tools/pr changed");
  }
  if (touchedAny(["e2e/specs/", "e2e/tests/", "e2e/lib/"])) {
    add("pnpm --filter @galyarder-design/e2e typecheck", "e2e/ changed");
...
function laneRules(lane: Lane, paths: string[]): string[] {
  ...
  switch (lane) {
    case "skill":
      ...
      items.push("rule [CONTRIBUTING.md skill hard rule 1]: real hand-crafted example.html present");
      items.push("rule [CONTRIBUTING.md skill hard rule 2]: anti-AI-slop list — purple gradients, generic emoji icons, Inter-as-display, invented numbers");
      items.push("rule [CONTRIBUTING.md skill hard rule 4]: references/checklist.md with at least the P0 gate");
      items.push("rule [CONTRIBUTING.md skill hard rule 5]: featured skill ⇒ docs/screenshots/skills/<slug>.png");
      items.push("rule [CONTRIBUTING.md skill hard rule 6]: self-contained — CDN scope ≤ existing skills, no unlicensed fonts, assets ≲ 250 KB");
      break;
    case "design-system":
      ...
      items.push("rule [CONTRIBUTING.md design-system hard rule 1]: 9 sections present (visual / color / typography / spacing / layout / components / motion / voice / anti-patterns)");
      items.push("rule [CONTRIBUTING.md design-system hard rule 5]: ASCII slug only (linear.app → linear-app)");
      items.push("rule [CONTRIBUTING.md design-system hard rule 2-3]: hex sampled from real source; OKLch acceptable for accent ramps");
      break;
    case "craft":
      items.push("rule [code-review-guidelines.md §4.5]: universal brand-agnostic craft (not brand-specific, not artifact-shape)");
      items.push("rule [code-review-guidelines.md §4.5]: at least one shipping skill opts in via gd.craft.requires, or follow-up named in PR description");
