...
const SAVED_PLUGIN_IDS_KEY = 'galyarder-design:saved-plugin-ids';
// Locale-aware label regex for the "Local CLI" tab (English + zh-CN +
// zh-TW); CJK kept verbatim as a UI detection pattern.
...
test('home starters html details modal exposes header actions and closes from the close button', async ({ page }) => {
  ...
  // Locale-aware label (English + zh-CN); CJK kept verbatim as a UI
  // detection pattern.
...
test('home starters html details modal shows metadata links, supports copy query, and opens the plugin share menu', async ({ page }) => {
  const htmlPlugin = makeStarterPlugin({
    ...
    previewEntry: './example.html',
    tags: ['deck', 'marketing'],
    authorName: 'Galyarder Design',
    authorUrl: 'https://github.com/galyarderlabs/galyarder-design',
  ...
  await expect(dialog).toBeVisible();
  await expect(page.getByTestId('plugin-details-author')).toContainText('Galyarder Design');
...
test('home starters direct Use keeps prompt empty and still allows a freeform submit', async ({ page }) => {
  ...
  expect(projectBody.pluginId).toBe('gd-default');
