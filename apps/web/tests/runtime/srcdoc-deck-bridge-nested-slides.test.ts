...
// Behavioral coverage for galyarderlabs/galyarder-design#1530. The deck bridge in
...
function extractDeckBridgeScript(srcdoc: string): string {
  const match = srcdoc.match(/<script data-gd-deck-bridge>([\s\S]*?)<\/script>/);
  if (!match || !match[1]) {
...
function lastSlideState(parentPostMessage: ReturnType<typeof vi.fn>) {
  const messages = parentPostMessage.mock.calls
    .map((call) => call[0])
    .filter((m) => m?.type === 'gd:slide-state');
