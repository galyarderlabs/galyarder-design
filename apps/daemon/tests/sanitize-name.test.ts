...
describe('sanitizeName', () => {
  ...
  it('preserves Unicode letters/digits (Chinese, Japanese, Cyrillic, accented)', () => {
    // UTF-8 / non-ASCII filename fixtures — these literals are
    // intentional non-English bytes that verify sanitizeName preserves
    // CJK, Cyrillic, and accented Latin characters instead of replacing
    // them with underscores. They MUST stay in their original scripts;
    // English placeholders would fail to exercise the bug from #144.
    //   测试文档-中文文件名.docx ≈ "test-document-chinese-filename.docx"
    //   資料.pdf                ≈ "materials.pdf" (traditional Chinese)
    //   документ.txt            ≈ "document.txt"  (Russian)
...
describe('decodeMultipartFilename', () => {
  it('restores UTF-8 names that multer parsed as latin1', () => {
    ...
    // The CJK literal here is a UTF-8 fixture: it verifies that
    // decodeMultipartFilename round-trips a Chinese filename whose
    // bytes were misread as latin1. Translating it would defeat the
    // test.
  ...
  it('returns input untouched when any code point exceeds 0xff', () => {
    ...
    // The CJK literal is a UTF-8 fixture verifying the no-op path.
