...
checked into the Galyarder Design repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Galyarder Design agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Galyarder Design and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Galyarder-Design-specific notes
   inside `SKILL.md`'s `> **Galyarder Design integration.**` blockquote.
...
flow (including how Galyarder Design behaves when Codex is not installed).
