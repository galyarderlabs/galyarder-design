...
//
// Previews are produced offline by `pnpm --filter @galyarder-design/landing-page
...
const PREVIEWS_ROOT_CANDIDATES = [
  // `pnpm --filter @galyarder-design/landing-page build` may keep cwd at the
...
const REPO_TREE = 'https://github.com/galyarderlabs/galyarder-design/tree/main';
const REPO_BLOB = 'https://github.com/galyarderlabs/galyarder-design/blob/main';
...
export function shapeSkill(
  ...
  return {
    ...
    mode: data.gd?.mode,
    platform: data.gd?.platform,
    scenario: data.gd?.scenario,
    category: data.gd?.category,
    featured: data.gd?.featured,
    upstream: data.gd?.upstream,
    examplePrompt: data.gd?.example_prompt,
...
export function shapeDesignTemplate(
  ...
  const summary =
    ...
    'Galyarder Design renderable design template.';
...
export function shapeLiveArtifactTemplate(
  ...
  const summary = extractFirstProseParagraph(body) || 'Galyarder Design Live Artifact template.';
...
// in sync with `gd.mode` / `gd.platform` across the SKILL.md corpus.
// ---------------------------------------------------------------------------
...
export interface CatalogCounts {
  ...
  templates: number;
  craft: number;
  /** SKILL.md `gd.mode` → count. Lowercase keys (e.g. `deck`, `prototype`). */
  byMode: Readonly<Record<string, number>>;
  /** SKILL.md `gd.platform` → count. Lowercase keys (e.g. `mobile`, `desktop`). */
...
// one route per concept. Without this, `gd.scenario: operation` and
// `gd.scenario: operations` would generate two separate `/skills/scenario/...`
...
// human label (preserving the original `gd.mode` casing for the heading).
// ---------------------------------------------------------------------------
