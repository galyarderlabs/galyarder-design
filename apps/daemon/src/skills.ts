...
export const SKILL_ID_ALIASES = Object.freeze({
  "editorial-collage": "galyarder-design-landing",
  "editorial-collage-deck": "galyarder-design-landing-deck",
});
...
export interface SkillInfo {
  ...
  /**
   * Per-skill Critique Theater override declared via `gd.critique.policy`
  ...
  for (let rootIdx = 0; rootIdx < roots.length; rootIdx += 1) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const mode = normalizeMode(data.gd?.mode, body, data.description);
        const surface = normalizeSurface(data.gd?.surface, mode);
        ...
        const category = normalizeCategory(data.gd?.category);
        const designSystemRequired =
          typeof data.gd?.design_system?.requires === "boolean"
            ? data.gd.design_system.requires
            : true;
        const upstream =
          typeof data.gd?.upstream === "string" ? data.gd.upstream : null;
        const previewType =
          typeof data.gd?.preview?.type === "string"
            ? data.gd.preview.type
            : "html";
        const description =
        ...
        const examplePromptI18n = localizedMapFromRecord(data.gd?.example_prompt_i18n);
        ...
        out.push({
          ...
          surface,
          source,
          craftRequires: normalizeCraftRequires(data.gd?.craft?.requires),
          platform,
          scenario,
          category,
          previewType,
          designSystemRequired,
          defaultFor: normalizeDefaultFor(data.gd?.default_for),
          upstream,
          featured: normalizeFeatured(data.gd?.featured),
          ...
          fidelity: normalizeFidelity(data.gd?.fidelity),
          speakerNotes: normalizeBoolHint(data.gd?.speaker_notes),
          animations: normalizeBoolHint(data.gd?.animations),
          examplePrompt: derivePrompt(data),
          ...(examplePromptI18n ? { examplePromptI18n } : {}),
          aggregatesExamples,
          critiquePolicy: normalizeCritiquePolicy(data.gd?.critique?.policy),
          body: parentBody,
          dir,
        });
...
// project folder (`.gd/projects/<id>/`), not the skill folder.
//
// We prepend a short preamble that advertises two paths:
//
//   1. A CWD-relative alias path (`.gd-skills/<folder>/`) — the primary one.
//      Before spawning the agent the chat handler copies the active skill
//      into `<cwd>/.gd-skills/<folder>/` (see `cwd-aliases.ts`), so this
...
// Optional `gd.fidelity` hint for prototype skills. Only 'wireframe' and
...
/**
 * Coerce `gd.critique.policy` from SKILL.md frontmatter into the
...
// Coerce `gd.featured` into a numeric priority. Lower numbers float to the
...
// Prefer an explicitly authored `gd.example_prompt`. Fall back to the
...
function derivePrompt(data: SkillFrontmatter): string {
  const explicit = data.gd?.example_prompt;
  if (typeof explicit === "string" && explicit.trim()) return explicit.trim();
  const desc =
    typeof data.description === "string" ? data.description.trim() : "";
  if (!desc) return "";
  const collapsed = desc.replace(/\s+/g, " ").trim();
  // Stop at the first sentence terminator. The character class includes
  // CJK fullwidth `。！？` (period / exclamation / question) so a
  // skill description authored in Chinese still gets clipped at the
...
function inferMode(body: unknown, description: unknown): SkillMode {
  ...
  // Each pattern keeps the English keyword alongside the Chinese
  // equivalents so a skill description written in either language
  // routes to the same surface. Chinese tokens (kept verbatim — they're
  // pattern matchers, not user-facing copy):
  //   图片 / 海报 / 插画  = image / poster / illustration
  //   视频 / 动效 / 短片  = video / motion / short film
  //   音频 / 音乐 / 配音 / 音效 = audio / music / voiceover / sound effect
  //   幻灯 / 投影         = slide / projection (i.e. deck)
...
function normalizePlatform(value: unknown, mode: SkillMode, body: unknown, description: unknown): SkillPlatform {
  ...
  const hay = `${description ?? ""}\n${body ?? ""}`.toLowerCase();
  // 手机 / 移动端 are Chinese for "phone" / "mobile"; kept here as
  // pattern tokens, not user copy.
...
function normalizeScenario(value: unknown, body: unknown, description: unknown): string {
  ...
  // 人事 = "HR" in Chinese; functional pattern, not user-facing.
