...
<div class="form-group">
  <label lang="ur" for="username">[Username]</label>
  <input 
    id="username" 
    type="text" 
    class="input" 
...
<input id="username" placeholder="[e.g. Ahmed Ali]" />

<!-- ✅ EXCELLENT: Label + aria-label for screen readers -->
<label for="username" lang="ur">[Username]</label>
<input 
  id="username" 
...
```html
<div class="card">
  <h3 lang="ur">[Card title]</h3>
  <p lang="ur">[Card body content goes here.]</p>
</div>
```
...
```html
<label for="email" lang="ur">[Email]</label>
...
<label for="pass" lang="ur">[Password]</label>
<input id="pass" type="password" aria-describedby="pass-hint" />
<p id="pass-hint" lang="ur">[Minimum 8 characters.]</p>
...
✅ "We apologize — your request could not be completed. Please try again." (rendered as a polite, formal Urdu sentence using the respectful register.)

❌ Curt, blunt phrasings such as a one-word "Error" — too informal for an official surface.

#### Friendly (Apps, Products)

✅ "Oops — please try again." (a warm, conversational Urdu phrasing.)

❌ Bare technical strings such as `Error 503` shown directly to users.

#### Technical (Developer Docs, Code)

✅ "API response error: [error_code]" (a localized Urdu sentence that still embeds the technical token verbatim.)

❌ Slangy or jokey phrasings ("Whoops, something went sideways") — wrong register for a developer-facing surface.
...
```html
```html
<!-- ✅ Correct: Pakistani Rupee glyph with localized digits -->
<span lang="ur">₨ 25,000</span>

<!-- ❌ Wrong: Indian Rupee glyph -->
<span lang="ur">₹ 25,000</span>
```
...
| English | Context |
|---------|---------|
| "Submit" | Form action — formal Urdu equivalents: "Send" / "Submit" |
| "Cancel" | Dismissal — formal Urdu equivalents: "Cancel" / "Go back" |
| "Loading..." | Wait state — Urdu equivalent: "Loading..." |
| "Error occurred" | Error state — formal Urdu equivalent: an apologetic "Sorry, something went wrong." |
| "No results" | Empty state — Urdu equivalent: "No results found." |
...
- **Truncate text with ellipsis in RTL:** `text-overflow: ellipsis` reads backwards. Use expand-button or full text instead.
  ...
  <p style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
    [A long Urdu sentence that gets truncated...]
  </p>
  
  <!-- ✅ GOOD: Full text or expand-button -->
  <details>
    <summary lang="ur">[See more]</summary>
    <p lang="ur">[A long Urdu paragraph the reader can expand to view in full.]</p>
  </details>
  ```
...
<body>
  <div class="container">
    <h1>[Welcome] — Urdu Modern Quick Start</h1>
    
    <p>
      [A complete example showing how to use the Urdu Modern design system.]
    </p>
    ...
    <ul style="margin-block-start: var(--space-md); margin-inline-start: var(--space-lg);">
      <li>[Nastaliq font automatically optimized for RTL.]</li>
      <li>[Color tokens such as <code>#0F595E</code> handle informational and warning states.]</li>
      <li>[Every Urdu string is marked with <code>lang="ur"</code>.]</li>
    </ul>
...
2. **Wrap Urdu text:** All Urdu strings wrapped in semantic tags with `lang="ur"`:
   ```html
   <h1 lang="ur">[Heading]</h1>
   <p lang="ur">[Body text.]</p>
   ```
