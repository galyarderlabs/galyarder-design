// Phase 4 / spec §14.1 — `gd plugin scaffold` unit test.
...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-scaffold-'));
});
...
describe('scaffoldPlugin', () => {
  it('writes SKILL.md + galyarder-design.json + README.md by default', async () => {
    ...
    expect(manifest.gd.taskKind).toBe('new-generation');
    expect(manifest.gd.useCase.query).toMatch(/sample plugin/i);
  });
