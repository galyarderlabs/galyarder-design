...
describe("win standalone prebundle policy", () => {
  ...
  it("keeps server-mode package topology unchanged", () => {
    expect(
      shouldInstallInternalPackageForWinPrebundle({
        packageName: "@galyarder-design/web",
        webOutputMode: "server",
      }),
    ).toBe(true);
    expect(
      shouldInstallInternalPackageForWinPrebundle({
        packageName: "@galyarder-design/packaged",
        webOutputMode: "server",
      }),
    ).toBe(true);
  });
  ...
  it("excludes internal packages replaced by win standalone prebundles", () => {
    for (const packageName of [
      ...
      "@galyarder-design/desktop",
      "@galyarder-design/packaged",
      "@galyarder-design/platform",
      "@galyarder-design/sidecar",
      "@galyarder-design/sidecar-proto",
      "@galyarder-design/web",
    ]) {
      expect(
        shouldInstallInternalPackageForWinPrebundle({
          packageName,
    ...
    expect(
      shouldInstallInternalPackageForWinPrebundle({
        packageName: "@galyarder-design/contracts",
        webOutputMode: "standalone",
      }),
    ).toBe(true);
  });
...
describe("assertWinPrebundleMetafile", () => {
  it("accepts a safe web sidecar metafile", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-win-prebundle-"));
  ...
  it("rejects a packaged main metafile that pulled in web runtime closure", async () => {
    ...
    try {
      await writeFile(
        metafilePath,
        JSON.stringify({ inputs: { "/repo/node_modules/@galyarder-design/web/dist/sidecar/index.js": {} } }),
        "utf8",
      );
...
describe("renderWinPackagedMainEntry", () => {
  it("renders the prebundled runtime entry shim", () => {
    ...
    expect(renderWinPackagedMainEntry(true)).not.toContain("@galyarder-design/packaged");
  });

  it("renders the package entry shim for non-prebundled mode", () => {
    expect(renderWinPackagedMainEntry(false)).toContain("@galyarder-design/packaged");
