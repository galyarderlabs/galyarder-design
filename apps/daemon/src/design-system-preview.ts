...
function extractColors(raw: string): ColorToken[] {
  ...
  // The character class `[:：]` matches either ASCII `:` or the
  // CJK fullwidth colon U+FF1A so author notes pasted from Chinese
  // markdown files (where IMEs auto-pick the fullwidth form) still parse.
...
function extractFonts(raw: string): FontHints {
  ...
  // matches both ASCII `:` and CJK fullwidth `：` U+FF1A.
