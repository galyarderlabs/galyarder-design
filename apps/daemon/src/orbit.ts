...
import type { OrbitRunSummary, OrbitStatusResponse } from '@galyarder-design/contracts/api/orbit';
...
export function buildOrbitSystemPrompt(now = new Date(), template?: OrbitTemplateSelection | null): string {
  ...
  const lines = [
    ...
    '- Code: “galyarder-design had 4 repositories updated. The most notable change was a daemon update that affects data refresh behavior, so review it before the next release.”',
  ...
  if (template) {
    lines.push(
      ...
      `- Staged root: .gd-skills/${path.basename(template.dir)}/`,
      '',
      `Before writing the artifact, read ".gd-skills/${path.basename(template.dir)}/SKILL.md" and, if present, ".gd-skills/${path.basename(template.dir)}/example.html". Follow that staged template's structure, layout, tokens, domain rules, and visual language as the source of truth. The staged template is for visual/domain guidance; still use the live-artifact workflow to register the final artifact.`,
      '',
