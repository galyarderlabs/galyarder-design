...
export interface InstallOptions {
  ...
  // `<GD_DATA_DIR>/gd-plugin-lock.json`; tests can point at temp dirs.
  lockfilePath?: string;
}
...
async function* installFromGithubContents(
  ...
  const tmpRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-plugin-github-contents-'));
...
async function* installFromArchiveUrl(
  ...
  const fetcher = opts.fetcher ?? defaultFetcher;
  const maxBytes = opts.maxBytes ?? DEFAULT_MAX_BYTES;
  const tmpRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-plugin-archive-'));
  try {
    const resp = await fetcher(url);
    if (!resp.ok || !resp.body) {
      yield {
...
export async function* installFromLocalFolder(
  ...
  // `gd plugin events tail` see the operation land in the in-
