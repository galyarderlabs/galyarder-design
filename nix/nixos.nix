# NixOS module for Galyarder Design — secondary interface for shared/server
...
#   };
{
  ...
  commonOpts = moduleCommon {
    ...
    defaultDataDir = "/var/lib/galyarder-design";
  };
  ...
  # `services.galyarder-design.extraBinPaths`.
  daemonPathEntries =
    [
      "/run/wrappers/bin"
  ...
  daemonEnvironment =
    ...
    // cfg.extraEnv;
in {
  options.services.galyarder-design =
    commonOpts
    // {
      user = lib.mkOption {
        type = lib.types.str;
        default = "galyarder-design";
      ...
      openFirewall = lib.mkOption {
        ...
        description = ''
          ...
          `services.galyarder-design.webFrontend.host = "0.0.0.0"` and
          declare `services.galyarder-design.webFrontend.allowedOrigins` so
          ...
          `extraEnv.GD_BIND_HOST` to the externally reachable address
          ...
          `GD_BIND_HOST = "0.0.0.0"` and add the externally reachable
          origin (e.g. `http://laptop.local:7456`) to
          `webFrontend.allowedOrigins`, which feeds `GD_ALLOWED_ORIGINS`.
        '';
      };
    };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    {
      users.users.${cfg.user} = {
        ...
        group = cfg.group;
        home = cfg.dataDir;
        description = "Galyarder Design daemon";
      };
    ...
    (lib.mkIf cfg.autoStart {
      systemd.services.galyarder-design = {
        description = "Galyarder Design daemon";
    ...
    (lib.mkIf cfg.webFrontend.enable {
      systemd.services.galyarder-design-web = {
