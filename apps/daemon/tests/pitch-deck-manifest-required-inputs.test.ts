...
import { parseManifest } from '@galyarder-design/plugin-runtime';
...
// to enforce in that shape, so an agent could route through gd-default
// and start generating with no facts collected. This test pins the three
// facts as structured `gd.inputs` fields so the gate fires before any
...
const manifestPath = path.join(
  repoRoot,
  'plugins/_official/examples/html-ppt-pitch-deck/galyarder-design.json',
);
...
describe('html-ppt-pitch-deck manifest inputs', () => {
  it('declares the financing facts the example prompt says must be confirmed first', async () => {
    ...
    const inputs = parsed.manifest.gd?.inputs ?? [];
