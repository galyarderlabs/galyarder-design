...
import type { ApplyResult, InstalledPluginRecord } from '@galyarder-design/contracts';
...
export function PluginDetailView(props: Props) {
  ...
  const gd = plugin.manifest?.gd ?? {};
  const surfaces = gd.genui?.surfaces ?? [];
  const required = gd.connectors?.required ?? [];
  const optional = gd.connectors?.optional ?? [];
  const capabilities = gd.capabilities ?? [];
  ...
  const hasPreview = typeof gd.preview?.entry === 'string' && gd.preview.entry.length > 0;
  const examples = (gd.useCase?.exampleOutputs ?? []) as Array<{
    path: string;
    title?: string;
  }>;
    ...
    <div className="plugin-detail" data-testid="plugin-detail">
      ...
      <header className="plugin-detail__header">
        ...
        <div className="plugin-detail__meta">
          ...
          {gd.taskKind ? <span>{gd.taskKind}</span> : null}
        </div>
      </header>
        ...
        <section className="plugin-detail__preview" data-testid="plugin-detail-preview-section">
          ...
          <iframe
            ...
            style={{
              width: '100%',
              minHeight: 360,
              border: '1px solid var(--gd-border, #ddd)',
              borderRadius: 6,
              background: '#fff',
            }}
          />
              ...
              return (
                <li key={`${e.path}-${idx}`}>
                  ...
                  {e.title && e.title !== stem ? (
                    <span style={{ marginLeft: '0.5em', color: 'var(--gd-muted, #888)' }}>
                      <code>{stem}</code>
                    </span>
                  ) : null}
                </li>
