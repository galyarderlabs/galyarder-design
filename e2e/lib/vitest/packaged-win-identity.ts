...
function displayNameForChannel(channel: ReleaseChannelIdentity): string {
  if (channel === "beta") return "Galyarder Design Beta";
  if (channel === "nightly") return "Galyarder Design Nightly";
  if (channel === "preview") return "Galyarder Design Preview";
...
export function resolvePackagedWinInstallIdentity(options: {
  ...
  const displayName = channel == null ? `Galyarder Design ${namespaceToken}` : displayNameForChannel(channel);
