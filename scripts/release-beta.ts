...
const metadataUrl = process.env.GALYARDER_DESIGN_BETA_METADATA_URL;
if (metadataUrl == null || metadataUrl.length === 0) {
  fail("GALYARDER_DESIGN_BETA_METADATA_URL is required");
}
validateHttpsUrl(metadataUrl, "GALYARDER_DESIGN_BETA_METADATA_URL");
...
const releaseName = `Galyarder Design Beta ${betaVersion}`;
