...
// Locations:
//
//   packaged (mac):    ~/Library/Application Support/Galyarder Design Nightly/installation.json
//   packaged (win):    %APPDATA%/Galyarder Design Nightly/installation.json
//   packaged (linux):  $XDG_CONFIG_HOME/Galyarder Design Nightly/installation.json
//   tools-dev / OSS:   <dataDir>/installation.json  (no namespace concept; fall back to dataDir)
//
// `GD_INSTALLATION_DIR` is the env override. Packaged sidecars.ts sets it
...
export function resolveInstallationDir(dataDir: string): string {
  const env = process.env.GD_INSTALLATION_DIR;
