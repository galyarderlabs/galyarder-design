...
// /api/media/config; the daemon persists them to .gd/media-config.json
...
// folder altogether (`GD_OPENAI_API_KEY=… node daemon/cli.js`).
//
// Storage location (precedence high → low):
//   1. GD_MEDIA_CONFIG_DIR=DIR   → <DIR>/media-config.json
//   2. GD_DATA_DIR=DIR           → <DIR>/media-config.json
//   3. (default)                 → <projectRoot>/.gd/media-config.json
...
// that already set GD_DATA_DIR for the rest of the daemon's runtime
...
// semantics as GD_DATA_DIR in server.ts:resolveDataDir(): the shared
...
//
// Migration note: a workspace install that sets a custom GD_DATA_DIR
// AND has a pre-existing `<projectRoot>/.gd/media-config.json` will
// start reading from `<GD_DATA_DIR>/media-config.json` instead. Move
// the file once or set GD_MEDIA_CONFIG_DIR=<projectRoot>/.gd to keep
// the old location.
//
...
// to dynamically lift `GD_MEDIA_MODEL_ALIAS_<id>=value` into a record
...
// their workflow:
//
//   GD_MEDIA_MODEL_ALIASES='{"doubao-seedream-3-0-t2i-250415":"doubao-seedream-5-0"}'
const ENV_MODEL_ALIASES = 'GD_MEDIA_MODEL_ALIASES';
...
const ENV_KEYS: Record<string, string[]> = {
  ...
  volcengine: ['GD_VOLCENGINE_API_KEY', 'ARK_API_KEY', 'VOLCENGINE_API_KEY'],
  // GD_GROK_API_KEY first (the project-reserved override, same shape as
  ...
  grok: ['GD_GROK_API_KEY', 'XAI_API_KEY'],
  nanobanana: ['GD_NANOBANANA_API_KEY', 'GOOGLE_API_KEY', 'GEMINI_API_KEY'],
  imagerouter: ['GD_IMAGEROUTER_API_KEY', 'IMAGEROUTER_API_KEY'],
  'custom-image': ['GD_CUSTOM_IMAGE_API_KEY', 'CUSTOM_IMAGE_API_KEY'],
  bfl: ['GD_BFL_API_KEY', 'BFL_API_KEY'],
  fal: ['GD_FAL_KEY', 'FAL_KEY'],
  replicate: ['GD_REPLICATE_API_TOKEN', 'REPLICATE_API_TOKEN'],
  google: ['GD_GOOGLE_API_KEY', 'GOOGLE_API_KEY', 'GEMINI_API_KEY'],
  kling: ['GD_KLING_API_KEY', 'KLING_API_KEY'],
  midjourney: ['GD_MIDJOURNEY_API_KEY'],
  minimax: ['GD_MINIMAX_API_KEY', 'MINIMAX_API_KEY'],
  suno: ['GD_SUNO_API_KEY'],
  udio: ['GD_UDIO_API_KEY'],
  elevenlabs: ['GD_ELEVENLABS_API_KEY', 'ELEVENLABS_API_KEY'],
  fishaudio: ['GD_FISHAUDIO_API_KEY', 'FISH_AUDIO_API_KEY'],
  senseaudio: ['GD_SENSEAUDIO_API_KEY', 'SENSEAUDIO_API_KEY'],
  tavily: ['GD_TAVILY_API_KEY', 'TAVILY_API_KEY'],
  leonardo: ['GD_LEONARDO_API_KEY', 'LEONARDO_API_KEY'],
};
...
function resolveOverrideDir(raw: string, projectRoot: string): string {
  // Share expandHomePrefix with resolveDataDir (server.ts) so GD_DATA_DIR
  // and GD_MEDIA_CONFIG_DIR cannot split state under a $HOME-style value.
  // A launcher passing GD_DATA_DIR=$HOME/.galyarder-design without a shell to
  // expand it would otherwise route SQLite/projects/artifacts to the
  // expanded path while media-config.json stayed under
  // <projectRoot>/$HOME/.galyarder-design, leaving stored credentials
...
export function mediaConfigDir(projectRoot: string): string {
  return (
    envOverrideDir('GD_MEDIA_CONFIG_DIR', projectRoot)
    ?? envOverrideDir('GD_DATA_DIR', projectRoot)
    ?? path.join(projectRoot, '.gd')
  );
}
...
async function resolveXAIOAuthCredential(
  ...
  // 1. GD-native xAI OAuth tokens (written by the daemon's own
  //    xai-oauth.ts client when the user authorizes inside GD).
  ...
  //    Hermes doesn't have to run a second OAuth dance inside GD.
