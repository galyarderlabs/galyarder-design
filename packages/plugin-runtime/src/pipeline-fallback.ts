...
//
// When a consumer plugin omits `gd.pipeline`, the runtime consults the
...
import type { PluginManifest, PluginPipeline } from '@galyarder-design/contracts';
...
export interface ResolvePipelineResult {
  ...
  // 'declared' = the manifest carried `gd.pipeline` itself.
...
export function resolveAppliedPipeline(input: ResolvePipelineInput): ResolvePipelineResult {
  const declared = input.manifest.gd?.pipeline;
  ...
  const kind = input.manifest.gd?.kind;
  if (kind === 'scenario') return { pipeline: undefined, source: 'none' };
  const taskKind = (input.manifest.gd?.taskKind ?? 'new-generation') as ScenarioRegistryEntry['taskKind'];
