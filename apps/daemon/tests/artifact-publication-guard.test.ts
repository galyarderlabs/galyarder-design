...
describe('artifact publication guard — wired into writeProjectFile', () => {
  it('rejects html artifacts that still contain pitch-deck placeholders', async () => {
    const projectsRoot = await mkdtemp(path.join(tmpdir(), 'gd-publication-guard-html-'));
    try {
      await expect(
        writeProjectFile(
          projectsRoot,
  ...
  it('rejects deck artifacts that still contain pitch-deck placeholders', async () => {
    const projectsRoot = await mkdtemp(path.join(tmpdir(), 'gd-publication-guard-deck-'));
    try {
      await expect(
        writeProjectFile(
          projectsRoot,
  ...
  it('lets non-guarded artifact kinds pass even when their body contains placeholder substrings', async () => {
    ...
    const projectsRoot = await mkdtemp(path.join(tmpdir(), 'gd-publication-guard-md-'));
    try {
  ...
  it('passes a clean deck artifact through writeProjectFile', async () => {
    const projectsRoot = await mkdtemp(path.join(tmpdir(), 'gd-publication-guard-clean-'));
    try {
      const meta = await writeProjectFile(
        projectsRoot,
        'project-1',
