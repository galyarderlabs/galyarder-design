...
export function lintArtifact(rawHtml: unknown): LintFinding[] {
  ...
  if (lam) {
    out.push({
      ...
      fix: 'Drop either the border-radius (set 0px) or the border-left. Cards in the GD seed use hairline borders all-round, no left accent.',
      snippet: clip(lam[0]),
    });
  }
  ...
  // Either `data-gd-id` (web/mobile prototypes) or `data-screen-label`
  ...
  const tagged = sections.filter(
    (s) => /data-gd-id\s*=/.test(s) || /data-screen-label\s*=/.test(s),
  ).length;
  if (sections.length > 0 && tagged < sections.length) {
    out.push({
      severity: 'P2',
      id: 'missing-section-anchor',
      message: `${sections.length - tagged} of ${sections.length} <section>s lack data-gd-id (or data-screen-label).`,
      fix: 'Add data-gd-id="kebab-slug" (or data-screen-label="01 Cover" for slides) to every top-level <section> so comment mode can target it.',
    });
  }
...
// the value all GD seed templates use), so `0.06rem` on a 48px heading
