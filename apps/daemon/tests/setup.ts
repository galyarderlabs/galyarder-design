...
const TEST_DATA_DIR_SYMBOL = Symbol.for('galyarder-design.daemon.vitestDataDir');
...
if (!globalState[TEST_DATA_DIR_SYMBOL]) {
  globalState[TEST_DATA_DIR_SYMBOL] = mkdtempSync(path.join(tmpdir(), 'gd-daemon-vitest-'));
...
// so tests can never read or overwrite the developer's real repo `.gd` data.
process.env.GD_DATA_DIR = globalState[TEST_DATA_DIR_SYMBOL];

// Publish/share endpoints shell out through GD_NODE_BIN + GD_BIN (dist/cli.js).
// Build the CLI artifact once per vitest process so package tests do not depend
// on a prior manual `pnpm --filter @galyarder-design/daemon build`.
ensureDaemonCliBuilt();
process.env.GD_DAEMON_CLI_PATH = daemonCliDist;
