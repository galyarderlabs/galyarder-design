...
describe('fetchSkillExample', () => {
  ...
  // Regression coverage for galyarderlabs/galyarder-design#897. Skills declared with
  // a non-html `gd.preview.type` ship no fetchable HTML — the daemon's
...
describe('deploy provider registry helpers', () => {
  ...
  it('passes the selected Cloudflare Pages provider id and custom domain through deploy requests', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      ...
      fileName: 'index.html',
      providerId: CLOUDFLARE_PAGES_PROVIDER_ID,
      url: 'https://galyarder-design-preview.pages.dev',
