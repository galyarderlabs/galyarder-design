...
//   - markSnapshotStale() — `gd plugin doctor` flips status='stale' after a
...
export function createSnapshot(db: SqliteDb, input: CreateSnapshotInput): AppliedPluginSnapshot {
  ...
  `).run(
    ...
    input.runId ?? null,
    input.pluginId,
    input.pluginSpecVersion ?? GALYARDER_DESIGN_PLUGIN_SPEC_VERSION,
    input.pluginVersion,
...
export interface PruneExpiredOptions {
  ...
  // projects while letting operators clean up after `gd project
...
function buildSnapshot(args: {
  ...
  const snapshot: AppliedPluginSnapshot = {
    ...
    pluginSpecVersion:    input.pluginSpecVersion ?? GALYARDER_DESIGN_PLUGIN_SPEC_VERSION,
