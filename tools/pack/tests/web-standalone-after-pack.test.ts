...
const CONFIG_ENV = "GD_TOOLS_PACK_WEB_STANDALONE_HOOK_CONFIG";
...
async function writeRootWebPackage(resourcesRoot: string): Promise<void> {
  const webPackageRoot = join(resourcesRoot, "app", "node_modules", "@galyarder-design", "web");
  await mkdir(join(webPackageRoot, "dist", "sidecar"), { recursive: true });
  await writeFile(join(webPackageRoot, "package.json"), "{\"name\":\"@galyarder-design/web\"}\n", "utf8");
...
async function runFixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), "galyarder-design-web-standalone-hook-"));
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "Galyarder Design.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "Galyarder Design.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "Galyarder Design Helper.app"), { recursive: true });
  }
    ...
    `${JSON.stringify(
      {
        ...
        resourceName: "galyarder-design-web-standalone",
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "Galyarder Design" } },
    });
  } catch (error) {
  ...
  return {
    appOutDir,
    auditReportPath,
    destinationRoot: join(resourcesRoot, "galyarder-design-web-standalone"),
    root,
  };
}
...
describe("web standalone afterPack hook", () => {
  it("deduplicates win32 copied standalone Next while retaining the app-local Next package", async () => {
    ...
    try {
      ...
      expect(resolvedNextPath).toMatch(
        /galyarder-design-web-standalone\/apps\/web\/node_modules\/next\/package\.json$/,
      );
  ...
  darwinSymlinkIt("rewrites darwin copied pnpm symlinks to stay inside the packaged resource", async () => {
    ...
    try {
      ...
      expect(report.copiedAudit.resolvedModules["next/package.json"].split(path.sep).join("/")).toMatch(
        /galyarder-design-web-standalone\/node_modules\/\.pnpm\/next@0\.0\.0\/node_modules\/next\/package\.json$/,
      );
    } finally {
      await rm(fixture.root, { force: true, recursive: true });
    }
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/Galyarder Design Helper\.app$/),
          expect.stringMatching(/Galyarder Design\.app$/),
        ]),
      );
      ...
      if (oldCodesignLog == null) {
        delete process.env.GD_FAKE_CODESIGN_LOG;
      } else {
        process.env.GD_FAKE_CODESIGN_LOG = oldCodesignLog;
      }
    }
  });
});
