...
# Builds the @galyarder-design/daemon workspace package — produces $out/bin/od.
#
...
in
  stdenv.mkDerivation (finalAttrs: {
    ...
    installPhase = ''
      runHook preInstall
      mkdir -p $out/lib/galyarder-design $out/bin
      ...
      # just apps/daemon.
      cp -r . $out/lib/galyarder-design/

      chmod +x $out/lib/galyarder-design/apps/daemon/dist/cli.js

      makeWrapper ${nodejs}/bin/node $out/bin/od \
        --add-flags $out/lib/galyarder-design/apps/daemon/dist/cli.js \
    ...
    meta = with lib; {
      description = "Galyarder Design daemon — local agent orchestrator + API (`gd` CLI)";
