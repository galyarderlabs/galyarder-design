...
const HOST = process.env.GD_HOST || "127.0.0.1";
if (process.env.GD_HOST != null && !/^[a-zA-Z0-9._\-:[\]@]+$/.test(process.env.GD_HOST)) {
  throw new Error(`GD_HOST contains invalid characters: ${process.env.GD_HOST}`);
}
...
const WEB_OUTPUT_MODE_ENV = "GD_WEB_OUTPUT_MODE";
const WEB_STANDALONE_ROOT_ENV = "GD_WEB_STANDALONE_ROOT";
const STANDALONE_PARENT_PID_ENV = "GD_STANDALONE_PARENT_PID";
const STANDALONE_STARTUP_TIMEOUT_ENV = "GD_STANDALONE_STARTUP_TIMEOUT_MS";
...
function resolveWebRoot(): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    try {
      ...
      if (packageJson.name === "@galyarder-design/web") return current;
    } catch {
  ...
  throw new Error("failed to resolve @galyarder-design/web package root");
}
...
function configuredAllowedDevHosts(): Set<string> {
  return new Set(
    (process.env.GD_ALLOWED_DEV_ORIGINS ?? "")
      .split(",")
...
async function startStandaloneBackend(webRoot: string | null): Promise<StandaloneBackend> {
  ...
  if (entryPath == null) {
    throw new Error(
      webRoot == null
        ? `missing Next.js standalone server under ${WEB_STANDALONE_ROOT_ENV}; configure ${WEB_STANDALONE_ROOT_ENV} or install @galyarder-design/web`
  ...
  console.log(`[galyarder-design web] starting standalone Next.js server from ${entryPath}`);
  ...
  child.once("exit", (code, signal) => {
    ...
    console.error(`[galyarder-design web] standalone Next.js server exited ${standaloneExitReason}`);
  });
...
async function startRegularNextSidecar(
  ...
  const app = createNextApp({ dev: process.env.GD_WEB_PROD !== "1" && runtime.mode === "dev", dir: webRoot });
