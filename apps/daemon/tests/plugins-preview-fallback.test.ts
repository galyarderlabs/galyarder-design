...
// declare `gd.preview.entry: "./index.html"` but ship the actual
...
// plugin's `gd.context.assets[]` HTMLs and `assets/*.html` so the
...
const serverRuntimeDataRoot = process.env.GD_DATA_DIR
  ? path.resolve(projectRoot, process.env.GD_DATA_DIR)
...
beforeEach(async () => {
  pluginRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-preview-fallback-'));
  ...
  await writeFile(
    path.join(folder, 'galyarder-design.json'),
    JSON.stringify({
...
describe('GET /api/plugins/:id/preview — fallback chain', () => {
  it('assembles example-slides fragments with the sibling template when the declared entry is missing', async () => {
    ...
    expect(body).toContain('Preview fallback fixture | Galyarder Design Example');
  });
});
