...
describe('preview comment attachment helpers', () => {
  it('builds compact target context from an iframe snapshot', () => {
    const target = targetFromSnapshot({
      ...
      selector: '[data-gd-id="hero-title"]',
      label: 'h1.hero-title',
      text: `  ${'Title '.repeat(80)}  `,
      htmlHint: `<h1 class="hero-title" data-gd-id="hero-title">${'x'.repeat(240)}</h1>`,
    ...
    const attachments = buildBoardCommentAttachments({
      target: {
        ...
        podMembers: [
          ...
          {
            elementId: 'chart',
            selector: '[data-gd-id="chart"]',
            label: 'section.chart',
            text: 'Chart value',
            position: { x: 120, y: 80, width: 190, height: 120 },
            htmlHint: '<section data-gd-id="chart">',
          },
        ],
      },
  ...
  it('keeps large queued board-note batches ordered in one send payload', () => {
    ...
    const attachments = buildBoardCommentAttachments({
      target: {
        filePath: 'atlas.html',
        elementId: 'pod-2',
        selector: '[data-gd-id="card"]',
        label: 'Card pod',
        text: 'Heading Body CTA',
        position: { x: 20, y: 30, width: 240, height: 160 },
        htmlHint: '<section data-gd-id="card">',
        selectionKind: 'pod',
        memberCount: 3,
        podMembers: [
          {
            elementId: 'card-heading',
            selector: '[data-gd-id="card-heading"]',
            label: 'h2.card-heading',
            text: 'Heading',
            position: { x: 24, y: 34, width: 100, height: 32 },
            htmlHint: '<h2 data-gd-id="card-heading">',
          },
          {
            elementId: 'card-body',
            selector: '[data-gd-id="card-body"]',
            label: 'p.card-body',
            text: 'Body',
            position: { x: 24, y: 72, width: 180, height: 48 },
            htmlHint: '<p data-gd-id="card-body">',
          },
          {
            elementId: 'card-cta',
            selector: '[data-gd-id="card-cta"]',
            label: 'button.card-cta',
            text: 'CTA',
            position: { x: 24, y: 128, width: 96, height: 32 },
            htmlHint: '<button data-gd-id="card-cta">',
          },
        ],
      },
      notes,
  ...
  it('rehydrates saved free-pin markers from persisted comment position after iframe reload', () => {
    const saved = comment({
      ...
      selector: '[data-gd-pin="pin-abc123"]',
      label: 'pin',
      text: '',
      htmlHint: '',
  ...
  it('serializes selected comments into API-mode prompt context without visible input', () => {
    ...
    expect(content).toContain('selector: [data-gd-id="hero-title"]');
...
function comment(patch: Partial<PreviewComment>): PreviewComment {
  return {
    ...
    text: 'Current title',
    position: { x: 1, y: 2, width: 3, height: 4 },
    htmlHint: '<h1 data-gd-id="hero-title">',
    note: 'Comment',
    status: 'open',
    createdAt: 1,
    updatedAt: 1,
