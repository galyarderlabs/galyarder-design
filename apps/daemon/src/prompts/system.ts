/**
 * Prompt composer. The base is the GD-adapted "expert designer" system
...
import { defaultCritiqueConfig, type CritiqueConfig } from '@galyarder-design/contracts/critique';
...
function renderDesignSystemImportModeGuidance(
  ...
  if (importMode === 'normalized') {
    return 'This package is normalized. Treat tokens.css and DESIGN.md as the contract, and prefer GD token names over source-project names. Use pull-layer source evidence only as optional background.';
  }
  if (importMode === 'hybrid') {
    return 'This package is hybrid. Build with GD-normalized tokens first, then inspect pull-layer source evidence or snippets only when original component behavior, density, or naming would materially improve fidelity.';
  }
...
export interface ComposeInput {
  ...
  // `kami`). `GD_DESIGN_TOKEN_CHANNEL=0` disables the channel as a kill
  ...
  // Craft references the active skill opted into via `gd.craft.requires`.
  ...
  // `renderActiveStageBlock()` when the GD_BUNDLED_ATOM_PROMPTS env
...
export function composeSystemPrompt({
  ...
  // `@galyarder-design/contracts`'s composer. The daemon hits this path for
  ...
  // agents still emit the `<todo-list>` / `[读取 X]` (Chinese for "[reading
  // X]") pseudo-tool markup described in #313. The CJK token is preserved
  // here as a literal pattern that has actually been observed in agent
  // output. Keep the wording byte-identical to the contracts copy so both
  // code paths produce the same observable behaviour.
  ...
  if (isDeckProject && !hasSkillSeed) {
    ...
    // is Chinese for "lecture / presentation" — kept as a pattern since
    // Chinese-speaking users frequently use it),
    ...
    parts.push(
      `\n\n---\n\n## If this brief is a slide deck / keynote / presentation\n\nThe user did not pre-select a "Slide deck" surface, but their request may still call for one. **If — and only if — the brief reads as slides, keynote, presentation, deck, PPT, or 讲解 (Chinese for "presentation"), follow the framework below.** Otherwise ignore everything in this section and continue with the freeform output you would have written anyway.\n\n${DECK_FRAMEWORK_DIRECTIVE}`,
    );
  }
 ...
 * precedence war and let `<todo-list>` / `[读取 X]` (Chinese for
 * "[reading X]") pseudo-tool markup
 * leak into the chat.
 */
...
- Fake-protocol prose such as \`[读取 template.html ...]\`, \`[读取 layouts.md ...]\`, \`[正在调用 TodoWrite ...]\`, or any \`[doing X]\` placeholder narrating a tool you cannot run. (Those CJK examples — "[读取 X]" / "[正在调用 X]" — translate to "[reading X]" / "[invoking X]". They appear here because we have observed the model emit them verbatim in Chinese; both English and Chinese forms are forbidden.)
...
instead of \`"$GD_NODE_BIN" "$GD_BIN" media generate\` for the first generation
...
Copy or move the selected generated file into \`$GD_PROJECT_DIR\` with a short
descriptive filename, then verify the exact destination file exists under
\`$GD_PROJECT_DIR\` before claiming success. If reading the source path,
...
After the file exists under \`$GD_PROJECT_DIR\`, reply with the project-local
...
path via \`"$GD_NODE_BIN" "$GD_BIN" media generate --surface image --model ${imageModel}\`.
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'image') {
    ...
    lines.push('');
    lines.push(
      'This is an **image** project. Plan the prompt carefully, then dispatch via the **media generation contract** using `"$GD_NODE_BIN" "$GD_BIN" media generate --surface image --model <imageModel>`. Do NOT emit `<artifact>` HTML for media surfaces.',
    );
  }
  if (metadata.kind === 'video') {
    ...
    lines.push('');
    lines.push(
      'This is a **video** project. Plan the shotlist and motion, then dispatch via the **media generation contract** using `"$GD_NODE_BIN" "$GD_BIN" media generate --surface video --model <videoModel> --length <seconds> --aspect <ratio>`. Do NOT emit `<artifact>` HTML.',
    );
  ...
  if (metadata.kind === 'audio') {
    ...
    lines.push('');
    lines.push(
      'This is an **audio** project. Lock the content intent first, then dispatch via the **media generation contract** using `"$GD_NODE_BIN" "$GD_BIN" media generate --surface audio --audio-kind <kind> --model <audioModel> --duration <seconds>` and add `--voice <voice-id>` for speech when you have a provider-specific voice id. Do NOT emit `<artifact>` HTML.',
    );
  }
  ...
  if (Array.isArray(metadata.contextConnectors) && metadata.contextConnectors.length > 0) {
    ...
    lines.push(
      'The user selected these connectors as context. Use daemon connector tools through the GD CLI wrapper when data from these sources is needed; do not ask the user to identify a source that is already selected.',
    );
