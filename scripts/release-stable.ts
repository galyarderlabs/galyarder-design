...
const stableTagPattern = /^galyarder-design-v(\d+\.\d+\.\d+)$/;
...
function parseChannel(value: string | undefined): ReleaseChannel {
  ...
  fail(`GALYARDER_DESIGN_RELEASE_CHANNEL must be stable or nightly; got ${value}`);
}
...
async function validateStableNightlyMetadata(options: {
  ...
  if (nightlyVersionInput.length === 0) {
    fail("GALYARDER_DESIGN_STABLE_NIGHTLY_VERSION is required when channel=stable; pass the exact validated nightly version");
  }

  const nightly = parseNightlyVersion(nightlyVersionInput, "GALYARDER_DESIGN_STABLE_NIGHTLY_VERSION");
  ...
  const publicOrigin = trimTrailingSlash(
    options.publicOrigin ?? fail("GALYARDER_DESIGN_RELEASES_PUBLIC_ORIGIN is required when channel=stable"),
  );
  validateHttpsUrl(publicOrigin, "GALYARDER_DESIGN_RELEASES_PUBLIC_ORIGIN");
...
const channel = parseChannel(process.env.GALYARDER_DESIGN_RELEASE_CHANNEL);
...
const versionTag = `galyarder-design-v${packagedVersion}`;
...
let releaseName = `Galyarder Design ${packagedVersion}`;
let nightlyNumber = "";
let stateSource = channel === "nightly" ? "R2 metadata.json" : "GitHub Releases";

if (channel === "nightly") {
  const metadataUrl = process.env.GALYARDER_DESIGN_NIGHTLY_METADATA_URL;
  if (metadataUrl == null || metadataUrl.length === 0) {
    fail("GALYARDER_DESIGN_NIGHTLY_METADATA_URL is required for nightly channel");
  }
  validateHttpsUrl(metadataUrl, "GALYARDER_DESIGN_NIGHTLY_METADATA_URL");
  ...
  releaseName = `Galyarder Design Nightly ${releaseVersion}`;
  ...
  const stableNightly = await validateStableNightlyMetadata({
    branch,
    commit,
    nightlyVersionInput: process.env.GALYARDER_DESIGN_STABLE_NIGHTLY_VERSION,
    packagedVersion,
    publicOrigin: process.env.GALYARDER_DESIGN_RELEASES_PUBLIC_ORIGIN,
    repository,
  });
