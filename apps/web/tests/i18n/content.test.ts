  ...
  it('uses inline skill display metadata before falling back to source fields', () => {
    const inlineSkill = {
      ...
      displayName: {
        ...
        // so the assertion below pins resolver output exactly.
        en: 'Inline Skill',
        'zh-CN': '内联技能',
      },
      description: ' English description from source. ',
    ...
    expect(localizeSkillName('zh-CN' as Locale, inlineSkill)).toBe('内联技能');
    expect(localizeSkillName('zh-CN' as Locale, inlineSkill)).toBe('内联技能');
    expect(localizeSkillName('fr', inlineSkill)).toBe('Inline Skill');
    expect(localizeSkillDescription('zh-CN' as Locale, inlineSkill)).toBe('中文内联描述。');
    expect(localizeSkillDescription('fr', inlineSkill)).toBe('English inline description.');
    expect(localizeSkillPrompt('zh-CN' as Locale, inlineSkill)).toBe('中文内联 prompt。');
