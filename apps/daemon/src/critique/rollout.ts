 ...
 * `GD_CRITIQUE_ENABLED=1` rather than relying on the client toggle,
 ...
 *      (`gd.critique.policy: required | opt-in | opt-out`).
 ...
 *   3. Environment override (`GD_CRITIQUE_ENABLED=1`). Useful for
 *      power users and CI fixtures.
 *   4. Global default. M0 / M1 = false. M2 = true for skills tagged
 *      `gd.critique.policy: required`. M3 = true globally.
 */
...
export interface RolloutInputs {
  /** Effective rollout phase. Reads from `GD_CRITIQUE_ROLLOUT_PHASE`
   *  in production; tests pass it directly. */
  phase: RolloutPhase;
  /** Skill's `gd.critique.policy` value, or `null` if the skill
   *  did not declare one. */
  ...
  /** Environment override (`GD_CRITIQUE_ENABLED=1`). */
...
/**
 * Parse the `GD_CRITIQUE_ROLLOUT_PHASE` env var into a `RolloutPhase`.
...
/**
 * Parse `GD_CRITIQUE_ENABLED`. Recognises the canonical truthy /
