...
async function withFakeAgent<T>(
  ...
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-bin-'));
...
`,
  ...
  it('spawns agent tests with draft allowlisted CLI env', async () => {
    const markerDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-env-'));
...
fs.writeFileSync(${JSON.stringify(envFile)}, JSON.stringify({
  ...
  SHOULD_NOT_PASS: process.env.GD_CONNECTION_TEST_SHOULD_NOT_PASS || null,
}));
console.log(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'ok' } }));
setImmediate(() => process.exit(0));
`,
            ...
            body: JSON.stringify({
              ...
              agentCliEnv: {
                codex: {
                  ...
                  GD_CONNECTION_TEST_SHOULD_NOT_PASS: 'leaked',
                },
                claude: {
...
`,
  ...
  it('uses CODEX_BIN overrides when testing agent connections', async () => {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-codex-bin-'));
    const oldPath = process.env.PATH;
    try {
  ...
  it('surfaces when an invalid configured CODEX_BIN was ignored in favor of PATH', async () => {
    await withFakeCodex(
      `console.log(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'ok' } }));\n`,
      async () => {
        const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-codex-invalid-'));
        try {
  ...
  it('falls back to PATH Codex during connection tests when a configured CODEX_BIN fails', async () => {
    await withFakeCodex(
      `console.log(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'ok' } }));\n`,
      async () => {
        const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-codex-fallback-'));
        try {
  ...
  it('falls back to PATH Codex when a configured shim spawns ENOENT', async () => {
    await withFakeCodex(
      `console.log(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'ok' } }));\n`,
      async () => {
        const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-codex-stale-shim-'));
        try {
...
`,
  ...
  it('launches OpenCode connection tests with 1.3-compatible JSON stdin args', async () => {
    const markerDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-opencode-argv-'));
...
`,
  ...
  it('rejects invalid custom model ids before spawning an agent', async () => {
    const markerDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-argv-'));
    const argvFile = path.join(markerDir, 'argv.json');
    try {
      await withFakeCodex(
        `
...
`,
  ...
  it('hard-cancels aborted agent probes before cleaning up', async () => {
    const markerDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-marker-'));
...
describe('connection test timeout overrides', () => {
  it('returns the fallback when the override is missing or empty', () => {
    expect(
      resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {}),
    ).toBe(12_000);
    expect(
      resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
        GD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '',
      }),
    ).toBe(45_000);
  });
  ...
  it('honors a positive integer override', () => {
    expect(
      resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {
        GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS: '30000',
      }),
    ).toBe(30_000);
    expect(
      resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
        GD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '120000',
      }),
    ).toBe(120_000);
  });
  ...
  it('warns and falls back on non-numeric, zero, negative, or non-integer overrides', () => {
    ...
    try {
      for (const bad of ['fast', '0', '-1', '1.5', 'NaN']) {
        expect(
          resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {
            GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS: bad,
          }),
        ).toBe(12_000);
      }
  ...
  it('rejects values above the Node setTimeout maximum (2^31-1)', () => {
    ...
    try {
      ...
      expect(
        resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          GD_CONNECTION_TEST_AGENT_TIMEOUT_MS: tooLarge,
        }),
      ).toBe(45_000);
      // The exact maximum is still accepted; anything past it is not.
      expect(
        resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          GD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '2147483647',
        }),
      ).toBe(2_147_483_647);
      expect(
        resolveConnectionTestTimeoutMs('GD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          GD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '2147483648',
        }),
      ).toBe(45_000);
      expect(warn).toHaveBeenCalled();
    } finally {
