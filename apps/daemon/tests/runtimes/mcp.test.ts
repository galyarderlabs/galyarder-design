...
test('MCP-capable agents can discover equivalent live artifact and connector tools', async () => {
  ...
  for (const tool of tools) {
    ...
    assert.match(tool.description, /POSIX equivalent: `"\$GD_NODE_BIN" "\$GD_BIN" tools /u);
  ...
  assert.equal(initialized.result.serverInfo.name, 'galyarder-design-live-artifacts');
...
test('live artifact MCP connector list forwards daily digest use case to daemon tools', async () => {
  process.env.GD_DAEMON_URL = 'http://127.0.0.1:17456/base';
  process.env.GD_TOOL_TOKEN = 'test-tool-token';
...
test('live artifact MCP create forwards input and artifact payload fields to daemon tools', async () => {
  process.env.GD_DAEMON_URL = 'http://127.0.0.1:17456';
  process.env.GD_TOOL_TOKEN = 'test-tool-token';
  const calls: Array<{ url: string; init: RequestInit | undefined }> = [];
  globalThis.fetch = async (url, init) => {
    calls.push({ url: String(url), init });
