...
function resolveDistDir(defaultValue: string) {
  if (process.env.GD_WEB_PROD === '1') return defaultValue;
  const configured = process.env.GD_WEB_DIST_DIR;
...
function resolveDevTsconfigPath() {
  const configured = process.env.GD_WEB_TSCONFIG_PATH;
...
function configuredAllowedDevHosts(): string[] {
  const configured = (process.env.GD_ALLOWED_DEV_ORIGINS ?? '')
    .split(',')
    .map(parseAllowedDevHost)
    .filter((host): host is string => host != null);

  const bindHost = parseAllowedDevHost(process.env.GD_HOST ?? '');
