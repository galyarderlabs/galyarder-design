...
const AUTHORING_PLUGIN = {
  ...
  manifest: {
    ...
    gd: {
      ...
      useCase: { query: 'Create an Galyarder Design plugin for {{pluginGoal}}.' },
      inputs: [
        {
          name: 'pluginGoal',
          type: 'string',
...
// the generic gd-new-generation router. Mirror that here so the
...
const LIVE_ARTIFACT_PLUGIN = {
  ...
  manifest: {
    ...
    title: 'Live Artifact',
    description: 'Create refreshable, auditable Galyarder Design artifacts.',
    gd: {
      kind: 'scenario',
      taskKind: 'new-generation',
      mode: 'prototype',
      scenario: 'live',
      useCase: {
        query: 'Create refreshable, auditable Galyarder Design artifacts backed by connector or local data.',
      },
      context: {
...
const LIVE_ARTIFACT_IMAGE_TEMPLATE_PLUGIN = {
  ...
  manifest: {
    ...
    gd: {
      ...LIVE_ARTIFACT_PLUGIN.manifest.gd,
      mode: 'image',
      surface: 'image',
      useCase: {
...
const AUTHORING_APPLY_RESULT = {
  ...
  contextItems: [],
  inputs: AUTHORING_PLUGIN.manifest.gd.inputs,
  assets: [],
  mcpServers: [],
  trust: 'trusted',
  capabilitiesGranted: ['prompt:inject'],
...
const WEB_PROTOTYPE_APPLY_RESULT = {
  ...AUTHORING_APPLY_RESULT,
  query: WEB_PROTOTYPE_PLUGIN.manifest.gd.useCase.query,
  inputs: WEB_PROTOTYPE_PLUGIN.manifest.gd.inputs,
  appliedPlugin: {
    ...AUTHORING_APPLY_RESULT.appliedPlugin,
...
const SIMPLE_DECK_APPLY_RESULT = {
  ...AUTHORING_APPLY_RESULT,
  query: SIMPLE_DECK_PLUGIN.manifest.gd.useCase.query,
  inputs: SIMPLE_DECK_PLUGIN.manifest.gd.inputs,
  appliedPlugin: {
    ...AUTHORING_APPLY_RESULT.appliedPlugin,
...
const LIVE_ARTIFACT_APPLY_RESULT = {
  ...AUTHORING_APPLY_RESULT,
  query: LIVE_ARTIFACT_PLUGIN.manifest.gd.useCase.query,
  inputs: [],
  appliedPlugin: {
    ...AUTHORING_APPLY_RESULT.appliedPlugin,
...
describe('HomeView prompt handoff', () => {
  ...
  it('consumes a plugin authoring handoff once and focuses the textarea', async () => {
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      ...
      if (typeof url === 'string' && url.includes('/api/plugins/gd-plugin-authoring/apply')) {
  ...
  it('falls back to gd-new-generation when gd-plugin-authoring is not registered yet', async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      if (typeof url === 'string' && url === '/api/plugins') {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      '/api/plugins/gd-new-generation/apply',
      expect.anything(),
    ));
    const applyCall = fetchMock.mock.calls.find(([url]) => (
      typeof url === 'string' && url.includes('/api/plugins/gd-new-generation/apply')
    ));
  ...
  it('binds gd-plugin-authoring before submitting the rail create-plugin prompt', async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      if (typeof url === 'string' && url === '/api/plugins') {
        return new Response(JSON.stringify({ plugins: [AUTHORING_PLUGIN, WEB_PROTOTYPE_PLUGIN] }), {
          status: 200,
    ...
    await clickHomeShortcut('create-plugin');
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      '/api/plugins/gd-plugin-authoring/apply',
      expect.anything(),
    ));
    await waitFor(() => {
