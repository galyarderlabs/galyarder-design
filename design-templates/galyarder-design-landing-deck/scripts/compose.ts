...
/**
 * galyarder-design-landing-deck — slide deck composer.
 *
 ...
 * stylesheet from the sister `galyarder-design-landing` skill, then layers
...
import type {
  GalyarderDesignLandingDeckInputs,
  Slide,
  CoverSlide,
  SectionSlide,
  ContentSlide,
...
const SISTER_STYLES = resolve(SKILL_ROOT, '..', 'galyarder-design-landing', 'styles.css');
...
/* ------------------------------------------------------------------ *
 * deck-specific stylesheet (layered on top of galyarder-design-landing CSS).
 *
...
function chromeStrip(brand: GalyarderDesignLandingDeckInputs['brand'], deckTitle: string): string {
...
function footStrip(idx: number, total: number, brand: GalyarderDesignLandingDeckInputs['brand']): string {
...
function renderSlide(
  ...
  i: number,
  total: number,
  inputs: GalyarderDesignLandingDeckInputs,
  assets: string,
): string {
...
(function () {
  ...
  function applySlide(n) {
    ...
    /* load-bearing: .slide.active is read by Galyarder Design's host bridge
  ...
  /* Host-driven navigation: Galyarder Design's host bridge classifies this deck
...
export function renderDeck(inputs: GalyarderDesignLandingDeckInputs, baseCss: string): string {
...
async function main(): Promise<void> {
  ...
  const inputs = JSON.parse(inputsRaw) as GalyarderDesignLandingDeckInputs;
