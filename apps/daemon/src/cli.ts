...
// ---- Subcommand router ----------------------------------------------------
//
// `gd` is two CLIs glued together:
//   - default mode: starts the daemon + opens the web UI.
//   - `gd media …`: a thin client that POSTs to the running daemon. This
...
// Flags accepted by `gd media generate`. Whitelisted so a hallucinated
...
// initialization") and crash every `gd media …` invocation.
...
// still in TDZ when the handler executes, so `gd status` /
// `gd atoms list` / etc. would crash with `Cannot access X before
...
// `gd automation …` mirrors the Automations tab. Same surface, same
// /api/routines store. The CLI form is the embeddability contract:
// external agents (hermes-agent, openclaw, etc.) can drive Galyarder Design
...
function printRootHelp() {
  console.log(`Usage:
  gd [--port <n>] [--host <addr>] [--no-open]
      Start the local daemon and open the web UI.

  gd tools live-artifacts <create|list|update|refresh> [options]
      Manage live artifacts through daemon wrapper commands.

  gd artifacts create --name <path> --input <file> [--project <id-or-name>]
      Create a normal project artifact through the local daemon.

  gd tools connectors <list|execute|github-design-context> [options]
      Discover and execute configured connectors.

  gd tools design-systems read --path <manifest-declared-path>
      Read active design-system pull-layer files through daemon wrapper commands.

  gd mcp live-artifacts
      Start the MCP server exposing live-artifact and connector tools.

  gd research search --query <text> [--max-sources 5] [--daemon-url <url>]
      Run agent-callable Tavily research through the local daemon.

  gd plugin <list|info|install|uninstall|apply|doctor|replay|trust> [args]
      Discover, install, and apply plugins through the local daemon.
  gd plugin publish-repo <folder>
      Create/update the author's GitHub repo for a local plugin folder.
  gd plugin galyarder-design-pr <folder>
      Push a community-catalog branch and open the Galyarder Design PR form.

  gd automation <list|get|create|update|run|runs|pause|resume|delete> [args]
  ...
  "$GD_NODE_BIN" "$GD_BIN" tools ...
      Recommended agent-runtime form; avoids relying on user PATH for gd or node.

  gd media generate --surface <image|video|audio> --model <id> [opts]
      Generate a media artifact and write it into the active project.
      Designed to be invoked by a code agent - picks up GD_DAEMON_URL
      and GD_PROJECT_ID from the env that the daemon injected on spawn.

  gd mcp [--daemon-url <url>]
      Run a stdio MCP server that proxies project tool calls to a
      running Galyarder Design daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Galyarder Design project and create
      project-scoped artifacts without exporting a zip.

Options:
  --port <n>       Port to listen on (default: 7456, env: GD_PORT).
  --host <addr>    Interface address to bind to (default: 127.0.0.1, env: GD_BIND_HOST).
...
What the daemon does:
  ...
  * exposes /api/projects/:id/media/generate — the unified image/video/audio
     dispatcher that the agent calls via \`gd media generate\`.`);
}

// ---------------------------------------------------------------------------
// Subcommand: gd research …
// ---------------------------------------------------------------------------
...
async function runResearch(args) {
  ...
  if (sub !== 'search') {
    console.error(`unknown subcommand: gd research ${sub}`);
    printResearchHelp();
    process.exit(2);
  }
...
Runs Tavily-backed shallow research through the local Galyarder Design daemon.
...
Flags:
  ...
  --daemon-url   Local daemon URL. Defaults to GD_DAEMON_URL, GD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456.`);
}
...
async function runMedia(args) {
  ...
  if (sub !== 'generate' && sub !== 'wait') {
    console.error(`unknown subcommand: gd media ${sub}`);
    printMediaHelp();
    process.exit(1);
  }
...
async function runMediaGenerate(rawArgs) {
  ...
  const daemonUrl = await cliDaemonUrl(flags);
  const projectId = flags.project || process.env.GD_PROJECT_ID;
  if (!projectId) {
    console.error(
      'project id required. Pass --project <id> or set GD_PROJECT_ID. The daemon injects this when it spawns the code agent.',
    );
    process.exit(2);
  }
...
async function runMediaWait(rawArgs) {
  ...
  if (!taskId) {
    console.error('usage: gd media wait <taskId> [--since <n>] [--daemon-url <url>]');
    process.exit(2);
  }
...
async function pollUntilDoneOrBudget(daemonUrl, taskId, sinceStart, options = {}) {
  ...
  process.stderr.write(
    `task ${taskId} still running after ${handoff.elapsed}s. ` +
      `Run \`"$GD_NODE_BIN" "$GD_BIN" media wait ${taskId} --since ${since}\` to continue in an agent runtime ` +
      `(${stillRunningHint}).\n`,
  );
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Galyarder Design daemon itself is unaffected - it can be ' +
...
function parseFlags(argv, opts = {}) {
  ...
  // positional args (e.g. `gd plugin info <id>`) re-scan `argv`
...
function printMediaHelp() {
  console.log(`Usage: gd media generate --surface <image|video|audio> --model <id> [opts]
       "$GD_NODE_BIN" "$GD_BIN" media generate --surface <image|video|audio> --model <id> [opts]

Required:
  --surface  image | video | audio
  --model    Model id from /api/media/models (e.g. gpt-image-2, seedance-2, suno-v5).
  --project  Project id. Auto-resolved from GD_PROJECT_ID when invoked by the daemon.
...
function printMcpHelp() {
  console.log(`Usage: gd mcp [--daemon-url <url>]

Run a stdio MCP (Model Context Protocol) server that proxies project
tool calls to a running Galyarder Design daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Galyarder Design
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   Galyarder Design daemon HTTP base URL. Resolution
                       order: this flag, GD_DAEMON_URL, GD_SIDECAR_IPC_PATH,
...
Tools exposed:
  list_projects                  list every Galyarder Design project
...
user has open in Galyarder Design; get_artifact and get_file additionally
...
→ MCP server in the Galyarder Design app. The daemon must be running locally
for tool calls to succeed.`);
}

// ---------------------------------------------------------------------------
// Subcommand: gd plugin …
// ---------------------------------------------------------------------------
...
async function runPlugin(args) {
  ...
  switch (sub) {
    ...
    case 'galyarder-design-pr': return runPluginGalyarderDesignPr(rest);
    case 'yank':     return runPluginYank(rest);
    default:
      console.error(`unknown subcommand: gd plugin ${sub}`);
      printPluginHelp();
      process.exit(2);
  }
}

// Phase 4 / spec §14.1 — `gd plugin scaffold` interactive starter.
//
// Side-effect: writes a SKILL.md + galyarder-design.json starter under
...
Writes <out|cwd>/<id>/{SKILL.md,galyarder-design.json,README.md}.`);
  ...
  if (!id) {
    console.error('Usage: gd plugin scaffold --id <id>');
    process.exit(2);
  }
  ...
  try {
    ...
    console.log(`\nNext: gd plugin install ${result.folder}`);
  } catch (err) {
...
// Phase 4 / spec §11.5 / plan §3.W1 — `gd plugin validate <folder>`.
//
...
async function runPluginValidate(rest) {
  ...
  if (flags.help || flags.h || rest.length === 0 || rest[0]?.startsWith('-')) {
    console.log(`Usage:
  gd plugin validate <folder> [--json] [--no-daemon] [--daemon-url <url>]
...
// Phase 4 / spec §14 / plan §3.X1 — `gd plugin pack <folder>`.
//
...
Exit codes:
  0  archive written
  2  CLI usage error
  4  pack-time error (missing galyarder-design.json, invalid JSON, etc)`);
    process.exit(rest.length === 0 ? 2 : 0);
  }
  const folder = rest[0];
  try {
    ...
    if (flags.json) {
      ...
      console.log(`\nNext: gd plugin install --source ${result.outPath}`);
    }
  } catch (err) {
...
async function runPluginLogin(rest) {
  ...
  if (flags.help || flags.h) {
    console.log(`Usage:
  gd plugin login [--host github.com]

Wraps GitHub CLI auth for Galyarder Design registry publishing. The token stays in gh.`);
    return;
  }
...
async function runPluginWhoami(rest) {
  ...
  if (flags.help || flags.h) {
    console.log(`Usage:
  gd plugin whoami [--host github.com] [--json]

Shows the GitHub account gh will use for Galyarder Design registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
  ...
  if (!auth.ok) {
    ...
    console.error(`[plugin whoami] gh is not authenticated for ${host}. Run: gd plugin login --host ${host}`);
...
// Phase 4 / spec §14 — `gd plugin export <projectId> --as <target>`.
//
...
// targets: 'gd', 'claude-plugin', 'agent-skill'.
async function runPluginExport(rest) {
  ...
  if (rest.length === 0 || flags.help || flags.h) {
    console.log(`Usage:
  gd plugin export <projectId> --as gd|claude-plugin|agent-skill --out <dir>
  gd plugin export --snapshot-id <id> --as gd|claude-plugin|agent-skill --out <dir>
...
view is the single source of truth.`);
  ...
  if (!projectId && !snapshotId) {
    console.error('Usage: gd plugin export <projectId> --as <target> --out <dir>');
    process.exit(2);
  }
  const target = String(flags.as ?? 'gd');
  if (target !== 'gd' && target !== 'claude-plugin' && target !== 'agent-skill') {
    console.error(`--as must be one of: gd, claude-plugin, agent-skill (got "${target}")`);
    process.exit(2);
  }
...
// Plan §3.B4 / spec §6: `gd marketplace …` minimum verbs. Add / list /
// refresh / remove / trust. The Phase 3 follow-up wires
// `gd plugin install <name>` resolution through these catalogs.
async function runMarketplace(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd marketplace add     <url> [--trust trusted|restricted]   Register a federated catalog.
  gd marketplace list                                         List registered marketplaces.
  gd marketplace info    <id>                                 Inspect one marketplace + cached manifest.
  gd marketplace plugins <id> [--json]                        List cached plugin entries for one marketplace.
  gd marketplace search  <query> [--json]                     Search cached marketplace entries.
  gd marketplace doctor  [id] [--strict] [--json]             Validate cached marketplace entries.
  gd marketplace login   <id|url> [--host github.com]         Authenticate gh for private GitHub catalogs.
  gd marketplace refresh <id>                                 Re-fetch the manifest.
  gd marketplace remove  <id>                                 Forget a marketplace.
  gd marketplace trust   <id> [--trust trusted|restricted|official]
                                                              Update the marketplace trust tier.

Common options:
  --daemon-url <url>   Galyarder Design daemon HTTP base (default GD_DAEMON_URL, GD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    case 'list': {
      ...
      if (rows.length === 0) {
        console.log('No marketplaces registered. Run `gd marketplace add <url>`.');
        return;
      }
      for (const m of rows) {
    ...
    case 'search': {
      ...
      if (!query) {
        console.error('Usage: gd marketplace search "<query>" [--tag <tag>]');
        process.exit(2);
      }
    ...
    case 'plugins': {
      ...
      if (!id) {
        console.error('Usage: gd marketplace plugins <id> [--json]');
        process.exit(2);
      }
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not Galyarder Design.`);
    ...
    case 'add': {
      ...
      if (!url) {
        console.error('Usage: gd marketplace add <url> [--trust trusted|restricted]');
        process.exit(2);
      }
    ...
    case 'trust': {
      const id = rest.find((a) => !a.startsWith('-')
        && a !== flags.trust);
      if (!id) {
        console.error(`Usage: gd marketplace ${sub} <id>`);
        process.exit(2);
      }
      let url;
      let method = 'GET';
    ...
    default:
      console.error(`unknown subcommand: gd marketplace ${sub}`);
      process.exit(2);
  }
}
...
// Two subcommands:
//   - `gd plugin snapshots list [--project <id>]` — list snapshots
//   - `gd plugin snapshots prune [--before <ts>]` — force-delete expired
//     (and optionally older-than-cutoff unreferenced) rows.
async function runPluginSnapshots(args) {
  const sub = args[0];
  if (!sub || sub === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd plugin snapshots list  [--project <id>]               List applied plugin snapshots.
  gd plugin snapshots show  <snapshotId> [--json]          Print one snapshot's full contents.
  gd plugin snapshots diff  <id-a> <id-b> [--json]         Compare two snapshots field-by-field.
  gd plugin snapshots prune [--before <unix-ms>]           Delete expired (or older-than-cutoff) snapshots.`);
    process.exit(args.length === 0 ? 2 : 0);
  }
  ...
  if (sub === 'show') {
    ...
    if (!id) {
      console.error('Usage: gd plugin snapshots show <snapshotId>');
      process.exit(2);
    }
  ...
  if (sub === 'diff') {
    ...
    if (positional.length < 2) {
      console.error('Usage: gd plugin snapshots diff <id-a> <id-b>');
      process.exit(2);
    }
  ...
  console.error(`unknown subcommand: gd plugin snapshots ${sub}`);
  process.exit(2);
}

// Plan §3.B3: `gd plugin run <id>` shorthand. Today this is a thin
// wrapper around `gd plugin apply` + `POST /api/runs` so a code agent
...
async function runPluginRun(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-')
    ...
    && a !== flags['grant-caps']);
  if (!id) {
    console.error('Usage: gd plugin run <id> --project <projectId> [--inputs <json>] [--agent <id>] [--message "<text>"] [--grant-caps a,b] [--follow]');
    process.exit(2);
  }
  if (!flags.project) {
  ...
  if (!runResp.ok) {
    if (runResp.status === 409 && runData?.error?.code === 'capabilities-required') {
      ...
      console.error(`[run] retry with --grant-caps ${missing} or run \`gd plugin trust ${id} --capabilities ${missing}\``);
      process.exit(66);
    }
...
// Plan §3.Y1 — filter knobs on `gd plugin list` (and feeds
// `gd plugin search` below). Recognising these as string flags
...
async function runPluginList(rest) {
  ...
  if (flags.help || flags.h) {
    console.log(`Usage:
  gd plugin list [--task-kind <kind>] [--mode <mode>] [--tag <tag>] \\
...
+ --tag=phase-7 returns only code-migration plugins tagged 'phase-7'.
  ...
  --task-kind   Match gd.taskKind (new-generation / figma-migration /
...
// Plan §3.Y1 — `gd plugin search <query>`.
async function runPluginSearch(rest) {
  ...
  if (flags.help || flags.h || !query) {
    console.log(`Usage:
  gd plugin search <query> [--task-kind <kind>] [--mode <mode>] \\
                           [--tag <tag>] [--trust <tier>] \\
...
// Plan §3.DD1 — `gd plugin stats`. Pretty-prints the
...
async function runPluginInfo(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('--')
    ...
    && a !== flags.version);
  if (!id) {
    console.error('Usage: gd plugin info <id-or-marketplace-name> [--version <version|tag|range>] [--json]');
    process.exit(2);
  }
  const base = (await pluginDaemonUrl(flags)).replace(/\/$/, '');
...
// Plan §3.MM1 — `gd plugin manifest <id>`. Prints just the parsed
// manifest JSON, no wrapper. Useful for plugin authors who want to
// compare the daemon's view to their on-disk galyarder-design.json
...
async function runPluginManifest(rest) {
  ...
  if (!id) {
    console.error('Usage: gd plugin manifest <id>');
    process.exit(2);
  }
...
// Plan §3.MM2 — `gd plugin sources`. Lists every distinct install
...
async function runPluginInstall(rest) {
  ...
  if (!source) {
    console.error('Usage: gd plugin install <source-or-name>\n' +
      '       gd plugin install ./local-folder\n' +
      '       gd plugin install github:owner/repo[@ref][/subpath]\n' +
      '       gd plugin install https://example.com/plugin.tar.gz\n' +
      '       gd plugin install <name>[@version|tag|range]  # resolves through configured marketplaces');
    process.exit(2);
  }
...
async function runPluginEvents(rest) {
  ...
  if (!sub || sub === 'help' || rest.includes('--help') || rest.includes('-h')) {
    console.log(`Usage:
  gd plugin events tail     [-f] [--since <id>] [--kind <k>] [--plugin-id <id>] [--json]
  gd plugin events snapshot [--since <id>] [--kind <k>] [--plugin-id <id>] [--json]
  gd plugin events stats    [--json]
  gd plugin events purge    [--confirm] [--json]    (loopback-only)
...
Lifecycle vocabulary:
  ...
  if (sub === 'purge') {
    // Refuse to run without an explicit --confirm so 'gd plugin
  ...
  if (sub !== 'tail') {
    console.error(`unknown subcommand: gd plugin events ${sub}`);
    process.exit(2);
  }
...
// Plan §3.FF1 — `gd plugin verify <pluginId>` CI meta-command.
//
// Reads an optional .gd-verify.json config from the plugin folder
...
async function runPluginVerify(rest) {
  ...
  if (flags.help || flags.h || !id) {
    console.log(`Usage:
  gd plugin verify <pluginId> [--config <path>] [--json]

CI meta-command. Reads an optional config from
'<plugin-folder>/.gd-verify.json' (or --config <path>) and runs:
...
Exit codes:
  ...
  // 2. Load .gd-verify.json from --config or <fsPath>/.gd-verify.json.
  const fs = await import('node:fs/promises');
  const path = await import('node:path');
  const configPath = typeof flags.config === 'string'
    ? path.resolve(flags.config)
    : (typeof plugin?.fsPath === 'string' ? path.join(plugin.fsPath, '.gd-verify.json') : null);
  ...
  if (enabledSet.has('simulate')) {
    const pipeline = plugin?.manifest?.gd?.pipeline;
...
// Plan §3.EE1 — `gd plugin simulate <pluginId> [-s key=value ...]`.
//
...
async function runPluginSimulate(rest) {
  ...
  if (flags.help || flags.h || !id) {
    console.log(`Usage:
  gd plugin simulate <pluginId> [-s key=value ...] [--cap <n>] [--json]
...
Examples:
  ...
  gd plugin simulate my-plugin -s critique.score=5

  # build-test devloop where both signals must hold
  gd plugin simulate code-migration \\
      -s build.passing=true -s tests.passing=true

  # raise the per-stage iteration cap (default 10)
  gd plugin simulate my-plugin -s critique.score=2 --cap 20
...
Closed signal vocabulary:
  ...
  if (!pipeline || !Array.isArray(pipeline.stages) || pipeline.stages.length === 0) {
    if (flags.json) {
      ...
      console.log(`[simulate] plugin ${id} has no gd.pipeline (or it is empty); nothing to walk.`);
    }
    return;
  }
...
// Plan §3.CC1 / §3.DD2 — `gd plugin canon <snapshotId>`. Prints the
...
async function runPluginCanon(rest) {
  ...
  if (flags.help || flags.h || !id) {
    console.log(`Usage:
  gd plugin canon <snapshotId> [--json]
  gd plugin canon <snapshotId> --check <expected-file>
...
// Plan §3.AA1 — `gd plugin diff <a> <b>`. Compares two installed
...
async function runPluginUpgrade(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-') && a !== flags['daemon-url'] && a !== flags.source);
  if (!id) {
    console.error('Usage: gd plugin upgrade <id> [--policy latest|pinned] [--json]');
    process.exit(2);
  }
...
async function runPluginUninstall(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-') && a !== flags['daemon-url'] && a !== flags.source);
  if (!id) {
    console.error('Usage: gd plugin uninstall <id>');
    process.exit(2);
  }
...
async function runPluginApply(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-')
    ...
    && a !== flags['grant-caps']);
  if (!id) {
    console.error('Usage: gd plugin apply <id> [--inputs <json>] [--input k=v ...] [--project <id>] [--grant-caps a,b]');
    process.exit(2);
  }
...
// Phase 4 / spec §14.1 — `gd plugin publish --to <catalog>`.
//
...
async function runPluginPublish(rest) {
  ...
  if (rest.length === 0 || flags.help || flags.h) {
    console.log(`Usage:
  gd plugin publish <pluginId> --to galyarder-design|anthropics-skills|awesome-agent-skills|clawhub|skills-sh
                    [--repo <github-url>] [--snapshot-id <id>] [--open] [--json]
  gd plugin publish <pluginId> --to marketplace-json --catalog ./galyarder-design-marketplace.json --repo <github-url>
...
publish from a frozen run snapshot rather than the live installed copy.`);
  ...
  if (!id) {
    console.error('Usage: gd plugin publish <pluginId> --to <catalog>');
    process.exit(2);
  }
  if (!target) {
    console.error('--to <catalog> is required (one of: galyarder-design, anthropics-skills, awesome-agent-skills, clawhub, skills-sh)');
    process.exit(2);
  }
  const base = (await pluginDaemonUrl(flags)).replace(/\/$/, '');
...
async function runPluginPublishRepo(rest) {
  ...
  if (rest.length === 0 || flags.help || flags.h) {
    console.log(`Usage:
  gd plugin publish-repo <folder> [--host github.com] [--owner github-login-or-org] [--dry-run] [--json]
...
GitHub API as a last resort. It never publishes to placeholder owners.`);
  ...
  if (!folder) {
    console.error('Usage: gd plugin publish-repo <folder>');
    process.exit(2);
  }
  ...
  const manifestPath = resolve(absFolder, 'galyarder-design.json');
  ...
  if (exists && !flags['dry-run']) {
    cleanupDir = await mkdtemp(join(os.tmpdir(), 'gd-plugin-publish-sync-'));
...
async function runPluginGalyarderDesignPr(rest) {
  const flags = parseFlags(rest, {
    string: new Set(['host', 'owner']),
    boolean: new Set(['help', 'h', 'json', 'dry-run']),
  });
  if (rest.length === 0 || flags.help || flags.h) {
    console.log(`Usage:
  gd plugin galyarder-design-pr <folder> [--host github.com] [--owner github-login-or-fork-owner] [--dry-run] [--json]

Copies a local plugin folder into plugins/community/<name>/ on the author's
fork of galyarderlabs/galyarder-design, pushes a branch, and opens the PR form with --web.`);
    process.exit(rest.length === 0 ? 2 : 0);
  }
  const folder = rest.find((a) => !a.startsWith('-') && a !== flags.host && a !== flags.owner);
  if (!folder) {
    console.error('Usage: gd plugin galyarder-design-pr <folder>');
    process.exit(2);
  }
  ...
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
  const target = await resolvePluginGithubTarget({ host, owner: flags.owner, manifest, purpose: 'galyarder-design-pr' });
  const name = String(manifest.name ?? '').trim();
  if (!name) {
    console.error('[galyarder-design-pr] manifest.name is required');
    process.exit(2);
  }
  const title = String(manifest.title ?? name).trim();
  const branch = `plugin/${name}-${Math.floor(Date.now() / 1000)}`;
  const tmpRoot = await fsp.mkdtemp(join(os.tmpdir(), 'gd-galyarder-design-pr-'));
  const checkout = join(tmpRoot, 'galyarder-design');
  const steps = [];
  const run = async (label, command, args, opts = {}) => {
    steps.push({ label, command: [command, ...args].join(' ') });
    if (flags['dry-run']) return { ok: true, stdout: '', stderr: '' };
  ...
  await run('fork', 'gh', ['repo', 'fork', 'galyarderlabs/galyarder-design'], {
  ...
  await run('clone fork', 'git', [
    ...
    '--filter=blob:none',
    '--sparse',
    `https://github.com/${target.owner}/galyarder-design.git`,
    checkout,
  ], { timeout: 240_000 });
  ...
  const pr = await run('open PR form', 'gh', [
    'pr', 'create',
    '--repo', 'galyarderlabs/galyarder-design',
  ...
  const prUrl = extractFirstUrl(pr.stdout || pr.stderr) ?? `https://github.com/${target.owner}/galyarder-design/pull/new/${branch}`;
  emitPluginWorkflowResult(flags, {
    ok: true,
    action: 'galyarder-design-pr',
    folder: absFolder,
...
async function resolvePluginGithubTarget({ host = 'github.com', owner, manifest, purpose }) {
  ...
  if (!resolvedOwner) {
    ...
    if (apiError && isGhApiRateLimit(apiError)) {
      const ownerHint = purpose === 'galyarder-design-pr' ? '<github-login-or-fork-owner>' : '<github-login-or-org>';
      ...
      console.error(purpose === 'galyarder-design-pr'
...
function isPlaceholderRepoOwner(owner) {
  return /^(galyarder-design-user|<vendor>|vendor|example-user|your-org|your-username|owner|user|username)$/i.test(String(owner ?? '').trim());
}
...
function emitPluginWorkflowResult(flags, payload) {
  ...
  if (payload.action === 'galyarder-design-pr') {
    if (payload.ownerSource) console.log(`[galyarder-design-pr] owner resolved from ${payload.ownerSource}: ${payload.owner}`);
    if (payload.apiRateLimited) console.log('[galyarder-design-pr] GitHub API was rate limited; continued with the locally resolved owner.');
...
async function runPluginYank(rest) {
  ...
  if (rest.length === 0 || flags.help || flags.h) {
    console.log(`Usage:
  gd plugin yank <vendor/plugin-name>@<version> --reason "<why>" [--to galyarder-design] [--json]
...
marks a version unresolvable for new installs while preserving lockfile replay.`);
  ...
  if (!parsed.name || !parsed.range) {
    console.error('Usage: gd plugin yank <vendor/plugin-name>@<version> --reason "<why>"');
    process.exit(2);
  }
  if (!reason) {
  ...
  const target = flags.to ?? 'galyarder-design';
  if (target !== 'galyarder-design') {
    console.error('Only --to galyarder-design is supported in this v1 GitHub-backed yank flow.');
    process.exit(2);
  }
  ...
  const body = [
    ...
    '```',
    '',
    'Generated by `gd plugin yank`.',
  ].join('\n');
  const params = new URLSearchParams({ title, body });
  const payload = {
    catalog: 'galyarder-design',
    name: parsed.name,
    version: parsed.range,
    reason,
    url: `https://github.com/galyarderlabs/galyarder-design/issues/new?${params.toString()}`,
    body,
  };
  if (flags.json) {
    process.stdout.write(JSON.stringify(payload, null, 2) + '\n');
...
async function runPluginDoctor(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-') && a !== flags['daemon-url'] && a !== flags.source);
  if (!id) {
    console.error('Usage: gd plugin doctor <id> [--strict] [--json]');
    process.exit(2);
  }
...
// `gd plugin replay <runId> --snapshot-id <id>` — re-emit the immutable
...
// the agent restarts the run via `gd plugin apply` followed by a normal
// `gd run start`. Future Phase 2C `gd plugin run` will collapse this
...
async function runPluginReplay(rest) {
  ...
  const runId = rest.find((a) => !a.startsWith('-')
    ...
    && a !== flags.capabilities);
  if (!runId) {
    console.error('Usage: gd plugin replay <runId> --snapshot-id <id>');
    process.exit(2);
  }
  const snapshotId = flags['snapshot-id'];
  if (!snapshotId) {
    console.error('--snapshot-id is required (runs are in-memory in Phase 2A; pass the snapshot id returned by gd plugin apply)');
    process.exit(2);
  }
  ...
  console.log('[replay] re-apply via: gd plugin apply ' + data.rerun?.pluginId + ' --inputs ' + JSON.stringify(JSON.stringify(data.rerun?.inputs ?? {})));
}

// `gd plugin trust <id> --capabilities <comma-sep>` — flip a plugin's
...
async function runPluginTrust(rest) {
  ...
  const id = rest.find((a) => !a.startsWith('-')
    ...
    && a !== flags.capabilities);
  if (!id) {
    console.error('Usage: gd plugin trust <id> --capabilities connector:figma,connector:notion [--revoke]');
    process.exit(2);
  }
...
// ---------------------------------------------------------------------------
// Subcommand: gd ui …  (spec §10.3.4 headless GenUI surface inbox)
// ---------------------------------------------------------------------------
...
async function runUi(args) {
  ...
  switch (sub) {
    ...
    default:
      console.error(`unknown subcommand: gd ui ${sub}`);
      printUiHelp();
      process.exit(2);
  }
}
...
async function runUiList(rest) {
  ...
  if (flags.run) url = `${base}/api/runs/${encodeURIComponent(flags.run)}/genui`;
    ...
    console.error('Usage: gd ui list --run <runId> | --project <projectId>');
    process.exit(2);
  }
  const resp = await fetch(url);
  if (!resp.ok) {
...
async function runUiShow(rest) {
  ...
  if (!runId || !surfaceId) {
    console.error('Usage: gd ui show --run <runId> <surfaceId>');
    process.exit(2);
  }
  ...
  // `gd ui respond --value-json "$(...)"` in headless / agent flows.
  if (flags.schema) {
...
async function runUiRespond(rest) {
  ...
  if (!runId || !surfaceId) {
    console.error('Usage: gd ui respond --run <runId> <surfaceId> [--value <text> | --value-json <json> | --skip]');
    process.exit(2);
  }
  let value = null;
  if (flags.skip) {
...
async function runUiRevoke(rest) {
  ...
  if (!projectId || !surfaceId) {
    console.error('Usage: gd ui revoke --project <projectId> <surfaceId>');
    process.exit(2);
  }
...
async function runUiPrefill(rest) {
  ...
  if (!projectId || !surfaceId || !snapshotId) {
    console.error('Usage: gd ui prefill --project <projectId> --snapshot-id <id> <surfaceId> [--value <text> | --value-json <json>] [--persist run|conversation|project] [--kind form|choice|confirmation|oauth-prompt]');
    process.exit(2);
  }
  let value = null;
...
function printUiHelp() {
  console.log(`Usage:
  gd ui list  --run <runId>                          List GenUI surfaces for a run.
  gd ui list  --project <projectId>                  List GenUI surfaces for a project.
  gd ui show  --run <runId> <surfaceId> [--schema]   Read a single surface (kind / schema / value). --schema prints just the JSON Schema.
  gd ui respond --run <runId> <surfaceId> [--value <txt> | --value-json <json> | --skip]
                                                     Answer a pending surface from any process.
  gd ui revoke --project <projectId> <surfaceId>     Invalidate a project-tier cached answer.
  gd ui prefill --project <projectId> --snapshot-id <id> <surfaceId>
...
function printPluginHelp() {
  console.log(`Usage:
  gd plugin list [--task-kind <kind>]     List installed plugins (filterable).
  gd plugin search <query> [--tag <t>]    Search installed plugins by id/title/desc/tag.
  gd plugin stats [--json]                Inventory + snapshot health report.
  gd plugin info <id>                     Print a plugin's manifest + trust state as JSON.
  gd plugin manifest <id>                 Print only the parsed manifest JSON (no wrapper).
  gd plugin sources                       List distinct install sources + counts.
  gd plugin install --source <path>       Install a plugin from a local folder (Phase 1).
  gd plugin upgrade <id>                  Re-install a plugin from its recorded source.
  gd plugin uninstall <id>                Remove a plugin from the registry + on-disk staging.
  gd plugin apply <id> [--inputs <json>]  Compute an ApplyResult (preview) for a plugin.
  gd plugin doctor <id>                   Lint a plugin's manifest, atoms and resolved refs.
  gd plugin canon <snapshotId>            Print the canonical system-prompt block for a snapshot.
                                          (--check <file> for byte-equality fixtures.)
  gd plugin simulate <pluginId> [-s k=v]  Walk the plugin's pipeline against caller-supplied
                                          signals; report stage convergence + iterations
                                          (no LLM in the loop).
  gd plugin verify <pluginId>             CI meta-command: doctor + simulate + canon --check
                                          driven by an .gd-verify.json config in the plugin folder.
  gd plugin events tail [-f] [--kind k]   Tail the in-memory plugin event ring buffer.
  gd plugin events snapshot               One-shot read (filterable, no SSE).
  gd plugin events stats                  Roll-up: counts by kind / pluginId / time range.
  gd plugin events purge                  Drop every event in the buffer (loopback-only).
  gd plugin diff <a> <b> [--json]         Compare two installed plugins by id.
  gd plugin replay <runId> --snapshot-id <id>
                                          Re-emit the immutable snapshot a run launched against.
  gd plugin trust <id> --capabilities a,b
                                          Stage a capability grant (full mutation lands Phase 3).
  gd plugin validate <folder> [--json]    Lint a plugin folder before installing
                                          (manifest parse + atom + ref checks).
  gd plugin pack <folder> [--out <path>]  Build a .tgz archive of a plugin
                                          folder for distribution.
  gd plugin publish-repo <folder>         Create/update the author's public
                                          GitHub repo for a plugin folder.
  gd plugin galyarder-design-pr <folder>       Push a community-catalog branch and
                                          open the galyarderlabs/galyarder-design PR form.
  gd plugin publish <folder> --to galyarder-design|anthropics-skills|awesome-agent-skills|clawhub|skills-sh
                                          Prepare a registry submission link.
  gd plugin login [--host github.com]      Authenticate registry publishing via gh.
  gd plugin whoami [--host github.com]     Show the gh account used for publishing.
...
// ---------------------------------------------------------------------------
// Subcommand: gd project / gd run / gd files / gd conversation
//
...
// these so a code agent can drive Galyarder Design end-to-end without
...
async function runProject(args) {
  ...
  gd project info <id>                    Print one project.
  gd project delete <id>                  Delete a project.
  gd project editors                      List locally-installed editors that
                                          can open a project (hand-off targets).
  gd project open-in <id> --editor <slug> Open the project's working directory
                                          in the chosen editor (cursor, zed,
                                          vscode, finder, terminal, …).
  gd project handoff <id> --conversation <id> --api-key <key> --model <model>
                    [--base-url <url>] [--max-tokens <n>]
                    Synthesize a resume-conversation handoff prompt.

Common options:
  --daemon-url <url>   Galyarder Design daemon HTTP base.
  --json               Emit raw JSON.`);
    process.exit(args.length === 0 ? 2 : 0);
  }
  const sub = args[0];
  ...
  // parser below so a malformed `gd project handoff` invocation
  ...
  switch (sub) {
    case 'list': {
      ...
      if (projects.length === 0) {
        console.log('No projects. Create one with `gd project create --name "..."`.');
        return;
      }
    ...
    case 'info': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd project info <id>');
        process.exit(2);
      }
    ...
    case 'delete': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd project delete <id>');
        process.exit(2);
      }
    ...
    case 'open-in': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd project open-in <id> --editor <slug>');
        process.exit(2);
      }
      const editor = typeof flags.editor === 'string' ? flags.editor : '';
      if (!editor) {
        console.error('--editor <slug> is required. Run `gd project editors` to list options.');
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd project ${sub}`);
      process.exit(2);
  }
}
...
async function runRun(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd run start --project <projectId> [--conversation <id>] [--message "<text>"]
               [--plugin <id>] [--inputs <json>] [--grant-caps a,b]
               [--agent claude|codex|gemini] [--model <id>] [--follow] [--json]
  gd run watch  <runId>                     ND-JSON event stream on stdout.
  gd run cancel <runId>                     Request cancellation.
  gd run list   [--project <id>]            List recent runs.
  gd run info   <runId>                     One run's status.
...
Common options:
  ...
  switch (sub) {
    ...
    case 'info': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd run info <runId>');
        process.exit(2);
      }
    ...
    case 'cancel': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd run cancel <runId>');
        process.exit(2);
      }
    ...
    case 'watch': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd run watch <runId>');
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd run ${sub}`);
      process.exit(2);
  }
}
...
async function runFiles(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd files list   <projectId>                  List files in a project.
  gd files read   <projectId> <relpath>        Stream file bytes to stdout.
  gd files write  <projectId> <relpath> [< stdin]
                                               Write content from stdin.
  gd files upload <projectId> <localpath> [--as <relpath>]
                                               Upload a local file.
  gd files delete <projectId> <name>           Delete a project file.
...
Common options:
  ...
  switch (sub) {
    case 'list': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd files list <projectId>');
        process.exit(2);
      }
    ...
    case 'read': {
      ...
      if (!id || !rel) {
        console.error('Usage: gd files read <projectId> <relpath>');
        process.exit(2);
      }
    ...
    case 'upload': {
      ...
      if (!id || !localPath) {
        console.error('Usage: gd files upload <projectId> <localpath> [--as <relpath>]');
        process.exit(2);
      }
    ...
    case 'write': {
      ...
      const [id, rel] = positional;
      if (!id || !rel) {
        console.error('Usage: gd files write <projectId> <relpath> [< stdin]');
        process.exit(2);
      }
    ...
    case 'delete': {
      ...
      if (!id || !name) {
        console.error('Usage: gd files delete <projectId> <name>');
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd files ${sub}`);
      process.exit(2);
  }
}
...
async function runConversation(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd conversation list <projectId>           List conversations in a project.
  gd conversation info <conversationId>      Print one conversation.
...
Common options:
  ...
  switch (sub) {
    case 'list': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd conversation list <projectId>');
        process.exit(2);
      }
    ...
    case 'info': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd conversation info <conversationId>');
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd conversation ${sub}`);
      process.exit(2);
  }
}

// ---------------------------------------------------------------------------
// Subcommand: gd daemon  (Phase 1.5 lifecycle, plan §6 / §3.F2)
//
// `gd daemon start [--headless] [--serve-web] [--port <n>] [--host <addr>]`
//   - --headless: implies --no-open, never tries to launch a browser.
//                 The default `gd` (no subcommand) keeps its
...
//
// `gd daemon status [--json] [--daemon-url <url>]` calls /api/daemon/status.
// `gd daemon stop   [--daemon-url <url>]`         calls POST /api/daemon/shutdown.
// ---------------------------------------------------------------------------

async function runDaemon(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd daemon start [--headless] [--serve-web] [--port <n>] [--host <addr>] [--no-open]
                                          Start the daemon (Phase 1.5 headless mode).
  gd daemon status [--json] [--daemon-url <url>]
                                          Print the daemon's runtime snapshot.
  gd daemon stop   [--daemon-url <url>]   Send a graceful shutdown signal.
  gd daemon db     status                 Print SQLite path + size + table row counts.
  gd daemon db     verify [--quick]       Run integrity_check + foreign_key_check.
  gd daemon db     vacuum                 Run SQLite VACUUM to reclaim space after deletes.
...
Common options:
  ...
  switch (sub) {
    ...
    default:
      console.error(`unknown subcommand: gd daemon ${sub}`);
      process.exit(2);
  }
}

// Plan §3.GG1 — `gd daemon db status`. Prints a SQLite inventory
// (file path, size on disk, schema version, per-table row counts).
async function runDaemonDb(rest, flags) {
  const sub = rest[0];
  if (!sub || sub === 'help' || rest.includes('--help') || rest.includes('-h')) {
    console.log(`Usage:
  gd daemon db status [--json] [--daemon-url <url>]
  gd daemon db verify [--quick] [--json] [--daemon-url <url>]
  gd daemon db vacuum [--json] [--daemon-url <url>]

status:
  Prints a structured inventory of the daemon's SQLite backend:
    - file path (under .gd/ by default; GD_DATA_DIR overrides)
...
vacuum:
  ...
  if (sub !== 'status') {
    console.error(`unknown subcommand: gd daemon db ${sub}`);
    process.exit(2);
  }
...
async function runDaemonStart(flags) {
  ...
  const port = Number(flags.port ?? process.env.GD_PORT ?? 7456);
  const host = String(flags.host ?? process.env.GD_BIND_HOST ?? '127.0.0.1');
  const headless = Boolean(flags.headless || flags['no-open'] || flags['serve-web']);
  process.env.GD_BIND_HOST = host;
  process.env.GD_PORT = String(port);
  ...
  console.log(`[gd] listening on ${url} (${headless ? 'headless' : 'desktop'})`);
  if (!headless) {
    const opener = process.platform === 'darwin' ? 'open'
      : process.platform === 'win32' ? 'start'
      : 'xdg-open';
...
// ---------------------------------------------------------------------------
// Subcommand: gd atoms / gd skills / gd design-systems / gd craft / gd status
//
...
async function runAtoms(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd atoms list             List first-party atoms (implemented + planned).
  gd atoms show <id>        Print one atom's metadata.
  gd atoms info <id>        Print metadata + the bundled SKILL.md body.
...
Common options:
  ...
  switch (sub) {
    ...
    case 'show': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd atoms show <id>');
        process.exit(2);
      }
      const resp = await fetch(`${base}/api/atoms`);
      if (!resp.ok) return structuredHttpFailure(resp);
    ...
    case 'info': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error('Usage: gd atoms info <id>');
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd atoms ${sub}`);
      process.exit(2);
  }
}
...
async function runLibraryList(name, args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd ${name} list           List ${name}.
  gd ${name} show <id>      Print one entry.`);
    process.exit(args.length === 0 ? 2 : 0);
  }
  const sub = args[0];
  const rest = args.slice(1);
  ...
  switch (sub) {
    ...
    case 'show': {
      const id = rest.find((a) => !a.startsWith('-'));
      if (!id) {
        console.error(`Usage: gd ${name} show <id>`);
        process.exit(2);
      }
    ...
    default:
      console.error(`unknown subcommand: gd ${name} ${sub}`);
      process.exit(2);
  }
}
...
async function runStatus(args) {
  // Alias of `gd daemon status`.
  return runDaemon(['status', ...args]);
}

// ---------------------------------------------------------------------------
// Subcommand: gd diagnostics export <path> [--json]
//
...
// `gd doctor` follow-ups, shell scripts) can collect a support bundle
...
async function runDiagnostics(args) {
  ...
  if (!sub || sub === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd diagnostics export [<path>] [--output <path>] [--json] [--daemon-url <url>]
...
diagnostics produces.
  ...
  <path>                 Where to write the zip. Defaults to
                         ./galyarder-design-diagnostics-<timestamp>.zip in the
  ...
  if (sub !== 'export') {
    console.error(`unknown subcommand: gd diagnostics ${sub}`);
    process.exit(2);
  }
  ...
  const { DIAGNOSTICS_EXPORT_PATH, DIAGNOSTICS_FILENAME_PREFIX, diagnosticsFileName } =
    await import('@galyarder-design/diagnostics');
  const fs = await import('node:fs/promises');
  const path = await import('node:path');
...
// ---------------------------------------------------------------------------
// Subcommand: gd doctor / gd config (Phase 4 CLI parity tail).
//
// Plan §3.I2 / spec §12.2.
//
// `gd doctor` — repo-wide diagnostics. Hits /api/daemon/status, lists
...
//
// `gd config get/set/list/unset` — wraps GET/PUT /api/app-config so a
...
async function runDoctor(args) {
  ...
  if (flags.help || flags.h) {
    console.log(`Usage:
  gd doctor [--json]   Print a daemon + plugin + design-library health summary.
...
async function runConfig(args) {
  if (args.length === 0 || args[0] === 'help' || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage:
  gd config list                      Print the full app config as JSON.
  gd config get <key>                 Print one top-level key.
  gd config set <key> <value>         Set a top-level key (string / number / boolean).
  gd config set <key> --value-json '<json>'
                                       Set a key to a JSON value.
  gd config unset <key>               Remove a top-level key.
...
Common options:
  ...
  switch (sub) {
    ...
    case 'set': {
      ...
      if (!key) {
        console.error('Usage: gd config set <key> <value> | gd config set <key> --value-json <json>');
        process.exit(2);
      }
      let parsed;
      if (typeof flags['value-json'] === 'string') {
    ...
    case 'unset': {
      const key = rest.find((a) => !a.startsWith('-'));
      if (!key) {
        console.error('Usage: gd config unset <key>');
        process.exit(2);
      }
      const cfg = await fetchConfig();
      const next = { ...cfg };
    ...
    default:
      console.error(`unknown subcommand: gd config ${sub}`);
      process.exit(2);
  }
}

// ---------------------------------------------------------------------------
// Subcommand: gd memory …
//
...
async function runMemory(args) {
  ...
  const topic = args[0];
  if (topic !== 'tree') {
    console.error(`unknown subcommand: gd memory ${topic}`);
    printMemoryHelp();
    process.exit(2);
  }
  const rest = args.slice(1);
  ...
  if (action === 'view') {
    const id = parts[1];
    if (!id) {
      console.error('Usage: gd memory tree view <id>');
      process.exit(2);
    }
  ...
  if (action === 'edit') {
    const id = parts[1];
    if (!id) {
      console.error('Usage: gd memory tree edit <id> [--name ...] [--description ...] [--type ...] [--body ...|--body-file ...]');
      process.exit(2);
    }
    const body = {};
  ...
  if (action === 'move') {
    ...
    if (!id || !type) {
      console.error('Usage: gd memory tree move <id> --type user|feedback|project|reference');
      process.exit(2);
    }
  ...
  console.error(`unknown subcommand: gd memory tree ${action}`);
  printMemoryHelp();
  process.exit(2);
}

// ---------------------------------------------------------------------------
// Subcommand: gd automation …
//
...
function printAutomationHelp() {
  console.log(`Usage:
  gd automation template list                                List built-in automation templates.
  gd automation template get <id>                            Print one built-in automation template.
  gd automation source ingest --source-kind <kind> --title <title>
                              [--source-ref <ref>] [--template <id>]
                              [--body <markdown> | --body-file <path|->]
                              [--connector <id>] [--compression off|balanced|aggressive]
                              [--json]
  gd automation source list [--limit 20] [--json]             List ingested source packets.
  gd automation source get <id> [--json]                      Print one source packet.
  gd automation proposal list [--status pending-review]       List self-evolution proposals.
  gd automation proposal get <id>                             Print one proposal.
  gd automation proposal apply <id>                           Apply a reviewable proposal.
  gd automation proposal reject <id> [--reason "<why>"]       Reject a reviewable proposal.
  gd automation list                                         List automations.
  gd automation get <id>                                     Print one automation.
  gd automation create --name "<title>" --prompt "<text>"
                       --schedule <spec>
                       ...
                       [--agent <id>]
  gd automation update <id> [--name ...] [--prompt ...]
                            [--schedule ...] [--target ...]
                            [--skill ...] [--plugin ...] [--mcp ...]
                            [--connector ...] [--enabled|--disabled]
                            Patch fields.
  gd automation run <id>                                       Trigger a manual run; prints projectId/conversationId.
  gd automation runs <id> [--limit 10]                         Print run history.
  gd automation crystallize-run <routineId> <runId> [--json]    Turn a succeeded run into skill/memory proposals.
  gd automation pause <id>                                     Mark disabled.
  gd automation resume <id>                                    Mark enabled.
  gd automation delete <id>                                    Remove the automation (history retained).
...
async function runAutomation(args) {
  ...
  const requireId = (label) => {
    ...
    if (!id) {
      console.error(`Usage: gd automation ${label} <id>`);
      process.exit(2);
    }
    return id;
  };
  ...
  switch (sub) {
    ...
    case 'templates': {
      ...
      if (action === 'get') {
        const id = parts[1];
        if (!id) {
          console.error('Usage: gd automation template get <id>');
          process.exit(2);
        }
        let resp;
        try {
      ...
      console.error(`unknown subcommand: gd automation template ${action}`);
      printAutomationHelp();
      process.exit(2);
    }
    case 'ingest':
    ...
    case 'sources': {
      ...
      if (action === 'ingest') {
        ...
        if (!sourceKind) {
          console.error('Usage: gd automation source ingest --source-kind <kind> --body-file <path|->');
          process.exit(2);
        }
      ...
      if (action === 'get') {
        const id = parts[1];
        if (!id) {
          console.error('Usage: gd automation source get <id>');
          process.exit(2);
        }
        let resp;
        try {
      ...
      console.error(`unknown subcommand: gd automation source ${action}`);
      printAutomationHelp();
      process.exit(2);
    }
    case 'proposal':
    case 'proposals': {
      ...
      if (action === 'get') {
        const id = parts[1];
        if (!id) {
          console.error('Usage: gd automation proposal get <id>');
          process.exit(2);
        }
        let resp;
        try {
      ...
      if (action === 'apply' || action === 'reject') {
        const id = parts[1];
        if (!id) {
          console.error(`Usage: gd automation proposal ${action} <id>`);
          process.exit(2);
        }
        let resp;
        try {
      ...
      console.error(`unknown subcommand: gd automation proposal ${action}`);
      printAutomationHelp();
      process.exit(2);
    }
    case 'list': {
      ...
      if (routines.length === 0) {
        console.log('No automations. Create one with `gd automation create --name "..." --prompt "..." --schedule daily:09:00`.');
        return;
      }
    ...
    case 'crystallize-run': {
      ...
      const runId = parts[1];
      if (!routineId || !runId) {
        console.error('Usage: gd automation crystallize-run <routineId> <runId> [--json]');
        process.exit(2);
      }
      let resp;
      try {
    ...
    default:
      console.error(`unknown subcommand: gd automation ${sub}`);
      printAutomationHelp();
      process.exit(2);
  }
}
