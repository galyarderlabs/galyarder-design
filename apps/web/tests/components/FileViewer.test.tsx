...
function srcDocActivationMessages(calls: readonly (readonly unknown[])[]) {
  return calls
    .map(([message]) => message)
    .filter((message): message is { type: 'gd:srcdoc-transport-activate'; html: string } => {
      if (typeof message !== 'object' || message === null) return false;
      const data = message as { type?: unknown; html?: unknown };
      return data.type === 'gd:srcdoc-transport-activate' && typeof data.html === 'string';
    });
}
...
describe('FileViewer SVG artifacts', () => {
  ...
  it('URL-loads a plain HTML preview iframe instead of inlining via srcDoc', () => {
    ...
    // booting the artifact in the inactive frame). `data-gd-active` and
    ...
    expect(markup).toContain('data-gd-render-mode="url-load"');
    expect(markup).toContain('data-gd-render-mode="url-load" data-gd-active="true"');
    expect(markup).toContain('data-gd-render-mode="srcdoc" data-gd-active="false"');
  ...
  it('keeps inactive HTML preview transports mounted without booting the artifact', async () => {
    ...
    const { container } = render(
      <FileViewer
        ...
        file={file}
        liveHtml='<html><body><script>window.__odArtifactBootCount = (window.__odArtifactBootCount || 0) + 1;</script><main data-gd-id="hero">Hero</main></body></html>'
      />,
    );

    const urlFrame = container.querySelector('iframe[data-gd-render-mode="url-load"]') as HTMLIFrameElement | null;
    const srcDocFrame = container.querySelector('iframe[data-gd-render-mode="srcdoc"]') as HTMLIFrameElement | null;

    expect(urlFrame).toBeTruthy();
    expect(srcDocFrame).toBeTruthy();
    expect(urlFrame?.getAttribute('data-gd-active')).toBe('true');
    expect(srcDocFrame?.getAttribute('data-gd-active')).toBe('false');
    expect(srcDocFrame?.srcdoc).toContain('data-gd-lazy-srcdoc-transport');
    ...
    const urlFrameAfter = container.querySelector('iframe[data-gd-render-mode="url-load"]') as HTMLIFrameElement | null;
    const srcDocFrameAfter = container.querySelector('iframe[data-gd-render-mode="srcdoc"]') as HTMLIFrameElement | null;

    expect(urlFrameAfter).toBe(urlFrame);
    expect(srcDocFrameAfter).toBe(srcDocFrame);
    expect(urlFrameAfter?.getAttribute('data-gd-active')).toBe('false');
    expect(urlFrameAfter?.getAttribute('src')).toBe('about:blank');
    expect(srcDocFrameAfter?.getAttribute('data-gd-active')).toBe('true');
    expect(srcDocFrameAfter?.srcdoc).toContain('data-gd-lazy-srcdoc-transport');
    ...
    await waitFor(() => {
      ...
      expect(activations.at(-1)?.html).toContain('data-gd-selection-bridge');
    });
  });
  ...
  it('allows downloads in the in-tab HTML presentation iframe', async () => {
    ...
    await waitFor(() => {
      ...
      expect(frame?.getAttribute('data-gd-render-mode')).toBe('url-load');
    });
  });
  ...
  it('keeps decks on the srcDoc path so the deck postMessage bridge can run', () => {
    ...
    expect(markup).toContain('data-testid="artifact-preview-frame"');
    expect(markup).toContain('data-gd-render-mode="srcdoc"');
    expect(markup).toContain('data-gd-render-mode="srcdoc" data-gd-active="true"');
    expect(markup).toContain('data-gd-render-mode="url-load" data-gd-active="false"');
    expect(markup).toContain('sandbox="allow-scripts allow-downloads"');
  });
...
describe('FileViewer tweaks toolbar', () => {
  ...
  it('renders the toolbar Draw entry alongside restored Comment and Inspect entries', () => {
    render(
      <FileViewer projectId="project-1" projectKind="prototype" file={htmlPreviewFile()}
        liveHtml='<html><body><main data-gd-id="hero">Hero</main></body></html>'
      />,
    );
  ...
  it('shows an inspect notice when a clicked child resolves to an annotated ancestor', async () => {
    render(
      <FileViewer projectId="project-1" projectKind="prototype" file={htmlPreviewFile()}
        liveHtml='<html><body><main data-gd-id="hero"><h1>Hero</h1></main></body></html>'
      />,
    );
    ...
    window.dispatchEvent(new MessageEvent('message', {
      ...
      data: {
        type: 'gd:comment-target',
        elementId: 'hero',
        selector: '[data-gd-id="hero"]',
        label: 'main',
        text: 'Hero',
        style: {},
        clickedDescendant: {
  ...
  it('keeps the preloaded selection bridge mounted while the Draw bar switches to click mode', async () => {
    ...
    expect((screen.getByTestId('artifact-preview-frame') as HTMLIFrameElement).getAttribute('data-gd-render-mode')).toBe('url-load');
    ...
    const frame = await waitFor(() => {
      ...
      expect(activeFrame.getAttribute('data-gd-render-mode')).toBe('srcdoc');
      expect(activeFrame.srcdoc).toContain('data-gd-lazy-srcdoc-transport');
      return activeFrame;
    });
    await waitFor(() => {
      expect(srcDocActivationMessages(postMessageSpy.mock.calls).at(-1)?.html).toContain('data-gd-selection-bridge');
    });
  ...
  it('hides non-open saved comments from preview markers when the side panel is empty', () => {
    const resolvedComment: PreviewComment = {
      ...
      selector: '[data-gd-pin="pin-applying"]',
      label: 'pin-applying',
      text: '',
      htmlHint: '',
  ...
  it('keeps saved comment marker numbers aligned with the side panel order', () => {
    const olderComment: PreviewComment = {
      ...
      filePath: 'preview.html',
      elementId: 'pin-older',
      selector: '[data-gd-pin="pin-older"]',
      label: 'pin-older',
      text: '',
      htmlHint: '',
      position: { x: 24, y: 32, width: 18, height: 18 },
    ...
    const newerComment: PreviewComment = {
      ...
      id: 'comment-newer',
      elementId: 'pin-newer',
      selector: '[data-gd-pin="pin-newer"]',
      label: 'pin-newer',
  ...
  it('does not preload non-open element comments into the picker composer', async () => {
    const applyingElementComment: PreviewComment = {
      ...
      label: 'Hero',
      text: 'Hero',
      htmlHint: '<main data-gd-id="hero">Hero</main>',
  ...
  it('closes an open saved-comment composer when that comment leaves the open state', async () => {
    const openComment: PreviewComment = {
      ...
      selector: '[data-gd-pin="pin-transition"]',
          ...
          comments={[
            {
              ...
              selector: '[data-gd-id="button.sso-btn"]',
              label: 'button.sso-btn',
              text: 'GitHub',
              htmlHint: '<button>GitHub</button>',
              position: { x: 16, y: 24, width: 160, height: 48 },
              // CJK fixture; '不要github，换成微信' (don't use GitHub,
              // switch to WeChat) is sample comment content verifying
              // the comment side-panel renders non-ASCII unmodified.
              note: '不要github，换成微信',
              status: 'open',
              createdAt: Date.now(),
              updatedAt: Date.now(),
  ...
  // https://github.com/galyarderlabs/galyarder-design/pull/1643#discussion_r3266838151.
  ...
  // https://github.com/galyarderlabs/galyarder-design/pull/1643#discussion_r3269955351.
...
describe('applyInspectOverridesToSource', () => {
  const base = `<!doctype html><html><head><title>X</title></head><body><main data-gd-id="hero">Hi</main></body></html>`;
  const css = `[data-gd-id="hero"] { color: #ff0000 !important }`;

  it('inserts the overrides block before </head>', () => {
    const next = applyInspectOverridesToSource(base, css);
    expect(next).toContain('<style data-gd-inspect-overrides>');
    expect(next).toContain('color: #ff0000');
    expect(next.indexOf('<style data-gd-inspect-overrides>')).toBeLessThan(next.indexOf('</head>'));
  });

  it('replaces an existing overrides block instead of duplicating', () => {
    const once = applyInspectOverridesToSource(base, css);
    const twice = applyInspectOverridesToSource(once, `[data-gd-id="hero"] { color: #00ff00 !important }`);
    const matches = twice.match(/<style data-gd-inspect-overrides>/g) ?? [];
  ...
  it('strips the overrides block when called with empty css', () => {
    ...
    expect(stripped).not.toContain('data-gd-inspect-overrides');
  });

  it('handles fragments without an explicit <head>', () => {
    const next = applyInspectOverridesToSource('<main data-gd-id="x">x</main>', css);
    expect(next).toContain('<style data-gd-inspect-overrides>');
    expect(next.indexOf('<style data-gd-inspect-overrides>')).toBeLessThan(next.indexOf('<main'));
  });

  // Regression for galyarderlabs/galyarder-design#362: if a source file has more than
  ...
  it('removes every existing overrides block, not just the first', () => {
    const dup = `<!doctype html><html><head>` +
      `<style data-gd-inspect-overrides>[data-gd-id="hero"] { color: #ff0000 !important }</style>` +
      `<style data-gd-inspect-overrides>[data-gd-id="hero"] { color: #00ff00 !important }</style>` +
      `<title>X</title></head><body><main data-gd-id="hero">Hi</main></body></html>`;
    const replaced = applyInspectOverridesToSource(dup, `[data-gd-id="hero"] { color: #0000ff !important }`);
    const matches = replaced.match(/<style data-gd-inspect-overrides>/g) ?? [];
    expect(matches).toHaveLength(1);
    ...
    expect(cleared).not.toContain('data-gd-inspect-overrides');
  });

  // Regression for galyarderlabs/galyarder-design#362: the splicer must be HTML-aware
  ...
  // `</head>` or `<style data-gd-inspect-overrides>` as text, e.g. a
  ...
  it('ignores </head> literals inside inline <script> and <style>', () => {
    ...
    const blockIdx = next.indexOf('<style data-gd-inspect-overrides>');
    ...
    const blockMatches = next.match(/<style data-gd-inspect-overrides>/g) ?? [];
    expect(blockMatches).toHaveLength(1);
  });

  it('ignores `<style data-gd-inspect-overrides>` literals inside <script>', () => {
    ...
    const sourceWithLiteral =
      `<!doctype html><html><head>` +
      `<script>const banner = "<style data-gd-inspect-overrides>color: red</style>";</script>` +
      `<title>X</title></head><body><main data-gd-id="hero">Hi</main></body></html>`;
    const next = applyInspectOverridesToSource(sourceWithLiteral, css);
    // The literal must survive verbatim inside the script body.
    expect(next).toContain('const banner = "<style data-gd-inspect-overrides>color: red</style>";');
    // The output still gains exactly one real override block.
    const blockMatches = next.match(/<style data-gd-inspect-overrides>\n\[data-gd-id="hero"\]/g) ?? [];
    expect(blockMatches).toHaveLength(1);
    // Stripping with empty css must NOT touch the script literal.
    const stripped = applyInspectOverridesToSource(sourceWithLiteral, '');
    expect(stripped).toContain('const banner = "<style data-gd-inspect-overrides>color: red</style>";');
    ...
    // override block.
    const allMatches = stripped.match(/data-gd-inspect-overrides/g) ?? [];
    expect(allMatches).toHaveLength(1);
  });

  // Regression for galyarderlabs/galyarder-design#362: the splicer must look at real
  // attribute names, not just substring-match the marker text against the
  // whole opening tag. A `\bdata-gd-inspect-overrides\b` regex over the
  ...
  it('does not strip <style> blocks whose attribute name only PREFIXES the marker', () => {
    const css2 = `[data-gd-id="hero"] { color: #00ffaa !important }`;
    const userBlock = `body { background: red !important }`;
    const sourceWithLongerName =
      `<!doctype html><html><head>` +
      // attribute is named data-gd-inspect-overrides-note, NOT the marker.
      // The note shouldn't be treated as an Inspect-owned style block.
      `<style data-gd-inspect-overrides-note="docs">${userBlock}</style>` +
      `<title>X</title></head><body><main data-gd-id="hero">Hi</main></body></html>`;
    ...
    expect(next).toContain('<style data-gd-inspect-overrides-note="docs">');
    ...
    expect(stripped).toContain('<style data-gd-inspect-overrides-note="docs">');
    expect(stripped).toContain(userBlock);
    expect(stripped).not.toContain('<style data-gd-inspect-overrides>');
  });
  ...
  it('does not strip <style> blocks that only mention the marker inside an attribute value', () => {
    ...
    const sourceWithMarkerInValue =
      `<!doctype html><html><head>` +
      // The literal text data-gd-inspect-overrides appears as an attribute
      // VALUE on a normal <style title="..."> — there is no real override
      // marker here, so the splicer must keep the block.
      `<style title="data-gd-inspect-overrides">${userBlock}</style>` +
      `<title>X</title></head><body><main data-gd-id="hero">Hi</main></body></html>`;
    const next = applyInspectOverridesToSource(sourceWithMarkerInValue, css2);
    expect(next).toContain('<style title="data-gd-inspect-overrides">');
    expect(next).toContain(userBlock);
    const blockMatches = next.match(/<style data-gd-inspect-overrides>/g) ?? [];
    expect(blockMatches).toHaveLength(1);
    const stripped = applyInspectOverridesToSource(sourceWithMarkerInValue, '');
    expect(stripped).toContain('<style title="data-gd-inspect-overrides">');
    expect(stripped).toContain(userBlock);
    expect(stripped).not.toContain('<style data-gd-inspect-overrides>');
  });

  it('still strips a real <style data-gd-inspect-overrides> block with assigned value', () => {
    // The marker is allowed both as a boolean attribute and with an
    // assigned value (`<style data-gd-inspect-overrides="">`). The splicer
    // must treat both as the override block, not just the boolean shape.
    const sourceWithValuedMarker =
      `<!doctype html><html><head>` +
      `<style data-gd-inspect-overrides="">` +
      `[data-gd-id="hero"] { color: #ff0000 !important }` +
      `</style>` +
  ...
  it('ignores </head> inside <textarea> and <title> raw-text elements', () => {
    ...
    const sourceWithTextarea =
      ...
      `<main data-gd-id="hero">Hi</main></body></html>`;
...
describe('serializeInspectOverrides', () => {
  it('emits validated declarations for legitimate overrides', () => {
    const out = serializeInspectOverrides({
      hero: { selector: '[data-gd-id="hero"]', props: { color: '#ff0000', 'font-size': '18px' } },
    });
    expect(out).toContain('[data-gd-id="hero"]');
  ...
  it('honours data-screen-label entries the bridge tagged that way', () => {
    ...
    expect(out).not.toContain('[data-gd-id="hero"]');
  });

  // Regression for galyarderlabs/galyarder-design#362: standard deck slides ship as
  ...
  // outright (or silently rewritten to `[data-gd-id="..."]`) and reload
  ...
  it('preserves data-screen-label values with whitespace and leading digits', () => {
    ...
    expect(out).not.toContain('[data-gd-id="01 Cover"]');
    expect(out).toContain('color: #ff0000 !important');
  ...
  it('rejects non-allow-listed properties', () => {
    const out = serializeInspectOverrides({
      hero: { selector: '[data-gd-id="hero"]', props: { position: 'absolute', color: '#fff' } },
    });
  ...
  // 'gd:inspect-overrides', overrides, css: '</style><script>...</script>' })
  ...
  it('refuses to surface a forged </style><script> payload', () => {
    const forged = {
      ...
      '"></style><script>alert(2)</script>': {
        selector: '[data-gd-id="x"]',
        props: { color: '#fff' },
      },
      // Hostile value: rejected by UNSAFE_VALUE.
      villain: {
        selector: '[data-gd-id="villain"]',
    ...
    expect(out).toContain('[data-gd-id="hero"] { color: #fff !important }');
...
// Regression for galyarderlabs/galyarder-design#362: the host owns the inspect override
...
// different element, and forging the iframe's gd:inspect-overrides reply
...
describe('parseInspectOverridesFromSource', () => {
  ...
  it('parses an existing override block into the host map', () => {
    const source =
      `<!doctype html><html><head>` +
      `<style data-gd-inspect-overrides>` +
      `[data-gd-id="hero"] { color: #ff0000 !important; font-size: 18px !important }` +
    ...
    expect(map.hero?.selector).toBe('[data-gd-id="hero"]');
  ...
  it('aggregates rules across multiple persisted blocks', () => {
    const source =
      `<style data-gd-inspect-overrides>[data-gd-id="a"] { color: #111 !important }</style>` +
      `<style data-gd-inspect-overrides>[data-gd-id="b"] { color: #222 !important }</style>`;
    const map = parseInspectOverridesFromSource(source);
  ...
  it('drops disallowed properties and rules whose only declarations are unsafe', () => {
    const source =
      `<style data-gd-inspect-overrides>` +
      `[data-gd-id="hero"] { position: absolute !important; color: #fff !important }` +
      `[data-gd-id="bad"] { background: red } ` +
      `</style>`;
    const map = parseInspectOverridesFromSource(source);
  ...
  it('refuses elementIds whose characters could break out of the attr value', () => {
    const hostile =
      `<style data-gd-inspect-overrides>` +
      `[data-gd-id="\"><script>alert(1)</script>"] { color: #fff !important }` +
      `</style>`;
  ...
  // Regression for galyarderlabs/galyarder-design#362: hydration must require an
  // actual `data-gd-inspect-overrides` attribute name, not a boundary-only
  ...
  it('does not seed phantom overrides from a longer attribute name', () => {
    const source =
      `<!doctype html><html><head>` +
      `<style data-gd-inspect-overrides-note="docs">` +
      `[data-gd-id="hero"] { color: #ff0000 !important }` +
      `</style></head><body></body></html>`;
    expect(parseInspectOverridesFromSource(source)).toEqual({});
  });
  ...
  it('does not seed phantom overrides when the marker text only appears in an attribute value', () => {
    const source =
      `<!doctype html><html><head>` +
      `<style title="data-gd-inspect-overrides">` +
      `[data-gd-id="hero"] { color: #ff0000 !important }` +
      `</style></head><body></body></html>`;
    expect(parseInspectOverridesFromSource(source)).toEqual({});
  });
  ...
  it('still parses a real override block when raw-text literals also mention one', () => {
    const phantomBlock =
      `<style data-gd-inspect-overrides>` +
      `[data-gd-id="phantom"] { color: #ff0000 !important }` +
      `</style>`;
    const source =
      `<!doctype html><html><head>` +
      `<script>const tmpl = \`${phantomBlock}\`;</script>` +
      `<style data-gd-inspect-overrides>` +
      `[data-gd-id="hero"] { color: #00ff00 !important }` +
      `</style>` +
      `</head><body></body></html>`;
    const map = parseInspectOverridesFromSource(source);
...
describe('updateInspectOverride', () => {
  const base: InspectOverrideMap = {
    hero: { selector: '[data-gd-id="hero"]', props: { color: '#ff0000' } },
  };

  it('adds a new property to an existing entry', () => {
    const next = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'font-size', '18px');
    expect(next).not.toBe(base);
    expect(next.hero?.props).toEqual({ color: '#ff0000', 'font-size': '18px' });
  });

  it('creates a new entry for a previously untouched element', () => {
    const next = updateInspectOverride(base, 'cta', '[data-gd-id="cta"]', 'color', '#00ff00');
  ...
  it('clears a single property when given an empty value', () => {
    const seeded = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'font-size', '18px');
    const cleared = updateInspectOverride(seeded, 'hero', '[data-gd-id="hero"]', 'font-size', '');
    expect(cleared.hero?.props).toEqual({ color: '#ff0000' });
  });

  it('drops the entry once the last property is cleared', () => {
    const cleared = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'color', '');
    expect(cleared.hero).toBeUndefined();
  });

  it('returns the same map reference when the change is a no-op', () => {
    const same = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'color', '#ff0000');
    expect(same).toBe(base);
    const noClear = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'font-size', '');
    expect(noClear).toBe(base);
  });

  it('rejects properties off the host allow-list', () => {
    const ignored = updateInspectOverride(base, 'hero', '[data-gd-id="hero"]', 'position', 'absolute');
...
describe('LiveArtifactRefreshHistoryPanel', () => {
  ...
  it('renders Indonesian strings end-to-end when wrapped in I18nProvider initial="id"', () => {
    const now = Date.now();
    const markup = renderToStaticMarkup(
    ...
    // the Indonesian past-tense suffix `lalu` and rule out the English
    // suffixes the legacy function emitted.
    expect(markup).toContain('lalu');
  ...
  it('renders the id empty hero ("Belum pernah") when lastRefreshedAt is missing', () => {
    const markup = renderToStaticMarkup(
      <I18nProvider initial="id">
        <LiveArtifactRefreshHistoryPanel
