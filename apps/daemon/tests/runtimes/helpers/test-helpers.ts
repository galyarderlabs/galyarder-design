...
const originalDisablePlugins = process.env.GD_CODEX_DISABLE_PLUGINS;
const originalPath = process.env.PATH;
const originalHome = process.env.HOME;
const originalAgentHome = process.env.GD_AGENT_HOME;
const originalDaemonUrl = process.env.GD_DAEMON_URL;
const originalToolToken = process.env.GD_TOOL_TOKEN;
...
afterEach(() => {
  if (originalDisablePlugins == null) {
    ...
    process.env.GD_CODEX_DISABLE_PLUGINS = originalDisablePlugins;
  }
  ...
  if (originalDaemonUrl == null) {
    delete process.env.GD_DAEMON_URL;
  } else {
    process.env.GD_DAEMON_URL = originalDaemonUrl;
  }
  if (originalToolToken == null) {
    delete process.env.GD_TOOL_TOKEN;
  } else {
    process.env.GD_TOOL_TOKEN = originalToolToken;
  }
