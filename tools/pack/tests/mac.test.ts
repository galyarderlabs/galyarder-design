...
describe("resolveSeededAppConfigPaths", () => {
  ...
  it("resolves relative GD_DATA_DIR against the workspace root", () => {
    process.env.GD_DATA_DIR = "e2e/ui/.gd-data";
    const config = makeConfig("/work");
    expect(resolveSeededAppConfigPaths(config)).toEqual({
      sourcePath: resolve("/work", "e2e", "ui", ".gd-data", "app-config.json"),
      targetPath: join("/work", ".tmp", "tools-pack", "runtime", "mac", "namespaces", "local-test", "data", "app-config.json"),
    });
  });

  it("expands $HOME-style GD_DATA_DIR values", () => {
    process.env.GD_DATA_DIR = "$HOME/.galyarder-design";
    const config = makeConfig("/work");
    expect(resolveSeededAppConfigPaths(config)).toEqual({
      sourcePath: join(os.homedir(), ".galyarder-design", "app-config.json"),
      targetPath: join("/work", ".tmp", "tools-pack", "runtime", "mac", "namespaces", "local-test", "data", "app-config.json"),
    });
  });
});
...
describe("seedPackagedAppConfig", () => {
  it("copies the current app-config into the packaged runtime namespace", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-tools-pack-mac-"));
    try {
      const config = makeConfig(root);
      const sourceDir = join(root, ".gd");
...
describe("writeLaunchPackagedConfig", () => {
  it("injects the tools-pack runtime namespace root without mutating the packaged app config", async () => {
    ...
    try {
      ...
      const appPath = join(root, "Galyarder Design.app");
      const embeddedConfigPath = join(appPath, "Contents", "Resources", "galyarder-design-config.json");
      ...
      expect(launchConfigPath).toBe(join(config.roots.runtime.namespaceRoot, "runtime", "galyarder-design-config.json"));
