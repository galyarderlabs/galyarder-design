// Phase 4 / spec §14.1 — `gd plugin scaffold` starter folder generator.
//
// Pure, file-system-side helper that materialises the §17.2 "enriched
// plugin" shape on disk: SKILL.md (canonical anchor, with the gd:
// frontmatter the skills protocol expects) + galyarder-design.json (sidecar
// with the v1 schema reference). Authors can drop the result into a
// new git repo and start iterating immediately; `gd plugin install ./<id>`
...
export async function scaffoldPlugin(input: ScaffoldInput): Promise<ScaffoldResult> {
  ...
  try {
    ...
    const conflicts = entries.filter((e) =>
      e === 'SKILL.md' || e === 'galyarder-design.json' || e === '.claude-plugin' || e === 'README.md',
    );
  ...
  const manifestPath = path.join(folder, 'galyarder-design.json');
  ...
  const readme = [
    ...
    '',
    '```bash',
    `gd plugin install ./${input.id}`,
    `gd plugin apply ${input.id} --input audience=VC`,
    '```',
    '',
    '## Files',
    '',
    '- `SKILL.md` — the canonical agent skill body.',
    '- `galyarder-design.json` — the versioned Galyarder Design marketplace sidecar.',
    '',
    'Edit `SKILL.md` to teach the agent how to perform the workflow.',
    'Edit `galyarder-design.json` to refine the marketplace card and inputs.',
    '',
  ].join('\n');
