 ...
 * shell: `<div class="gd-loading-shell">Loading Galyarder Design…</div>`. That
...
beforeEach(() => {
  ...
  document.documentElement.removeAttribute('data-gd-app-mounted');
});
...
describe('observability/white-screen', () => {
  it('fires client_white_screen when only the dynamic-import loading shell is in the DOM after the timeout', () => {
    ...
    // "Loading Galyarder Design…" is longer than the legacy 10-char floor.
    const shell = document.createElement('div');
    shell.className = 'gd-loading-shell';
    shell.textContent = 'Loading Galyarder Design…';
  ...
  it('does NOT fire when the app sets the data-gd-app-mounted marker before the timeout', () => {
