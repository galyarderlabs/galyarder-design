...
// (`.gd-skills/<folder>/...`). The chat handler invokes
...
// `.gd-skills/<id>/SKILL.md` resolves through the symlink and mutates
// the shipped resource itself. Per-project copies eliminate that
// channel — every byte under `.gd-skills/` is a private working copy,
...
export const SKILLS_CWD_ALIAS = '.gd-skills';
...
/**
 * Copy `<sourceDir>` to `<cwd>/.gd-skills/<folderName>/` so an agent can
...
export async function stageActiveSkill(
  ...
  // The alias root is GD-reserved. If the user (or some unrelated tool)
  ...
  try {
    ...
    if (aliasStat.isSymbolicLink()) {
      log(
        `[gd] skill-stage: replacing legacy symlink at ${aliasRoot} with a real directory`,
      );
      await rm(aliasRoot, { recursive: true, force: true });
    } else if (!aliasStat.isDirectory()) {
      log(
        `[gd] skill-stage: ${aliasRoot} exists and is not a directory; refusing to stage`,
      );
      return {
        staged: false,
  ...
  try {
    ...
    await cp(sourceDir, stagedPath, {
      ...
      // `.gd-skills/...` can resolve back to a real file outside the
      // project cwd.
      dereference: true,
    ...
    log(`[gd] skill-stage failed: ${(err as Error).message}`);
