...
// DoctorReport shape the post-install `gd plugin doctor <id>`
// command emits.
//
// The lift from `gd plugin doctor`:
...
//
// Rationale: spec §16 Phase 4 ships `gd plugin scaffold`, `gd
// plugin export`, `gd plugin publish` for the author tooling slice.
// `gd plugin validate` closes the loop: the author can run lint
...
import type { RegistryView } from '@galyarder-design/plugin-runtime';
...
export interface ValidatePluginFolderInput {
  ...
  // `galyarder-design.json` / `SKILL.md` / `.claude-plugin/plugin.json`
