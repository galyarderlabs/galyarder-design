...
real repo (cloned via `gd project import` or a path the user
...
| --- | --- | --- |
| `repoPath` | yes | Absolute path to the repo's root, supplied via `gd project import` or an upstream `od.context.assets[]` reference |
...
walk respects a budget (`GD_CODE_IMPORT_BUDGET_MS`, default 60s) so
