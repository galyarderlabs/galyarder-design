...
| # | Bet | [Anthropic Claude Design][cd] | [Open CoDesign][ocod] | GD |
|---|---|---|---|---|
...
User opens the web app, types *"Airbnb-style search page, use our internal design system"*, GD picks the `prototype-skill`, resolves the user's `DESIGN.md`, dispatches to Claude Code with both files plus the brief, streams tool calls into the UI, and renders the resulting HTML in an iframe preview. User clicks an element, drops a comment, the agent rewrites just that region.

### S2 — "Make me a deck"
User says *"8-slide magazine-style pitch deck for my seed round"*. GD routes to `deck-skill` (a fork of [`guizang-ppt-skill`][guizang]). Output is a single-file HTML deck; preview is the deck itself with arrow-key navigation; export is PDF/PPTX.
...
User uploads a screenshot, brand guide PDF, or Figma link. GD runs `design-system-skill` which produces a `DESIGN.md` following the 9-section format. That file is then referenced by every subsequent generation — prototypes, decks, templates all pick up the tokens.

### S5 — "Let the design agent evolve"
User connects sources such as GitHub, Notion, Drive, Slack, or a local folder, then picks an Automation template like "Ingest into memory tree," "Extract design system," or "Crystallize this run into a skill." GD canonicalizes the source, optionally compresses it, proposes memory / skill / design-system changes, and only applies them after the configured review policy. Future agent runs consume those accepted nodes automatically.
...
In short: Claude Design is a product; GD is a **substrate**.
...
- A design system author can write a `DESIGN.md`, point GD at it, and have the style propagate across prototype / deck / template outputs.
