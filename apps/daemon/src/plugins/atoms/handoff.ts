...
// `gd plugin export`) can reverse-resolve the artifact's lineage
...
// `<cwd>/<manifest-path>` or `.gd/artifacts/<id>/manifest.json`).
//
...
export interface RunHandoffAtomInput {
  ...
  // 'cli' when gd plugin export wrote to disk; 'docker' when the
