...
function readPreview(record: InstalledPluginRecord): PreviewBlock | null {
  const gd = record.manifest?.gd as { preview?: unknown } | undefined;
  if (!gd || typeof gd.preview !== 'object' || gd.preview === null) return null;
  return gd.preview as PreviewBlock;
}

function exampleOutputCount(record: InstalledPluginRecord): number {
  const gd = record.manifest?.gd as
    | { useCase?: { exampleOutputs?: unknown } }
    | undefined;
  const list = gd?.useCase?.exampleOutputs;
  return Array.isArray(list) ? list.length : 0;
}

function modeOf(record: InstalledPluginRecord): string {
  const gd = record.manifest?.gd as { mode?: unknown } | undefined;
  return typeof gd?.mode === 'string' ? gd.mode.toLowerCase() : '';
}

function surfaceOf(record: InstalledPluginRecord): string {
  const gd = record.manifest?.gd as { surface?: unknown } | undefined;
  return typeof gd?.surface === 'string' ? gd.surface.toLowerCase() : '';
}

function kindOf(record: InstalledPluginRecord): string {
  const gd = record.manifest?.gd as { kind?: unknown } | undefined;
  return typeof gd?.kind === 'string' ? gd.kind.toLowerCase() : '';
}

function featuredRank(record: InstalledPluginRecord): number | null {
  const gd = (record.manifest?.gd ?? {}) as Record<string, unknown>;
  if (gd.featured === true) return 0;
  if (typeof gd.featured !== 'number' || !Number.isFinite(gd.featured)) return null;
  return Math.max(0, gd.featured);
}
