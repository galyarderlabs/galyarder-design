...
import { createJsonIpcServer, type JsonIpcServerHandle } from "@galyarder-design/sidecar";
import { SIDECAR_ENV, SIDECAR_MESSAGES } from "@galyarder-design/sidecar-proto";
import { resolveDaemonUrl, DEFAULT_DAEMON_URL } from "../src/daemon-url.js";

// Verifies the resolution chain: --daemon-url > GD_DAEMON_URL > sidecar
// IPC status discovery > legacy default. Each layer must short-circuit the next
// so `gd` clients follow the live daemon across ephemeral-port restarts.

describe("resolveDaemonUrl", () => {
  let ipcBaseDir: string;

  beforeAll(() => {
    ipcBaseDir = fs.mkdtempSync(path.join(os.tmpdir(), "gd-mcp-resolve-"));
  });
  ...
  it("prefers the explicit --daemon-url flag", async () => {
    const url = await resolveDaemonUrl({
      ...
      env: {
        GD_DAEMON_URL: "http://env.example:2222",
  ...
  it("falls back to GD_DAEMON_URL when no flag given", async () => {
    const url = await resolveDaemonUrl({
      env: {
        GD_DAEMON_URL: "http://env.example:2222",
        [SIDECAR_ENV.IPC_PATH]: path.join(ipcBaseDir, "daemon.sock"),
  ...
  it("discovers the live daemon URL via the concrete sidecar IPC status endpoint", async () => {
    const socketPath = process.platform === "win32"
      ? `\\\\.\\pipe\\galyarder-design-daemon-url-${process.pid}-${Date.now()}`
