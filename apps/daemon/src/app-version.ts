...
// Keep this structurally aligned with `@galyarder-design/contracts` AppVersionInfo.
...
// it: `dist/app-version.js` (rootDir=src, used by the `gd` CLI) and
...
// inject the version via `GD_APP_VERSION` (packaged runtime) keep working
...
export function resolveAppVersionInfo({
  ...
  const version = cleanString(env.GD_APP_VERSION)
    ?? cleanString(packageMetadata?.version)
    ?? APP_VERSION_FALLBACK;
  const inferredChannel = inferReleaseChannelFromVersion(version);
  const channel = cleanString(env.GD_RELEASE_CHANNEL)
    ?? cleanString(env.GD_APP_CHANNEL)
    ?? inferredChannel
