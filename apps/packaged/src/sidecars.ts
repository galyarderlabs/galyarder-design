 ...
 * boots, but the GD_LEGACY_DATA_DIR one-shot recovery flow can synch-
 * copy a multi-GB legacy `.gd/` payload before SQLite even opens, and
...
export function resolveDaemonStatusTimeoutMs(
  ...
  const raw = env.GD_LEGACY_DATA_DIR;
 ...
 * #710 migrator's LegacyMigrationError on invalid GD_LEGACY_DATA_DIR,
...
// @galyarder-design/platform's wellKnownUserToolchainBins so the daemon
...
function createPackagedDaemonManagedPathEnv(
  ...
  return {
    GD_DATA_DIR: paths.dataRoot,
    GD_RESOURCE_ROOT: paths.resourceRoot,
    GD_INSTALLATION_DIR: paths.installationRoot,
  };
}
...
export function buildPackagedDaemonSpawnEnv(
  ...
  return {
    ...
    ...(options.requireDesktopAuth ? { GD_REQUIRE_DESKTOP_AUTH: "1" } : {}),
    ...
    ...(options.appVersion == null ? {} : { GD_APP_VERSION: options.appVersion }),
    ...(options.telemetryRelayUrl == null || options.telemetryRelayUrl.length === 0
      ? {}
      : { GALYARDER_DESIGN_TELEMETRY_RELAY_URL: options.telemetryRelayUrl }),
    // GD_LEGACY_DATA_DIR is the one-shot recovery handle for users
    // upgrading from 0.3.x .gd/ layouts. The daemon's startup
    ...
    ...(options.legacyDataDir == null || options.legacyDataDir.length === 0
      ? {}
      : { GD_LEGACY_DATA_DIR: options.legacyDataDir }),
...
async function spawnSidecarChild(options: {
  ...
  const child = spawn(
    command,
    [options.entryPath, ...createProcessStampArgs(stamp, GALYARDER_DESIGN_SIDECAR_CONTRACT)],
    {
      cwd: process.cwd(),
      env: childEnv,
...
export async function startPackagedSidecars(
  ...
  try {
    const daemon = await spawnSidecarChild({
      app: APP_KEYS.DAEMON,
      entryPath: options.daemonSidecarEntry ?? resolveSidecarEntry("@galyarder-design/daemon", "sidecar"),
      env: buildPackagedDaemonSpawnEnv(paths, {
        appVersion: options.appVersion,
        daemonCliEntry: options.daemonCliEntry,
        legacyDataDir: process.env.GD_LEGACY_DATA_DIR ?? null,
    ...
    const daemonStatus = await waitForStatus<DaemonStatusSnapshot>(
      ...
      // invalid GD_LEGACY_DATA_DIR, existing target payload, symlink,
    ...
    const web = await spawnSidecarChild({
      app: APP_KEYS.WEB,
      entryPath: options.webSidecarEntry ?? resolveSidecarEntry("@galyarder-design/web", "sidecar"),
      env: {
        [SIDECAR_ENV.DAEMON_PORT]: extractPort(daemonStatus.url),
        [SIDECAR_ENV.WEB_PORT]: "0",
        ...(options.webStandaloneRoot == null ? {} : { GD_WEB_STANDALONE_ROOT: options.webStandaloneRoot }),
        GD_WEB_OUTPUT_MODE: options.webOutputMode,
        PORT: "0",
      },
      nodeCommand: options.nodeCommand,
      paths,
