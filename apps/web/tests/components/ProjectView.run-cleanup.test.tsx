...
describe('ProjectView daemon cleanup', () => {
  ...
  // the run streamed. The user-bug-report quoted in CJK said: "It looks
  // like it sent the input box's query but didn't clear it" (CJK kept
  // verbatim because it's the original report text). With the fix,
  // auto-send projects must hand the composer
  ...
  it('does not seed composer initialDraft when auto-send sessionStorage flag is set', async () => {
    ...
    window.sessionStorage.setItem('gd:auto-send-first:project-2', '1');
    ...
    try {
      ...
      window.sessionStorage.removeItem('gd:auto-send-first:project-2');
    }
  });
  ...
  it('auto-sends Home-staged design files as first-turn daemon attachments', async () => {
    ...
    chatPaneSpy.mockClear();
    window.sessionStorage.setItem('gd:auto-send-first:project-files', '1');
    window.sessionStorage.setItem(
      'gd:auto-send-attachments:project-files',
      JSON.stringify([
    ...
    try {
      ...
      expect(window.sessionStorage.getItem('gd:auto-send-first:project-files')).toBeNull();
      expect(window.sessionStorage.getItem('gd:auto-send-attachments:project-files')).toBeNull();
    } finally {
      window.sessionStorage.removeItem('gd:auto-send-first:project-files');
      window.sessionStorage.removeItem('gd:auto-send-attachments:project-files');
    }
  });
  ...
  it('audits design-system workspace output after first auto-send and seeds a bounded repair prompt', async () => {
    ...
    chatPaneSpy.mockClear();
    window.sessionStorage.setItem('gd:auto-send-first:project-ds', '1');
    ...
    expect(window.sessionStorage.getItem('gd:design-system-audit-auto-repair:project-ds')).toBe('1');
    await waitFor(() => {
  ...
  it('clears design-system auto-repair budget when the first audit passes', async () => {
    ...
    chatPaneSpy.mockClear();
    window.sessionStorage.setItem('gd:auto-send-first:project-ds-pass', '1');
    ...
    expect(window.sessionStorage.getItem('gd:design-system-audit-auto-repair:project-ds-pass')).toBeNull();
  });
  ...
  it('seeds composer initialDraft with pendingPrompt when auto-send flag is absent', async () => {
    ...
    chatPaneSpy.mockClear();
    window.sessionStorage.removeItem('gd:auto-send-first:project-3');
