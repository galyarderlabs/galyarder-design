/**
 * One-shot legacy `.gd/` data migrator.
 *
 * Galyarder Design 0.3.x ran from the repo and wrote runtime state to
 * `<repo>/.gd/` (SQLite at `app.sqlite`, agent CWDs under `projects/`,
 ...
 * disappear. The data was never lost (the 0.3.x `.gd/` folder is still
 * on disk wherever they used to run from), but the new daemon had no
 * way to know about it. See https://github.com/galyarderlabs/galyarder-design/issues/710.
 *
 * This module gives operators a recovery path. When `GD_LEGACY_DATA_DIR`
 ...
 *      out of the data root via crafted .gd/ symlinks).
...
export interface MigrateLegacyDataDirOptions {
  /** Path to the legacy `.gd/` directory (typically GD_LEGACY_DATA_DIR). */
...
/**
 * Daemon startup throws this when GD_LEGACY_DATA_DIR is explicitly set
...
// Directories and files that are part of the GD runtime payload. Anything
 ...
 * a real GD SQLite database. We deliberately do NOT just check for an
 ...
 * shipped chat history wrote one, and a stray empty `.gd/` folder won't
 * have it.
 */
...
export function migrateLegacyDataDirSync(
  ...
  const log = options.logger ?? {
    info: (m) => console.log(`[gd-migrate] ${m}`),
    warn: (m) => console.warn(`[gd-migrate] ${m}`),
  };

  const raw = options.legacyDir;
  if (raw === undefined || raw.length === 0) {
    return { status: 'noop', reason: 'GD_LEGACY_DATA_DIR not set' };
  }
  const legacyDir = path.resolve(raw);
  const dataDir = path.resolve(options.dataDir);

  if (legacyDir === dataDir) {
    return { status: 'noop', reason: 'GD_LEGACY_DATA_DIR equals GD_DATA_DIR' };
  }

  // Marker check runs before legacyDirHasPayload on purpose: once a
  // migration has succeeded, the marker is the canonical "do not
  // touch" signal. The user may leave GD_LEGACY_DATA_DIR set and then
  // delete or move the old repo `.gd/` later; without this ordering
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    // Loud: env was set but the path doesn't look like an GD data dir.
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `GD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit Galyarder Design, fix the path, and relaunch.`,
    );
  }
  ...
  if (existing.length > 0) {
    throw new LegacyMigrationError(
      'data_dir_not_empty',
      `GD_DATA_DIR="${dataDir}" already contains payload entries (${existing.join(', ')}); refusing to merge legacy data on top. Move the existing data aside or pick a fresh data root before re-running with GD_LEGACY_DATA_DIR.`,
    );
  }
  ...
  const stagingDir = path.join(
    ...
    `${path.basename(dataDir)}.gd-migrate-${process.pid}-${Date.now()}`,
  );
