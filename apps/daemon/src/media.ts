...
//   `gd media generate --surface … --model … --output … --prompt …`
...
//
// The fallback stub handlers are gated behind GD_MEDIA_ALLOW_STUBS=1; in
...
type MediaContext = {
   ...
   * to the user-supplied alias from `GD_MEDIA_MODEL_ALIASES` /
...
// behind GD_MEDIA_ALLOW_STUBS=1 and otherwise return a 503 (mapped from
...
function stubsAllowed() {
  const v = process.env.GD_MEDIA_ALLOW_STUBS;
  return v === '1' || v === 'true';
}
 ...
 *
 * @param {Object} args
 * @param {string} args.projectRoot   - Repo root (.gd/ lives directly under).
 * @param {string} args.projectsRoot  - Absolute path to <repo>/.gd/projects.
...
export async function generateMedia(args: {
  ...
  try {
    if (
      ...
      // stub fallback behind GD_MEDIA_ALLOW_STUBS so release builds don't
...
async function renderImageRouterImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no ImageRouter API key — configure it in Settings or set GD_IMAGEROUTER_API_KEY',
    );
  }
...
async function renderVolcengineVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  const configuredMaxMs = Number(process.env.GD_VOLCENGINE_VIDEO_MAX_POLL_MS);
  const maxMs =
...
function volcengineRatioFor(aspect?: string): string {
  // Seedance accepts a fixed list of ratios; map the GD vocabulary to
...
async function renderGrokImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no xAI credentials — sign in with your SuperGrok subscription (in GD or via `hermes auth add xai-oauth`), set XAI_API_KEY, or configure a key in Settings',
    );
  }
...
async function renderNanoBananaImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no Nano Banana API key — configure it in Settings or set GD_NANOBANANA_API_KEY',
    );
  }
...
async function renderGrokVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  if (!videoUrl && requestId) {
    const startedAt = Date.now();
    const configuredMaxMs = Number(process.env.GD_GROK_VIDEO_MAX_POLL_MS);
    const maxMs =
      Number.isFinite(configuredMaxMs) && configuredMaxMs >= 60_000
        ? configuredMaxMs
        : 8 * 60 * 1000;
    ...
    if (!videoUrl) {
      ...
      throw new Error(
        ...
        + `If your jobs legitimately need longer, raise GD_GROK_VIDEO_MAX_POLL_MS.`,
      );
    }
  }
...
async function renderElevenLabsTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no ElevenLabs API key - configure it in Settings or set GD_ELEVENLABS_API_KEY',
    );
  }
...
async function renderMinimaxTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no MiniMax API key — configure it in Settings or set GD_MINIMAX_API_KEY',
    );
  }
...
async function renderSenseAudioTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no SenseAudio API key — configure it in Settings or set GD_SENSEAUDIO_API_KEY',
    );
  }
...
//     slot with the TTS path (GD_SENSEAUDIO_API_KEY / SENSEAUDIO_API_KEY).
...
// containing the literal Chinese error string `参数错误：size`
// ("parameter error: size"). The CJK string is documented here as the
// observed upstream error to make this comment a useful crash-report
// breadcrumb. Keep this table in sync with byok-tools.ts's
...
async function renderFishAudioTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no FishAudio API key — configure it in Settings or set GD_FISHAUDIO_API_KEY',
    );
  }
...
async function renderHyperFramesViaCli(ctx: MediaContext, projectDir: string, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  if (typeof compRel !== 'string' || !compRel.trim()) {
    throw new Error(
      'hyperframes-html requires --composition-dir <project-relative-path> ' +
        ...
        '$GD_PROJECT_DIR/.hyperframes-cache/<id>/ and pass that path here.',
    );
  }
  ...
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'galyarder-design-hf-'));
