...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Galyarder Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Galyarder Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Galyarder Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Galyarder Design", "Galyarder Design.exe"),
    ...
    setupPath: join(namespaceRoot, "builder", "Galyarder Design-second-setup.exe"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Galyarder Design", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Galyarder Design.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-win-builder-"));
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Galyarder Design.exe"), "exe\n", "utf8");
      await writeFile(
        join(cachedUnpackedRoot, "resources", "galyarder-design-config.json"),
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "galyarder-design-packaged-app", version: "0.5.0-beta.1" })}\n`,
        "utf8",
      );
      ...
      await expect(readFile(join(paths.unpackedRoot, "Galyarder Design.exe"), "utf8")).resolves.toBe("exe\n");
      await expect(readFile(join(paths.unpackedRoot, "resources", "galyarder-design-config.json"), "utf8")).resolves.toContain(
        '"namespace":"second"',
      );
