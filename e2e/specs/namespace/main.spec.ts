...
describe('namespace isolation spec', () => {
  test('keeps namespace in lifecycle infrastructure while daemon clients use URL or concrete IPC transport', async () => {
    ...
    await suite.with.toolsDev(async ({ runtime, status, webUrl }) => {
      ...
      const ipcPath = installInfo.env.GD_SIDECAR_IPC_PATH;
      if (typeof ipcPath !== 'string' || ipcPath.length === 0) {
        throw new Error('MCP install-info did not include GD_SIDECAR_IPC_PATH');
      }
      expect(ipcPath).toEqual(expect.any(String));
      expect(installInfo.args).not.toContain('--daemon-url');
      expect(installInfo.env.GD_DATA_DIR).toBe(suite.dataDir);
      expect(installInfo.env).not.toHaveProperty('GD_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('GD_SIDECAR_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('GD_SIDECAR_IPC_BASE');

      const cliStatus = await runDaemonCliJson<DaemonStatusResponse>(
        ['daemon', 'status', '--json'],
        {
          GD_DATA_DIR: suite.dataDir,
          GD_NAMESPACE: 'wrong-daemon-namespace',
          GD_SIDECAR_IPC_BASE: path.join(suite.scratchDir, 'wrong-ipc-base'),
          GD_SIDECAR_IPC_PATH: ipcPath,
          GD_SIDECAR_NAMESPACE: 'wrong-sidecar-namespace',
        },
      );
      ...
      await access(path.join(expectedPluginRoot, 'galyarder-design.json'));
...
async function writeLocalPluginFixture(root: string): Promise<string> {
  ...
  await writeFile(
    path.join(pluginRoot, 'galyarder-design.json'),
    JSON.stringify(
      {
...
async function runDaemonCli(
  ...
  delete mergedEnv.GD_DAEMON_URL;
  delete mergedEnv.GD_PORT;
