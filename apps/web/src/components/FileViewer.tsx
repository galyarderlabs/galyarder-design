...
const EXPORT_READY_NUDGE_STORAGE_PREFIX = 'galyarder-design:export-ready-nudge:';
...
export function LiveArtifactViewer({
  ...
  // missing artifact file or a stuck `gd://` resolver — surface in
        ...
        <div className="inspect-ancestor-notice" data-testid="inspect-ancestor-notice">
          ...
          <div className="inspect-ancestor-notice-text">
            ...
            {target.clickedDescendant.text
              ...
              : ''}
            , but it has no <code>data-gd-id</code> annotation. Editing{' '}
...
// it travels in gd:inspect-overrides messages. The host's persisted map is
// owned and mutated only by host-driven onApply / reset actions plus the
// initial parse of the source's <style data-gd-inspect-overrides> block;
...
// structured `overrides` field of an gd:inspect-overrides message. The host
...
export function serializeInspectOverrides(overrides: unknown): string {
  ...
  for (const elementId of Object.keys(map)) {
    ...
    // digit) and silently downgrade them to `[data-gd-id="..."]`. The
...
// Parse any persisted <style data-gd-inspect-overrides> blocks in the
...
// initial hydration step — inbound iframe gd:inspect-overrides messages are
...
//
// HTML-aware: enumerates `<style data-gd-inspect-overrides>` elements via
// the same walker used by the splicer, so a `<style data-gd-inspect-overrides>`
...
export function parseInspectOverridesFromSource(source: string): InspectOverrideMap {
  ...
  for (const body of stripInspectOverridesAndIndex(source).bodies) {
    const ruleRe = /(\[data-(?:gd-id|screen-label)="([^"]*)"\])\s*\{\s*([^}]*)\}/g;
...
// `<style data-gd-inspect-overrides>` written as text inside one of them
...
// `data-gd-inspect-overrides` attribute, as opposed to merely mentioning
// the marker text inside another attribute name or value. The naive
// `\bdata-gd-inspect-overrides\b` test against the whole tag text is
// over-broad in two cases:
//
//   1. A longer attribute name that has the marker as a prefix, e.g.
//      `<style data-gd-inspect-overrides-note="docs">`. The `-` after
...
//      `<style title="data-gd-inspect-overrides">`. The whole tag text
...
// names is exactly `data-gd-inspect-overrides` (boolean attribute or
...
function styleTagIsInspectOverrideBlock(tagText: string): boolean {
  ...
  while (i < end) {
    ...
    if (name === 'data-gd-inspect-overrides') return true;
  }
  return false;
}
...
type InspectSpliceScan = {
  ...
  // Raw inner-text of every real `<style data-gd-inspect-overrides>` element
...
// `<style data-gd-inspect-overrides>...</style>` block removed, while
...
// `<style data-gd-inspect-overrides>` written inside an inline script or
...
export function applyInspectOverridesToSource(source: string, css: string): string {
  ...
  const block = `<style data-gd-inspect-overrides>\n${trimmed}\n</style>\n`;
  if (headCloseStart >= 0) {
...
function HtmlViewer({
    ...
    const apply = () => {
      ...
      try {
        ...
        iframeRef.current?.contentWindow?.postMessage({
          type: 'gd:preview-scroll-restore',
  ...
  // serializes that host map directly — iframe gd:inspect-overrides messages
  ...
  // invisible to the iframe — live updates flow through gd-edit-preview-style
  ...
  // first load: the bridge that emits `gd:tweaks-available` is only injected
  ...
  // next shell will post `gd:srcdoc-transport-ready` (or fire onLoad) and
  ...
  useEffect(() => {
    function onMessage(ev: MessageEvent) {
      ...
      if (data?.type !== 'gd:srcdoc-transport-ready') return;
  ...
  const activateSrcDocTransport = useCallback((target: HTMLIFrameElement | null = srcDocPreviewIframeRef.current) => {
    ...
    if (!win) return false;
    win.postMessage({ type: 'gd:srcdoc-transport-activate', html: srcDoc }, '*');
  ...
  useEffect(() => {
    function onMessage(ev: MessageEvent) {
      ...
      if (!data || data.type !== 'gd:preview-scroll') return;
    ...
    function onRestoreRequest(ev: MessageEvent) {
      ...
      if (!isActivePreviewIframeSource(ev.source)) return;
      const data = ev.data as { type?: string } | null;
      if (!data || data.type !== 'gd:preview-scroll-request') return;
  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      const data = ev?.data as
        ...
        | null;
      if (!data || data.type !== 'gd:slide-state') return;
  ...
  useEffect(() => {
    const win = iframeRef.current?.contentWindow;
    if (!win) return;
    win.postMessage({ type: 'gd-edit-mode', enabled: manualEditMode }, '*');
  ...
  const previewStyleToIframe = useCallback((id: string, styles: Partial<ManualEditStyles>, version: number) => {
    const win = iframeRef.current?.contentWindow;
    if (!win) return false;
    win.postMessage({ type: 'gd-edit-preview-style', id, styles, version }, '*');
    return true;
  }, []);
  ...
  function postSelectedManualEditTargetToIframe(id: string | null, target: HTMLIFrameElement | null = iframeRef.current) {
    const win = target?.contentWindow;
    if (!win) return;
    win.postMessage({ type: 'gd-edit-selected-target', id }, '*');
  }
  ...
  function syncBridgeModes(target: HTMLIFrameElement | null = iframeRef.current) {
    ...
    win.postMessage({ type: 'gd:tweaks-panel-visible', visible: tweaksMode }, '*');
    win.postMessage({ type: tweaksMode ? '__activate_edit_mode' : '__deactivate_edit_mode' }, '*');
    win.postMessage({ type: 'gd:inspect-mode', enabled: inspectMode }, '*');
    const palette = previewPalette ?? selectedPalette;
    win.postMessage({ type: 'gd:palette', palette }, '*');
  }
  ...
  // Mirror the bridge's `gd:comment-targets` broadcast into
  ...
  // elements on the page carry data-gd-id / data-screen-label".
  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      const data = ev.data as
        ...
        | null;
      if (data?.type !== 'gd:comment-targets' || !Array.isArray(data.targets)) return;
  ...
  useEffect(() => {
    ...
    //  - `gd:tweaks-panel-visible` is the bridge protocol used by class-based
  ...
  // (`gd:tweaks-*` for skill-template artifacts) and the artifact-native
  ...
  useEffect(() => {
    function onMessage(ev: MessageEvent) {
      ...
      if (!data?.type) return;
      if (data.type === 'gd:tweaks-available') {
      ...
      } else if (data.type === 'gd:tweaks-panel-state') {
  ...
  // artifact JS forging an gd:inspect-overrides message cannot tamper
  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      if (!data?.type) return;
      if (data.type === 'gd:comment-targets' && Array.isArray(data.targets)) {
        const next = new Map<string, PreviewCommentSnapshot>();
        data.targets.forEach((item) => {
      ...
      if (data.type === 'gd:comment-leave') {
        setHoveredCommentTarget(null);
        return;
      }
      if (data.type === 'gd:comment-hover') {
      ...
      if (data.type === 'gd:comment-target') {
        const snapshot = snapshotFromData(data);
        if (!snapshot.elementId) return;
      ...
      if (data.type === 'gd:pod-clear') {
        setStrokePoints([]);
        return;
      }
      if (data.type === 'gd:pod-stroke' && Array.isArray(data.points)) {
        setStrokePoints(
      ...
      if (data.type === 'gd:pod-select' && Array.isArray(data.points)) {
  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      if (!data?.type) return;
      if (data.type === 'gd-edit-targets' && Array.isArray(data.targets)) {
  ...
  // Inspect-mode picker: same `gd:comment-target` payload, different sink.
  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      const data = ev.data as
        ...
        | null;
      if (!data || data.type !== 'gd:comment-target') return;
  ...
  function postSlide(action: 'next' | 'prev' | 'first' | 'last') {
    const win = iframeRef.current?.contentWindow;
    if (!win) return;
    win.postMessage({ type: 'gd:slide', action }, '*');
  }
  ...
  function postInspectSet(elementId: string, selector: string, prop: string, value: string) {
    ...
    if (!win) return;
    win.postMessage(
      { type: 'gd:inspect-set', elementId, selector, prop, value },
      '*',
    );
  }
  ...
  function postInspectReset(elementId?: string) {
    const win = iframeRef.current?.contentWindow;
    if (!win) return;
    win.postMessage({ type: 'gd:inspect-reset', elementId }, '*');
  }
  ...
  // bridge would also have accepted via gd:inspect-set.
  //
  ...
  function replayInspectOverridesToIframe(target: HTMLIFrameElement | null = iframeRef.current) {
    ...
    const overrides = inspectHydratedSourceRef.current === source
      ? inspectOverrides
      : (typeof source === 'string' ? parseInspectOverridesFromSource(source) : {});
    win.postMessage({ type: 'gd:inspect-replay', overrides }, '*');
  }

  // Persist accumulated inspect overrides into the artifact source: replace
  // (or insert) a single <style data-gd-inspect-overrides> block in <head>.
  ...
  // contentWindow as the bridge and could forge an gd:inspect-overrides
        ...
        {source === null ? (
          ...
          >
            ...
            <div className={manualEditMode ? 'manual-edit-canvas' : 'comment-frame-clip'}>
              ...
              >
                <PreviewDrawOverlay
                  ...
                  sendDisabledReason="A task is currently executing"
                >
                  <div className="artifact-preview-transport-stack">
                    <iframe
                      ...
                      data-gd-render-mode="url-load"
                      data-gd-active={useUrlLoadPreview ? 'true' : 'false'}
                    ...
                    <iframe
                      ...
                      data-gd-render-mode="srcdoc"
                      data-gd-active={useUrlLoadPreview ? 'false' : 'true'}
            ...
            {/*
              ...
              `apps/web/src/runtime/srcdoc.ts` posts `gd:comment-targets`
              with every element annotated with `data-gd-id` /
              ...
              - "has targets": the existing copy ("Click any element with
                `data-gd-id` to tune its style.") for users who just don't
              ...
              - "no targets" (issue #890): a freeform-generated artifact
                ...
                without a skill) ships zero `data-gd-id` annotations. The
            ...
            {(inspectMode || (boardMode && boardTool === 'inspect'))
              ...
              >
                {liveCommentTargets.size === 0 ? (
                  ...
                  >
                    This artifact has no <code>data-gd-id</code>{' '}
                  ...
                  >
                    Click any element with <code>data-gd-id</code> to{' '}
      ...
      fetchProjectRelativeText(projectId, fileName, href).then((css) =>
        css == null
          ...
          : {
              from: tag,
              to:
                `<style data-gd-inline-asset="${escapeHtmlAttr(href)}">\n` +
...
function resolveProjectRelativePath(ownerFileName: string, assetRef: string): string | null {
  ...
  try {
    const url = new URL(assetRef, `https://gd.local/${baseDirFor(ownerFileName)}`);
    if (url.origin !== 'https://gd.local') return null;
