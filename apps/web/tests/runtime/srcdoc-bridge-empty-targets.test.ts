...
// Behavioral coverage for galyarderlabs/galyarder-design#890. The Examples / Tweaks
// preview iframe runs the bridge script generated by `buildSrcdoc`. When
// the agent emits an artifact without `data-gd-id` / `data-screen-label`
// (e.g. a freeform PRD → HTML pass through a Claude-Code-compatible CLI
// without a skill), the bridge must:
//
//   1. Still post `gd:comment-targets` with `targets: []` so the host
//      can detect the empty-annotation state and surface a clearer
//      hint than "Click any element with `data-gd-id` …" — without
...
//      `gd:comment-target` — the click handler walks up to <html>,
...
function extractBridgeScript(srcdoc: string): string {
  // The bridge is wrapped in `<script data-gd-selection-bridge>(function(){…})()</script>`.
  const match = srcdoc.match(
    /<script data-gd-selection-bridge>([\s\S]*?)<\/script>/,
  );
  if (!match || !match[1]) {
...
function extractSelectionBridgeStyle(srcdoc: string): string {
  const match = srcdoc.match(
    /<style data-gd-selection-bridge-style>[\s\S]*?<\/style>/,
  );
  return match?.[0] ?? '';
}
...
describe('selection bridge — empty annotation surface (#890)', () => {
  it('posts gd:comment-targets with an empty list when the artifact has no annotated elements', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      // PRD-style mockup: real DOM, zero `data-gd-id` / `data-screen-label`.
    ...
    const targetMessages = parentPostMessage.mock.calls
      .map((call) => call[0])
      .filter((message) => message?.type === 'gd:comment-targets');
  ...
  it('does not post gd:comment-target when the user clicks an unannotated element', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
    ...
    // Click directly on the <h1> — it has no `data-gd-id` / `data-screen-label`,
    ...
    const clickMessages = parentPostMessage.mock.calls
      .map((call) => call[0])
      .filter((message) => message?.type === 'gd:comment-target');
    expect(clickMessages).toEqual([]);
  });

  it('still posts gd:comment-target when the click traverses up to an annotated ancestor', async () => {
    // Sanity check / contract pin: the no-op behavior above is specific
    // to the no-annotation case. When ANY ancestor carries `data-gd-id`,
    // the click must still resolve to that ancestor — this is the
    // happy path the gallery has shipped since the bridge landed.
    const { win, parentPostMessage } = setupBridgeDom(
      '<main data-gd-id="hero"><h1 id="title">Hero</h1></main>',
      'inspect',
    );
  ...
  it('does not add clickedDescendant when the annotated target itself is clicked', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<main data-gd-id="hero">Hero</main>',
      'inspect',
    );
    ...
    win.document.querySelector('[data-gd-id="hero"]')!.dispatchEvent(
      new win.MouseEvent('click', { bubbles: true, cancelable: true }),
    );
  ...
  it('does not use Picker DOM fallback on mixed annotated and unannotated pages', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<main><section data-gd-id="hero">Hero</section><button id="cta">Launch</button></main>',
      'comment',
      ['#cta'],
    );
  ...
  it('applies pointer-events none to body iframes while Comment picker is active', async () => {
    const { win } = setupBridgeDom(
      '<article data-gd-id="card"><div>Tablet edition</div><iframe id="f" src="about:blank"></iframe></article>',
      'comment',
      ['#f'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));
  ...
  it('applies pointer-events none to body iframes while Inspect picker is active', async () => {
    const { win } = setupBridgeDom(
      '<article data-gd-id="card"><iframe id="f" src="about:blank"></iframe></article>',
      'inspect',
      ['#f'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));
  ...
  it('restores iframe pointer events after Comment mode is disabled', async () => {
    ...
    win.dispatchEvent(
      new win.MessageEvent('message', {
        data: { type: 'gd:comment-mode', enabled: false, mode: 'picker' },
      }),
    );
    expect(win.getComputedStyle(iframe).pointerEvents).toBe('auto');
  });

  it('posts gd:comment-target for the annotated card when the device-frame iframe is clicked', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<article data-gd-id="tablet-card" class="frame-card"><div class="meta">Tablet edition</div><iframe id="f" class="tablet-frame" title="Tablet edition" src="about:blank"></iframe></article>',
      'comment',
      ['#f', '.frame-card'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));
