...
IMAGE_NAMESPACE="${IMAGE_NAMESPACE:-galyarderlabs}"
IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-galyarder-design}"
...
Options:
  ...
  --image_namespace <namespace>   default: galyarderlabs
  --image_repository <name>       default: galyarder-design
...
node_local_base_image() {
  ...
  local arch
  arch="$(platform_to_arch "$platform")" || die "unsupported platform '$platform'"
  printf 'galyarder-design-base-node:24-alpine-%s' "$arch"
}
...
runtime_local_base_image() {
  ...
  local arch
  arch="$(platform_to_arch "$platform")" || die "unsupported platform '$platform'"
  printf 'galyarder-design-runtime-base:24-alpine-%s' "$arch"
}
