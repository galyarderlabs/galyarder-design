...
describe("assertMacPrebundleMetafile", () => {
  it("accepts a safe web sidecar metafile", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-mac-prebundle-"));
...
describe("renderMacPackagedMainEntry", () => {
  it("renders the prebundled runtime entry shim", () => {
    ...
    expect(renderMacPackagedMainEntry(true)).not.toContain("@galyarder-design/packaged");
  });

  it("renders the package entry shim for non-prebundled mode", () => {
    expect(renderMacPackagedMainEntry(false)).toContain("@galyarder-design/packaged");
