// Content fixtures for the "Use Galyarder Design everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'Galyarder Design works wherever your agent works',
    intro:
      'Galyarder Design is more than a window — it is a local privileged daemon ' +
      "(`gd`) plus a Skills + Design-Systems + Atoms registry. Once it's " +
    ...
    bullets: [
      'CLI — `gd <command>` for headless scripts, CI, and shell automation.',
      'MCP server — wires Galyarder Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Galyarder Design at all.',
    ...
    snippets: [
      {
        ...
        language: 'bash',
        body: 'pnpm tools-dev\n# or, if `gd` is on your PATH (packaged install):\nod --port 7456',
      },
      {
      ...
      },
      {
        ...
        language: 'bash',
        body:
          'pnpm seed:test-projects --offline --data-dir ./.gd \\\n' +
          '  --decks 2 --webs 2 --default-plugins 3 --community-plugins 3\n' +
          '# Then start Galyarder Design in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
      'The daemon writes to `./.gd/` (project-local) by default. Set ' +
      '`GD_DATA_DIR=~/.galyarder-design` to share data across projects.',
  },
  {
    id: 'cli',
    tabLabel: 'CLI · gd',
    heading: 'Drive Galyarder Design from any shell',
    intro:
      'The `gd` bin ships with the daemon and is the same binary used by ' +
    ...
    bullets: [
      '`gd` (no args) — boots the daemon and opens the web UI.',
      '`gd media generate ...` — produce image / video / audio bytes through the unified media protocol.',
      '`gd run ...` — start a project run from a prompt + skill.',
      '`gd plugin install <source>` / `gd plugin apply <id>` — install and apply community plugins.',
      '`gd skills list` / `gd design-systems list` — inspect what is available locally.',
      '`gd status` / `gd doctor` — verify daemon health and detect agent CLIs on your PATH.',
    ],
    snippets: [
      {
      ...
      },
      {
        ...
        language: 'bash',
        body: 'gd skills list --json\nod design-systems list --json',
      },
      {
        label: 'Check seeded artifacts through the CLI',
        language: 'bash',
        body:
          'gd project list --daemon-url http://127.0.0.1:7456\n' +
          'gd files list <seed-project-id> --daemon-url http://127.0.0.1:7456\n' +
          'gd files read <seed-project-id> index.html --daemon-url http://127.0.0.1:7456 | head',
      },
      {
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose Galyarder Design as an MCP server to any coding agent',
    intro:
      'Galyarder Design ships with a Model Context Protocol server (`gd mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover Galyarder Design tools ' +
    ...
    bullets: [
      'Stdio transport — no extra port, the client spawns `gd mcp` directly.',
      ...
      'Pins `GD_DATA_DIR` so the spawned MCP process writes to the same place the daemon already uses (avoids EPERM in packaged macOS app bundles).',
    ],
    snippets: [
      {
        ...
        body:
          ...
          '      "env": { "GD_DATA_DIR": "~/.galyarder-design" }\n' +
          '    }\n' +
          '  }\n' +
          '}',
      },
      ...
      },
      {
        ...
        language: 'bash',
        body: 'gd mcp live-artifacts',
      },
    ],
    footer:
      'In the Galyarder Design app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    intro:
      ...
      'Server-Sent Events with the contract types in `@galyarder-design/contracts`.',
    bullets: [
    ...
    footer:
      ...
      '`@galyarder-design/contracts` — import them in your script for full ' +
      'autocomplete without wiring a generator.',
  },
  {
    id: 'skills',
    tabLabel: 'Skills & headless',
    heading: 'Drop-in Skills for any agent — even without Galyarder Design running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). Galyarder Design extends the format with the ' +
      '`gd:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside Galyarder Design and by a vanilla ' +
    ...
    bullets: [
      ...
      'Headless: an agent with `gd` on its PATH can call `gd skills list` then run any skill; the daemon is optional for read-only flows.',
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + Galyarder Design extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        language: 'bash',
        body: 'gd skills list --json | jq \'.skills[].name\'',
      },
      {
        ...
        body:
          ...
          '# Shell 1: start Galyarder Design after ingesting.\n' +
          'pnpm tools-dev\n' +
          '# Shell 2: inspect the produced projects.\n' +
          'gd project list --json --daemon-url http://127.0.0.1:7456',
      },
    ],
    footer:
