...
} from '@galyarder-design/contracts/api/connectionTest';

export { validateBaseUrl } from '@galyarder-design/contracts/api/connectionTest';
...
// Override with GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS for slow networks
...
// Override with GD_CONNECTION_TEST_AGENT_TIMEOUT_MS.
...
// the budget — e.g. `GD_CONNECTION_TEST_AGENT_TIMEOUT_MS=3000000000` —
...
export function resolveConnectionTestTimeoutMs(
  key: 'GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS' | 'GD_CONNECTION_TEST_AGENT_TIMEOUT_MS',
  fallback: number,
...
function providerTimeoutMs(): number {
  return resolveConnectionTestTimeoutMs(
    'GD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS',
...
function codexExecutableGuidance(
  ...
  return ` Configured Codex path failed: ${configuredOverridePath}. Galyarder Design also detected a PATH Codex CLI at ${pathResolvedPath}. Update CODEX_BIN or clear the custom path to use the detected binary.`;
}
...
async function testAgentConnectionInternal(
  ...
  const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'gd-conn-test-'));
