...
hard-coded — they are polled from a connector at runtime. The GD daemon
...
> `"$GD_NODE_BIN" "$GD_BIN" tools live-artifacts {create,update}` and
...
3. Resolve `auth_ref` against the daemon's `media-config.json`. The
   ...
   - `<GD_MEDIA_CONFIG_DIR>/media-config.json` when that env var is set;
   - else `<GD_DATA_DIR>/media-config.json` when `GD_DATA_DIR` is set
