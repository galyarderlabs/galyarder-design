{
  description = "Galyarder Design — local-first design product. Daemon (`gd` CLI) + Next.js static web frontend.";
  ...
  outputs = {
    ...
    in {
      ...
      # Wrap `gd` with `--no-open` for `nix run`: the daemon package
      # builds the daemon workspace only, not `apps/web/out/`, so the
      # browser would otherwise auto-open onto an empty static dir.
      #
      # Set GD_DATA_DIR to a writable location when unset. The Nix store
      ...
      apps.default = {
        ...
        program = "${pkgs.writeShellScript "od-nix-run" ''
          export GD_DATA_DIR="''${GD_DATA_DIR:-$HOME/.od}"
          exec ${daemon}/bin/od --no-open "$@"
        ''}";
        meta.description = "Galyarder Design local daemon (`gd`)";
      };
      ...
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "🎨 Galyarder Design dev shell loaded!"
          echo ""
  ...
  in
    ...
    // {
      homeManagerModules = rec {
        galyarder-design = import ./nix/home-manager.nix {
      ...
      nixosModules = rec {
        galyarder-design = import ./nix/nixos.nix {
          inherit moduleCommon;
          flake = self;
        };
        default = galyarder-design;
