...
function fresh(): string {
  return mkdtempSync(path.join(tmpdir(), 'gd-skills-'));
}
...
describe('listSkills', () => {
  it('surfaces optional localized display metadata from SKILL.md frontmatter', async () => {
    ...
    try {
      ...
      mkdirSync(dir, { recursive: true });
      // i18n locale fixture; the CJK strings are zh-CN translations the resolver must surface.
      writeFileSync(
        path.join(dir, 'SKILL.md'),
        [
          '---',
          'name: localized',
          // i18n locale fixture; CJK is the zh-CN translation the resolver must surface.
          'zh_name: "本地化技能"',
          'en_name: "Localized Skill"',
          'description: "English fallback description."',
          // i18n locale fixture; CJK zh-CN translation kept verbatim.
          'zh_description: "中文描述。"',
          'en_description: "English localized description."',
          'gd:',
          '  example_prompt: "English fallback prompt."',
          '  example_prompt_i18n:',
          // i18n locale fixture; CJK zh-CN example prompt.
          '    zh-CN: "中文 prompt。"',
          '---',
          '',
      ...
      expect(skills[0]).toMatchObject({
        id: 'localized',
        displayName: {
          // i18n locale fixture; CJK is the zh-CN translation the resolver must return.
          en: 'Localized Skill',
          'zh-CN': '本地化技能',
        },
        descriptionI18n: {
  ...
  it('includes the built-in live-artifact skill catalog entry', async () => {
    ...
    expect(skill.body).toContain('"$GD_NODE_BIN" "$GD_BIN" tools live-artifacts create --input artifact.json');
    ...
    expect(skill.body).toContain('`GD_DAEMON_URL`');
    expect(skill.body).toContain('`GD_TOOL_TOKEN`');
  });
  ...
  it('includes the agent-browser skill as an external CLI integration', async () => {
    ...
    expect(skill.body).toContain('Galyarder Design Smoke Path');
  });
