/**
 * galyarder-design-landing-deck — input schema.
 *
 * Sister skill to `galyarder-design-landing`. Produces a single-file slide
...
import type { MixedText, BrandBlock, ImageryConfig } from '../galyarder-design-landing/schema';
...
export interface CoverSlide {
  kind: 'cover';
  /** Eyebrow above the title — `'Galyarder Design · Vol. 01'`. */
  eyebrow: string;
...
export interface EndSlide {
  kind: 'end';
  /** The huge kicker — `'Galyarder Design.'`. */
  mega: MixedText;
...
export interface GalyarderDesignLandingDeckInputs {
  $schema?: string;
  brand: BrandBlock;
  /** Deck-wide title shown in the HUD — `'Galyarder Design · Vol. 01'`. */
  deck_title: string;
  slides: Slide[];
...
/**
 * @deprecated Use `GalyarderDesignLandingDeckInputs`.
 *
 ...
 */
export type EditorialCollageDeckInputs = GalyarderDesignLandingDeckInputs;
