...
import { createSidecarLaunchEnv, requestJsonIpc } from "@galyarder-design/sidecar";
import {
...
function printRunForegroundResult(started: Partial<Record<ToolDevAppName, unknown>>, options: CliOptions): void {
  ...
  if (webUrl != null || daemonUrl != null) {
    process.stdout.write("\n  Galyarder Design dev server ready\n\n");
...
function createAppStamp(config: ToolDevConfig, appName: ToolDevAppName) {
  ...
  return {
    args: createProcessStampArgs(stamp, GALYARDER_DESIGN_SIDECAR_CONTRACT),
  ...
  try {
    ...
    return await spawnSidecarRuntime({
      ...
      env: {
        ...
        ...(spawnOptions.requireDesktopAuth ? { GD_REQUIRE_DESKTOP_AUTH: "1" } : {}),
      },
      logHandle,
    });
  } finally {
  ...
  try {
    ...
    return await spawnSidecarRuntime({
      ...
      env: {
        ...
        ...(options.parentPid == null ? {} : { [TOOLS_DEV_PARENT_PID_ENV]: String(options.parentPid) }),
        ...(options.prod === true
          ? { NODE_ENV: "production", GD_WEB_OUTPUT_MODE: "server", GD_WEB_PROD: "1" }
          : {}),
      },
      logHandle,
    });
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @galyarder-design/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@galyarder-design/desktop", "build"], process.env);
...
async function ensureDaemonCliBuild(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  ...
  await logHandle.write(`\n[tools-dev] building @galyarder-design/daemon because ${reason} at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@galyarder-design/daemon", "build"], process.env);
  await runLoggedCommand({
    args: invocation.args,
    command: invocation.command,
    cwd: config.workspaceRoot,
...
async function runForeground(config: ToolDevConfig, appName: string | undefined, options: CliOptions) {
  ...
  await new Promise<void>((resolveDone) => {
    const shutdown = () => {
      ...
      process.stderr.write("\nStopping Galyarder Design dev server...\n");
...
function addPortOptions(command: ReturnType<typeof cli.command>) {
  return command
    ...
    .option("--prod", "use production build (requires pnpm --filter @galyarder-design/web build first)");
}
