 ...
 *
 * The web-app/BYOK path goes through `@galyarder-design/contracts`'s
...
describe('daemon composeSystemPrompt — API mode (#313)', () => {
  ...
  describe('plain stream (streamFormat: plain)', () => {
    ...
    it('forbids the pseudo-tool markup observed in #313', () => {
      ...
      // Detection pattern; verifies the prompt forbids the Chinese pseudo-tool markup
      // (e.g. `[读取 …]`, where 读取 means "Read") observed leaking from the model in #313.
