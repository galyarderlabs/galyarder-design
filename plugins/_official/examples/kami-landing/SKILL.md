...
name: kami-landing
description: >
  Produce a print-grade single-page kami (kami, "paper") document — warm
...
Tag every editable element with `data-gd-id="<unique-slug>"` so the
...
└── example.html             # canonical Galyarder Design rendering
```
