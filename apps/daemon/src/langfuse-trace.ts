...
// official Galyarder Design telemetry relay or, for local smoke tests, directly to
// Langfuse. Without GALYARDER_DESIGN_TELEMETRY_RELAY_URL or LANGFUSE_PUBLIC_KEY /
...
export interface RuntimeInfo {
  ...
  arch?: string;
  /** Galyarder Design app version reported by the daemon. */
  appVersion?: string;
...
export function readTelemetrySinkConfig(
  ...
  const relayUrl = env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL?.trim();
  if (relayUrl) {
    return {
      kind: 'relay',
      relayUrl: relayUrl.replace(/\/+$/, ''),
      timeoutMs: parsePositiveInt(
        env.GALYARDER_DESIGN_TELEMETRY_TIMEOUT_MS ?? env.LANGFUSE_TIMEOUT_MS,
        DEFAULT_FETCH_TIMEOUT_MS,
      ),
      retries: parseNonNegativeInt(
        env.GALYARDER_DESIGN_TELEMETRY_RETRIES ?? env.LANGFUSE_RETRIES,
...
function buildTagList(ctx: ReportContext): string[] {
  const tags = ['galyarder-design', `project:${ctx.projectId}`];
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetchImpl(config.relayUrl, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-Galyarder-Design-Telemetry': 'langfuse-ingestion-v1',
        },
        signal: AbortSignal.timeout(config.timeoutMs),
        body,
      });
