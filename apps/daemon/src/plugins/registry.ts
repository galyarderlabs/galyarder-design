...
//
// - Scans `<daemonDataDir>/plugins/<id>/` (the GD-canonical install root) for
//   manifest folders.
// - Resolves a plugin folder into either an `galyarder-design.json`-anchored
...
export function defaultRegistryRoots(): RegistryRoots {
  return registryRootsForDataDir(path.resolve(process.env.GD_DATA_DIR ?? path.join(process.cwd(), '.gd')));
}
...
export async function resolvePluginFolder(opts: ResolveOptions): Promise<ResolveResult> {
  ...
  const sidecarPath = path.join(folder, 'galyarder-design.json');
  ...
  if (fs.existsSync(sidecarPath)) {
    ...
    if (!parsed.ok) {
      errors.push(...parsed.errors.map((e) => `galyarder-design.json: ${e}`));
    } else {
  ...
  if (!sidecar && adapters.length === 0) {
    return {
      ok: false,
      errors: [...errors, `Plugin folder contains no SKILL.md, no .claude-plugin/plugin.json, and no galyarder-design.json: ${folder}`],
      warnings,
    };
  }
