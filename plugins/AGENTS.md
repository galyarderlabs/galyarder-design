...
This directory owns Galyarder Design plugin content and plugin authoring material.
...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `galyarder-design.json` only as the GD sidecar.
- Keep `SKILL.md` bodies free of OD-only marketplace metadata. Put GD display, inputs, preview, pipeline, capabilities, and source information in `galyarder-design.json`.
- Do not import app-private code from plugin content. A plugin may reference GD atoms, design systems, craft docs, assets, scripts, MCP servers, or connectors through the manifest.
...
- Use the v1 JSON schema at `docs/schemas/galyarder-design.plugin.v1.json`.
...
```bash
gd plugin validate ./plugins/spec/examples/<plugin-id>
```
