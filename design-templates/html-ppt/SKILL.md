---
name: html-ppt
description: HTML PPT Studio — author professional static HTML presentations in many styles, layouts, and animations, all driven by templates. Use when the user asks for a presentation, PPT, slides, keynote, deck, slideshow, "Slides", "Speech Draft", "Create a PPT", "Create a slide deck", a reveal-style HTML deck, a Xiaohongshu Graphics, or any kind of multi-slide pitch/report/sharing document that should look tasteful and be usable with keyboard navigation. Triggers include keywords like "presentation", "ppt", "slides", "deck", "keynote", "reveal", "slideshow", "Slides", "Speech Draft", "Sharing Draft", "Xiaohongshu Graphics", "talk slides", "pitch deck", "tech sharing", "technical presentation".
triggers:
  - "ppt"
  - "deck"
  - "slides"
...
od:
  ...
  animations: true
  example_prompt: >-
    Create a 12-page HTML presentation using html-ppt. First let's align on three things: content/page-count/audience, design theme (recommend 2-3 out of 36 sets), and the starting template (pitch-deck, tech-sharing, weekly-report, xhs-post, or presenter-mode-reveal). Once aligned, begin writing the slides.
---
...
- **15 full-deck templates** (`templates/full-decks/<name>/`) — complete multi-slide decks with scoped `.tpl-<name>` CSS. 8 extracted from real-world decks (xhs-white-editorial, graphify-dark-graph, knowledge-arch-blueprint, hermes-cyber-terminal, obsidian-claude-gradient, testing-safety-alert, xhs-pastel-card, dir-key-nav-minimal), 7 scenario scaffolds (pitch-deck, product-launch, tech-sharing, weekly-report, xhs-post 3:4, course-module, **presenter-mode-reveal** — Dedicated Presenter Mode)
...
### 🎤 Presenter Mode (Presenter Mode + Verbatim Transcript)

If the user mentions any of: **Speech / Sharing / Keynote / Transcript / speaker notes / presenter view / Presenter View / Teleprompter**, or says things like "I need to present xxx to my team", "I want to do a tech sharing session", "I'm worried about not speaking smoothly", "I want a PPT with speaker transcripts" — **use the `presenter-mode-reveal` full-deck template** and write 150–300 words of speaker transcript in each slide's `<aside class="notes">`.
...
1. **Not a script, but visual cues** — bold key terms + make transition sentences independent paragraphs
2. **150–300 words per page** — maintaining a pace of 2–3 minutes per page
3. **Use conversational language, not formal prose** — "therefore" → "so", "this solution" → "this way"
...
- 🟠 **SPEAKER SCRIPT** — large-font verbatim transcript (scrollable)
...
Only `presenter-mode-reveal` is designed from the ground up around the feature with proper example speaker transcript on every slide.
...
2. **Style / theme.** Which of the 36 themes fits? If unsure, recommend 2-3
   ...
   - Xiaohongshu Graphics → `xiaohongshu-white`, `soft-pastel`, `rainbow-gradient`,
     `magazine-bold`
...
3. **Starting point.** One of the 14 full-deck templates, or scratch? Point
   ...
   user's content suggests something obvious (e.g. "I want to host a product launch" →
   `product-launch`), propose it confidently instead of asking blindly.

A good opening message looks like:

> I can build this PPT for you! First, let's confirm three things:
> 1. Outline content / page count / who is the audience?
> 2. Style preference? I recommend choosing one of these 3 themes: `tokyo-night` (looks great by default for tech sharing), `xiaohongshu-white` (Xiaohongshu style), `corporate-clean` (formal reporting).
> 3. Do you want to bootstrap with my existing `tech-sharing` full-deck template?
...
- **NEVER put presenter-only text on the slide itself.** Descriptive text like
  "This page displays..." or "Speaker: You can supplement here..." or small explanatory captions
...
- [references/presenter-mode.md](references/presenter-mode.md) — **Presenter Mode + speaker notes Writing Guide (Must-Read for Tech Sharing/Presentations)**.
