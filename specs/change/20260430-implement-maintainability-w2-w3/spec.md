...
- Root scripts run daemon, web, build, tests, and typecheck through pnpm filters; root `typecheck` currently targets only `@galyarder-design/web`. Source: `package.json:12-25`
...
- Decision: Broaden typechecking from web-only to contracts, daemon, scripts, and e2e support. Root `typecheck` currently filters only `@galyarder-design/web`; daemon has tests but no typecheck script; e2e already uses TypeScript configs and MJS/CJS operational files. Source: `package.json:19-25`, `apps/daemon/package.json:9-23`, `apps/web/tsconfig.json:2-23`, `e2e/package.json:6-12`, `e2e/playwright.config.ts:1-58`
...
- A compiled daemon TypeScript target is the safest full migration end state for the package bin and root `gd` entrypoint. Source: `apps/daemon/package.json:6-13`, `package.json:9-10`
...
- Contracts: run `pnpm --filter @galyarder-design/contracts typecheck`; add lightweight type-level coverage via exported example payloads or `tsc`-checked fixture files. Source: `specs/current/maintainability-roadmap.md:57-58`
- Web adoption: run `pnpm --filter @galyarder-design/web typecheck` and existing web tests after importing shared DTO/SSE/error types. Source: `apps/web/package.json:6-10`, `apps/web/tsconfig.json:2-23`
- Daemon adoption: add and run `pnpm --filter @galyarder-design/daemon typecheck`, then `pnpm --filter @galyarder-design/daemon test`; daemon already uses Vitest with TypeScript config. Source: `apps/daemon/package.json:9-23`, `apps/daemon/vitest.config.ts:1-8`
...
- Runtime migration: after each TypeScript conversion batch, run daemon tests and root typecheck; after script/e2e migration, run `pnpm --filter @galyarder-design/e2e test` and a Playwright reporter smoke run when feasible. Source: `package.json:19-25`, `e2e/package.json:6-12`, `e2e/playwright.config.ts:22-37`
...
- The root `gd` bin and daemon package bin currently point at JavaScript entrypoints; conversion to `.ts` requires a compiled output target before script/bin paths change. Source: `package.json:9-10`, `apps/daemon/package.json:6-13`
...
- `packages/contracts/package.json` - added `@galyarder-design/contracts` package metadata, source exports, and `typecheck` script.
...
- `apps/web/package.json` and `apps/daemon/package.json` - added workspace dependencies on `@galyarder-design/contracts` for boundary type adoption.
...
- `package.json` - updated the root `gd` bin to the compiled daemon CLI path.
...
- `corepack pnpm --filter @galyarder-design/contracts typecheck` - passed.
- `corepack pnpm --filter @galyarder-design/web typecheck` - passed as a package graph/type resolution sanity check.
...
- `corepack pnpm install` - passed after adding app dependencies on `@galyarder-design/contracts`; lockfile links web and daemon to the workspace package.
- `corepack pnpm --filter @galyarder-design/contracts typecheck` - passed after Step 2 adoption.
- `corepack pnpm --filter @galyarder-design/web typecheck` - passed after Step 2 adoption.
- `corepack pnpm --filter @galyarder-design/web test -- src/providers/sse.test.ts` - passed; Vitest also ran existing artifact manifest tests in the web package.
- `corepack pnpm --filter @galyarder-design/daemon test -- sse-response.test.mjs` - passed; Vitest also ran existing daemon artifact manifest and json event stream tests.
- `corepack pnpm install` - passed after adding daemon TypeScript/type dependencies.
- `corepack pnpm --filter @galyarder-design/daemon typecheck` - passed after adding the daemon TypeScript foundation.
- `corepack pnpm --filter @galyarder-design/daemon test` - passed; all 18 daemon tests passed.
...
- `corepack pnpm --filter @galyarder-design/daemon typecheck` - passed after daemon module conversion.
- `corepack pnpm --filter @galyarder-design/daemon build` - passed and emitted compiled daemon output under `apps/daemon/dist`.
- `corepack pnpm --filter @galyarder-design/daemon test` - passed after daemon module conversion; all 18 daemon tests passed.
...
- `corepack pnpm --filter @galyarder-design/e2e exec tsc -p ../scripts/tsconfig.json --noEmit` - passed for root TypeScript scripts.
- `corepack pnpm --filter @galyarder-design/daemon build` - passed before e2e support typechecking and live-script import validation.
- `corepack pnpm --filter @galyarder-design/e2e typecheck` - passed for Playwright config, reporter, report metadata, and e2e support scripts.
...
- `corepack pnpm --filter @galyarder-design/e2e test:ui:clean` - passed against the TypeScript cleanup script.
- `corepack pnpm --filter @galyarder-design/e2e exec playwright test -c playwright.config.ts --list` - passed as a Playwright config/reporter loading smoke check and listed 15 Chromium UI tests.
...
- `corepack pnpm --filter @galyarder-design/e2e exec tsc -p ../scripts/tsconfig.json --noEmit` - passed after adding the residual JavaScript scanner.
