...
const PACKAGED_CONFIG_PATH_ENV = "GD_PACKAGED_CONFIG_PATH";
const UPDATE_ACTION_TIMEOUT_MS = 10 * 60 * 1000;

function desktopStamp(config: ToolPackConfig): SidecarStamp {
  return {
    app: APP_KEYS.DESKTOP,
    ipc: resolveAppIpcPath({ app: APP_KEYS.DESKTOP, contract: GALYARDER_DESIGN_SIDECAR_CONTRACT, namespace: config.namespace }),
...
async function findManagedDesktopProcessTree(config: ToolPackConfig): Promise<number[]> {
  ...
  const stampedRootPids = processes
    .filter((processInfo) =>
      matchesStampedProcess(processInfo, { mode: SIDECAR_MODES.RUNTIME, namespace: config.namespace, source: SIDECAR_SOURCES.TOOLS_PACK }, GALYARDER_DESIGN_SIDECAR_CONTRACT),
    )
