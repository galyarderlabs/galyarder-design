...
// galyarderlabs/galyarder-design#368).
//
...
// developer's machine — see galyarder_design_architecture.md), so the
...
function buildInlineStyleBlock(tag: string, href: string, css: string): string {
  ...
  return (
    `<style data-gd-inline-asset="${escapeHtmlAttr(href)}"${attrString}>\n` +
