...
import { validateBaseUrl } from '@galyarder-design/contracts/api/connectionTest';
import {
  agentIdToTracking,
...
export function SettingsDialog({
  ...
  const handleInstallLatest = useCallback(async () => {
    ...
    try {
      const res = await fetch('https://api.github.com/repos/galyarderlabs/galyarder-design/releases/latest', {
    ...
    window.open('https://github.com/galyarderlabs/galyarder-design/releases', '_blank', 'noopener,noreferrer');
  ...
  // Global Escape is handled by the DS Dialog primitive (dismissable=true).
  // The manual keydown listener has been removed to avoid double-handling.
  ...
  const sectionHeader: Record<SettingsSection, { title: string; subtitle: string }> = {
    ...
    telemetry: { title: t('settings.telemetry'), subtitle: t('settings.telemetryHint') },
  ...
  // Narrow-breakpoint detection for NavRail collapse (accordion at <768px).
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        {/* Accessible title — visually hidden via .sr-only when the header renders it.
            No custom id — Radix manages the id internally for aria-labelledby. */}
        <DialogTitle className="sr-only">
          {t('settings.title')}
        </DialogTitle>

        {/* Top-right chrome strip */}
        ...
        <header className="modal-head" id="settings-dialog-title-visible">
          {welcome ? (
            <>
        ...
        <div className="modal-body">
          {/* ── Vertical sub-rail navigation ─────────────────────────────── */}
          {isNarrow ? (
            /* Accordion at narrow widths */
            <nav className="settings-sidebar settings-sidebar-accordion" aria-label="Settings sections">
              {/* Five required sections first */}
              <details open={activeSection === 'privacy' || activeSection === 'telemetry'}>
                <summary
                  className={`settings-accordion-summary${activeSection === 'privacy' ? ' active' : ''}`}
              ...
              <details open={activeSection === 'execution'}>
                <summary
                  className={`settings-accordion-summary${activeSection === 'execution' ? ' active' : ''}`}
                  onClick={(e) => { e.preventDefault(); setActiveSection('execution'); }}
                >
                  <Icon name="sliders" size={16} />
                  <span>{t('settings.envConfigure')}</span>
                </summary>
              </details>
              <details open={activeSection === 'language'}>
                <summary
                  className={`settings-accordion-summary${activeSection === 'language' ? ' active' : ''}`}
                  onClick={(e) => { e.preventDefault(); setActiveSection('language'); }}
                >
                  <Icon name="languages" size={16} />
                  <span>{t('settings.language')}</span>
                </summary>
              </details>
              <details open={activeSection === 'telemetry'}>
                <summary
                  className={`settings-accordion-summary${activeSection === 'telemetry' ? ' active' : ''}`}
                  onClick={(e) => { e.preventDefault(); setActiveSection('telemetry'); }}
                >
                  <DsIcon name="BarChart2" size={16} />
                  <span>{t('settings.telemetry')}</span>
                </summary>
              </details>
              {/* Additional sections */}
              {([
                { id: 'instructions', icon: 'edit', label: 'Instructions / Rules' },
                { id: 'memory', icon: 'history', label: t('settings.memory') },
                { id: 'media', icon: 'image', label: t('settings.mediaProviders') },
                { id: 'skills', icon: 'grid', label: t('settings.skills') },
                { id: 'mcpClient', icon: 'sparkles', label: t('settings.externalMcpTitle') },
                { id: 'composio', icon: 'sliders', label: t('connectors.title') },
                { id: 'integrations', icon: 'link', label: t('settings.mcpServerTitle') },
                { id: 'appearance', icon: 'sun-moon', label: t('settings.appearance') },
                { id: 'critiqueTheater', icon: 'comment', label: t('critiqueTheater.settingsNav') },
                { id: 'notifications', icon: 'bell', label: t('settings.notifications') },
                { id: 'pet', icon: 'sparkles', label: t('pet.navTitle') },
                { id: 'designSystems', icon: 'draw', label: t('settings.designSystems') },
                { id: 'about', icon: 'settings', label: t('settings.about') },
              ] as Array<{ id: SettingsSection; icon: string; label: string }>).map(({ id, icon, label }) => (
                <details key={id} open={activeSection === id}>
                  <summary
                    className={`settings-accordion-summary${activeSection === id ? ' active' : ''}`}
            ...
            >
              {/* ── Five required sections (Req 13.1) ── */}
              <NavItem
              ...
              <NavItem
                ...
                label={t('settings.envConfigure')}
                active={activeSection === 'execution'}
                onClick={() => setActiveSection('execution')}
              />
              <NavItem
                icon={<DsIcon name="Cpu" size={20} />}
              ...
              />
              <NavItem
                icon={<DsIcon name="BarChart2" size={20} />}
              ...
              <NavItem
                ...
                label={t('settings.mediaProviders')}
              ...
              <NavItem
                ...
                label={t('settings.externalMcpTitle')}
                active={activeSection === 'mcpClient'}
                onClick={() => setActiveSection('mcpClient')}
              />
              <NavItem
                icon={<Icon name="sliders" size={20} />}
                label={t('connectors.title')}
                active={activeSection === 'composio'}
                onClick={() => setActiveSection('composio')}
              />
              <NavItem
                icon={<Icon name="link" size={20} />}
                label={t('settings.mcpServerTitle')}
                active={activeSection === 'integrations'}
                onClick={() => setActiveSection('integrations')}
              />
              <NavItem
                icon={<Icon name="sun-moon" size={20} />}
              ...
              <NavItem
                ...
                label={t('critiqueTheater.settingsNav')}
                active={activeSection === 'critiqueTheater'}
              ...
              <NavItem
                icon={<Icon name="sparkles" size={20} />}
                label={t('pet.navTitle')}
                active={activeSection === 'pet'}
                onClick={() => setActiveSection('pet')}
              />
              <NavItem
                icon={<Icon name="draw" size={20} />}
                label={t('settings.designSystems')}
                active={activeSection === 'designSystems'}
          ...
          <ScrollArea
            ...
            ariaLabel={activeHeader.title || undefined}
            viewportClassName="settings-content-viewport"
          >
            <div ref={settingsContentRef} className="settings-content-inner">
              ...
              <label className="field">
                ...
                >
                  {apiModelOptions.map((m) => (
                    <SelectItem value={m.id} key={m.id}>{apiModelOptionLabel(m)}</SelectItem>
                  ))}
                  <SelectItem value={CUSTOM_MODEL_SENTINEL}>{t('settings.modelCustom')}</SelectItem>
                </Select>
              ...
              {apiProtocol === 'senseaudio' ? (
                <label className="field">
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      updateApiConfig({ byokImageModel: val })
                    }
                    placeholder={
          ...
          <section className="settings-section">
            <Combobox
              items={LOCALES.map((code) => ({
                ...
                keywords: [LOCALE_LABEL[code], code],
              }))}
              value={locale}
              onValueChange={(code) => {
              ...
              label={t('settings.language')}
              emptyState={t('settings.language')}
              className="settings-locale-combobox"
            />
          </section>
          ) : null}
              ...
              <div className="memory-field-block instructions-rules-card">
                <div className="memory-block-head">
                  <div>
                    ...
                    <p className="hint">
                      Fixed instructions GalyarderDesign follows in every chat. These are
          ...
          {activeSection === 'telemetry' ? (
            <TelemetrySection cfg={cfg} setCfg={setCfg} />
          ) : null}
...
/**
 * Telemetry section — exposes the three telemetry toggles (metrics,
 * content, artifact manifest) from the privacy config as a dedicated
 * section so the five required sections (Req 13.1) are each visibly
 * labeled. Uses DS `Switch` primitives for the toggle controls.
 */
function TelemetrySection({
  ...
  function patchTelemetry(patch: Partial<typeof telemetry>): void {
    setCfg((c) => {
    ...
    <section className="settings-section">
      <div className="settings-privacy-toggles">
        <div className="settings-telemetry-row">
          <div className="settings-telemetry-row-text">
            <span className="settings-telemetry-row-label">{t('settings.privacyMetrics')}</span>
            <span className="settings-telemetry-row-hint">{t('settings.privacyMetricsHint')}</span>
          </div>
          <DsSwitch
            checked={telemetry.metrics === true}
            aria-label={t('settings.privacyMetrics')}
            onCheckedChange={(v) => {
        ...
        <div className="settings-telemetry-row">
          <div className="settings-telemetry-row-text">
            <span className="settings-telemetry-row-label">{t('settings.privacyContent')}</span>
            <span className="settings-telemetry-row-hint">{t('settings.privacyContentHint')}</span>
          </div>
          <DsSwitch
            checked={telemetry.content === true}
            aria-label={t('settings.privacyContent')}
            onCheckedChange={(v) => {
              trackSettingsPrivacyClick(analytics.track, {
                page_name: 'settings',
                area: 'privacy',
        ...
        <div className="settings-telemetry-row">
          <div className="settings-telemetry-row-text">
            <span className="settings-telemetry-row-label">{t('settings.privacyArtifacts')}</span>
            <span className="settings-telemetry-row-hint">{t('settings.privacyArtifactsHint')}</span>
          </div>
          <DsSwitch
            checked={telemetry.artifactManifest === true}
            aria-label={t('settings.privacyArtifacts')}
            onCheckedChange={(v) => {
              trackSettingsPrivacyClick(analytics.track, {
                page_name: 'settings',
                area: 'privacy',
...
// and Linux ship a system /usr/bin/gd (octal-dump) that shadows any
// `gd` we might add to PATH, and most Galyarder Design users run from
// source where `gd` is not installed globally. The installer panel
// must NOT reference bare `gd`.
type McpClientId =
  | 'claude'
  | 'codex'
  | 'cursor'
...
[mcp_servers.galyarder-design.env]
...
function IntegrationsSection() {
  ...
  const MCP_CLIENTS: McpClient[] = [
    {
      ...
      buildSnippet: (info) => {
        ...
        return `claude mcp add-json --scope user galyarder-design '${inner}'`;
      },
    ...
    },
    {
      ...
      buildDeeplink: (info) => {
        ...
        return `cursor://anysphere.cursor-deeplink/mcp/install?name=galyarder-design&config=${encoded}`;
      },
  ...
  // so snippets work even when `gd` isn't on PATH (the realistic
  // case for source clones, plus macOS/Linux ship a /usr/bin/gd that
      ...
      >
        {THEMES.map(({ value, labelKey, icon }) => (
          <ToggleGroupItem key={value} value={value}>
            {icon ? <Icon name={icon} size={16} aria-hidden="true" /> : null}
 ...
 *     `galyarder-design:config` localStorage blob; this is what gates
