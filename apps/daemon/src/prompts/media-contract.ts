 ...
 * binary). Instead it shells out to a single command — `gd media
...
shell command through \`GD_NODE_BIN\` + \`GD_BIN\` is HOW you actually produce bytes.
...
- \`GD_NODE_BIN\`    — absolute path to the Node-compatible runtime that started the daemon. Packaged desktop installs provide this even when the user has no system \`node\` on PATH.
- \`GD_BIN\`         — absolute path to the GD CLI script. On POSIX shells run with \`"$GD_NODE_BIN" "$GD_BIN" …\`.
- \`GD_PROJECT_ID\`  — the active project's id. Pass it as \`--project "$GD_PROJECT_ID"\`.
- \`GD_PROJECT_DIR\` — the project's files folder (your cwd). Generated files land here.
- \`GD_DAEMON_URL\`  — base URL of the local daemon, e.g. \`http://127.0.0.1:7456\`.

If any of these are unset, the user is running you outside the GD daemon —
ask them to relaunch from the GD app (or pass the values explicitly).
TODO (post-v1): teach the media dispatcher to auto-spawn a transient
daemon when invoked outside the GD app, so a user running \`claude\`
...
\`\`\`bash
"$GD_NODE_BIN" "$GD_BIN" media generate \\
  --project "$GD_PROJECT_ID" \\
...
For media projects, \`"$GD_NODE_BIN" "$GD_BIN" media generate …\` is the **only**
...
before the first generate attempt. Treat \`GD_NODE_BIN\`, \`GD_BIN\`,
\`GD_PROJECT_ID\`, and \`GD_DAEMON_URL\` as the source of truth and try the dispatcher
first.
...
For most GD requests ("test video", "5s product reveal", "demo clip"),
...
# — GD projects already have their own design-system layer. Default to
...
drive the render to completion by calling \`media wait <taskId>\` through \`GD_NODE_BIN\` + \`GD_BIN\` in
...
- \`exit 2\` — still **running**. Final stdout line is
  ...
  \`"$GD_NODE_BIN" "$GD_BIN" media wait <taskId> --since <n>\` to continue from where you left
...
\`\`\`bash
out=$("$GD_NODE_BIN" "$GD_BIN" media generate --surface video --model … --image …)
ec=$?
if [ "$ec" -ne 0 ]; then
  echo "$out" >&2; exit "$ec"
fi
...
while [ -n "$task_id" ]; do
  out=$("$GD_NODE_BIN" "$GD_BIN" media wait "$task_id" --since "$since")
  ec=$?
...
A note on \`fetch failed\` to \`127.0.0.1\`. The GD daemon runs on
...
agent's sandbox / network policy. Do not claim "the GD daemon is down"
...
- **video**:   ${VIDEO_IDS}
  ...
  \`--image <project-relative-path>\` to \`"$GD_NODE_BIN" "$GD_BIN" media generate\`. The
...
2. **One discovery turn before generating.** Even with metadata defaults
   ...
   message first, and do not call \`"$GD_NODE_BIN" "$GD_BIN" media generate\` (that path is
   intentionally rejected for this model).
3. **Generate by shell, narrate in chat.** When you actually invoke
   \`"$GD_NODE_BIN" "$GD_BIN" media generate\`, do it inside a clearly-labelled tool call. After
...
4. **Iterate by re-running.** To revise, call \`"$GD_NODE_BIN" "$GD_BIN" media generate\` again
...
1. **HTTP status.** When stubs are disabled (the default release-build
   ...
   error message. Set \`GD_MEDIA_ALLOW_STUBS=1\` to write a labelled
   placeholder instead.
2. **Exit code.** \`"$GD_NODE_BIN" "$GD_BIN" media generate\` exits \`0\` for
   both immediate completion and successful queued/running handoff; inspect
   the final stdout JSON for either \`file\` or \`taskId\`. \`"$GD_NODE_BIN"
   "$GD_BIN" media wait\` exits \`0\` on terminal **done**, \`2\` when the
