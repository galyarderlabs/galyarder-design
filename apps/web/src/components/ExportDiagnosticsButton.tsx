...
interface GalyarderDesignDesktopApi {
  exportDiagnostics(): Promise<DesktopExportResult>;
}

declare global {
  interface Window {
    galyarderDesignDesktop?: GalyarderDesignDesktopApi;
  }
}

const DIAGNOSTICS_EXPORT_PATH = '/api/diagnostics/export';
const DIAGNOSTICS_FILENAME_PREFIX = 'galyarder-design-diagnostics';
 ...
 * save dialog via window.galyarderDesignDesktop) and the browser (triggers a
...
export function ExportDiagnosticsRow() {
  ...
  const handleClick = async () => {
    ...
    setStatus({ kind: 'busy' });
    try {
      if (window.galyarderDesignDesktop != null) {
        const result = await window.galyarderDesignDesktop.exportDiagnostics();
        if (result.ok) {
