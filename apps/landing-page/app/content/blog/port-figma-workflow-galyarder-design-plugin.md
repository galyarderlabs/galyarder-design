---
title: "How to port a Figma workflow into an Galyarder Design plugin"
date: 2026-05-18
category: "Use cases"
readingTime: 6
...
The 0.8.0-preview thread makes a sharper ask: port that muscle memory into a plugin. Not a panel bolted onto a canvas. Not a private script only one team can run. A reusable Galyarder Design workflow that an agent can pick up, execute, review, and hand off through the same local-first loop as any other design task.

This is the practical version of the [0.8.0-preview call for plugins](https://github.com/galyarderlabs/galyarder-design/discussions/1727). If your team has one repeatable design workflow today, this post shows what it looks like to turn it into a plugin-shaped contribution.
...
That is why Galyarder Design's [skill layer](/blog/why-we-built-galyarder-design-as-a-skill-layer/) matters. A plugin is not an opaque runtime extension. It is a folder of files: a `SKILL.md` contract, optional design systems, optional examples, and an `galyarder-design.json` sidecar that tells Galyarder Design how to display and apply the workflow.

## The Galyarder Design angle is portability

The plugin spec states the contract plainly: `SKILL.md` stays the executable agent contract, while `galyarder-design.json` adds marketplace metadata, input fields, defaults, previews, and context wiring.

That gives one workflow two lives. In Galyarder Design, it appears as a plugin with a preview, inputs, provenance, and a one-click "use" path. In Claude Code, Cursor, Codex, Gemini CLI, OpenClaw, or another skill catalog, the same folder still works as a plain agent skill because the core behavior lives in Markdown.
...
| Galyarder Design metadata | `galyarder-design.json` |
...
3. **Add the Galyarder Design sidecar.** Create `galyarder-design.json` so the marketplace can show the title, description, required inputs, preview, and source repo.
...
registry names at scaffold time; `gd plugin scaffold` creates
...
with `gd plugin login` and `gd plugin whoami --json`, then follow the
publishing docs for the current review path. Galyarder Design does not store
...
| Review section IDs, copy, density, and responsive behavior | Human in the Galyarder Design preview |
...
If your team has a Figma export, token sync, brand kit, or deck template that keeps coming back, port the smallest repeatable slice first. Start with a `SKILL.md`, add `galyarder-design.json`, validate it, and open the PR before the workflow grows into a private tool nobody else can reuse. The screenshot-to-prototype example shows the plugin-shaped version: a portable skill plus an Galyarder Design sidecar.

[Try this workflow](https://github.com/galyarderlabs/galyarder-design/tree/main/plugins/spec/examples/import-screenshot-to-prototype).

## Related reading

- [31 skills, 72 systems: how the Galyarder Design library works](/blog/31-skills-72-systems-how-the-library-works/) — the primitives a plugin wraps
- [Why we built Galyarder Design as a skill layer, not a product](/blog/why-we-built-galyarder-design-as-a-skill-layer/) — why the workflow is file-shaped instead of account-shaped
