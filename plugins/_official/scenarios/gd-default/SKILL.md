---
name: gd-default
description: Hidden fallback scenario for free-form Home prompts. Ask the task type first, then continue through the matching Galyarder Design flow.
od:
  scenario: default-router
  mode: scenario
---

# gd-default (hidden scenario)
...
{
  "description": "I will route the free-form prompt through the right Galyarder Design workflow.",
  "questions": [
    {
      "id": "taskType",
...
- `Image`: plan a concrete image prompt, then use the GD media generation
  CLI for image output.
- `Video`: plan shots, duration, aspect, and motion, then use the GD media
...
- `Audio`: plan voice/music/SFX intent, then use the GD media generation
  CLI for audio output.
- `Other`: ask only the minimum follow-up needed, then choose the closest
  Galyarder Design workflow and continue.
