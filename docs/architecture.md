...
GD is a web app plus a local daemon. The split means the same UI can run in three shapes:
...
- **`artifact.json` metadata** → GD can reconstruct the artifact tree without a DB.
...
| File | Purpose |
|---|---|
| `~/.galyarder-design/config.toml` | daemon-global: default agent preference, keys (optional, BYOK), telemetry opt-in (default off) |
| `~/.galyarder-design/agents.json` | cached agent detection results |
...
`baseDir` is stored on `metadata.baseDir` and GD reads / writes directly
...
- **Fail-closed.** Two coordinated mechanisms prevent the gate from
  ...
  - An **orchestrator-pinned mode** via the `GD_REQUIRE_DESKTOP_AUTH=1`
...
- **Daemon restart edge.** If the daemon is restarted while desktop
  keeps running, the new daemon process will be in `GD_REQUIRE_DESKTOP_AUTH`
...
- **tools-dev split-start hardening.** `tools-dev start desktop`
  ...
  `GD_REQUIRE_DESKTOP_AUTH=1` (the env var is only injected when
...
services:
  daemon:
    ...
    volumes: [ "~/.galyarder-design:/root/.galyarder-design", "./:/workspace" ]
    ports: ["7456:7456"]
  web:
    image: openclaudedesign/web
    ports: ["3000:3000"]
    environment: [ "GD_DAEMON_URL=http://daemon:7456" ]
```
...
# flip VERCEL env flag GD_MODE=direct to hide daemon-connect UI
```
...
- First generation latency: dominated by agent model time; GD overhead should be < 50 ms.
