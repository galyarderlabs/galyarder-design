...
describe('PluginShareMenu', () => {
  ...
  it('copies an install command for marketplace plugins using the registry entry name', async () => {
    renderMenu(
      make({
        id: 'mp-plugin',
        sourceKind: 'github',
        source: 'github:galyarder-design/plugins/mp-plugin',
        marketplaceId: 'official',
        marketplaceEntryName: 'galyarder-design/mp-plugin',
      }),
    );
    openPopover();
    clickItem('Copy install command');
    await Promise.resolve();
    expect(writes).toContain('gd plugin install galyarder-design/mp-plugin');
  });
  ...
  it('copies the github source string for github-installed plugins', async () => {
    ...
    clickItem('Copy install command');
    await Promise.resolve();
    expect(writes).toContain('gd plugin install github:owner/repo@main/sub');
  });
  ...
  it('renders official bundled repo links as anchors', () => {
    ...
    const repoLinks = Array.from(
      container.querySelectorAll<HTMLAnchorElement>(
        'a.plugin-share-item[href="https://github.com/galyarderlabs/galyarder-design"]',
      ),
    );
