...
// Adapter from a portable SKILL.md (with optional `gd:` frontmatter, see
...
// any SKILL.md that already carries the `gd:` frontmatter we use today.
//
// Inputs:
...
// fields (e.g. gd.parameters live sliders that v1 manifest does not surface).
...
const ROLE_PARAMETER_KEYS = ['gd.parameters'];
...
export function adaptAgentSkill(
  ...
  const gd = isObject(frontmatter['gd']) ? frontmatter['gd'] : {};
  ...
  const designSystemFm = isObject(gd['design_system']) ? gd['design_system'] : null;
  ...
  const craftFm = isObject(gd['craft']) ? gd['craft'] : null;
  ...
  const inputs: InputField[] | undefined = mapInputs(gd['inputs'], warnings);

  // gd.parameters are deferred to Phase 4 per spec §5.4; record a warning so
  // doctor surfaces them instead of silently dropping.
  for (const key of ROLE_PARAMETER_KEYS) {
    const [namespace, sub] = key.split('.');
    if (namespace === 'gd' && sub && Array.isArray(gd[sub])) {
      warnings.push(`SKILL.md ${key} is preserved as adapter metadata; v1 manifest does not expose live sliders`);
    }
  }

  const previewFm = isObject(gd['preview']) ? gd['preview'] : null;
  ...
  const manifest: PluginManifest = {
    ...
    gd: {
      kind: 'skill',
      taskKind: stringOr(gd['taskKind'], 'new-generation') as PluginManifest['gd'] extends infer T ? T extends { taskKind?: infer K } ? K : never : never,
      mode: stringOr(gd['mode'], '') || undefined,
      platform: stringOr(gd['platform'], '') || undefined,
      scenario: stringOr(gd['scenario'], '') || undefined,
      preview,
...
function examplePromptFromFrontmatter(fm: FrontmatterObject, body: string): string {
  const gd = isObject(fm['gd']) ? fm['gd'] : {};
  const direct = stringOr(gd['example_prompt'], '').trim();
