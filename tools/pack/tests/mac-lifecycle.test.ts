...
const resolveAppIpcPath = vi.fn(() => "/tmp/galyarder-design/ipc/test/desktop.sock");
...
vi.mock("@galyarder-design/sidecar", () => ({
  createSidecarLaunchEnv,
  requestJsonIpc,
  resolveAppIpcPath,
}));

vi.mock("@galyarder-design/platform", () => ({
...
describe("startPackedMacApp", () => {
  it("writes a launch override when the bundled config is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-tools-pack-mac-lifecycle-"));
    try {
      const config = makeConfig(root);
      const paths = resolveMacPaths(config);
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "Galyarder Design");
      ...
      const launchConfigPath = join(config.roots.runtime.namespaceRoot, "runtime", "galyarder-design-config.json");
      const launchEnv = spawnLoggedProcess.mock.calls[0]?.[0]?.env as NodeJS.ProcessEnv | undefined;

      expect(result.source).toBe("installed");
      expect(result.status?.state).toBe("running");
      expect(launchEnv?.GD_PACKAGED_CONFIG_PATH).toBe(launchConfigPath);
  ...
  it("passes a launch override config path for portable mac starts", async () => {
    ...
    try {
      ...
      const paths = resolveMacPaths(config);
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "Galyarder Design");
      const bundledConfigPath = join(paths.installedAppPath, "Contents", "Resources", "galyarder-design-config.json");
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          appVersion: "1.2.3",
          daemonCliEntryRelative: "galyarder-design/bin/gd",
  ...
  it("uses the preview executable name for preview release namespaces", async () => {
    ...
    try {
      ...
      const paths = resolveMacPaths(config);
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "Galyarder Design Preview");
