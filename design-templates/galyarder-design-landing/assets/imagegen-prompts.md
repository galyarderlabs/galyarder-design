...
This pack is consumed by the `galyarder-design-landing` skill. Every page-level
...
## 4. Localized project input template

A compact, brand-agnostic prompt skeleton suitable for projects that
need verbatim copy in a non-English language. Replace the placeholders
with the localized strings; keep the visual constraints intact.

```text
Generate a 16:9 landscape web page visual in the style of a high-end
creative AI studio site: modern Swiss editorial layout, Bauhaus
geometry, classical plaster sculpture collage, minimal architecture,
handmade paper texture, hairline drafting marks.

Brand text:        "<BRAND_NAME>"
Navigation:        "<NAV_1>", "<NAV_2>", "<NAV_3>", "<NAV_4>"
Eyebrow label:     "<EYEBROW>"
Main headline (render verbatim):  "<MAIN_HEADLINE>"
Italic emphasis (italic serif):   "<ITALIC_WORDS>"
Body copy (render verbatim):      "<BODY_COPY>"
Button text:       "<PRIMARY_CTA>", "<SECONDARY_CTA>"

Main subject: <subject description>
Texture motifs: <sky / stone / plants / figures / eyes / mountains / water / UI screenshots, etc.>
Composition: <text-left/image-right | image-left/text-right | top headline with horizontal cards below | centered collage | timeline columns>
Color: warm ivory paper, black text, limestone/concrete gray,
       charcoal, low-saturation coral-red accents, occasional
       mustard yellow or pale sky blue.

Constraints: text must be crisp and legible, no extra random words, no
            watermarks, no cartoon style, no neon, no heavy shadows,
            no clichéd tech blue-purple gradients.
```
