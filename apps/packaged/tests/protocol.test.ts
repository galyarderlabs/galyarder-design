/**
 * Regression coverage for the `gd://` protocol proxy in
 * apps/packaged/src/protocol.ts.
 *
 * The packaged Electron entry registers `gd://` as the loader for the
 ...
 *
 * @see https://github.com/galyarderlabs/galyarder-design/issues/895
 */
...
describe('gd:// protocol proxy', () => {
  it('proxies the request through fetchImpl with the rewritten target URL', async () => {
    ...
    const request = new Request('gd://app/api/codex-pets/sync', { method: 'POST' });
  ...
  it('preserves the request path, search, and hash when rewriting to the web sidecar', async () => {
    ...
    const request = new Request('gd://app/api/projects?limit=5#section', { method: 'GET' });
  ...
  it('returns a 502 Response when the underlying fetch rejects (issue #895)', async () => {
    ...
    expect(body.error).toBe('GD_PROTOCOL_PROXY_FAILED');
  ...
  it('does not throw when fetch rejects (the actual #895 root-cause guard)', async () => {
    ...
    await expect(
      handleOdRequest(new Request('gd://app/'), 'http://127.0.0.1:1/', fetchImpl),
