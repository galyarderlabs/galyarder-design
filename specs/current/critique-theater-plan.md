...
```bash
cd /c/Users/ekada/OneDrive/Desktop/Githubcontributing/galyarder-design
git status
...
```bash
pnpm --filter @galyarder-design/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @galyarder-design/contracts test critique.test.ts
```
...
```bash
pnpm --filter @galyarder-design/contracts test sse.test.ts
```
...
import type { PanelEvent } from '@galyarder-design/contracts/critique';
...
```bash
pnpm --filter @galyarder-design/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@galyarder-design/contracts/critique';
...
```bash
pnpm --filter @galyarder-design/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@galyarder-design/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@galyarder-design/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@galyarder-design/contracts/critique';
...
```bash
pnpm --filter @galyarder-design/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @galyarder-design/daemon test orchestrator.test.ts
```
...
} from '@galyarder-design/contracts/critique';
import { panelEventToSse } from '@galyarder-design/contracts/sse';
import { parseCritiqueStream } from './parser';
...
it('routes through critique orchestrator when GD_CRITIQUE_ENABLED=true', async () => {
  // mock CLI emitting the happy fixture
  process.env.GD_CRITIQUE_ENABLED = 'true';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@galyarder-design/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@galyarder-design/contracts/critique';
...
```bash
pnpm --filter @galyarder-design/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@galyarder-design/contracts/sse';
import type { PanelistRole } from '@galyarder-design/contracts/critique';
...
- [ ] **Step 1–5:** Add the toggle bound to `GD_CRITIQUE_ENABLED`. Persist through the existing settings endpoint. Test that the daemon reads the new value at run start. Commit.
...
docs/critique-theater.md
  ...
  3. Settings reference
     - GD_CRITIQUE_ENABLED env var and the in-app toggle
...
- [ ] **Step 1: Default `GD_CRITIQUE_ENABLED=false`.**
...
```bash
git commit -m "chore(rollout): M0 ships behind GD_CRITIQUE_ENABLED=false"
```
