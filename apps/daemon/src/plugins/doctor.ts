...
//
//   - Validates the manifest using @galyarder-design/plugin-runtime's validateSafe.
...
// the CLI renders them as `gd plugin doctor <id>` output. Spec §11.5 promises
// a richer report (severity / kind enum) which we'll layer in once Phase 4
// adds the diagnostics endpoint.

import { manifestSourceDigest, resolveContext, validateSafe, type RegistryView } from '@galyarder-design/plugin-runtime';
...
export function doctorPlugin(
  ...
  for (const atomId of manifest.gd?.context?.atoms ?? []) {
    if (!isKnownAtom(atomId)) {
      issues.push({
        severity: 'error',
        code: 'atom.unknown',
        message: `Unknown atom id: '${atomId}'.`,
        field: 'gd.context.atoms',
      });
  ...
  for (const stage of manifest.gd?.pipeline?.stages ?? []) {
    for (const atomId of stage.atoms ?? []) {
      if (!isKnownAtom(atomId)) {
        issues.push({
          ...
          field: `gd.pipeline.stages.${stage.id}`,
        });
      }
    }
  ...
  for (const surface of manifest.gd?.genui?.surfaces ?? []) {
    if (!surface.component) continue;
    const declared = new Set(manifest.gd?.capabilities ?? []);
...
// matches the live plugin's freshly computed digest. Called by `gd plugin
