...
describe('plugins/_official/atoms roster', () => {
  ...
  it('every atom folder ships SKILL.md + galyarder-design.json (the spec §3 cross-catalog floor)', async () => {
    for (const id of EXPECTED_ATOMS) {
      const folder = path.join(atomsRoot, id);
      const skill = await stat(path.join(folder, 'SKILL.md'));
      const manifest = await stat(path.join(folder, 'galyarder-design.json'));
