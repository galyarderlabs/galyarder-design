...
const CONNECTORS_USAGE = `Usage:
  gd tools connectors list [--use-case personal_daily_digest] [--format compact]
  gd tools connectors execute --connector <id> --tool <name> --input input.json
  gd tools connectors github-design-context --repo owner/repo [--ref main] [--output context/github/owner-repo.md] [--max-files 48] [--require-connector]
  gd tools connectors local-design-context --path /path/to/project [--output context/local-code/project.md] [--max-files 48]
  gd tools connectors design-system-package-audit --path /path/to/project [--reference-package] [--fail-on-warnings]

Environment:
  GD_NODE_BIN     Node-compatible runtime for agent wrapper invocations
  GD_BIN          Galyarder Design CLI script for agent wrapper invocations
  GD_DAEMON_URL   Daemon base URL injected into agent runs
  GD_TOOL_TOKEN   Bearer token injected into agent runs

Agent runtime invocation:
  "$GD_NODE_BIN" "$GD_BIN" tools connectors list --use-case personal_daily_digest --format compact
`;
...
function daemonUrl(): URL | { error: string } {
  const rawUrl = process.env.GD_DAEMON_URL;
  if (!rawUrl) return { error: 'GD_DAEMON_URL is required' };
  try {
    ...
    return { error: 'GD_DAEMON_URL must be a valid URL' };
  }
}

function toolToken(): string | { error: string } {
  const token = process.env.GD_TOOL_TOKEN;
  if (!token) return { error: 'GD_TOOL_TOKEN is required' };
  return token;
}
...
async function collectGithubEvidenceWithGitClone(
  ...
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-github-context-'));
...
function renderGithubDesignEvidenceMarkdown(evidence: GithubDesignEvidence): string {
  ...
  const lines = [
    ...
    evidence.method === 'connector'
      ? '- Connector platform fallback was used through `gd tools connectors`.'
...
function renderLocalDesignEvidenceMarkdown(evidence: LocalDesignEvidence): string {
  ...
  const lines = [
    ...
    '## Intake Status',
    '',
    '- Local source folder was read through bounded `gd tools connectors local-design-context` intake.',
  ];
  if (evidence.warnings.length > 0) {
    lines.push('', '## Warnings', '', ...evidence.warnings.map((warning) => `- ${warning}`));
  }
  ...
  try {
    ...
    if (connectorReady) {
      ...
      const connectorReason = 'error' in baseUrl
        ? baseUrl.error
        : typeof token === 'string'
          ? 'GD_TOOL_TOKEN is not available'
          : token.error;
      if (options.requireConnector) {
        return fail('Required GitHub repository intake could not read the repository through git, GitHub CLI, or connector', {
          repo: `${repo.owner}/${repo.repo}`,
...
export async function auditDesignSystemPackage(
  ...
  if (options.referencePackage === true) {
    if (!fileSet.has('DESIGN.md')) {
      addIssue('warning', 'missing_galyarder_design_rules', 'Reference packages may omit DESIGN.md, but generated Galyarder Design packages must include it as the canonical rules file.', 'DESIGN.md');
    }
  } else {
