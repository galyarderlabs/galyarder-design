...
This package captures a source-backed Galyarder Design design system for a desktop AI chat workspace. It includes reusable rules, token CSS, focused review previews, preserved assets, preserved fonts, and an applied UI kit.
...
- DESIGN.md is the canonical Galyarder Design rules document.
...
Use this skill when creating Galyarder Design artifacts that should match the Cherry Studio desktop AI chat workspace.
...
description: Use this skill when creating Galyarder Design artifacts that should match the Cherry Studio desktop AI chat workspace.
user-invocable: true
---
...
This package is intended for reusable Galyarder Design work, so future agents should keep the output grounded in captured evidence, use preserved assets instead of redrawing brand marks, keep app surfaces compact, and inspect preview cards before introducing any new component pattern. Treat it as a focused product design kit, not a generic style summary.
...
function auditComponent(componentName: string): string {
  ...
  { id: 'review', label: '${componentName} review state', detail: 'Reusable review surface for future Galyarder Design projects.' },
];
...
`, 'utf8');
  ...
  it('appends curated useCase query params for connector listing', async () => {
    process.env.GD_DAEMON_URL = 'http://127.0.0.1:7456/base/';
    process.env.GD_TOOL_TOKEN = 'agent-run-token';
  ...
  it('includes curation in compact connector output', async () => {
    process.env.GD_DAEMON_URL = 'http://127.0.0.1:7456';
    process.env.GD_TOOL_TOKEN = 'agent-run-token';
  ...
  it('writes GitHub design evidence through connected connector tools', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-connectors-cli-'));
  ...
  it('writes bounded local design evidence snapshots from a linked folder', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-local-context-'));
    process.chdir(tmpDir);
  ...
  it('prioritizes core app surfaces over nested tool buttons during local intake', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-local-core-context-'));
    process.chdir(tmpDir);
  ...
  it('passes a Claude Design-style design-system package audit', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-pass-'));
    process.chdir(tmpDir);
  ...
  it('fails a design-system package audit when manifest docs point at old scaffold paths', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-stale-docs-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('fails a design-system package audit when package titles come from URL protocol text', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-protocol-title-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when SKILL.md is missing agent-discoverable frontmatter', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-skill-frontmatter-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when SKILL.md lacks Claude-style reusable skill sections', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-skill-sections-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when README.md lacks a source-backed product overview', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-readme-overview-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when README.md lacks a Claude-style package reuse guide', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-readme-package-guide-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when README.md lacks a concrete preview manifest', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-readme-preview-manifest-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when the applied UI-kit README lacks a reuse guide', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-uikit-readme-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await writeFile(path.join(tmpDir, 'DESIGN.md'), AUDIT_DESIGN_MD);
  ...
  it('warns when build runtime icon evidence is not preserved in the package', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-build-assets-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('warns when preserved build runtime assets do not match captured evidence bytes', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-fake-build-assets-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('accepts preserved build runtime assets that match captured evidence bytes', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-source-build-assets-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('warns when the brand-assets preview redraws instead of referencing preserved assets', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-brand-preview-assets-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('fails a design-system package audit when modular UI-kit components are placeholders', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-thin-components-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('fails a design-system package audit when the UI-kit entry does not load its modules or token CSS', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-disconnected-uikit-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('fails a design-system package audit when the UI-kit entry lists modules without rendering them', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-unmounted-uikit-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('fails a design-system package audit when JSX components are loaded without browser runtime scripts', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-missing-jsx-runtime-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/pages/home'), { recursive: true });
  ...
  it('fails a design-system package audit when script-loaded JSX components do not expose browser globals', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-missing-browser-global-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/pages/home'), { recursive: true });
  ...
  it('fails a design-system package audit when chat evidence lacks UI-kit role coverage', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-missing-roles-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('fails a design-system package audit when the app shell does not compose role components', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-uncomposed-app-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/pages/home'), { recursive: true });
  ...
  it('fails a design-system package audit when rich binary evidence is collapsed to one asset and font', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-thin-binaries-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
  ...
  it('fails a design-system package audit when preserved fonts are not bound in token CSS', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-font-binding-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'fonts/ubuntu'), { recursive: true });
  ...
  it('warns when visual artifacts do not reference source-backed component names', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-generic-visuals-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('warns when focused preview cards do not apply tokens to source components', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-preview-source-context-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('warns when rich component evidence is not preserved as source examples outside context', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-source-examples-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
    await mkdir(path.join(tmpDir, 'context/local-code/cherry/files/src/components'), { recursive: true });
  ...
  it('warns when source-backed examples are only tiny stubs', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-thin-source-examples-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app/components'), { recursive: true });
  ...
  it('fails a design-system package audit when evidence-backed artifacts are missing', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-fail-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
  ...
  it('can audit an external Claude Design reference package without DESIGN.md', async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-package-audit-reference-'));
    process.chdir(tmpDir);
    await mkdir(path.join(tmpDir, 'preview'), { recursive: true });
    await mkdir(path.join(tmpDir, 'ui_kits/app'), { recursive: true });
    await mkdir(path.join(tmpDir, 'assets'), { recursive: true });
    ...
    expect(JSON.parse(stdoutOutput.join(''))).toMatchObject({
      ...
      errors: [],
      warnings: expect.arrayContaining([
        expect.objectContaining({ code: 'missing_galyarder_design_rules', path: 'DESIGN.md' }),
      ]),
    });
