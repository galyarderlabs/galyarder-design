  ...
  return (
    <section className="home-hero" data-testid="home-hero">
      <div className="home-hero__brand" aria-hidden>
        ...
        <span className="home-hero__brand-name">Galyarder Design</span>
      </div>
            ...
            {hoveredPlugin ? (
              ...
              >
                ...
                <div className="home-hero__plugin-hover-meta">
                  <span>{t('homeHero.parameters', { n: (hoveredPlugin.manifest?.gd?.inputs ?? []).length })}</span>
...
function getPluginQueryPreview(plugin: InstalledPluginRecord): string {
  const raw = plugin.manifest?.gd?.useCase?.query;
  const value =
    typeof raw === 'string'
      ? raw
      : raw && typeof raw === 'object' && !Array.isArray(raw)
        ? raw.en ?? raw.id ?? Object.values(raw).find((entry): entry is string => (
...
function pluginRecordSlugs(record: InstalledPluginRecord): Set<string> {
  ...
  const rawValues = [
    ...
    fieldString(gd, 'mode'),
    fieldString(gd, 'surface'),
    fieldString(gd, 'scenario'),
    fieldString(gd, 'taskKind'),
...
function renderPluginPresetQuery(record: InstalledPluginRecord, query: string): string {
  const fields = record.manifest?.gd?.inputs ?? [];
...
function describeStructuredPresetPrompt(
  ...
  const artifact = pluginPresetArtifactLabel(chipId);
...
function pluginPresetArtifactLabel(chipId: string): string {
  switch (chipId) {
...
function homeHeroChipPromptExamples(chipId: string, _locale: Locale): string[] {
  switch (chipId) {
    case 'prototype':
      return [
