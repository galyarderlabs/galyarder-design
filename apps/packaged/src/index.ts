...
import { applyOsLocaleSwitch } from "@galyarder-design/desktop/main";
...
async function main(): Promise<void> {
  ...
  const argvStamp = readProcessStamp(process.argv.slice(1), GALYARDER_DESIGN_SIDECAR_CONTRACT);
  ...
  const { runDesktopMain } = await import("@galyarder-design/desktop/main");
  await runDesktopMain(runtime, {
    ...
    // gd://app/ renderer URL, which Node/undici cannot resolve through
