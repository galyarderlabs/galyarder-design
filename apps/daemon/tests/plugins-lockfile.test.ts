...
const plugin: InstalledPluginRecord = {
  ...
  version: '0.1.0',
  sourceKind: 'github',
  source: 'github:galyarderlabs/galyarder-design@main/plugins/community/registry-starter',
  ...
  resolvedSource: 'github:galyarderlabs/galyarder-design@main/plugins/community/registry-starter',
  resolvedRef: 'main',
...
describe('plugin lockfile', () => {
  ...
  it('writes stable .gd/gd-plugin-lock.json content', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'gd-lock-'));
    try {
      const filePath = path.join(dir, '.gd', 'gd-plugin-lock.json');
