...
import {
  APP_KEYS,
  GALYARDER_DESIGN_SIDECAR_CONTRACT,
  SIDECAR_MESSAGES,
  SIDECAR_MODES,
  SIDECAR_SOURCES,
  ...
  type SidecarStamp,
} from "@galyarder-design/sidecar-proto";
import { createSidecarLaunchEnv, requestJsonIpc, resolveAppIpcPath } from "@galyarder-design/sidecar";
import {
  ...
  stopProcesses,
} from "@galyarder-design/platform";
...
const PRODUCT_NAME = "Galyarder Design";
const APP_IMAGE_PRODUCT_NAME = "Galyarder-Design";
const DESKTOP_LOG_ECHO_ENV = "GD_DESKTOP_LOG_ECHO";
...
const PRODUCTION_INSTALL_PNPM_BIN_ENV = "GD_TOOLS_PACK_PNPM_BIN";
...
export function buildDockerArgs(
  ...
  //     @galyarder-design/sidecar-proto (restricted to namespace charset)
  ...
  if (config.telemetryRelayUrl != null) {
    dockerArgs.push("-e", `GALYARDER_DESIGN_TELEMETRY_RELAY_URL=${config.telemetryRelayUrl}`);
  }
  dockerArgs.push(
    "-w",
    "/project",
...
function desktopFileName(namespace: string): string {
  return `galyarder-design-${sanitizeNamespace(namespace)}.desktop`;
}

function iconFileName(namespace: string): string {
  return `galyarder-design-${sanitizeNamespace(namespace)}.png`;
}
...
function resolveLinuxPaths(config: ToolPackConfig): LinuxPaths {
  ...
  return {
    ...
    packagedConfigPath: join(namespaceRoot, "galyarder-design-config.json"),
    resourceRoot: join(namespaceRoot, "resources", "galyarder-design"),
...
// buildDockerArgs sets GD_TOOLS_PACK_PNPM_BIN to the standalone pnpm binary it
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  await runPnpm(config, ["--filter", "@galyarder-design/contracts", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/registry-protocol", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/sidecar-proto", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/sidecar", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/platform", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/agui-adapter", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/plugin-runtime", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/diagnostics", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/daemon", "build"]);
  try {
    await runPnpm(config, ["--filter", "@galyarder-design/web", "build"], { GD_WEB_OUTPUT_MODE: "server" });
    await runPnpm(config, ["--filter", "@galyarder-design/web", "build:sidecar"]);
  ...
  await runPnpm(config, ["--filter", "@galyarder-design/desktop", "build"]);
  await runPnpm(config, ["--filter", "@galyarder-design/packaged", "build"]);
}
...
async function writeAssembledApp(
  ...
  const packageJson = {
    name: "galyarder-design-packaged",
  ...
  const mainStub = `"use strict";\nrequire("@galyarder-design/packaged");\n`;
    ...
    `${JSON.stringify(
      {
        appVersion: version,
        namespace: config.namespace,
        nodeCommandRelative: "galyarder-design/bin/node",
...
async function writeLinuxBuilderConfig(config: ToolPackConfig, paths: LinuxPaths): Promise<void> {
  ...
  const builderConfig: Record<string, unknown> = {
    appId: "io.galyarder-design.desktop",
    ...
    extraMetadata: {
      main: "./main.cjs",
      name: "galyarder-design-packaged-app",
      productName: PRODUCT_NAME,
      version: packageVersion,
      ...(config.portable ? {} : { odToolsPackRuntimeRoot: config.roots.runtime.namespaceBaseRoot }),
    },
    extraResources: [
      { from: paths.resourceRoot, to: "galyarder-design" },
      { from: paths.packagedConfigPath, to: "galyarder-design-config.json" },
    ],
    ...
    linux: {
      ...
      synopsis: "Galyarder Design",
      maintainer: "Galyarder Design Contributors",
    },
...
export async function installPackedLinuxApp(config: ToolPackConfig): Promise<LinuxInstallResult> {
  ...
  const rendered = renderDesktopTemplate(template, {
    ...
    iconName: `galyarder-design-${sanitizeNamespace(config.namespace)}`,
  });
...
async function validateDesktopAppImageMarker(
  ...
  // GD_PACKAGED_NAMESPACE -- so apps/packaged falls back to a SIDECAR_SOURCES.PACKAGED
  ...
  const expectedIpc = resolveAppIpcPath({
    ...
    contract: GALYARDER_DESIGN_SIDECAR_CONTRACT,
    namespace: config.namespace,
  });
  const stampOk =
...
export async function startPackedLinuxApp(config: ToolPackConfig): Promise<LinuxStartResult> {
  ...
  const args = ["--appimage-extract-and-run", ...createProcessStampArgs(stamp, GALYARDER_DESIGN_SIDECAR_CONTRACT)];
...
//   <assembledAppRoot>/node_modules/@galyarder-design/packaged/dist/headless.mjs
// The bundled Node binary lives at:
//   <namespaceRoot>/resources/galyarder-design/bin/node  (populated by copyResourceTree)

function resolveHeadlessEntryPath(paths: LinuxPaths): string {
  return join(paths.assembledAppRoot, "node_modules", "@galyarder-design", "packaged", "dist", "headless.mjs");
}
...
function headlessLauncherPath(config: ToolPackConfig): string {
  return join(homedir(), ".local", "bin", `galyarder-design-headless-${sanitizeNamespace(config.namespace)}`);
}
...
export async function installPackedLinuxHeadless(config: ToolPackConfig): Promise<LinuxHeadlessInstallResult> {
  ...
  // pre-sanitized by sidecar-proto to [A-Za-z0-9._-]. GD_DATA_DIR is baked
  ...
  const script = [
    "#!/bin/sh",
    `# Galyarder Design headless launcher — namespace: ${config.namespace}`,
    `GD_PACKAGED_NAMESPACE=${JSON.stringify(config.namespace)} GD_DATA_DIR=${JSON.stringify(dataDir)} GD_RESOURCE_ROOT=${JSON.stringify(paths.resourceRoot)} exec ${JSON.stringify(nodePath)} ${JSON.stringify(entryPath)} "$@"`,
  ].join("\n") + "\n";
  ...
  try {
    child = await spawnBackgroundProcess({
      ...
      env: {
        ...
        GD_PACKAGED_NAMESPACE: config.namespace,
        ...
        // headless.ts computes: join(GD_DATA_DIR, "namespaces") which must
        // equal config.roots.runtime.namespaceBaseRoot.
        GD_DATA_DIR: dirname(config.roots.runtime.namespaceBaseRoot),
        GD_RESOURCE_ROOT: paths.resourceRoot,
      },
      logFd: logHandle.fd,
    });
  } finally {
