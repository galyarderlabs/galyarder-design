...
//
//   - `gd`            → SKILL.md + galyarder-design.json (canonical GD shape).
...
// installed plugin's fs_path, and reconstructs galyarder-design.json from
// the cached `manifest_json` so a publishable snapshot is reproducible
// even after an `gd plugin update` rotates the live source.
...
export type ExportTarget = 'gd' | 'claude-plugin' | 'agent-skill';
...
export async function exportPlugin(input: ExportInput): Promise<ExportResult> {
  ...
  if (input.target !== 'gd') {
    ...
    // 'gd' target: still ship SKILL.md as the portable anchor (per
  ...
  const readme = [
    ...
    '',
    'This folder was produced by `gd plugin export`.',
    '',
  ].join('\n');
