...
//   - Copy install command    (`gd plugin install <ref>`)
//   - Copy share link         (link to the marketplace detail page)
//   - Copy markdown badge     (Galyarder Design powered, includes link)
...
function buildInstallCommand(record: InstalledPluginRecord): string {
  ...
  if (typeof record.sourceMarketplaceEntryName === 'string') {
    return `gd plugin install ${record.sourceMarketplaceEntryName}`;
  }
  if (record.sourceKind === 'marketplace' && typeof record.sourceMarketplaceId === 'string') {
    return `gd plugin install ${record.sourceMarketplaceId}`;
  }
  return `gd plugin install ${record.source}`;
}
...
function buildMarkdownBadge(record: InstalledPluginRecord): string {
  ...
  return `[![${record.title} — Galyarder Design plugin](https://img.shields.io/badge/Open%20Design-${encodeURIComponent(record.title)}-d65a31?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2C)](${url})`;
}
