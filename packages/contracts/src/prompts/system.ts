...
export function composeSystemPrompt({
  ...
  // events — see #313. The CJK token `读取` (Chinese for "read") is
  // preserved verbatim because the system prompt below names it as a
  // forbidden literal output pattern and downstream tests assert on it.
  // Pin this preamble ABOVE DISCOVERY_AND_PHILOSOPHY so it beats the
  // discovery layer's own "these override anything later" header.
  ...
  if (isDeckProject && !hasSkillSeed) {
    ...
    // land here. If the user's brief is a deck/keynote/slides
    // (the CJK token `讲解` means "lecture / explanation" and is kept as
    // a Chinese-language brief-detection trigger), the agent used to
    // invent its own scale-to-fit + slide visibility + nav script from
    // scratch and shipped subtle CSS specificity bugs (per-slide layout
    // classes overriding `.slide { display:none }`). Inject the same
    // framework here, prefixed with a one-line conditional so the agent
    // only adopts it when the brief actually is a deck — otherwise the
    // directive is read as background reference and ignored.
    parts.push(
      `\n\n---\n\n## If this brief is a slide deck / keynote / presentation\n\nThe user did not pre-select a "Slide deck" surface, but their request may still call for one. **If — and only if — the brief reads as slides, keynote, presentation, deck, PPT, or 讲解 (Chinese-language deck/lecture trigger), follow the framework below.** Otherwise ignore everything in this section and continue with the freeform output you would have written anyway.\n\n${DECK_FRAMEWORK_DIRECTIVE}`,
    );
  }
 ...
 * markup (`<todo-list>...`, `[读取 X]` — `读取` is Chinese for "read";
 * kept verbatim because the prompt below names this exact literal as a
 * forbidden output pattern and downstream tests assert on it) instead of
 * producing structured `tool_use` events the UI can render — bug #313. Pinning the override
