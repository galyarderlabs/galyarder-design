...
`GD_CRITIQUE_*`. The user-facing label is sourced from a single i18n key,
...
3. **`GD_CRITIQUE_ENABLED` env override.** Power-user lane / CI
   fixtures.
...
|---|---|---|
| `malformed_block` | The adapter emitted a `<CRITIQUE>` block the parser rejects. | Re-run the conformance harness locally (`pnpm --filter @galyarder-design/daemon vitest run tests/critique-conformance.test.ts`) to confirm the adapter's transcript shape. The Phase 12 dashboard surfaces this status as a Prometheus series once that PR lands; until then the harness is the authoritative source. |
