...
describe('senseaudio media generation', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), 'gd-senseaudio-'));
    ...
    delete process.env.GD_SENSEAUDIO_API_KEY;
  ...
  it('renders SenseAudio speech with the documented defaults', async () => {
    ...
    const fetchMock = vi.fn(async (input: unknown, init?: RequestInit) => {
      ...
      expect(JSON.parse(String(init?.body))).toEqual({
        model: WIRE_MODEL,
        text: 'Hello, welcome to the SenseAudio text-to-speech service.',
        stream: false,
        voice_setting: {
    ...
    const result = await generateMedia({
      ...
      audioKind: 'speech',
      prompt: 'Hello, welcome to the SenseAudio text-to-speech service.',
  ...
  it('reads the API key from GD_SENSEAUDIO_API_KEY when storage is empty', async () => {
    process.env.GD_SENSEAUDIO_API_KEY = 'env-sense-key';
    const fetchMock = vi.fn(async (_input: unknown, init?: RequestInit) => {
      expect(init?.headers).toMatchObject({
