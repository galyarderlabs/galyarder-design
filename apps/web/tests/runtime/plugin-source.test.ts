...
describe('derivePluginSourceLinks · github sources', () => {
  it('builds tree URL with ref + subpath when source has both', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:galyarder-design/plugins@v1.2.0/make-a-deck',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/galyarder-design/plugins/tree/v1.2.0/make-a-deck');
    expect(out.sourceLabel).toBe('galyarder-design/plugins @v1.2.0/make-a-deck');
    expect(out.sourceKindLabel).toBe('GitHub');
    expect(out.contributeUrl).toBe('https://github.com/galyarder-design/plugins/issues/new');
  ...
  it('falls back to repo-root URL when no ref / subpath', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:galyarder-design/plugins',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/galyarder-design/plugins');
    expect(out.sourceLabel).toBe('galyarder-design/plugins');
  });
  ...
  it('uses pinnedRef when source has no inline ref', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/galyarder-design/plugins/tree/a1b2c3d4');
    expect(out.sourceLabel).toBe('galyarder-design/plugins @a1b2c3d4');
  });
  ...
  it('preserves slash-separated branch refs (release/1.0)', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/galyarder-design/plugins/tree/release/1.0');
  });
...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the Galyarder Design repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
        source:     'plugins/_official/scenarios/gd-code-migration',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/galyarderlabs/galyarder-design');
    expect(out.sourceKindLabel).toBe('Official');
    expect(out.sourceLabel).toBe('galyarderlabs/galyarder-design');
    expect(out.authorProfileUrl).toBe('https://github.com/galyarderlabs/galyarder-design');
    expect(out.homepageUrl).toBe('https://github.com/galyarderlabs/galyarder-design');
  });
});
...
describe('derivePluginSourceLinks · author + contribute', () => {
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'Galyarder Design', url: 'https://github.com/galyarderlabs' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('Galyarder Design');
    expect(out.authorProfileUrl).toBe('https://github.com/galyarderlabs');
    expect(out.authorAvatarUrl).toBe('https://github.com/galyarderlabs.png?size=80');
  });
  ...
  it('falls back to homepage for the contribute link when source is not github', () => {
    ...
    expect(out.contributeUrl).toBe('https://github.com/galyarderlabs/galyarder-design/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/galyarderlabs/galyarder-design');
  });
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('Galyarder Design')).toBe('GD');
