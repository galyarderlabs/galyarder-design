...
This note documents the preferred integration shape for a new Galyarder Design agent runtime.

## Recommendation

New agent runtimes should expose an **ACP over stdio** CLI mode.

In practice, Galyarder Design expects to spawn a local executable and speak JSON-RPC over the child process streams:

```text
Galyarder Design daemon
...
That wrapper keeps Galyarder Design on the standard ACP subprocess transport and avoids requiring a daemon-side network transport adapter.
...
ACP's remote HTTP/WebSocket transport is still described as a draft/proposal rather than the established compatibility path. Galyarder Design's implemented ACP adapters therefore use stdio subprocesses today.

## Messages Galyarder Design sends

For `streamFormat: 'acp-json-rpc'`, Galyarder Design currently drives a session with these JSON-RPC methods:

1. `initialize`
   - Sent first.
   - Includes Galyarder Design client metadata and `clientCapabilities`.
2. `session/new`
   - Creates a working session.
   - Includes the project working directory.
   - May include MCP server descriptors when the runtime is allowed to use Galyarder Design-provided tools.
3. `session/set_config_option` or `session/set_model` *(optional)*
   - Sent when the user selected a non-default model.
   - Galyarder Design prefers `session/set_config_option` when `session/new` reports a model config option; otherwise it falls back to `session/set_model`.
4. `session/prompt`
...
## Messages Galyarder Design expects from the agent
...
3. Notifications using `session/update`.
   - Galyarder Design currently maps:
...
4. Optional `session/request_permission` requests.
   - Galyarder Design auto-selects an approve/allow-style option when available.
...
5. Response to `session/prompt`.
   ...
   - This response tells Galyarder Design the turn is finished.
...
- After `session/prompt` completes, either exit cleanly when stdin closes or tolerate Galyarder Design sending `SIGTERM` after a short grace period.
- Implement `session/cancel` if possible. Galyarder Design falls back to process termination when the transport is no longer usable.
- Avoid interactive terminal prompts. If permission is required, use ACP permission requests instead.

## Galyarder Design adapter shape

An ACP runtime definition in Galyarder Design is intentionally small:
...
- Galyarder Design implementation:
