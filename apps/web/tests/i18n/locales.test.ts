...
const EXPECTED_LOCALES = ['en', 'id', 'de', 'pt-BR', 'es-ES', 'ru', 'fa', 'ar', 'ja', 'ko', 'pl', 'hu', 'fr', 'uk', 'tr', 'th', 'it'];
...
describe('i18n locales', () => {
  it('resolves the initial locale from browser language preferences', () => {
    ...
    expect(resolveSystemLocale(['zh-Hant-HK', 'en-US'])).toBe('en');
  ...
  it('registers every supported locale in the language menu', () => {
    ...
    // CJK literal; '日本語' is the Japanese self-name registered in
    // LOCALE_LABEL — see i18n/types.ts for the full canonical map.
