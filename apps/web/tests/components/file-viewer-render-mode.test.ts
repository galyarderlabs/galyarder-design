...
describe('shouldUrlLoadHtmlPreview', () => {
  ...
  it('falls back to srcDoc when the artifact ships the class based tweaks template', () => {
    ...
    // disabled (no `gd:tweaks-available` ever fires).
...
describe('hasUrlModeBridge', () => {
  it('detects an artifact-owned direct-edit bridge script', () => {
    expect(hasUrlModeBridge('<script src="gd-direct-edit.js"></script>')).toBe(true);
    expect(hasUrlModeBridge('<script defer src="./assets/gd-direct-edit.js?v=1"></script>')).toBe(true);
  });

  it('ignores comments, text nodes, and inline script bodies that only mention the bridge name', () => {
    expect(hasUrlModeBridge('<!-- TODO: ship gd-direct-edit.js -->')).toBe(false);
    expect(hasUrlModeBridge('<p>Use gd-direct-edit.js for editing</p>')).toBe(false);
    expect(hasUrlModeBridge('<script>console.log("gd-direct-edit.js")</script>')).toBe(false);
  });
