...
const SAMPLE_MANIFEST = (id: string) =>
  JSON.stringify({
    ...
    license: 'MIT',
    gd: { kind: 'atom', capabilities: ['prompt:inject'] },
  });

const SAMPLE_SKILL = (id: string) => `---\nname: ${id}\ndescription: bundled fixture\n---\n# ${id}\n`;

beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-bundled-'));
...
describe('registerBundledPlugins', () => {
  it('registers every <bundledRoot>/<tier>/<id>/ folder under source_kind=bundled', async () => {
    ...
    //   <bundledRoot>/atoms/discovery-question-form/{galyarder-design.json,SKILL.md}
    //   <bundledRoot>/scenarios/gd-new-generation/{galyarder-design.json,SKILL.md}
    const atomDir = path.join(tmpRoot, 'atoms', 'discovery-question-form');
    const sceneDir = path.join(tmpRoot, 'scenarios', 'gd-new-generation');
    await mkdir(atomDir, { recursive: true });
    await mkdir(sceneDir, { recursive: true });
    await writeFile(path.join(atomDir, 'galyarder-design.json'), SAMPLE_MANIFEST('discovery-question-form'));
    await writeFile(path.join(atomDir, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form'));
    await writeFile(path.join(sceneDir, 'galyarder-design.json'), SAMPLE_MANIFEST('gd-new-generation'));
    await writeFile(path.join(sceneDir, 'SKILL.md'), SAMPLE_SKILL('gd-new-generation'));

    const result = await registerBundledPlugins({ db, bundledRoot: tmpRoot });
    expect(result.registered.map((r) => r.id).sort()).toEqual(['discovery-question-form', 'gd-new-generation']);
  ...
  it('can stamp official registry provenance on bundled preinstalls', async () => {
    ...
    await writeFile(path.join(folder, 'galyarder-design.json'), SAMPLE_MANIFEST('starter'));
    ...
    expect(result.registered[0]?.sourceMarketplaceEntryName).toBe('galyarder-design/starter');
    ...
    expect(row?.sourceMarketplaceEntryName).toBe('galyarder-design/starter');
  });
  ...
  it('also registers a direct <bundledRoot>/<plugin-id>/ folder', async () => {
    ...
    await mkdir(folder, { recursive: true });
    await writeFile(path.join(folder, 'galyarder-design.json'), SAMPLE_MANIFEST('sample-plugin'));
  ...
  it('is idempotent — re-running upserts the same row', async () => {
    ...
    await mkdir(folder, { recursive: true });
    await writeFile(path.join(folder, 'galyarder-design.json'), SAMPLE_MANIFEST('sample'));
  ...
  it('skips folders without galyarder-design.json without warning', async () => {
