...
// <projectRoot>/.gd/projects/<projectId>/. The frontend's project list
...
function buildDesignManifest(entries, projectLabel) {
  ...
  return JSON.stringify({
    ...
    title: projectLabel || 'Galyarder Design project',
    entryFile,
    sourceFiles: {
      all: files,
      html: htmlFiles,
...
function buildDesignHandoff(entries, projectLabel) {
  ...
  return `# ${projectLabel || 'Galyarder Design project'} implementation handoff
...
async function uniqueRenameTempPath(source) {
  ...
  for (let i = 0; i < 10; i++) {
    const temp = path.join(dir, `.gd-rename-${process.pid}-${Date.now()}-${i}-${base}.tmp`);
    try {
      await stat(temp);
    } catch (err) {
...
// '_', so a Chinese filename like '测试文档.docx' (literally
// "test document.docx") became '____.docx' (issue #144). The CJK
// example is preserved here verbatim so this comment still describes
...
// directory (`.gd/projects/<id>`, `design-systems/<id>`, etc.). The character
...
// finalize / write operations outside `.gd/projects/`.
