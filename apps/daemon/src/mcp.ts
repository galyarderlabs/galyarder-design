// `gd mcp` - stdio MCP server that proxies project tool calls to the
// running daemon's HTTP API. Lets a coding agent in a *different* repo
// (Claude Code, Cursor, Zed) pull files from a local Galyarder Design
...
// every tool resolves to a fetch() against `GD_DAEMON_URL`. Spawn the
...
const SERVER_NAME = 'galyarder-design';
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no Galyarder Design activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every Galyarder Design project on this daemon.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Galyarder Design projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in Galyarder Design right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with Galyarder Design (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no Galyarder Design activity.",
        },
        include: {
          type: 'string',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no Galyarder Design activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal Galyarder Design project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          ...
          description: 'Optional ArtifactManifest sidecar. If omitted, Galyarder Design infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Galyarder Design artifact' },
  },
  // Catalog (skills, design systems) is intentionally NOT exposed as
  // MCP tools. Skills are recipes that Galyarder Design itself uses to
  ...
  // user can opt into via the resource URIs (gd://design-systems/...)
...
export async function runMcpStdio({ daemonUrl }: RunMcpOptions): Promise<void> {
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'Galyarder Design (GD) is a local-first design workspace. The user typically',
        'has GD running on their machine; each project contains a rendered',
        ...
        'default to the project the user has open in GD right now; get_file',
        ...
        '    [gd:file-window ...] marker when the file is longer; page',
        ...
        'gd://design-systems/<id>/DESIGN.md when you need the brand spec',
        'for a design (palette, typography, voice). Skills are similarly',
        'available at gd://skills/<id>/SKILL.md but are mostly relevant',
        'when the user asks about how a particular artifact was generated.',
        '',
        'When extending an Galyarder Design design in another codebase, pull',
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    const resources = [
      {
        uri: 'gd://focus/active',
        name: 'Active Galyarder Design context',
        description: 'The project/file the user has open in Galyarder Design right now.',
        mimeType: 'application/json',
      },
    ];
    for (const s of skillsData?.skills || []) {
      resources.push({
        uri: `gd://skills/${encodeURIComponent(s.id)}/SKILL.md`,
    ...
    for (const d of dsData?.designSystems || []) {
      resources.push({
        uri: `gd://design-systems/${encodeURIComponent(d.id)}/DESIGN.md`,
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
    ...
    const m = String(uri || '').match(/^gd:\/\/(skills|design-systems)\/([^/]+)\/(.+)$/);
    if (!m) {
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'Galyarder Design has no active project right now. The active context expires about 5 minutes after the last user interaction with Galyarder Design, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// renamed in the Galyarder Design UI shows up within a few seconds.
...
// open in Galyarder Design. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(baseUrl: string, arg: unknown): Promise<{ id: string; resolved: ResolvedProject | null; active: ActiveContext | null }> {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and Galyarder Design has no active project. The active context expires about 5 minutes after the last user interaction with Galyarder Design - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
async function getFile(baseUrl: string, project: string, relPath: string, active: ActiveContext | null, resolved?: ResolvedProject | null, offset = 0, limit = 2000) {
  ...
  if (!isTextualMime(mime)) {
    return errorResult(
      `file at "${relPath}" has mime "${mime}"; binary content is not yet supported by gd mcp. Use list_files to inspect its metadata.`,
    );
  }
  ...
  if (resolved && (resolved.source === 'slug' || resolved.source === 'substring')) {
    extra.push(`[gd:resolved-project id="${resolved.id}" name="${resolved.name}" via="${resolved.source}"]`);
  }
  if (truncated || start > 0) {
    ...
    extra.push(
      `[gd:file-window offset=${start} returnedLines=${returnedLines} totalLines=${totalLines}${next}]`,
    );
  }
  return {
    content: [
...
function formatActiveEchoLine(active: ActiveContext, resolvedPath: string): string {
  ...
  const note = `[gd:active-context project="${proj}" file="${resolvedPath}"]`;
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Galyarder Design daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
