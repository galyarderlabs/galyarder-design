...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-snapshot-'));
...
describe('snapshots writer', () => {
  it('createSnapshot inserts a row and round-trips via getSnapshot', () => {
    ...
    const snap = createSnapshot(db, baseInput({
      ...
      sourceMarketplaceEntryName: 'galyarder-design/sample-plugin',
      sourceMarketplaceEntryVersion: '1.0.0',
      marketplaceTrust: 'official',
      resolvedSource: 'github:galyarder-design/plugins@abc123/sample-plugin',
      resolvedRef: 'abc123',
    ...
    expect(fetched!.sourceMarketplaceEntryName).toBe('galyarder-design/sample-plugin');
    expect(fetched!.sourceMarketplaceEntryVersion).toBe('1.0.0');
    expect(fetched!.marketplaceTrust).toBe('official');
    expect(fetched!.resolvedSource).toBe('github:galyarder-design/plugins@abc123/sample-plugin');
