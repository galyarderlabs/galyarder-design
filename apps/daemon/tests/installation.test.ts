...
const SAVED_INSTALL_ENV = process.env.GD_INSTALLATION_DIR;

beforeEach(async () => {
  rootDir = await mkdtemp(join(tmpdir(), 'gd-install-test-'));
  dataDir = join(rootDir, 'namespace', 'data');
  installDir = join(rootDir, 'channel-root');
  await mkdir(dataDir, { recursive: true });
  await mkdir(installDir, { recursive: true });
  process.env.GD_INSTALLATION_DIR = installDir;
});

afterEach(async () => {
  if (SAVED_INSTALL_ENV == null) {
    delete process.env.GD_INSTALLATION_DIR;
  } else {
    process.env.GD_INSTALLATION_DIR = SAVED_INSTALL_ENV;
  }
  if (rootDir != null) {
...
describe('installation.json migration', () => {
  ...
  it('falls back to dataDir when GD_INSTALLATION_DIR is unset (dev / OSS / tools-dev paths)', async () => {
    delete process.env.GD_INSTALLATION_DIR;
