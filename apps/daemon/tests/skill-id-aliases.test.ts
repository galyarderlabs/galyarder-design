...
// Regression coverage for the editorial-collage → galyarder-design-landing rename.
...
beforeAll(async () => {
  skillsRoot = await mkdtemp(path.join(tmpdir(), 'gd-skills-aliases-'));
  ...
  await mkdir(path.join(skillsRoot, 'galyarder-design-landing'), { recursive: true });
  await writeFile(
    path.join(skillsRoot, 'galyarder-design-landing', 'SKILL.md'),
    '---\nname: galyarder-design-landing\ndescription: Atelier Zero landing.\n---\n\nbody\n',
    'utf8',
  );
  await mkdir(path.join(skillsRoot, 'galyarder-design-landing-deck'), {
    recursive: true,
  });
  await writeFile(
    path.join(skillsRoot, 'galyarder-design-landing-deck', 'SKILL.md'),
...
describe('SKILL_ID_ALIASES', () => {
  it('maps the editorial-collage rename to its current canonical id', () => {
    expect(SKILL_ID_ALIASES['editorial-collage']).toBe('galyarder-design-landing');
...
describe('resolveSkillId', () => {
  it('forwards deprecated ids to their canonical replacement', () => {
    expect(resolveSkillId('editorial-collage')).toBe('galyarder-design-landing');
...
describe('findSkillById', () => {
  it('resolves a project saved with the old editorial-collage id to the renamed skill', async () => {
    ...
    expect(skill.id).toBe('galyarder-design-landing');
  ...
  it('resolves a project saved with the old editorial-collage-deck id to the renamed deck skill', async () => {
    ...
    expect(skill.id).toBe('galyarder-design-landing-deck');
  });

  it('still resolves current ids exactly', async () => {
    const skills = await listSkills(skillsRoot);
    expect(findSkillById(skills, 'galyarder-design-landing')?.id).toBe(
  ...
  it('returns undefined for unknown ids and missing inputs', async () => {
    ...
    expect(findSkillById(null, 'galyarder-design-landing')).toBeUndefined();
  });
});
