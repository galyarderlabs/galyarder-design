...
describe('DesignSystemCreationFlow', () => {
  it('opens the project as soon as the workspace exists and prepares the first chat task afterward', async () => {
    ...
    await waitFor(() => expect(mocks.patchProject).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        pendingPrompt: expect.stringContaining('Create this project as a complete Galyarder Design design system workspace.'),
      }),
    ));
  ...
  it('creates a project-backed design system and hands the first task to the normal project chat', async () => {
    ...
    expect(window.sessionStorage.getItem(`gd:auto-send-first:${project.id}`)).toBe('1');
  ...
  it('allows GitHub repo links without Composio by using local GitHub intake', () => {
    ...
    expect(screen.getByText('Galyarder Design account')).toBeTruthy();
    expect(screen.getByText('Connector platform')).toBeTruthy();
    expect(screen.getByText('Coming soon')).toBeTruthy();
    expect(screen.getByText('Not configured')).toBeTruthy();

    fireEvent.change(input, { target: { value: 'https://github.com/galyarderlabs/galyarder-design/' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add' }));

    expect(screen.getByText('galyarderlabs/galyarder-design')).toBeTruthy();
  ...
  it('keeps a manual GitHub authorization action when the automatic popup is blocked', async () => {
    ...
    try {
      ...
      expect(screen.getByText('Popup blocked. Allow popups for Galyarder Design and try again.')).toBeTruthy();
  ...
  it('records connected GitHub repository sources in the project source manifest', async () => {
    ...
    expect(mocks.writeProjectTextFile).toHaveBeenCalledWith(
      project.id,
      'context/source-context.md',
      expect.stringContaining('https://github.com/galyarderlabs/galyarder-design'),
    );
    expect(mocks.writeProjectTextFile).toHaveBeenCalledWith(
      project.id,
      'context/source-context.md',
    ...
    expect(mocks.writeProjectTextFile).toHaveBeenCalledWith(
      project.id,
      'context/source-context.md',
      expect.stringContaining('"$GD_NODE_BIN" "$GD_BIN" tools connectors github-design-context --repo \'https://github.com/galyarderlabs/galyarder-design\' --output context/github/galyarderlabs-galyarder-design.md'),
    );
