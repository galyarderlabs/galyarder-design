  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    if (bold) {
      ...
      if (hex) {
        ...
        // Match either ASCII `:` or CJK fullwidth `：` (U+FF1A) — the
        // latter is auto-substituted by Chinese IMEs.
    ...
    // and "Text" labels. `[:：]` matches both ASCII and CJK fullwidth
    // colons.
...
function extractFonts(raw: string): FontHints {
  const out: FontHints = {};
  // `[:：]` accepts ASCII `:` or CJK fullwidth `：` (U+FF1A) so notes
  // pasted from Chinese-language design docs still parse.
