...
od:
  ...
  capabilities_required:
    ...
    - surgical_edit
  example_prompt: >-
    Build me a Notion-style team dashboard for Acme Studio. KPIs: total tasks, done this week, active members, docs in review. Wire it to the Notion connector and let it refresh on demand.
---
...
- Real connector mode: `POST /api/od/connectors/poll` with a JSON body
  ...
  values declared in `connectors.json`. The GD daemon resolves the
...
- `index.html` — single self-contained file, no external CSS / JS
  imports beyond a system font stack and a single GD `<live-counter>`
  custom element.
