...
function designSystemRef(record: InstalledPluginRecord): string | null {
  const ds = (record.manifest?.gd?.context as { designSystem?: ContextRef } | undefined)
    ?.designSystem;
  if (!ds) return null;
...
function specAssetPath(record: InstalledPluginRecord): string {
  ...
  // but `gd.context.assets[0]` may point at a different relpath when
  ...
  const assets = (record.manifest?.gd?.context?.assets ?? []) as string[];
