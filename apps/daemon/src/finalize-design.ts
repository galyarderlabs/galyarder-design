...
// tsconfig does not resolve the `@galyarder-design/contracts/api/chat` subpath
...
} from '@galyarder-design/contracts/api/finalize';
...
// canonical definitions live in @galyarder-design/contracts/api/finalize
 ...
 * look under `.gd/projects/<id>` and miss the real artifacts.
 *
...
export async function finalizeDesignPackage(
  ...
  // `.gd/projects/<id>`. resolveProjectDir handles both shapes; calling
  ...
  // For native projects, it lazily creates `.gd/projects/<id>`.
  ...
  try {
    ...
    // under metadata.baseDir rather than the empty `.gd/projects/<id>` dir.
