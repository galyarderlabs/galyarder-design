...
import type { ArtifactManifest } from '@galyarder-design/contracts';
...
beforeEach(async () => {
  cwd = await mkdtemp(path.join(os.tmpdir(), 'gd-figma-pipeline-e2e-'));
});
...
describe('figma-migration pipeline — full atom chain', () => {
  ...
  it('crosses the bundled-scenario fallback line cleanly: a figma-migration plugin without a pipeline still resolves', async () => {
    ...
    // gd.pipeline + has taskKind='figma-migration'. The smoke test
    ...
    const scenariosRoot = path.resolve(__dirname, '../../..', 'plugins', '_official', 'scenarios', 'gd-figma-migration', 'galyarder-design.json');
    const manifest = JSON.parse(await readFile(scenariosRoot, 'utf8'));
    expect(manifest.gd.pipeline.stages.map((s: { id: string }) => s.id)).toEqual([
