...
//
// Per memory `project_galyarder_design_493_merged.md`: this file uses
...
function setupResolverFixture(): { db: any; projectsRoot: string } {
  tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-finalize-'));
...
describe('finalizeDesignPackage (pipeline integration)', () => {
  function setupPipeline(
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-finalize-pipe-'));
  ...
  // unconditionally so DESIGN.md landed in the hidden .gd/projects/<id>
  ...
  it('writes DESIGN.md under metadata.baseDir for imported-folder projects', async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-finalize-imported-'));
    const designSystemsRoot = path.join(tempDir, 'design-systems');
    fs.mkdirSync(designSystemsRoot, { recursive: true });
    ...
    // The user's actual folder lives outside .gd/projects/.
...
// `.gd/projects/` via `path.join`. The HTTP layer happens to reject this
