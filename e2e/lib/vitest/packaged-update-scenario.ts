...
export function resolvePackagedUpdateScenario(input: {
  ...
  if (releaseChannel == null || releaseChannel === '' || releaseVersion == null || releaseVersion === '') {
    throw new Error('GD_PACKAGED_E2E_RELEASE_CHANNEL and GD_PACKAGED_E2E_RELEASE_VERSION must be set together');
  }
...
export function applyPackagedUpdateEnv(
  ...
  env.GD_UPDATE_ENABLED = '1';
  env.GD_UPDATE_METADATA_URL = metadataUrl;
  env.GD_UPDATE_OPEN_DRY_RUN = '1';
  env.GD_UPDATE_AUTO_CHECK = '1';
  if (scenario.currentVersionOverride == null) {
    delete env.GD_UPDATE_CURRENT_VERSION;
  } else {
    env.GD_UPDATE_CURRENT_VERSION = scenario.currentVersionOverride;
  }
}
