...
const PLUGIN_SHARE_DETAILS: Record<PluginShareAction, {
  ...
  'publish-github': {
    ...
    fallbackDescription:
      'Creates a public GitHub repository for this local Galyarder Design plugin.',
    confirmLabel: 'Start publishing',
    steps: [
      'Create a new Galyarder Design project for the publish workflow.',
  ...
  'contribute-galyarder-design': {
    eyebrow: 'Galyarder Design pull request',
    fallbackTitle: 'Contribute Plugin to Galyarder Design',
    fallbackDescription:
      'Opens a pull request that adds this plugin to the Galyarder Design community catalog.',
    confirmLabel: 'Start contribution',
    steps: [
      'Create a new Galyarder Design project for the contribution workflow.',
      'Copy this plugin into that project as isolated source context.',
...
export function PluginsView({
  ...
  // DS Dialog state for installed plugin detail (Req 14.2, 14.6)
  const [dsDetailOpen, setDsDetailOpen] = useState(false);
  const [dsDetailRecord, setDsDetailRecord] = useState<InstalledPluginRecord | null>(null);
  // Ref to the card button that opened the DS dialog — for focus return on close (Req 14.6)
  ...
  useEffect(() => {
    void refresh();
    window.addEventListener('galyarder-design:plugins-changed', refresh);
    return () => window.removeEventListener('galyarder-design:plugins-changed', refresh);
  }, []);
  ...
  // Open DS dialog for an installed plugin card (Req 14.2)
  const openDsDetail = useCallback(
    (record: InstalledPluginRecord, cardEl: HTMLButtonElement | null) => {
      activatingCardRef.current = cardEl;
      setDsDetailRecord(record);
      setDsDetailOpen(true);
    },
    [],
  );

  // Close DS dialog and return focus to the activating card (Req 14.6)
  const closeDsDetail = useCallback(() => {
    setDsDetailOpen(false);
  }, []);

  function handleDsDetailOpenChange(open: boolean) {
    if (!open) {
      setDsDetailOpen(false);
      // Return focus to the card that opened the dialog (Req 14.6)
      window.setTimeout(() => {
        activatingCardRef.current?.focus();
        activatingCardRef.current = null;
      }, 0);
    }
  }
  ...
  return (
    <section className="plugins-view" aria-labelledby="plugins-title">
      ...
      {/* Error state with retry (Req 14.4) */}
      {loadError ? (
        <Banner
          variant="danger"
          title="Failed to load plugins"
          description="Plugin metadata could not be retrieved. Check that the daemon is reachable."
          actions={
      ...
      <div className="plugins-view__gallery">
        {loading ? (
          <div className="plugins-view__empty" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <Spinner size="sm" aria-label={t('pluginsView.loading')} />
            <span>{t('pluginsView.loading')}</span>
          </div>
        ) : null}

        {!loading && activeTab === 'installed' ? (
          userPlugins.length === 0 ? (
            /* Empty state (Req 14.5) */
            <EmptyState
              icon={<DsIcon name="Puzzle" size={24} />}
              title="No plugins available"
              description="Install plugins from the Available tab or import one directly."
              action={
                <Button
                  variant="secondary"
                  size="sm"
            ...
            /* Responsive Card grid (Req 14.1) */
            <div className="plugins-view__ds-grid" role="list" aria-label={t('pluginsView.installedTitle')}>
              {userPlugins.map((record) => (
                <PluginGridCard
                  key={record.id}
                  record={record}
                  locale={locale}
                  isPending={pendingApplyId === record.id}
                  onOpenDetail={openDsDetail}
                  onUse={(r) => {
                    trackPluginsInstalledTabClick(analytics.track, {
                      ...
                      template_id: r.id,
                      template_type: r.sourceKind,
                    });
                    void handleUsePlugin(r, 'use');
                  }}
                />
              ))}
            </div>
      ...
      {/* DS Dialog for installed plugin detail — Req 14.2, 14.6 */}
      <Dialog open={dsDetailOpen} onOpenChange={handleDsDetailOpenChange}>
        {dsDetailRecord ? (
          <DialogContent size="md" dismissable>
            <PluginDsDetailBody
              record={dsDetailRecord}
              locale={locale}
              isPending={pendingApplyId === dsDetailRecord.id}
              onUse={(record) => {
                trackPluginsInstalledTabClick(analytics.track, {
                  page_name: 'plugins',
                  area: 'installed_tab',
...
function readLocalizedUseCaseQuery(record: InstalledPluginRecord | null): string | null {
  const query = record?.manifest?.gd?.useCase?.query;
...
function pluginShareSlug(name: string): string {
  return (
    name
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, '-')
      .replace(/(^[-._]+|[-._]+$)/g, '') || 'galyarder-design-plugin'
  );
}
...
function pluginCategory(record: InstalledPluginRecord): string {
  const tags = record.manifest?.tags ?? [];
  const first = tags.find((tag) => !NOISE_TAGS.has(tag.toLowerCase()));
...
interface PluginGridCardProps {
  ...
  locale: string;
  isPending: boolean;
  onOpenDetail: (record: InstalledPluginRecord, el: HTMLButtonElement | null) => void;
  onUse: (record: InstalledPluginRecord) => void;
}

function PluginGridCard({ record, locale, isPending, onOpenDetail, onUse }: PluginGridCardProps) {
  ...
  return (
    <div role="listitem" className="plugins-view__ds-card-wrapper">
      <Card
        elevation="resting"
        padding="md"
        className="plugins-view__ds-card"
      >
        <div className="plugins-view__ds-card-head">
          <span className="plugins-view__ds-card-title" title={title}>{title}</span>
          <Badge variant="neutral" size="sm">{category}</Badge>
        </div>
        {description ? (
          <p className="plugins-view__ds-card-desc">{description}</p>
        ) : null}
        <div className="plugins-view__ds-card-meta">
          <span className="plugins-view__ds-card-version">v{record.version}</span>
          <TrustBadge trust={record.trust} />
        </div>
        <div className="plugins-view__ds-card-actions">
          <Button
            ref={cardRef}
            variant="secondary"
            size="sm"
            onClick={() => onOpenDetail(record, cardRef.current)}
            aria-label={`View details for ${title}`}
            data-testid={`plugins-ds-details-${record.id}`}
          >
            Details
          </Button>
          <Button
            ...
            data-testid={`plugins-ds-use-${record.id}`}
          >
...
// ─── DS Dialog body for installed plugin detail (Req 14.2) ───────────────────

interface PluginDsDetailBodyProps {
  record: InstalledPluginRecord;
  locale: string;
  isPending: boolean;
  onUse: (record: InstalledPluginRecord) => void;
...
function PluginDsDetailBody({ record, locale, isPending, onUse, onClose }: PluginDsDetailBodyProps) {
  const title = localizePluginTitle(locale, record);
  const description = localizePluginDescription(locale, record);
  const category = pluginCategory(record);

  return (
    <div className="plugins-view__ds-detail">
      <div className="plugins-view__ds-detail-head">
        <div className="plugins-view__ds-detail-title-row">
          <DialogTitle className="plugins-view__ds-detail-title">
            {title}
          </DialogTitle>
          <Badge variant="neutral" size="sm">{category}</Badge>
        </div>
        <div className="plugins-view__ds-detail-meta">
        ...
        <DialogClose asChild>
          <Button
            ...
            className="plugins-view__ds-detail-close"
          >
      ...
      <div className="plugins-view__ds-detail-body">
        {description ? (
          <p className="plugins-view__ds-detail-desc">{description}</p>
        ) : (
          <p className="plugins-view__ds-detail-desc plugins-view__ds-detail-desc--empty">
            No description provided.
          </p>
        )}
        <dl className="plugins-view__ds-detail-facts">
          <div>
            <dt>ID</dt>
          ...
          {record.manifest?.tags && record.manifest.tags.length > 0 ? (
            <div>
              <dt>Tags</dt>
              <dd className="plugins-view__ds-detail-tags">
                {record.manifest.tags.map((tag) => (
      ...
      <div className="plugins-view__ds-detail-foot">
        <DialogClose asChild>
        ...
        <Button
          ...
          loading={isPending}
          onClick={() => onUse(record)}
          data-testid={`plugins-ds-detail-use-${record.id}`}
        >
          {isPending ? 'Applying…' : 'Use plugin'}
        </Button>
      </div>
    </div>
  );
...
function AvailablePluginsPanel({
  ...
  return (
    <section className="plugins-view__section" aria-labelledby="plugins-available-title">
      ...
      {plugins.length === 0 ? (
        /* Zero available entries — EmptyState per Req 23.2 */
        <EmptyState
          icon={<DsIcon name="PackageSearch" size={24} />}
          title={t('pluginsView.availableTitle')}
          description={t('pluginsView.availableEmptyInstalled')}
        />
      ) : filteredPlugins.length === 0 ? (
        /* Filtered empty — EmptyState per Req 23.2 */
        <EmptyState
          icon={<DsIcon name="SearchX" size={24} />}
          title={filterActive ? t('pluginsView.availableEmptyFiltered') : t('pluginsView.availableEmptyNoSources')}
        />
      ) : (
  ...
  return (
    <section className="plugins-view__section" aria-labelledby="plugins-sources-title">
      ...
      >
        ...
        <div className="plugins-view__source-row">
          <input
            ...
            placeholder="https://example.com/galyarder-design-marketplace.json"
      ...
      {marketplaces.length === 0 ? (
        /* Zero sources — EmptyState per Req 23.2 */
        <EmptyState
          icon={<DsIcon name="PackagePlus" size={24} />}
          title="No registry sources"
          description={t('pluginsView.sourcesEmpty')}
        />
      ) : (
      ...
      >
        ...
        <div className="plugins-import-modal__body">
          ...
          {kind === 'zip' ? (
            <FileImportPanel
              title="Upload zip"
              body="Choose a .zip archive containing galyarder-design.json, SKILL.md, or .claude-plugin/plugin.json."
...
function buildAvailableInstallCommand(
  ...
  return `gd plugin install ${entry.name}${suffix}`;
}
