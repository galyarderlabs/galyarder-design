    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__GalyarderDesignComponent ||
...
window.__GalyarderDesignComponent = window.__GalyarderDesignComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'GalyarderDesignComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'GalyarderDesignComponent';
    return 'const GalyarderDesignComponent = ';
  });
  code = code.replace(
