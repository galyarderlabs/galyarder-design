...
export function configuredAllowedOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env.GD_ALLOWED_ORIGINS || '';
  if (!raw.trim()) return [];
  return raw
    .split(',')
    ...
    .map((origin) => {
      ...
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new Error('GD_ALLOWED_ORIGINS only supports http:// and https:// origins');
      }
...
export function allowedBrowserPorts(
  ...
  const webPort = Number(env.GD_WEB_PORT);
...
export function isLocalSameOrigin(
  ...
  const bindHost = env.GD_BIND_HOST || '127.0.0.1';
  ...
  // origin in GD_ALLOWED_ORIGINS. Trust the Origin header in that case:
