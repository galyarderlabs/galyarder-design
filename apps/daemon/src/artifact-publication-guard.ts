...
// `gd.inputs` (preferred) or contribute its specific placeholder markers
// here together with a fixture that proves they cannot appear in
// finished output.
//
// Pairs with `gd.inputs` on `plugins/_official/examples/html-ppt-pitch-deck/
// galyarder-design.json`. The inputs gate is the primary contract — this guard
// is the defense-in-depth invariant that catches agents that bypassed the
// structured contract (e.g. routed through `gd-default`) but still tried
