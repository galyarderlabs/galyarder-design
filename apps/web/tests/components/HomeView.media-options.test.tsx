...
const MEDIA_PLUGIN = pluginRecord('gd-media-generation', 'Media generation');
...
const PROMPT_TEMPLATES: PromptTemplateSummary[] = [
  {
    ...
    model: 'gpt-image-2',
    aspect: '16:9',
    source: { repo: 'galyarder-design/image-prompts', license: 'MIT' },
  },
  {
    id: 'video-reveal',
    surface: 'video',
    ...
    aspect: '16:9',
    source: { repo: 'galyarder-design/video-prompts', license: 'MIT' },
  },
  {
...
describe('HomeView media composer options', () => {
  ...
  it('uses the Audio text input as the audio source and plugin subject', async () => {
    ...
    fireEvent.change(input, {
      target: {
        value: "Create premium product-studio audio from Welcome to Galyarder Design. using minimax-tts for 10 seconds: polished, restrained, clear, and brand-ready.",
      },
    });
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({
        pluginInputs: expect.objectContaining({
          subject: 'Welcome to Galyarder Design.',
          text: 'Welcome to Galyarder Design.',
        }),
      }));
    });
  });

  it('preserves gd-media-generation required inputs when applying media chips', async () => {
    ...
    await waitFor(() => {
      expect(fetchMock.mock.calls.some(([url, init]) => (
        typeof url === 'string' &&
        url.includes('/api/plugins/gd-media-generation/apply') &&
        JSON.parse(String(init?.body)).inputs.subject === 'a polished product concept'
      ))).toBe(true);
    });
    const applyCall = fetchMock.mock.calls.find(([url]) => (
      typeof url === 'string' && url.includes('/api/plugins/gd-media-generation/apply')
    ));
...
function stubFetch(options: { elevenLabsVoices?: Array<{ voiceId: string; name: string; category?: string }>; elevenLabsVoiceError?: string } = {}) {
  ...
  const fetchMock = vi.fn<typeof fetch>(async (url, init) => {
    ...
    if (typeof url === 'string' && url.includes('/apply')) {
      const pluginId = url.split('/api/plugins/')[1]?.split('/apply')[0] ?? 'gd-media-generation';
      if (pluginId === 'gd-media-generation') {
