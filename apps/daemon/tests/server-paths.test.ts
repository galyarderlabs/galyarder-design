...
describe('resolveDaemonCliPath', () => {
  it('resolves the gd CLI from the daemon package root', () => {
  ...
  it('uses the packaged daemon CLI path override before package resolution', () => {
    expect(resolveDaemonCliPath({ GD_DAEMON_CLI_PATH: '/app/prebundled/daemon-cli.mjs' })).toBe(
      '/app/prebundled/daemon-cli.mjs',
    );
  });

  it('uses GD_BIN as a fallback override for bundled wrapper invocations', () => {
    expect(resolveDaemonCliPath({ GD_BIN: '/app/prebundled/daemon-cli.mjs' })).toBe(
      '/app/prebundled/daemon-cli.mjs',
    );
  });
});
...
describe('resolveDaemonResourceRoot', () => {
  ...
  it('rejects resource roots outside the safe bases', () => {
    ...
    expect(() => resolveDaemonResourceRoot({ configured, safeBases: [safeBase] })).toThrow(
      /GD_RESOURCE_ROOT must be under/,
    );
  });
});
