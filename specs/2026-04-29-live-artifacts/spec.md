...
This spec defines how to bring Monet's **connectors** and **live artifacts** ideas into Galyarder Design, but implement the agent-facing surface as **file-based agent skills plus daemon-owned local tools**, not as an in-process tool registry or MCP-first integration.

---

## 1. Product goal

Galyarder Design should let an agent create previewable artifacts that are not just one-off generated files, but **live, refreshable, auditable views** backed by external or local data sources.
...
The user experience should feel like the existing GD artifact flow:

1. User chats with the selected agent.
2. Agent uses a skill to plan and create a live artifact.
3. GD persists the artifact as project-scoped files and metadata.
...
Monet exposes connectors and live artifacts through a controller-owned tool registry. GD should not copy that exact runtime shape because OD's core architecture is different: GD delegates to external CLI agents such as Claude Code, Codex, Cursor Agent, Gemini CLI, OpenCode, and Qwen.
...
- **Skill-native product model:** OD's extension point is already `skills/` + `SKILL.md`, so live artifacts should feel like another GD capability, not a separate agent protocol.
...
Controller-owned agent loop | External CLI agent loop; GD only injects skills and receives output/events
...
name: live-artifact
description: |
  Create refreshable, auditable Galyarder Design artifacts backed by connector or local data.
...
Prefer a small `gd` wrapper command over raw `curl` in the skill body:
...
- Require a valid `GD_TOOL_TOKEN` bound to the active run/project.
...
The `/api/tools/*` endpoints are optimized for agent consumption: compact JSON, concise errors, and explicit machine-readable validation failures. They never accept an arbitrary `projectId`; project/run scope comes from `GD_TOOL_TOKEN`. The `/api/live-artifacts/*` endpoints are optimized for UI state and use the web app's normal project context.
...
Use project-scoped files under the daemon runtime data directory first. `GD_DATA_DIR` may override the default; otherwise `<RUNTIME_DATA_DIR>` is `<repo>/.od`:
...
User asks for a live dashboard
  ↓
GD selects/injects live-artifact skill
  ↓
...
- OAuth connection state and credentials live outside project artifacts, under a daemon-controlled global store such as `~/.galyarder-design/connectors/` or an app database.
...
- Add run-scoped `GD_TOOL_TOKEN` for tool endpoints.
...
MCP-capable agent
  ⇄ stdio MCP protocol
gd mcp live-artifacts          # TypeScript source under apps/daemon/src, built into the gd bin
  ⇄ local HTTP with Authorization: Bearer $GD_TOOL_TOKEN
...
- **Single policy path:** the MCP server must call the existing `/api/tools/*` endpoints using `GD_DAEMON_URL` and `GD_TOOL_TOKEN`. It must not import store/service modules to bypass token scoping, connector policy, output redaction, rate limits, or route validation.
...
- **No project overrides:** tool input schemas must not accept `projectId`; project/run scope is always derived from `GD_TOOL_TOKEN` by daemon routes.
- **No global config mutation:** GD may display or generate an ephemeral MCP launch descriptor for compatible agents, but must not edit user-level MCP config files automatically.
...
- `/api/tools/*` requires a short-lived bearer `GD_TOOL_TOKEN`.
