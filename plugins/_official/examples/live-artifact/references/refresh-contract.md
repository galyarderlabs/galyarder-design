...
1. List connectors with `"$GD_NODE_BIN" "$GD_BIN" tools connectors list --format compact`.
...
3. Execute once with `"$GD_NODE_BIN" "$GD_BIN" tools connectors execute --connector <id> --tool <name> --input input.json` to produce compact normalized preview data.
