...
// follow-up will layer on `gd plugin install <name>` resolution +
...
const VALID_MANIFEST = JSON.stringify({
  ...
  plugins: [
    { name: 'sample-plugin', source: 'github:galyarder-design/sample-plugin', version: '0.1.0' },
  ],
});
...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-mp-'));
...
describe('marketplaces', () => {
  ...
  it('normalizes public marketplace urls to the canonical raw registry', async () => {
    ...
    const result = await addMarketplace(db, {
      url: 'https://galyarder-design.ai/marketplace/community/galyarder-design-marketplace.json',
      fetcher: async (url) => {
        seenUrls.push(url);
        return {
          ok: true,
  ...
  it('normalizes legacy branch raw urls to the canonical raw registry', () => {
    expect(resolveMarketplaceFetchUrl(
      'https://raw.githubusercontent.com/galyarderlabs/galyarder-design/garnet-hemisphere/plugins/registry/community/galyarder-design-marketplace.json',
    )).toBe(marketplaceManifestUrlForRegistry('community'));
  });

  it('requires a raw galyarder-design-marketplace.json document, not a GitHub tree page', async () => {
    const result = await addMarketplace(db, {
      url: 'https://github.com/galyarderlabs/galyarder-design/tree/garnet-hemisphere/plugins/registry/community',
  ...
  it('refresh re-fetches and updates refreshed_at', async () => {
    ...
    updatedManifest.plugins.push({
      name: 'new-plugin',
      source: 'github:galyarder-design/new-plugin',
      version: '0.2.0',
    });
  ...
  it('upserts a fixed built-in marketplace manifest', () => {
    const result = ensureMarketplaceManifest(db, {
      id: 'official',
      url: 'https://galyarder-design.ai/marketplace/galyarder-design-marketplace.json',
      trust: 'official',
      manifestText: VALID_MANIFEST,
      now: 123,
    });
  ...
  it('seeds the checked-in default community registry as restricted and resolvable', async () => {
    const communityManifest = await readFile(
      new URL('../../../plugins/registry/community/galyarder-design-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(resolved?.source).toMatch(
      /^github:galyarderlabs\/galyarder-design(?:@[^/]+)?\/plugins\/community\/registry-starter$/,
    );
  });

  it('keeps the checked-in official registry populated from bundled plugins', async () => {
    const officialManifestText = await readFile(
      new URL('../../../plugins/registry/official/galyarder-design-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(officialManifest.plugins?.some((plugin) => plugin.name === 'galyarder-design/build-test')).toBe(true);
    expect(officialManifest.plugins?.every((plugin) =>
      /^github:galyarderlabs\/galyarder-design(?:@[^/]+)?\/plugins\/_official\//.test(plugin.source ?? ''),
    )).toBe(true);
    ...
    const resolved = resolvePluginInMarketplaces(db, 'galyarder-design/build-test');
  ...
  it('keeps checked-in community registry entries pointed at source folders that can pack', async () => {
    ...
    const sourceSubpath = entry!.source!.replace(/^github:galyarderlabs\/galyarder-design(?:@[^/]+)?\//, '');
    expect(sourceSubpath).toBe('plugins/community/registry-starter');

    const sourceManifest = await readFile(
      new URL(`../../../${sourceSubpath}/galyarder-design.json`, import.meta.url),
      'utf8',
    );
    expect(JSON.parse(sourceManifest)).toMatchObject({
      name: 'community-registry-starter',
      plugin: {
        repo: expect.stringContaining('github.com/galyarderlabs/galyarder-design'),
      },
    });
  });
});
...
describe('resolvePluginInMarketplaces', () => {
  it('returns the canonical source string for a known plugin name', async () => {
    ...
    expect(resolved!.source).toBe('github:galyarder-design/sample-plugin');
