// @ts-nocheck
import type { DesktopExportPdfInput, DesktopExportPdfResult } from '@galyarder-design/sidecar-proto';
...
import { createCommandInvocation } from '@galyarder-design/platform';
import { SIDECAR_DEFAULTS, SIDECAR_ENV } from '@galyarder-design/sidecar-proto';
import {
...
} from '@galyarder-design/contracts/analytics';
import {
  redactSecrets,
  testAgentConnection,
...
import { DIAGNOSTICS_EXPORT_PATH } from '@galyarder-design/diagnostics';
import {
  buildProjectArchive,
  buildBatchArchive,
...
/** @typedef {import('@galyarder-design/contracts').ApiErrorCode} ApiErrorCode */
/** @typedef {import('@galyarder-design/contracts').ApiError} ApiError */
/** @typedef {import('@galyarder-design/contracts').ApiErrorResponse} ApiErrorResponse */
/** @typedef {import('@galyarder-design/contracts').ChatRequest} ChatRequest */
/** @typedef {import('@galyarder-design/contracts').ChatSseEvent} ChatSseEvent */
/** @typedef {import('@galyarder-design/contracts').ProxyStreamRequest} ProxyStreamRequest */
/** @typedef {import('@galyarder-design/contracts').ProxySseEvent} ProxySseEvent */
/** @typedef {import('@galyarder-design/contracts').ProjectConversationCreatedSsePayload} ProjectConversationCreatedSsePayload */

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const require = createRequire(import.meta.url);
const DAEMON_CLI_PATH_ENV = 'GD_DAEMON_CLI_PATH';
...
export function resolveDaemonCliPath(env: NodeJS.ProcessEnv = process.env): string {
  const configured = cleanOptionalPath(env[DAEMON_CLI_PATH_ENV]) ?? cleanOptionalPath(env.GD_BIN);
  if (configured) return configured;

  const packageJsonPath = require.resolve('@galyarder-design/daemon/package.json');
  return path.join(path.dirname(packageJsonPath), 'dist', 'cli.js');
}

const PROJECT_ROOT = resolveProjectRoot(__dirname);
const RESOURCE_ROOT_ENV = 'GD_RESOURCE_ROOT';
...
export function validateCodexGeneratedImagesDir(
  ...
  const warnSkipped = (reason: string) =>
    warn(`[gd] codex generated_images allowlist skipped: ${reason}`);
  ...
  try {
    ...
    warn(`[gd] codex generated_images allowlist mkdir failed: ${message}`);
    return null;
  }
}
...
function resolveProcessResourcesPath() {
  ...
  const windowsResourceBinMarker =
    `${path.sep}resources${path.sep}galyarder-design${path.sep}bin${path.sep}`.toLowerCase();
...
const GD_BIN = resolveDaemonCliPath();
const GD_NODE_BIN = process.execPath;
...
// so they respect GD_DATA_DIR overrides (test isolation, packaged runs).
...
const OFFICIAL_PLUGIN_SOURCE_REPO = 'github:galyarderlabs/galyarder-design@main';
...
async function marketplaceSeedManifestText(id, bundledMarketplaceEntries) {
  const manifestPath = path.join(PLUGIN_REGISTRY_DIR, id, 'galyarder-design-marketplace.json');
...
export function resolveDataDir(raw, projectRoot) {
  if (!raw) return path.join(projectRoot, '.gd');
  // expandHomePrefix is shared with media-config.ts so GD_DATA_DIR and
  // GD_MEDIA_CONFIG_DIR can never split state under a $HOME-style value.
  // Some launchers (systemd unit files, NixOS modules, certain Docker
  // entrypoints, Windows scheduled tasks) pass GD_DATA_DIR with literal
  ...
  try {
    ...
    throw new Error(
      [
        `GD_DATA_DIR "${resolved}" is not writable: ${e.message}`,
...
const RUNTIME_DATA_DIR = resolveDataDir(process.env.GD_DATA_DIR, PROJECT_ROOT);
const PLUGIN_LOCKFILE_PATH = path.join(RUNTIME_DATA_DIR, 'gd-plugin-lock.json');
...
// itself as the stable, user-shaped path so GD_DATA_DIR resolution stays
...
// One-shot legacy data migration. When GD_LEGACY_DATA_DIR is set and the
// new data root is fresh (no app.sqlite), copy the 0.3.x .gd/ payload
...
// and https://github.com/galyarderlabs/galyarder-design/issues/710.
migrateLegacyDataDirSync({
  legacyDir: process.env.GD_LEGACY_DATA_DIR,
 ...
 *
 *   1. `GD_PUBLIC_BASE_URL` env var. Cloud and packaged-electron deployments
...
function getPublicBaseUrl(req) {
  const env = process.env.GD_PUBLIC_BASE_URL;
  if (env && /^https?:\/\//i.test(env)) {
    return env.replace(/\/+$/u, '');
  }
  const proto = req.protocol || 'http';
  const host = req.get('host');
  if (!host) return `http://localhost:${process.env.GD_PORT ?? '7456'}`;
  return `${proto}://${host}`;
}
...
const PROMPT_TEMP_FILE = () =>
  '.gd-prompt-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8) + '.md';
...
// Load Critique Theater config once at startup so a bad GD_CRITIQUE_* value
...
export function createAgentRuntimeEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...baseEnv,
    GD_DATA_DIR: RUNTIME_DATA_DIR,
    GD_DAEMON_URL: daemonUrl,
    GD_NODE_BIN: nodeBin,
  };
  ...
  if (toolTokenGrant?.token) {
    env.GD_TOOL_TOKEN = toolTokenGrant.token;
  } else {
    delete env.GD_TOOL_TOKEN;
  }
...
export function createAgentRuntimeToolPrompt(
  ...
  const tokenLine = toolTokenGrant?.token
    ? '- `GD_TOOL_TOKEN` is available in your environment for this run. Use it only through project wrapper commands; do not print, persist, or override it.'
    : '- `GD_TOOL_TOKEN` is not available for this run, so `/api/tools/*` wrapper commands may be unavailable.';

  return [
    '## Runtime tool environment',
    '',
    `- Daemon URL: \`${daemonUrl}\` (also available as \`GD_DAEMON_URL\`).`,
    '- `GD_NODE_BIN` is the absolute path to the Node-compatible runtime that started the daemon; packaged desktop installs provide this even when the user has no system `node` on PATH.',
    '- `GD_BIN` is the absolute path to the Galyarder Design CLI script. On POSIX shells run wrappers with `"$GD_NODE_BIN" "$GD_BIN" tools ...`; do not call bare `gd`, which may resolve to the system octal-dump command on Unix-like systems.',
    '- On PowerShell use `& $env:GD_NODE_BIN $env:GD_BIN tools ...`; on cmd.exe use `"%GD_NODE_BIN%" "%GD_BIN%" tools ...`.',
    tokenLine,
    '- Prefer project wrapper commands through `GD_NODE_BIN` + `GD_BIN` over raw HTTP. The wrappers read these environment values automatically.',
  ].join('\n');
}
...
function renderRunContextPrompt(selection, metadata) {
  ...
  if (Array.isArray(context.connectorIds) && context.connectorIds.length > 0) {
    ...
    lines.push(
      'The user selected these connectors for this run. Discover available read-only connector tools first with `"$GD_NODE_BIN" "$GD_BIN" tools connectors list --format compact`, then execute relevant tools through `tools connectors execute`; do not ask for a data source that is already selected.',
    );
...
async function readProjectPluginManifest(folder) {
  const raw = await fs.promises.readFile(path.join(folder, 'galyarder-design.json'), 'utf8');
...
function githubRepoNameFromPluginName(name) {
  const slug = String(name)
    ...
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/(^[-._]+|[-._]+$)/g, '');
  return slug || 'galyarder-design-plugin';
}

const PLUGIN_SHARE_ACTION_LABELS = {
  'publish-github': 'Publish to GitHub',
  'contribute-galyarder-design': 'Contribute to Galyarder Design',
};
...
function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }) {
  ...
  if (action === 'publish-github') {
    return [
      `Publish the local Galyarder Design plugin "${title}" as a new public GitHub repository.`,
      '',
      `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
      'Use the local daemon share endpoint so the publish flow runs through Galyarder Design\'s validated GitHub path:',
      '',
      '```bash',
      `curl -sS -X POST "$GD_DAEMON_URL/api/projects/$GD_PROJECT_ID/plugins/publish-github" \\`,
  ...
  return [
    `Open a pull request to add the local Galyarder Design plugin "${title}" to the Galyarder Design repository.`,
    '',
    `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
    'Use the local daemon share endpoint so the contribution flow runs through Galyarder Design\'s validated GitHub path:',
    '',
    '```bash',
    `curl -sS -X POST "$GD_DAEMON_URL/api/projects/$GD_PROJECT_ID/plugins/contribute-galyarder-design" \\`,
    `  -H 'content-type: application/json' \\`,
    `  -d '${JSON.stringify({ path: stagedPath })}'`,
    '```',
    '',
...
function renderOAuthResultPage(opts) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to Galyarder Design.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — Galyarder Design</title>
  ...
  <script>
    try {
      ...
      if (window.BroadcastChannel) {
        var bc = new BroadcastChannel('galyarder-design-mcp-oauth');
...
const UPLOAD_DIR = path.join(os.tmpdir(), 'gd-uploads');
  ...
  storage: multer.diskStorage({
    destination: async (req, _file, cb) => {
      try {
        ...
        // .gd/projects/<id>/ behavior for non-imported projects.
...
function pluginShareActionToCli(action) {
  if (action === 'publish-github') {
    return {
      ...
      title: 'Publish repo',
      command: 'gd plugin publish-repo',
      successMessage: 'Published plugin to GitHub.',
      failureCode: 'publish-repo-failed',
    };
  }
  return {
    argv: ['plugin', 'galyarder-design-pr'],
    title: 'Galyarder Design PR',
    command: 'gd plugin galyarder-design-pr',
    successMessage: 'Opened Galyarder Design PR flow.',
    failureCode: 'galyarder-design-pr-failed',
  };
}
...
function pluginShareProgressPlan(action) {
  ...
  return [
    'Ensure the Galyarder Design fork exists',
...
async function runPluginShareTask(task, folder) {
  ...
  const result = await execCommandViaLoginShell(GD_NODE_BIN, [
    GD_BIN,
    ...share.argv,
    folder,
    '--json',
  ...
  task.result = {
    message: url
      ? (task.action === 'publish-github'
          ...
          : `Opened Galyarder Design PR flow at ${url}.`)
...
function resolveChatRunInactivityTimeoutMs() {
  const raw = Number(process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS);
...
function resolveChatRunShutdownGraceMs() {
  const raw = Number(process.env.GD_CHAT_RUN_SHUTDOWN_GRACE_MS);
...
function resolveAcpStageTimeoutMs(): number | undefined {
  ...
  // sets `GD_ACP_STAGE_TIMEOUT_MS`. Bounded to the same 24h ceiling as the
  // outer chat inactivity watchdog so an oversized override doesn't get
  // clamped to 1ms by Node's signed-32-bit delay limit.
  const raw = Number(process.env.GD_ACP_STAGE_TIMEOUT_MS);
...
export async function startServer({
  port = 7456,
  host = process.env.GD_BIND_HOST || '127.0.0.1',
  returnServer = false,
  ...
  // GD_API_TOKEN is set. This is the spec §16 Phase 5 safety floor:
  // a hosted operator can no longer accidentally publish an unsecured
  // daemon by setting GD_BIND_HOST=0.0.0.0 without a token.
  //
  // Loopback hosts (127.0.0.1 / ::1 / localhost) are always allowed —
  // the desktop / dev flow remains unchanged. Setting GD_API_TOKEN is
  ...
  const apiToken = (process.env.GD_API_TOKEN ?? '').trim();
  if (!isLoopbackHostname(host) && apiToken.length === 0) {
    throw new Error(
      `GD_BIND_HOST=${host} requires GD_API_TOKEN to be set. ` +
  ...
  //
  // Active only when GD_API_TOKEN is set. Loopback origins skip the
  // check (the desktop UI / local CLI never carry a bearer); every
  // other request must present `Authorization: Bearer <token>` with a
  // value matching `GD_API_TOKEN`. Health / version / status remain
    ...
    app.use('/api', (req, res, next) => {
      ...
      if (!match || match[1] !== apiToken) {
        return res.status(401).json({
          error: { code: 'API_TOKEN_REQUIRED', message: 'Authorization: Bearer <GD_API_TOKEN> required' },
        });
      }
      return next();
    });
  ...
  function isReplaceableDesignSystemWorkspaceFile(filePath, file) {
    ...
    if (/^ui_kits\/app\/components\/.+\.(jsx|tsx|js|ts|css|html)$/u.test(filePath)) {
      return buffer.length < 700 && /gd-ui-kit-[a-z-]+/u.test(text);
    }
  ...
  if (process.env.GD_CODEX_DISABLE_PLUGINS === '1') {
    console.log('[gd] Codex plugins disabled via GD_CODEX_DISABLE_PLUGINS=1');
  }
  ...
  try {
    const result = await registerBundledPlugins({
      ...
      marketplaceProvenance: {
        ...
        entryNamePrefix:     'galyarder-design',
      },
    });
    bundledMarketplaceEntries = result.registered.map((plugin) => ({
      name:        `galyarder-design/${plugin.id}`,
      title:       plugin.title,
      ...
      publisher:   { id: 'galyarder-design', url: 'https://galyarder-design.ai' },
      homepage:    plugin.manifest.homepage,
      license:     plugin.manifest.license,
      tags:        plugin.manifest.tags,
      capabilitiesSummary: Array.isArray(plugin.manifest.gd?.capabilities)
        ? plugin.manifest.gd.capabilities
        : undefined,
    }));
  ...
  // when GD_SNAPSHOT_GC_INTERVAL_MS is 0; otherwise one-time bootstrap
  ...
  await recoverStaleLiveArtifactRefreshes({ projectsRoot: PROJECTS_DIR }).catch((error) => {
    console.warn('[gd] Failed to recover stale live artifact refreshes:', error);
  });
  ...
  // so `gd daemon status --json` can render a one-shot health snapshot
  ...
  app.get('/api/daemon/status', async (_req, res) => {
    ...
    res.json({
      ...
      bindHost: process.env.GD_BIND_HOST ?? '127.0.0.1',
      port: Number(process.env.GD_PORT ?? 7456),
      dataDir: RUNTIME_DATA_DIR,
      mediaConfigDir: process.env.GD_MEDIA_CONFIG_DIR ?? null,
      pid: process.pid,
  ...
  // Plan §3.GG1 — `gd daemon db status`. Inventory of the SQLite
  ...
  // Useful for dashboards + the `gd plugin events snapshot` CLI
  ...
  // Plan §3.NN1 — `gd plugin events purge`. Operator escape
  ...
  // Plan §3.II1 — `gd plugin events tail`. SSE-backed live event
  ...
  // Plan §3.LL1 — `gd daemon db verify`. Runs SQLite
  ...
  // Plan §3.HH2 — `gd daemon db vacuum`. Runs SQLite VACUUM to
  ...
  // `gd daemon stop`; the actual close path goes through the same
  ...
  // set `GD_METRICS_ENDPOINT=disabled`; the route is registered only
  // when that env value is not the literal string 'disabled'.
  if (process.env.GD_METRICS_ENDPOINT !== 'disabled') {
  ...
  // design: the recommendation does not flip GD_CRITIQUE_ROLLOUT_PHASE
  ...
  app.get('/api/critique/conformance', async (req, res) => {
    try {
      ...
      const decision = evaluateRollout({
        current: parseRolloutPhase(process.env.GD_CRITIQUE_ROLLOUT_PHASE),
        history,
        windowDays,
        shippedThreshold,
        cleanParseThreshold,
  ...
  app.delete('/api/projects/:id', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').OkResponse} */
      const body = { ok: true };
      res.json(body);
    } catch (err) {
  ...
  async function folderLooksLikePlugin(folder) {
    const names = ['galyarder-design.json', 'SKILL.md', path.join('.claude-plugin', 'plugin.json')];
  ...
  app.post('/api/plugins/upload-zip', (req, res) => {
    pluginUpload.single('file')(req, res, async (err) => {
      ...
      try {
        ...
        const stagedFolder = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'gd-plugin-zip-'));
  ...
  app.post('/api/plugins/upload-folder', (req, res) => {
    pluginUpload.array('files', 500)(req, res, async (err) => {
      if (err) return sendMulterError(res, err);
      const stagedFolder = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'gd-plugin-folder-'));
      try {
      ...
      if (!resolved) {
        return res.status(404).json({
          error: {
            ...
            message: `No marketplace plugin named "${source}". Add a marketplace via 'gd marketplace add <url>' or pass a github: / https:// / local source.`,
            data: { name: source },
          },
        });
      }
  ...
  // Plan §3.Z2 — `gd plugin upgrade <id>` re-installs a plugin from
  ...
  app.post('/api/plugins/:id/upgrade', async (req, res) => {
    ...
    if (!source) {
      return res.status(409).json({
        error: {
          code: 'missing-source',
          message: `Plugin "${id}" has no recorded install source — cannot upgrade. Reinstall via 'gd plugin install --source <...>' to set one.`,
          data: { id },
        },
      });
    }
  ...
  async function loadPluginRegistryView() {
    ...
    // consumer plugin omits gd.pipeline. Each scenario carries a
  ...
  // AND gd.kind='scenario' AND gd.pipeline is non-empty become entries
  ...
  // bundled scenario may share a `taskKind` (e.g. `gd-media-generation`
  ...
  // dedupes and prefers the canonical id `gd-<taskKind>` as the
  ...
  // to hijack a consumer plugin that omitted `gd.pipeline`.
  function collectBundledScenarios() {
    type ScenarioEntry = {
      id: string;
      taskKind: 'new-generation' | 'figma-migration' | 'code-migration' | 'tune-collab';
      pipeline: NonNullable<NonNullable<import('@galyarder-design/contracts').PluginManifest['gd']>['pipeline']>;
    };
    ...
    try {
      ...
      for (const row of all) {
        ...
        const gd = row.manifest.gd;
        if (!gd || gd.kind !== 'scenario') continue;
        if (!gd.pipeline || !Array.isArray(gd.pipeline.stages) || gd.pipeline.stages.length === 0) continue;
        const taskKind = (gd.taskKind ?? 'new-generation') as ScenarioEntry['taskKind'];
        if (taskKind !== 'new-generation' && taskKind !== 'figma-migration' &&
            taskKind !== 'code-migration' && taskKind !== 'tune-collab') continue;
        const entry: ScenarioEntry = { id: row.id, taskKind, pipeline: gd.pipeline };
        const existing = byTaskKind.get(taskKind);
        if (!existing || entry.id === `gd-${taskKind}`) {
  ...
  app.post('/api/plugins/:id/share-project', async (req, res) => {
    try {
      ...
      if (!action) {
        sendApiError(res, 400, 'BAD_REQUEST', 'action must be publish-github or contribute-galyarder-design');
        return;
      }
  ...
  // Plan §3.AA2 — `gd atoms info <id>`. Returns the catalog row +
  ...
  //     `gd.preview.entry`, with fallbacks that walk the plugin's
  //     own context.assets[] HTMLs, examples/*.html and assets/*.html).
  //   - `/example/:name` serves an entry from `gd.useCase.exampleOutputs[]`,
  ...
  // …) declare `gd.preview.entry: "./index.html"` but actually ship
  ...
  function collectPluginPreviewCandidates(plugin: unknown): string[] {
    ...
    const gd = (manifest.gd ?? {}) as Record<string, unknown>;
    const preview = (gd.preview ?? {}) as Record<string, unknown>;

    push(preview.entry);

    const ctx = (gd.context ?? {}) as Record<string, unknown>;
    ...
    const useCase = (gd.useCase ?? {}) as Record<string, unknown>;
  ...
  app.get('/api/plugins/:id/example/:name', async (req, res) => {
    ...
    await servePluginSandboxedHtml(req, res, async (plugin) => {
      const examples = ((plugin as { manifest?: { gd?: { useCase?: { exampleOutputs?: Array<{ path?: unknown; title?: unknown }> } } } })
        .manifest?.gd?.useCase?.exampleOutputs ?? []) as Array<{ path?: unknown; title?: unknown }>;
  ...
  // discovery surface so `gd craft list` is a thin wrapper over a
  ...
  // Plan §3.DD1 — `gd plugin stats`. Aggregates the installed-
  ...
  // Plan §3.CC1 — `gd plugin canon <snapshotId>`. Returns the
  ...
  // Phase 3 layers in `gd plugin install <name>` resolution + the trust
  ...
  // Plan §3.A5: list all applied snapshots; useful for `gd plugin
  ...
  app.post('/api/applied-plugins/export', requireLocalDaemonRequest, async (req, res) => {
    try {
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const target = body.target === 'gd' || body.target === 'claude-plugin' || body.target === 'agent-skill'
        ? body.target
        : null;
      if (!target) {
        return res.status(400).json({ error: 'target must be one of: gd, claude-plugin, agent-skill' });
      }
  ...
  app.get('/api/runs/:runId/genui/:surfaceId', (req, res) => {
    try {
      ...
      // pinned AppliedPluginSnapshot. This is what `gd ui show`
      // returns to headless callers so a code agent can inspect the
      // JSON Schema before responding via `gd ui respond --value-json`.
  ...
  app.get('/api/deploy/config', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').DeployConfigResponse} */
  ...
  app.get('/api/deploy/cloudflare-pages/zones', async (_req, res) => {
    try {
      /** @type {import('@galyarder-design/contracts').CloudflarePagesZonesResponse} */
  ...
  app.get('/api/projects/:id/deployments', (req, res) => {
    try {
      /** @type {import('@galyarder-design/contracts').ProjectDeploymentsResponse} */
  ...
  app.post('/api/projects/:id/deploy', async (req, res) => {
    try {
      ...
      const now = Date.now();
      /** @type {import('@galyarder-design/contracts').DeployProjectFileResponse} */
  ...
  app.post('/api/projects/:id/deploy/preflight', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').DeployPreflightResponse} */
    ...
    async (req, res) => {
      try {
        ...
        if (existing.providerId === CLOUDFLARE_PAGES_PROVIDER_ID && existing.cloudflarePages?.pagesDev?.url) {
          ...
          const now = Date.now();
          /** @type {import('@galyarder-design/contracts').CheckDeploymentLinkResponse} */
          const body = upsertDeployment(db, {
            ...existing,
            ...checked,
  ...
  // Project files. Each project owns a flat folder under .gd/projects/<id>/
  ...
  app.get('/api/projects/:id/files', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').ProjectFilesResponse} */
      const body = { files };
      res.json(body);
    } catch (err) {
      sendApiError(res, 400, 'BAD_REQUEST', String(err));
  ...
  app.post('/api/projects/:id/plugins/contribute-galyarder-design', async (req, res) => {
    try {
      const project = getProject(db, req.params.id);
      if (!project) {
        sendApiError(res, 404, 'PROJECT_NOT_FOUND', 'project not found');
      ...
      if (!result.ok || !payload?.ok) {
        res.status(500).json({
          ok: false,
          code: payload?.error?.label || 'galyarder-design-pr-failed',
          message: payload?.error?.stderr || payload?.error?.stdout || 'Galyarder Design PR creation failed.',
          log: payload?.steps?.map((step) => step.stderr || step.stdout || step.command).filter(Boolean) ?? [result.stderr || result.stdout || 'galyarder-design-pr failed'],
        });
        return;
      }
      res.json({
        ok: true,
        message: payload.prUrl ? `Opened Galyarder Design PR flow at ${payload.prUrl}.` : 'Opened Galyarder Design PR flow.',
  ...
  app.post('/api/projects/:id/plugins/share-tasks', async (req, res) => {
    ...
    try {
      ...
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const action = body.action === 'publish-github' || body.action === 'contribute-galyarder-design'
        ? body.action
        : null;
      if (!action) {
  ...
  app.delete('/api/projects/:id/raw/*', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').DeleteProjectFileResponse} */
      const body = { ok: true };
      res.json(body);
    } catch (err) {
      const status = err && err.code === 'ENOENT' ? 404 : 400;
    ...
    async (req, res) => {
      try {
        ...
        if (req.file) {
          ...
          /** @type {import('@galyarder-design/contracts').ProjectFileResponse} */
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        /** @type {import('@galyarder-design/contracts').UploadProjectFilesResponse} */
  ...
  const composeDaemonSystemPrompt = async ({
    ...
    // `gd.critique.policy` in the resolved skill's SKILL.md frontmatter.
    ...
    // `gd.context.skills[{ path: './SKILL.md' }]`) over the global
    ...
    //
    // `GD_DESIGN_TOKEN_CHANNEL=0` is the kill switch: it forces the
    ...
    // matrix: skill `gd.critique.policy` veto > project override > env
    ...
    // `gd.critique.policy` in the active skill's SKILL.md frontmatter
    ...
    const critiqueEnabledForRun = isCritiqueEnabled({
      phase: parseRolloutPhase(process.env.GD_CRITIQUE_ROLLOUT_PHASE),
      skillPolicy: skillCritiquePolicy,
      projectOverride: projectCritiqueOverride,
      envOverride: parseEnvEnabled(process.env.GD_CRITIQUE_ENABLED),
    });
    ...
    // every Phase 6/7/8 atom); set GD_BUNDLED_ATOM_PROMPTS=0 to opt
    ...
    const bundledAtomPromptsEnabled = process.env.GD_BUNDLED_ATOM_PROMPTS !== '0';
    if (
      ...
      try {
        ...
        if (stages.length > 0) {
          const { loadAtomBodies } = await import('./plugins/atom-bodies.js');
          const { renderActiveStageBlock } = await import('@galyarder-design/contracts');
          const blocks = [];
    ...
    const prompt = composeSystemPrompt({
      ...
      // critiqueCfg.enabled is loaded from GD_CRITIQUE_ENABLED only, so a
  ...
  // the default stage runner; set GD_PIPELINE_RUNNER=stub to fall
  ...
  const firePipelineForRun = (args) => {
    ...
    const projectIdForRun = run.projectId
      ...
      ?? 'project-unknown';
    const runnerMode = process.env.GD_PIPELINE_RUNNER === 'stub'
      ? 'stub'
      : 'registry';
    let runStage;
    if (runnerMode === 'stub') {
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // ("我是 X" is the Chinese-language equivalent of "I am X")
    ...
    const { prompt: daemonSystemPrompt, activeSkillDir, critiqueShouldRun } =
      await composeDaemonSystemPrompt({
        ...
        // Default ON; set GD_BUNDLED_ATOM_PROMPTS=0 to opt out.
    ...
    //      `<cwd>/.gd-skills/<folder>/` so any agent CLI — not just the
    ...
    if (cwd && activeSkillDir) {
      ...
      if (!result.staged) {
        console.warn(
          `[gd] skill-stage skipped: ${result.reason ?? 'unknown reason'}; falling back to absolute paths`,
        );
      }
    }
    ...
    // Galyarder Design relays them to the agent so the model can call those tools.
  ...
  orbitService.setRunHandler(async ({
    ...
    if (template?.dir) {
      ...
      if (!result.staged) {
        console.warn(
          `[gd] orbit template skill-stage skipped: ${result.reason ?? 'unknown reason'}; falling back to prompt-embedded instructions`,
        );
      }
    }
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside Galyarder Design.',
        'You must discover connectors and connector tools yourself through the GD CLI; the daemon has not chosen tools for you.',
        ...
        'Keep connector credentials and GD_TOOL_TOKEN private; never print or persist secrets.',
      ].join('\n'),
    }, run));
  ...
  app.post('/api/runs', async (req, res) => {
    ...
    // bridge). Prefer the explicit `x-gd-client` header from desktop /
    ...
    const declaredClient = String(req.get('x-gd-client') ?? '').toLowerCase();
    ...
    /** @type {import('@galyarder-design/contracts').ChatRunCreateResponse} */
    const body = {
      runId: run.id,
      ...(resolvedSnapshot?.ok
        ? {
  ...
  app.get('/api/runs', (req, res) => {
    ...
    /** @type {import('@galyarder-design/contracts').ChatRunListResponse} */
  ...
  // GD adds internal-only events later.
  app.get('/api/runs/:id/agui', async (req, res) => {
    const run = design.runs.get(req.params.id);
    if (!run) return sendApiError(res, 404, 'NOT_FOUND', 'run not found');
    const { encodeOdEventForAgui } = await import('@galyarder-design/agui-adapter');
    const sse = createSseResponse(res);
  ...
  app.post('/api/runs/:id/cancel', (req, res) => {
    ...
    /** @type {import('@galyarder-design/contracts').ChatRunCancelResponse} */
    const body = { ok: true };
    res.json(body);
  });
  ...
  routineService.setRunHandler(async ({ routine, trigger, startedAt, runId }) => {
    ...
    // `ProjectConversationCreatedSsePayload` from `@galyarder-design/contracts`
    ...
    try {
      server = app.listen(port, host, () => {
        ...
        if (!boundPort) {
          reject(
            new Error(
              `[gd] daemon failed to resolve listening port (address=${JSON.stringify(address)})`,
            ),
          );
          return;
        }
        ...
        if (!returnServer) {
          console.log(`[gd] daemon listening on ${url}`);
        }
        daemonUrl = url;
...
function assembleExample(templateHtml, slidesHtml, title) {
  return templateHtml
    ...
    .replace(
      /<title>.*?<\/title>/,
      `<title>${title} | Galyarder Design Example</title>`,
    );
}
