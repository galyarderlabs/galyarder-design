// Plan §3.Z2 — `gd plugin upgrade` re-installs from recorded source.
//
...
async function writeSource(version: string, opts?: { description?: string }) {
  await writeFile(
    path.join(sourceFolder, 'galyarder-design.json'),
    JSON.stringify({
...
beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-upgrade-'));
...
describe('gd plugin upgrade — installer round-trip', () => {
  ...
  it('persists the new manifest atomically — ENOENT on read does not regress to the old version', async () => {
    ...
    const onDisk = JSON.parse(await readFile(path.join(recorded!.fsPath, 'galyarder-design.json'), 'utf8'));
