...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-gc-'));
...
describe('snapshot GC', () => {
  ...
  //
  // Operators who opt into GD_SNAPSHOT_RETENTION_DAYS expect referenced
  ...
  describe('GD_SNAPSHOT_RETENTION_DAYS referenced-row TTL', () => {
