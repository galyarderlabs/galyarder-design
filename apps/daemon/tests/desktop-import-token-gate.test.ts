 ...
 * carries a valid HMAC-signed `X-GD-Desktop-Import-Token` minted by
...
describe('desktop-import-token gate', () => {
  ...
  function makeFolder(): string {
    const d = mkdtempSync(path.join(tmpdir(), 'gd-import-token-'));
    tempDirs.push(d);
    return d;
  }
  ...
  it('rejects imports with a malformed token', async () => {
    ...
    const resp = await importFolder(
      { baseDir: folder },
      { 'x-gd-desktop-import-token': 'totally.bogus' },
    );
    expect(resp.status).toBe(403);
  });
  ...
  it('rejects tokens minted with the wrong secret', async () => {
    ...
    const resp = await importFolder(
      { baseDir: folder },
      { 'x-gd-desktop-import-token': token },
    );
    expect(resp.status).toBe(403);
  });
