 ...
 * push the scaled stage off-screen (see the GD srcdoc bridge's deck-fix
 ...
 *     and the translate centers it correctly even inside the GD viewer's
...
<body>
  ...
  <script>
    (function () {
      ...
      // transforms (e.g. when the GD viewer wraps the iframe in its own
      ...
      // Capture phase + listen on both targets — inside the GD iframe,
...
- ❌ Don't write your own \`fit()\` function or \`transform: scale()\` script. The framework already does it, and ad-hoc versions drift inside the GD viewer's nested transform wrapper.
...
The framework is a contract with the host viewer. The GD iframe sits inside a transformed wrapper (the zoom control); the keyboard handler needs capture phase + dual targets; "Share → PDF" reads the print stylesheet; the position survives reloads via localStorage. If a turn rewrites any of these — even with "equivalent" code — the next turn diverges, and three turns in the deck has subtly broken nav and a one-page PDF. Treat the framework as load-bearing infrastructure.
