#!/usr/bin/env node
// Seed Galyarder Design with pre-baked test projects so the UI has real slide
...
//   pnpm seed:test-projects --offline          # ingest into ./.gd before boot
//   pnpm seed:test-projects --clear            # remove previously seeded projects
//
// The daemon URL is resolved in this order: --daemon flag > $GD_DAEMON_URL >
// http://127.0.0.1:$GD_PORT > whatever `pnpm tools-dev status --json` reports
// for the daemon app. --namespace is only passed to that tools-dev discovery
// step; it is not forwarded to the gd CLI or stored in daemon data. The
...
const WEBS: SeedFixture[] = [
  {
    skillId: 'galyarder-design-landing',
    sourceKind: 'skill',
    kind: 'prototype',
...
Seeds Galyarder Design with pre-baked, real HTML artifacts from:
  - Skills examples
...
Options:
  --daemon <url>     Daemon base URL. When omitted, the script reads
                     \$GD_DAEMON_URL, then \$GD_PORT, and finally falls back
  ...
  --data-dir <dir>   Offline target data dir (default: \$GD_DATA_DIR or ./.gd).
  --namespace <name> Tools-dev namespace for online auto-discovery. This does
                     not affect gd CLI behavior. Offline mode requires
                     --data-dir or GD_DATA_DIR when --namespace is set.
...
Online daemon URL resolution (first match wins):
  ...
  2. \`GD_DAEMON_URL\` env var.
  3. \`http://127.0.0.1:\$GD_PORT\` when \`GD_PORT\` is set to a real port.
...
function isDiscoverablePort(value: string | undefined): value is string {
  ...
  // tools-dev sets GD_PORT=0 to mean "ephemeral, look at runtime status",
...
async function resolveDaemonUrl(args: Args, { required }: { required: boolean }): Promise<string | null> {
  ...
  if (process.env.GD_DAEMON_URL) return process.env.GD_DAEMON_URL;
  if (isDiscoverablePort(process.env.GD_PORT)) {
    return `http://127.0.0.1:${process.env.GD_PORT}`;
  }
  const discovered = await discoverDaemonUrlFromToolsDev(args.namespace);
  if (discovered) return discovered;
  if (!required) return null;
  throw new Error(
    'cannot determine daemon URL: no --daemon flag, no GD_DAEMON_URL, ' +
      'no usable GD_PORT, and `pnpm tools-dev status --json` did not report a ' +
...
function resolveDataDir(raw: string | null): string {
  const value = raw ?? process.env.GD_DATA_DIR ?? path.join(REPO_ROOT, '.gd');
...
function assertOfflineDataDirIsExplicit(args: Args): void {
  if (args.namespace && !args.dataDir && !process.env.GD_DATA_DIR) {
    throw new Error(
      '--namespace is only a tools-dev discovery selector. Offline mode with ' +
        '--namespace requires --data-dir or GD_DATA_DIR so the script does not ' +
