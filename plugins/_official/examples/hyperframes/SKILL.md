...
## Galyarder Design integration (load-bearing for this surface)

When this skill runs inside Galyarder Design (i.e. `$GD_PROJECT_DIR` is set), the
...
**Render workflow inside GD — fast path**:
...
# 4. Dispatch render through the GD daemon. Do NOT run `npx hyperframes
...
gate is for standalone HF projects. **GD projects already have their
own design-system layer** — the user picked their visual direction at
project creation time. For an GD test render, default to: dark canvas
...
**Do NOT** call `"$GD_NODE_BIN" "$GD_BIN" media generate --model hyperframes-html` — that
