...
//     --skill galyarder-design-landing \
//     --agent claude
//
...
Run/project options:
  --daemon <url>               Daemon base URL. Falls back to GD_DAEMON_URL,
                               GD_PORT, then tools-dev status discovery.
...
async function resolveDaemonUrl(config: BatchConfig): Promise<string> {
  ...
  if (isDiscoverablePort(process.env.GD_PORT)) return `http://127.0.0.1:${process.env.GD_PORT}`;
...
async function main(): Promise<void> {
  ...
  if (resolved.dryRun) {
    ...
    const daemonUrl =
      config.daemonUrl ??
      process.env.GD_DAEMON_URL ??
      (isDiscoverablePort(process.env.GD_PORT) ? `http://127.0.0.1:${process.env.GD_PORT}` : '(not resolved in dry-run)');
