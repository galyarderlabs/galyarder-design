...
export interface PluginLoopSubmit {
  ...
  // video / audio → gd-media-generation, others → gd-new-generation).
  // Null means the caller did not stamp an explicit kind. HomeView's
  // free-form fallback uses `other` and binds the hidden gd-default
...
export function PluginLoopHome({ onSubmit }: Props) {
  ...
  const sortedPlugins = useMemo(() => {
    return [...plugins].sort((a, b) => {
      const aHasQuery = Boolean(a.manifest?.gd?.useCase?.query);
      const bHasQuery = Boolean(b.manifest?.gd?.useCase?.query);
      if (aHasQuery !== bHasQuery) return aHasQuery ? -1 : 1;
      const aScenario = a.manifest?.gd?.kind === 'scenario';
      const bScenario = b.manifest?.gd?.kind === 'scenario';
  ...
  async function usePlugin(record: InstalledPluginRecord) {
    ...
    const query = result.query || resolvePluginQueryFallback(record.manifest?.gd?.useCase?.query, locale);
    if (query) {
    ...
    <div className="plugin-loop-home" data-testid="plugin-loop-home">
      ...
      <div className="plugin-loop-home__grid" role="list">
        {loading ? (
          ...
          sortedPlugins.map((p) => {
            const hasQuery = Boolean(p.manifest?.gd?.useCase?.query);
            ...
            return (
              ...
              >
                ...
                <div className="plugin-loop-home__card-meta">
                  {p.manifest?.gd?.taskKind ? (
                    <span>{p.manifest.gd.taskKind}</span>
                  ) : null}
                  {p.manifest?.gd?.kind ? <span>· {p.manifest.gd.kind}</span> : null}
                </div>
