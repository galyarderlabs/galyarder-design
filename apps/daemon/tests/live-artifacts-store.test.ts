...
async function makeProjectsRoot() {
  const root = await mkdtemp(path.join(tmpdir(), 'gd-live-artifacts-'));
  tempRoots.push(root);
...
describe('live artifact store layout', () => {
  ...
  it('maps document refresh data paths directly even when a legacy transform is present', () => {
    const document: any = {
      ...
      dataJson: {
        repository: {
          fullName: 'galyarderlabs/galyarder-design',
          url: 'https://github.com/galyarderlabs/galyarder-design',
          starCount: 100,
      ...
      sourceJson: {
        ...
        input: { url: 'https://api.github.com/repos/galyarderlabs/galyarder-design' },
        outputMapping: {
          dataPaths: [
    ...
    const candidate = buildLiveArtifactRefreshCandidate({
      ...
      documentOutput: {
        output: {
          ...
          full_name: 'galyarderlabs/galyarder-design',
          html_url: 'https://github.com/galyarderlabs/galyarder-design',
