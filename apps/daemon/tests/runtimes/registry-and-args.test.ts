...
test('local agent profiles inherit a base adapter and can pin the default model', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'gd-local-agent-profiles-'));
  try {
    await withEnvSnapshot(['GD_AGENT_PROFILES_CONFIG'], async () => {
      ...
      process.env.GD_AGENT_PROFILES_CONFIG = config;
...
test('local agent profiles skip explicit unknown baseAgent without falling back', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'gd-local-agent-profiles-invalid-'));
  try {
    await withEnvSnapshot(['GD_AGENT_PROFILES_CONFIG'], async () => {
      const config = join(dir, 'agents.local.json');
      writeFileSync(
...
test('codex args disable plugins when GD_CODEX_DISABLE_PLUGINS is 1', () => {
  process.env.GD_CODEX_DISABLE_PLUGINS = '1';

  withPlatform('darwin', () => {
    const args = codex.buildArgs('', [], [], {}, { cwd: '/tmp/gd-project' });
...
test('codex args use workspace-write sandbox on macOS and Linux', () => {
  delete process.env.GD_CODEX_DISABLE_PLUGINS;
...
test('codex args keep plugins enabled when GD_CODEX_DISABLE_PLUGINS is unset', () => {
  delete process.env.GD_CODEX_DISABLE_PLUGINS;
...
test('codex args keep plugins enabled when GD_CODEX_DISABLE_PLUGINS is not 1', () => {
  process.env.GD_CODEX_DISABLE_PLUGINS = 'true';
...
test('codex model picker includes current OpenAI choices in priority order', async () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'gd-agents-codex-models-'));
  try {
...
test('codex detection surfaces live debug models separately from fallback models', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'gd-agents-codex-live-models-'));
  try {
    await withEnvSnapshot(['PATH', 'GD_AGENT_HOME'], async () => {
      const codexBin = join(dir, 'codex');
      writeFileSync(
...
test('codex args do not include the literal `-` stdin sentinel (regression of #237)', () => {
  ...
  const baseArgs = codex.buildArgs('', [], [], {}, { cwd: '/tmp/gd-project' });
