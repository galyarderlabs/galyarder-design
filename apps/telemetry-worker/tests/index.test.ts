...
function makeRequest(body: unknown): Request {
  return new Request('https://telemetry.galyarder-design.ai/api/langfuse', {
    method: 'POST',
    headers: {
...
describe('telemetry worker', () => {
  it('returns a health response for browser checks', async () => {
    const response = await worker.fetch(
      new Request('https://telemetry.galyarder-design.ai/api/langfuse'),
      env,
    );
    ...
    expect(await response.json()).toEqual({
      ok: true,
      service: 'galyarder-design-telemetry-relay',
      configured: true,
  ...
  it('reports unconfigured health without exposing secrets', async () => {
    const response = await worker.fetch(new Request('https://telemetry.galyarder-design.ai/health'), {});
        ...
        batch: [
          {
            ...
            body: { id: 'trace-1', name: 'galyarder-design-turn' },
          },
        ],
      }),
      env,
  ...
  it('rejects requests without the Galyarder Design client marker', async () => {
    const response = await worker.fetch(
      new Request('https://telemetry.galyarder-design.ai/api/langfuse', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
