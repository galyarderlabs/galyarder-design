...
| File | Responsibility |
|---|---|
| `config.ts` | `CritiqueConfig` defaults + `GD_CRITIQUE_*` env-var parsing. The single source of truth for thresholds, weights, max-rounds, timeouts. |
...
- **Designer weight is frozen at 0.0 until v2 cast config lands.**
  ...
  overrides driven off `gd.critique.cast` in `SKILL.md` frontmatter
