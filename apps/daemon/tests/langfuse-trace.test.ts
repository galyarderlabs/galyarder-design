...
describe('readTelemetrySinkConfig', () => {
  it('prefers the Galyarder Design telemetry relay when configured', () => {
    const cfg = readTelemetrySinkConfig({
      GALYARDER_DESIGN_TELEMETRY_RELAY_URL: 'https://telemetry.galyarder-design.ai/api/langfuse//',
      LANGFUSE_PUBLIC_KEY: 'pk',
      LANGFUSE_SECRET_KEY: 'sk',
    });
    expect(cfg).toEqual({
      kind: 'relay',
      relayUrl: 'https://telemetry.galyarder-design.ai/api/langfuse',
      timeoutMs: 20_000,
      retries: 1,
    });
  });
  ...
  it('uses relay-specific timeout and retry tuning when present', () => {
    const cfg = readTelemetrySinkConfig({
      GALYARDER_DESIGN_TELEMETRY_RELAY_URL: 'https://telemetry.galyarder-design.ai/api/langfuse',
      GALYARDER_DESIGN_TELEMETRY_TIMEOUT_MS: '30000',
      GALYARDER_DESIGN_TELEMETRY_RETRIES: '3',
      LANGFUSE_TIMEOUT_MS: '1',
      LANGFUSE_RETRIES: '0',
    });
...
describe('buildTracePayload', () => {
  ...
  it('truncates by UTF-8 bytes, not by JS string length, for multi-byte text', () => {
    ...
    // UTF-8 / CJK fixture; verifies the truncation path measures UTF-8 bytes (not code points).
    ...
    // And every character is still '设' (the single CJK fixture char), i.e. we didn't mangle the encoding.
...
describe('reportRunCompleted', () => {
  ...
  it('POSTs serialized ingestion batches to the Galyarder Design telemetry relay', async () => {
    ...
    const url = call[0] as string;
    const init = call[1] as RequestInit & { headers: Record<string, string> };
    expect(url).toBe('https://telemetry.galyarder-design.ai/api/langfuse');
    expect(init.method).toBe('POST');
    expect(init.headers.Authorization).toBeUndefined();
    expect(init.headers['Content-Type']).toBe('application/json');
    expect(init.headers['X-Galyarder-Design-Telemetry']).toBe('langfuse-ingestion-v1');
    const body = JSON.parse(init.body as string);
    expect(Array.isArray(body.batch)).toBe(true);
  });
