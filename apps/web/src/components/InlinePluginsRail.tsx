...
interface Props {
  ...
  // applied to `gd.kind` so the ChatComposer rail can hide bundled
...
function filterPlugins(
  ...
  return rows.filter((r) => {
    ...
    if (filter.taskKind && r.manifest?.gd?.taskKind !== filter.taskKind) {
      return false;
    }
    if (filter.mode && r.manifest?.gd?.mode !== filter.mode) {
      return false;
    }
    if (filter.kinds && filter.kinds.length > 0) {
      const k = r.manifest?.gd?.kind;
