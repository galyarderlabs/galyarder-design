...
import { normalizeCustomReason } from '@galyarder-design/contracts/analytics';
import {
...
import type { AppliedPluginSnapshot } from '@galyarder-design/contracts';
import type {
  AgentEvent,
  AgentInfo,
  AppConfig,
...
const CHAT_PANEL_WIDTH_STORAGE_KEY = 'galyarder-design.project.chatPanelWidth';
...
function autoSendFirstMessageKey(projectId: string): string {
  return `gd:auto-send-first:${projectId}`;
}

function autoSendAttachmentsKey(projectId: string): string {
  return `gd:auto-send-attachments:${projectId}`;
}

function designSystemAuditAutoRepairKey(projectId: string): string {
  return `gd:design-system-audit-auto-repair:${projectId}`;
}
...
export function ProjectView({
  ...
  useEffect(() => {
    ...
    // Onboarding's 4th step (the "Generation progress page" — CJK in
    // the original product spec, kept verbatim as a step-name reference)
    // fires here, not in
  ...
  const handleSend = useCallback(
    async (
      ...
      if (config.mode === 'daemon') {
        ...
        // (so an explicit "remember: X" marker — or its Chinese
        // equivalent "我是 X", meaning "I am X" — in this turn's user
        // message lands in memory in time for this turn's system
  ...
  const handlePluginFolderAgentAction = useCallback(
    async (relativePath: string, action: PluginFolderAgentAction) => {
      ...
      const shareAction = action === 'publish' ? 'publish-github' : 'contribute-galyarder-design';
      setActivePluginActionPaths((prev) => new Set(prev).add(relativePath));
      let taskStart;
      try {
  ...
  // sets `sessionStorage['gd:auto-send-first:<projectId>']` and routes
  ...
  // `galyarder-design:config` localStorage blob and stays in sync with the
  // platform `storage` event (cross-tab) plus the same-tab
  // `galyarder-design:critique-theater-toggle` CustomEvent. The mount itself
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Galyarder Design PR';
}

function pluginWorkflowCliCommand(action: PluginFolderAgentAction, relativePath: string): string {
  return action === 'publish'
    ? `gd plugin publish-repo ${relativePath}`
    : `gd plugin galyarder-design-pr ${relativePath}`;
}
