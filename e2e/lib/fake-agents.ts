...
export async function createFakeAgentRuntimes(
  ...
  const root = Array.isArray(input)
    ? path.join(tmpdir(), `galyarder-design-fake-agents-${process.pid}`)
    : (input.root ?? path.join(tmpdir(), `galyarder-design-fake-agents-${process.pid}`));
...
async function createOrbitLiveArtifact() {
  const baseUrl = process.env.GD_DAEMON_URL;
  const token = process.env.GD_TOOL_TOKEN;
  if (!baseUrl || !token) {
    throw new Error('Orbit fake run requires GD_DAEMON_URL and GD_TOOL_TOKEN');
  }
