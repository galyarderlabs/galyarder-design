...
Galyarder Design plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Galyarder Design PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `galyarder-design.json` as the Galyarder Design sidecar.
...
2. Push a public GitHub repository or open an Galyarder Design PR.
3. Add README install instructions for Galyarder Design and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Galyarder Design | GD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `galyarder-design.json` | Open a PR to Galyarder Design or publish a marketplace index entry pointing to the plugin repo. |
...
For Galyarder Design plugin authors:

- Ensure the repo or subpath contains a valid `SKILL.md`.
- Keep `galyarder-design.json` additive; generic skill clients should be able to ignore it.
- Put a short install block in your README:

```bash
npx skills add owner/repo --skill my-plugin
gd plugin install https://github.com/owner/repo
```
...
- Use a GitHub topic and README keywords that match the plugin's lane, such as `galyarder-design-plugin`, `agent-skill`, `prototype`, `deck`, `hyperframes`, or `design-system`.
...
Use this path for normal Galyarder Design plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Galyarder Design PR.
- Keep changelog text honest and versioned.
- Keep `galyarder-design.json` `specVersion` fixed to the spec kit version and bump plugin `version` for every publishable behavior change.
