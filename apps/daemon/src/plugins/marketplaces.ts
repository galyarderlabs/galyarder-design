...
// `plugin_marketplaces`. The actual `gd plugin install <name>` resolution
...
// validation lives in `@galyarder-design/plugin-runtime`'s parser and we only
...
} from '@galyarder-design/plugin-runtime';
import {
...
const DEFAULT_MARKETPLACE_REPO = 'galyarderlabs/galyarder-design';
const DEFAULT_MARKETPLACE_REPO_REF = 'main';
const DEFAULT_MARKETPLACE_REGISTRY_PATH = 'plugins/registry';
const PUBLIC_MARKETPLACE_BASE_URL = 'https://galyarder-design.ai/marketplace';
const PUBLIC_PLUGINS_BASE_URL = 'https://galyarder-design.ai/plugins';

function marketplaceRegistryRepo(): string {
  return (process.env.GD_MARKETPLACE_REPO?.trim() || DEFAULT_MARKETPLACE_REPO)
    .replace(/^\/+|\/+$/g, '');
}

export function marketplaceRegistryBaseUrl(): string {
  const explicit = process.env.GD_MARKETPLACE_REGISTRY_BASE_URL?.trim();
  if (explicit) return explicit.replace(/\/+$/, '');

  const repo = marketplaceRegistryRepo();
  const ref = (process.env.GD_MARKETPLACE_REPO_REF?.trim() || DEFAULT_MARKETPLACE_REPO_REF)
    .replace(/^\/+|\/+$/g, '');
  const registryPath = (process.env.GD_MARKETPLACE_REGISTRY_PATH?.trim() || DEFAULT_MARKETPLACE_REGISTRY_PATH)
    .replace(/^\/+|\/+$/g, '');
...
export function marketplaceManifestUrlForRegistry(id: string): string {
  ...
  return `${marketplaceRegistryBaseUrl()}/${registryId}/galyarder-design-marketplace.json`;
}

function registryIdFromBaseUrl(url: string, baseUrl: string): string | null {
  const base = baseUrl.replace(/\/+$/, '');
  if (!url.startsWith(`${base}/`) || !url.endsWith('/galyarder-design-marketplace.json')) {
    return null;
  }
  const id = url
    .slice(base.length + 1)
    .replace(/\/galyarder-design-marketplace\.json$/, '');
...
export function marketplaceRegistryIdFromUrl(url: string): string | null {
  ...
  for (const base of publicBases) {
    if (trimmed === `${base}/galyarder-design-marketplace.json`) return 'official';
    if (trimmed.startsWith(`${base}/`) && trimmed.endsWith('/galyarder-design-marketplace.json')) {
      const id = trimmed
        .slice(base.length + 1)
        .replace(/\/galyarder-design-marketplace\.json$/, '');
  ...
  try {
    ...
    return id && filename === 'galyarder-design-marketplace.json' ? id : null;
  } catch {
    return null;
  }
}
...
function safeParseManifest(raw: string): MarketplaceManifest {
  ...
  return {
    specVersion: GALYARDER_DESIGN_PLUGIN_SPEC_VERSION,
    name: 'unknown',
    version: '0.0.0',
    plugins: [],
  } as MarketplaceManifest;
