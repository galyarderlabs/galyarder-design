...
import { installMockGalyarderDesignHost } from '@galyarder-design/host/testing';
import {
  archiveFilenameFrom,
...
describe('buildDesignHandoffContent', () => {
  ...
  it('builds a machine-readable design manifest for coding tools', () => {
    ...
    expect(manifest.schema).toBe('galyarder-design.design-manifest.v1');
...
describe('exportAsMd', () => {
  ...
  it('keeps multi-byte content (UTF-8) intact end-to-end', async () => {
    // UTF-8 / CJK / Arabic fixture; verifies the export pipeline writes
    // multi-byte content (Chinese, Japanese, Arabic) through a Blob
    // without re-encoding or truncation.
...
describe('sandboxed preview Blob exports', () => {
  ...
  it('passes srcdoc options through the sandboxed new-tab wrapper', async () => {
    ...
    expect(wrapper).toContain('gd:slide');
  });
  ...
  it('uses the desktop native print bridge when the host PDF bridge is available', async () => {
    ...
    const restoreHost = installMockGalyarderDesignHost({
    ...
    // 'GD_PRINT_READY' to the parent once fonts and images are loaded.
    expect(htmlArg).toContain('GD_PRINT_READY');
    // Verify the parent-wrapper cache script is present so the handshake is
    // never missed even if 'GD_PRINT_READY' fires before the listener attaches.
  ...
  it('injects image-waiting logic into the print-ready handshake for the desktop bridge', async () => {
    ...
    expect(htmlArg).toContain("type:'GD_PRINT_READY'");
    expect(htmlArg).toContain("nonce:'");
    // The cache script also validates the nonce and event source.
    expect(htmlArg).toContain("e.data.type==='GD_PRINT_READY'");
...
describe('requestPreviewSnapshot', () => {
  ...
  it('resolves with snapshot data when the bridge responds', async () => {
    ...
    window.dispatchEvent(
      { type: 'message', source: contentWindow, data: { type: 'gd:snapshot:result', id, dataUrl: 'data:image/png;base64,abc', w: 100, h: 50 } } as unknown as Event,
    );
  ...
  it('resolves null when the bridge responds with an error', async () => {
    ...
    window.dispatchEvent(
      { type: 'message', source: contentWindow, data: { type: 'gd:snapshot:result', id, error: 'snapshot image failed' } } as unknown as Event,
    );
  ...
  it('ignores messages with a mismatched id', async () => {
    ...
    window.dispatchEvent(
      { type: 'message', source: contentWindow, data: { type: 'gd:snapshot:result', id: 'wrong-id', dataUrl: 'data:image/png;base64,abc', w: 100, h: 50 } } as unknown as Event,
    );
  ...
  it('ignores messages from a different source window', async () => {
    ...
    window.dispatchEvent(
      { type: 'message', source: { other: true }, data: { type: 'gd:snapshot:result', id, dataUrl: 'data:image/png;base64,abc', w: 100, h: 50 } } as unknown as Event,
    );
