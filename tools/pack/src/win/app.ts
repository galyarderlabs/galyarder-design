...
import { createCommandInvocation, createPackageManagerInvocation } from "@galyarder-design/platform";
...
async function runEsbuild(config: ToolPackConfig, args: string[]): Promise<void> {
  await runPnpm(config, ["--filter", "@galyarder-design/packaged", "exec", "esbuild", ...args]);
}
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  try {
    await runPnpm(config, ["--filter", "@galyarder-design/web", "build"], { GD_WEB_OUTPUT_MODE: config.webOutputMode });
    ...
    `${JSON.stringify(
      {
        ...
        description: "Galyarder Design packaged runtime",
        main: "./main.cjs",
...
async function buildPrebundledStandaloneRuntime(
  ...
  await writeFile(
    ...
    [
      ...
      "process.env.GD_BIN ??= selfPath;",
      "process.env.GD_DAEMON_CLI_PATH ??= selfPath;",
