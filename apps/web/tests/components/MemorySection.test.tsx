...
describe('MemorySection', () => {
  ...
  it('renders the memory tree and opens an entry editor from a tree node', async () => {
    ...
    const entry = {
      ...
      description: 'Galyarder Design should evolve from accepted work',
      type: 'project',
          ...
          suggestions: [
            {
              ...
              type: 'project',
              body: 'GalyarderDesign connector memory should focus on design preferences, UI decisions, and visual references from Notion.',
              source: {
                kind: 'connector',
                connectorId: 'notion',
          ...
          connectors: [
            {
              ...
              toolName: 'notion.notion_search',
              toolTitle: 'Search Notion',
              // CJK fixture; '设计思路' (design approach) verifies the
              // memory-section UI displays connector summaries with
              // non-ASCII content untouched.
  ...
  it('renders connector model authentication failures without raw provider JSON', async () => {
    ...
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
          ...
          extractions: [
            {
              ...
              userMessagePreview: 'Suggest durable GalyarderDesign memories from connected apps.',
	  ...
	  expect(
	    screen.queryByText('Suggest durable GalyarderDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(
	    within(document.querySelector('.memory-unified-list') as HTMLElement)
	      .queryByText('Suggest durable GalyarderDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(screen.getByText('OpenAI authentication expired')).toBeTruthy();
    expect(
      screen.getByText('Connected apps were read, but GalyarderDesign could not turn that context into memory.'),
    ).toBeTruthy();
    expect(
