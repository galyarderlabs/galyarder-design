...
description: Dark terminal review deck — featuring an ultra-dark background (#0a0c10) + a 56px cyber grid, CRT corner shading + scanlines, custom window chrome buttons, '$ prompt' command-line headers, mint green (#7ed3a4) display text, JetBrains Mono typography, stroke-only charts, blinking cursor, and tri-color status badges. Perfect for CLI utilities, autonomous agents, and dev tool evaluations (including trace flows, diffs, and benchmarks).
triggers:
  - "terminal review"
  - "cli review"
  - "agent review"
...
od:
  ...
  animations: true
  example_prompt: >-
    Create a CLI or agent review deck using the html-ppt-hermes-cyber-terminal template. Build a terminal-styled slide with CRT scanlines, command-line headers, and a clean benchmark chart. First confirm: target system, 3-5 comparison metrics, and baseline benchmark data.
---
