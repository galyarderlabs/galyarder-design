...
// + galyarder-design.json pair) and register every match into the
...
// daemon code.
//
// `gd plugin uninstall` of a bundled plugin is rejected by the
...
import type { InstalledPluginRecord, MarketplaceTrust } from '@galyarder-design/contracts';
...
export async function registerBundledPlugins(
  ...
  for (const tier of topLevel) {
    ...
    const tierManifest = path.join(tierAbs, 'galyarder-design.json');
    if (await pathExists(tierManifest)) {
      // Direct: <bundledRoot>/<plugin-id>/galyarder-design.json
    ...
    for (const entry of inner) {
      ...
      const manifest = path.join(folder, 'galyarder-design.json');
