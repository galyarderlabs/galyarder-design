...
import {
  ...
  GalyarderDesignSpecVersionSchema,
} from './manifest.js';
...
// `galyarder-design-marketplace.json` schema (v1). Mirrors
// `docs/schemas/galyarder-design.marketplace.v1.json`. The federated catalog
// format is intentionally permissive — community catalogs can carry extra
// fields (e.g. clawhub category tags) without breaking GD installs.
...
export const MarketplaceManifestSchema = z.object({
  ...
  specVersion: GalyarderDesignSpecVersionSchema.default(GALYARDER_DESIGN_PLUGIN_SPEC_VERSION),
  name:        z.string().min(1),
  version:     z.string().min(1),
  owner: z.object({
