...
describe('findPriorArtifactSiblings', () => {
  ...
  async function makeDir() {
    const dir = await mkdtemp(path.join(tmpdir(), 'gd-stub-guard-'));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it('returns an empty list when the directory does not exist', async () => {
    const priors = await findPriorArtifactSiblings('/nonexistent/gd/projects/missing', 'dashboard');
  ...
  it('falls back to the "artifact" basename when the identifier slugifies to empty', async () => {
    ...
    // Detection pattern; CJK identifier '测试' here exercises the empty-slug fallback path.
  ...
  it('does NOT match a fallback sibling whose sidecar identifier differs (lefarcen/mrcfps round 4)', async () => {
    ...
    // Detection pattern; CJK identifiers '测试' / '首页' here verify two non-ASCII
    // identifiers with the same fallback basename are kept distinct.
    await writeArtifactPair(dir, 'artifact.html', 'a'.repeat(40_000), '测试');
...
describe('artifactIdentifiersMatch', () => {
  ...
  it('does not match distinct identifiers that both slugify to empty', () => {
    // Detection pattern; CJK identifiers '测试' / '首页' both slugify to empty,
    // verifying the matcher distinguishes them by their raw form.
  ...
  it('matches a non-ASCII identifier with itself even when its slug is empty', () => {
    // Detection pattern; CJK identifier '测试' verifies raw-form equality wins
    // when the slug-form is empty.
...
describe('evaluateArtifactStubGuard (integration with disk scan)', () => {
  ...
  async function makeDir() {
    const dir = await mkdtemp(path.join(tmpdir(), 'gd-stub-guard-eval-'));
    tempDirs.push(dir);
    return dir;
  }
...
describe('readArtifactStubGuardConfigFromEnv', () => {
  ...
  it('parses recognised mode values', () => {
    expect(readArtifactStubGuardConfigFromEnv({ GD_ARTIFACT_STUB_GUARD: 'reject' }).mode).toBe('reject');
    expect(readArtifactStubGuardConfigFromEnv({ GD_ARTIFACT_STUB_GUARD: 'WARN' }).mode).toBe('warn');
    expect(readArtifactStubGuardConfigFromEnv({ GD_ARTIFACT_STUB_GUARD: 'off' }).mode).toBe('off');
  });

  it('falls back to default when mode is unrecognised', () => {
    expect(readArtifactStubGuardConfigFromEnv({ GD_ARTIFACT_STUB_GUARD: 'maybe' }).mode).toBe(
  ...
  it('honours numeric overrides within range', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      GD_ARTIFACT_STUB_GUARD_MIN_RATIO: '0.35',
      GD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES: '8192',
    });
  ...
  it('accepts ratio = 1 to reject any shrinkage', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      GD_ARTIFACT_STUB_GUARD_MIN_RATIO: '1',
    });
  ...
  it('rejects out-of-range numeric overrides', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      GD_ARTIFACT_STUB_GUARD_MIN_RATIO: '5',
      GD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES: '-12',
    });
