...
describe('composeSystemPrompt', () => {
  ...
  it('injects live-artifact skill guidance and metadata intent', () => {
    ...
    expect(prompt).toContain('The wrapper reads injected `GD_NODE_BIN`, `GD_BIN`, `GD_DAEMON_URL`, and `GD_TOOL_TOKEN`');
    expect(prompt).toContain('Do not include or invent `projectId`; the daemon derives project/run scope from the token.');
    expect(prompt).toContain('"$GD_NODE_BIN" "$GD_BIN" tools live-artifacts create --input artifact.json');
  ...
  // `GD_DESIGN_TOKEN_CHANNEL=0` as the kill switch). These tests pin
