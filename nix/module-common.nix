# Shared option definitions for the Galyarder Design Home Manager and NixOS
...
# `options.<scope>.galyarder-design`.
#
# The two callers differ only in:
#   - default `dataDir` (HM: $HOME/.od; NixOS: /var/lib/galyarder-design)
...
in {
  enable = lib.mkEnableOption "Galyarder Design — local-first design product daemon";

  package = lib.mkOption {
    type = lib.types.package;
    default =
      flakePackages.daemon or (throw
        "galyarder-design: no daemon package available for ${pkgs.stdenv.hostPlatform.system}; set services.galyarder-design.package explicitly");
    defaultText = lib.literalExpression "galyarder-design.packages.\${pkgs.stdenv.hostPlatform.system}.daemon";
    description = "The Galyarder Design daemon package providing the `gd` binary.";
  };
  ...
  port = lib.mkOption {
    ...
    description = ''
      ...
      `GD_DAEMON_URL` injection — see section (4) of `nix/README.md`.
    '';
  };
  ...
  dataDir = lib.mkOption {
    ...
    defaultText =
      ...
      (
        if defaultDataDir == "/var/lib/galyarder-design"
        then "\"/var/lib/galyarder-design\""
  ...
  environmentFile = lib.mkOption {
    ...
    '';
    example = "/run/secrets/galyarder-design.env";
  };
  ...
  extraEnv = lib.mkOption {
    ...
    description = ''
      ...
      service (e.g. `GD_CODEX_DISABLE_PLUGINS = "1"`). Secrets belong
      in `environmentFile`, not here.
    '';
    example = lib.literalExpression ''
      {
        GD_CODEX_DISABLE_PLUGINS = "1";
      }
    '';
  };
  ...
  webFrontend = {
    # The Galyarder Design web frontend is a static SPA built by
    # `apps/web` → `apps/web/out/`. The daemon is a separate Express
    # process that serves the JSON API at `/api/*`. The SPA is built
    # with `GD_DAEMON_URL=""`, so the bundled JS issues relative
    ...
    host = lib.mkOption {
      ...
      description = ''
        ...
        must additionally set `services.galyarder-design.openFirewall =
    ...
    allowedOrigins = lib.mkOption {
      ...
      description = ''
        ...
        to set `extraEnv.GD_WEB_PORT = "<proxy-port>"`, which whitelists
        that port on every loopback host without enumerating origins.

        Each entry is forwarded to the daemon via the `GD_ALLOWED_ORIGINS`
    ...
    package = lib.mkOption {
      ...
      default =
        flakePackages.web or (throw
          "galyarder-design: no web package available for ${pkgs.stdenv.hostPlatform.system}; set services.galyarder-design.webFrontend.package explicitly");
      defaultText = lib.literalExpression "galyarder-design.packages.\${pkgs.stdenv.hostPlatform.system}.web";
