...
async function buildFixtureTarball(args: {
  ...
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'gd-fixture-'));
...
beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-installer-archive-'));
...
describe('archive installer', () => {
  it('extracts a github:owner/repo source via the codeload tarball URL', async () => {
    ...
    expect(urlSeen).toBe('https://codeload.github.com/galyarder-design/sample-plugin/tar.gz/HEAD');
    const row = db.prepare(`SELECT source_kind, source FROM installed_plugins WHERE id = 'sample-plugin'`).get();
    expect(row).toEqual({ source_kind: 'github', source: 'github:galyarder-design/sample-plugin' });
  });
  ...
  it('extracts a github source with a ref and plugin subpath', async () => {
    ...
    const apiUrl =
      'https://api.github.com/repos/galyarderlabs/galyarder-design/contents/plugins/community/registry-starter?ref=garnet-hemisphere';
    ...
    let success = false;
    let error: string | undefined;
    const source = 'github:galyarderlabs/galyarder-design@garnet-hemisphere/plugins/community/registry-starter';
    for await (const ev of installPlugin(db, {
      source,
      roots: { userPluginsRoot: pluginsRoot },
      fetcher,
    ...
    expect(urlsSeen).not.toContain('https://codeload.github.com/galyarderlabs/galyarder-design/tar.gz/garnet-hemisphere');
    const row = db.prepare(`SELECT source_kind, source FROM installed_plugins WHERE id = 'sample-plugin'`).get();
  ...
  it.each([
    ...
    const tarball = await buildFixtureTarball({
      rootPrefix: 'galyarder-design-main',
      pluginSubpath: 'plugins/community/import-smoke-test',
    });
    const urlsSeen: string[] = [];
    const contentsUrl =
      'https://api.github.com/repos/galyarderlabs/galyarder-design/contents/plugins/community/import-smoke-test?ref=main';
    const tarballUrl = 'https://codeload.github.com/galyarderlabs/galyarder-design/tar.gz/main';
    const fetcher: ArchiveFetcher = async (u) => {
      urlsSeen.push(u);
      if (u === contentsUrl) {
    ...
    let success = false;
    let error: string | undefined;
    const source = 'github:galyarderlabs/galyarder-design@main/plugins/community/import-smoke-test';
    for await (const ev of installPlugin(db, {
      source,
      roots: { userPluginsRoot: pluginsRoot },
      fetcher,
