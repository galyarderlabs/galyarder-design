 ...
 *
 * Phase 3 dry-run: propose a `gd.scenario` value for every visible plugin
 * manifest under `plugins/_official/**`, mapped onto the 7 scenario lanes
 * derived from the user-query analysis report.
 *
 *   business-system  | Business systems / admin / data dashboards
 *   presentation     | Slide decks / reports / courseware
 *   app-prototype    | App / multi-screen product prototype
 *   landing          | Marketing site / landing / promo pages
 *   brand-visual     | Brand visual / logo / design system
 *   dev-tool         | Developer tools / engineering collaboration
 *   media-asset      | Image / video / showcase assets
 *   general          | Fallback: existing signals can't classify reliably
 *
...
const SCENARIO_LABEL: Record<Scenario, string> = {
  ...
  'dev-tool': 'Developer tools / engineering collaboration',
  'media-asset': 'Image / video / showcase assets',
  general: 'Fallback (needs human review)',
};
...
function classify(manifest: Manifest, relPath: string): { scenario: Scenario; reason: string } {
  const gd = manifest.gd ?? {};
  const mode = String(gd.mode ?? '').toLowerCase();
  const taskKind = String(gd.taskKind ?? '').toLowerCase();
  const scenario = String(gd.scenario ?? '').toLowerCase();
...
async function main() {
  ...
  for (const manifestPath of manifests) {
    ...
    // `gd.kind !== 'atom'` so this matches the visible-plugin contract.
    if (manifest.gd?.kind === 'atom') continue;
    ...
    candidates.push({
      ...
      currentScenario: String(manifest.gd?.scenario ?? ''),
      currentMode: String(manifest.gd?.mode ?? ''),
      currentTaskKind: String(manifest.gd?.taskKind ?? ''),
      tags: manifest.tags ?? [],
      proposedScenario: scenario,
      reason,
    });
