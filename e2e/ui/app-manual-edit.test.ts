...
test('manual edit inspector previews and persists page and selected element styles', async ({ page }) => {
  ...
  await galyarderDesignFile(page, 'manual-edit.html');

  await expect(page.getByTestId('artifact-preview-frame')).toBeVisible();
  const frame = page.frameLocator('[data-testid="artifact-preview-frame"]');
  await expect(frame.getByRole('heading', { name: 'Original Hero' })).toBeVisible();
  const responsivePair = frame.locator('[data-gd-id="responsive-pair"]');
  ...
  const selectedTitleMarker = frame.locator('[data-gd-id="hero-title"][data-gd-edit-selected="true"]');
  ...
  await frame.locator('body').evaluate(() => {
    window.parent.postMessage({ type: 'gd-edit-targets', targets: [] }, '*');
  });
  await expect(page.locator('.manual-edit-modal')).toContainText('TYPOGRAPHY');
  ...
  await expectFileSourceExcludes(page, projectId, 'manual-edit.html', ['data-gd-edit-selected']);
  await expect(page.locator('.manual-edit-modal')).toContainText('TYPOGRAPHY');
  await expect(page.locator('.manual-edit-modal')).not.toContainText('PAGE');
  await expect(selectedTitleMarker).toHaveCount(1);
  await expect(page.locator('.manual-edit-error')).toHaveCount(0);
...
test('manual edit mode preserves preview actions after style edits', async ({ page }) => {
  ...
  await page.getByTestId('manual-edit-mode-toggle').click();
  const fontSizeInput = await selectStyleRowInput(page, frame, '[data-gd-id="hero-title"]', 'TYPOGRAPHY', 'Size');
  await fontSizeInput.fill('48');
...
test('manual edit mode keeps deck navigation available for deck-shaped HTML', async ({ page }) => {
  ...
  await galyarderDesignFile(page, 'manual-deck.html');
...
async function gotoEntryHome(page: Page) {
  ...
  await waitForLoadingToClear(page);
  const privacyDialog = page.getByRole('dialog').filter({ hasText: 'Help us improve Galyarder Design' });
...
async function seedDeckArtifact(
  ...
  const slideHtml = slides
    .map((slide, index) => `<section class="slide" data-gd-id="slide-${index + 1}"${index === 0 ? '' : ' hidden'}><h1>${slide}</h1></section>`)
    .join('\n');
  const resp = await page.request.post(
    `/api/projects/${projectId}/files`,
    {
...
async function galyarderDesignFile(page: Page, fileName: string) {
...
<html>
  ...
  <body style="font-family: Inter, system-ui, sans-serif; font-size: 16px; letter-spacing: 0.01em;">
    <main>
      <section data-gd-id="responsive-pair" data-gd-label="Responsive pair" class="responsive-pair">
        <div data-gd-id="pair-a">Left panel</div>
        <div data-gd-id="pair-b">Right panel</div>
      </section>
      <section data-gd-id="hero" data-gd-label="Hero section" style="display:flex;gap:8px;align-items:center;">
        <h1 data-gd-id="hero-title" data-gd-label="Hero title">Original Hero</h1>
        <a data-gd-id="cta" data-gd-label="Primary CTA" href="/start">Start now</a>
        <img data-gd-id="hero-image" data-gd-label="Hero image" src="/hero.png" alt="Hero" style="width:64px;height:64px;">
      </section>
    </main>
  </body>
</html>`;
...
<html>
  <body>
    <section class="slide" data-gd-id="slide-1"><h1>Slide One</h1></section>
    <section class="slide" data-gd-id="slide-2" hidden><h1>Slide Two</h1></section>
    <script>
      let active = 0;
      const slides = Array.from(document.querySelectorAll('.slide'));
      function render() { slides.forEach((slide, index) => { slide.hidden = index !== active; }); }
      window.addEventListener('message', (event) => {
        if (!event.data || event.data.type !== 'gd:slide') return;
        if (event.data.action === 'next') active = Math.min(slides.length - 1, active + 1);
        if (event.data.action === 'prev') active = Math.max(0, active - 1);
        render();
        window.parent.postMessage({ type: 'gd:slide-state', active, count: slides.length }, '*');
      });
      render();
      window.parent.postMessage({ type: 'gd:slide-state', active, count: slides.length }, '*');
    </script>
