...
// pre-render. The daemon mirrors these values via the x-gd-analytics-*
// headers (see @galyarder-design/contracts/analytics).

import type { AnalyticsClientType } from '@galyarder-design/contracts/analytics';
import { detectGalyarderDesignHostClientType } from '@galyarder-design/host';

const ANONYMOUS_ID_KEY = 'galyarder-design:analytics.anonymous_id';
const SESSION_ID_KEY = 'galyarder-design:analytics.session_id';
...
// Desktop packaged builds install the Galyarder Design host bridge so the
...
export function detectClientType(): AnalyticsClientType {
  ...
  return detectGalyarderDesignHostClientType();
}
