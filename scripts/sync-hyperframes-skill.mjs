...
// Code, Codex, Cursor, Trae, Factory, etc.) — but GD supports a wider
...
// content through GD's own skill scanner (which injects the SKILL.md
// body into the system prompt), every GD-supported agent — including
// BYOK setups — gets HyperFrames guidance uniformly.
//
// This script does NOT auto-merge. Reasons:
//   1. We add an GD-specific frontmatter shim (gd.mode/surface/preview/…)
//      and an "Galyarder Design integration" section near the top of
//      SKILL.md. An auto-merge would either drop the shim (breaking GD
...
//                                                       by hand
//
// To actually apply: copy the upstream files in by hand, re-add the GD
// frontmatter shim and the "Galyarder Design integration" section.
...
async function main() {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-hf-sync-'));
  try {
    ...
    for (const [rel, up] of upstreamMap) {
      ...
      // SKILL.md gets local edits (frontmatter shim + GD integration
    ...
    console.log(
      '\n  Upstream copy lives at:\n' +
        ...
        '   commit the merge by hand. Re-add GD frontmatter shim if it\n' +
    ...
    if (process.env.GD_KEEP_HF_SYNC_TMP) {
      console.log(`[sync] GD_KEEP_HF_SYNC_TMP set — leaving ${tmpRoot}`);
    } else {
