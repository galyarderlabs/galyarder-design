...
describe('daemon origin validation middleware', () => {
  ...
  it('blocks local guarded routes without Origin when Host only matches a configured deployment origin', async () => {
    process.env.GD_ALLOWED_ORIGINS = 'https://gd.example.com';
    try {
      const res = await request(port, 'POST', '/api/active', {
        headers: {
          Host: 'gd.example.com',
      ...
      delete process.env.GD_ALLOWED_ORIGINS;
    }
  });
  ...
  it('allows local guarded routes without Origin when Host matches a configured non-loopback IP origin', async () => {
    ...
    process.env.GD_ALLOWED_ORIGINS = `http://${lanHost}`;
    try {
      const res = await request(port, 'POST', '/api/active', {
        headers: {
          Host: lanHost,
  ...
  it('allows explicitly configured deployment origins', async () => {
    process.env.GD_ALLOWED_ORIGINS = `https://gd.example.com,http://203.0.113.10:${port}`;
    try {
      const res = await request(port, 'GET', '/api/projects', {
        origin: 'https://gd.example.com',
      });
  ...
  // --- GD_WEB_PORT (split-port proxy) ---

  it('allows requests from GD_WEB_PORT (web proxy port)', async () => {
    const webPort = port + 1000;
    process.env.GD_WEB_PORT = String(webPort);
    const res = await request(port, 'GET', '/api/projects', {
      origin: `http://127.0.0.1:${webPort}`,
    });
    delete process.env.GD_WEB_PORT;
    expect(res.status).toBe(200);
  });

  it('blocks requests from unknown ports even with GD_WEB_PORT set', async () => {
    const webPort = port + 1000;
    process.env.GD_WEB_PORT = String(webPort);
    const res = await request(port, 'GET', '/api/projects', {
...
// browser origin is explicitly listed in GD_ALLOWED_ORIGINS, because
...
// documented escape-hatch behavior of GD_ALLOWED_ORIGINS.
describe('isLocalSameOrigin: GD_ALLOWED_ORIGINS bypass for reverse-proxy deployments', () => {
  const ALLOWED = 'http://192.168.8.168:7457';
  const previousAllowedOrigins = process.env.GD_ALLOWED_ORIGINS;
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    GD_ALLOWED_ORIGINS: ALLOWED,
    GD_BIND_HOST: '0.0.0.0',
  };

  beforeAll(() => {
    process.env.GD_ALLOWED_ORIGINS = ALLOWED;
  });
  afterAll(() => {
    if (previousAllowedOrigins === undefined) delete process.env.GD_ALLOWED_ORIGINS;
    else process.env.GD_ALLOWED_ORIGINS = previousAllowedOrigins;
  });

  it('accepts a request whose Origin matches GD_ALLOWED_ORIGINS even when the Host header is the proxy upstream', () => {
    const req = {
      headers: {
  ...
  it('still rejects a request whose Origin is not in GD_ALLOWED_ORIGINS', () => {
    const req = {
      headers: {
