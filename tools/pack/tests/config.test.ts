...
const savedTelemetryRelayUrl = process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL;
...
afterEach(() => {
  if (savedTelemetryRelayUrl == null) {
    delete process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL;
  } else {
    process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL = savedTelemetryRelayUrl;
  }
...
describe("resolveToolPackConfig telemetry relay", () => {
  it("reads and normalizes GALYARDER_DESIGN_TELEMETRY_RELAY_URL for packaged config", () => {
    process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL = "https://telemetry.galyarder-design.ai/api/langfuse//";
    const config = resolveToolPackConfig("mac", { namespace: "telemetry-test" });
    expect(config.telemetryRelayUrl).toBe("https://telemetry.galyarder-design.ai/api/langfuse");
  });

  it("rejects invalid telemetry relay URLs", () => {
    process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL = "not-a-url";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /GALYARDER_DESIGN_TELEMETRY_RELAY_URL must be an absolute https URL/,
    );
  });

  it("rejects plaintext telemetry relay URLs for packaged config", () => {
    process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL = "http://telemetry.galyarder-design.ai/api/langfuse";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /GALYARDER_DESIGN_TELEMETRY_RELAY_URL must use https/,
    );
  });
});
