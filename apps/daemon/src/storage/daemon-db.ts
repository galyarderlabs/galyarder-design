...
// `GD_DAEMON_DB=postgres` returns a stub that throws when used so
...
export function resolveDaemonDbConfig(env?: Record<string, string | undefined>): DaemonDbConfig {
  ...
  const kind = (e.GD_DAEMON_DB ?? 'sqlite').trim().toLowerCase();
  if (kind === 'postgres') {
    const host = e.GD_PG_HOST ?? '';
    const portStr = e.GD_PG_PORT ?? '5432';
    const database = e.GD_PG_DATABASE ?? '';
    const user = e.GD_PG_USER ?? '';
    const sslMode = e.GD_PG_SSL_MODE === 'disable' || e.GD_PG_SSL_MODE === 'verify-full'
      ? e.GD_PG_SSL_MODE
      : 'require';
    if (!host || !database || !user) {
      throw new DaemonDbConfigError(
        'GD_DAEMON_DB=postgres requires GD_PG_HOST, GD_PG_DATABASE, GD_PG_USER. ' +
        'GD_PG_PORT defaults to 5432; GD_PG_SSL_MODE defaults to "require".',
      );
    }
    return {
      kind: 'postgres',
  ...
  if (kind !== 'sqlite' && kind !== '') {
    throw new DaemonDbConfigError(
      `unknown GD_DAEMON_DB value '${kind}'. Accepted: 'sqlite' (default), 'postgres'.`,
    );
  }
  return { kind: 'sqlite' };
}
