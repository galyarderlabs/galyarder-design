...
Local packaging control plane for Galyarder Design.
...
Release artifacts keep the canonical `Galyarder Design.app` bundle shape; local `tools-pack install` copies it as
`Galyarder Design.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
...
- Finder/manual launches on macOS may not inherit your shell-managed `PATH`. If packaged Galyarder Design cannot detect agent CLIs that work in Terminal, expose those binaries to the GUI login environment or launch the packaged app from a shell session that already sees them.
...
- AppImage: `~/.local/bin/Galyarder-Design.<namespace>.AppImage`
- Menu entry: `~/.local/share/applications/galyarder-design-<namespace>.desktop`
- Icon: `~/.local/share/icons/hicolor/512x512/apps/galyarder-design-<namespace>.png`

The `<namespace>` suffix is unconditional so multiple developer namespaces can coexist on the same desktop. The `.desktop` file registers the `gd://` scheme via `MimeType=x-scheme-handler/gd;` and pre-sets `GD_PACKAGED_NAMESPACE` on the `Exec=` line so menu launches identify the correct namespace.
...
`--headless` makes `install`, `start`, `stop`, `uninstall`, and `cleanup` operate on the headless entry (`@galyarder-design/packaged/dist/headless.mjs`) instead of the AppImage. Headless mode runs daemon + web without Electron.

- `install --headless` writes a shell launcher at `~/.local/bin/galyarder-design-headless-<namespace>` that bakes in the namespace and resource paths. The launcher is self-contained, but the assembled app directory at those paths must remain in place — don't move it after install.
