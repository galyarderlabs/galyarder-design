...
} from "@galyarder-design/download";
import {
...
export const DESKTOP_UPDATE_ENV = Object.freeze({
  ARCH: "GD_UPDATE_ARCH",
  AUTO_CHECK: "GD_UPDATE_AUTO_CHECK",
  AUTO_DOWNLOAD: "GD_UPDATE_AUTO_DOWNLOAD",
  AUTO_OPEN: "GD_UPDATE_AUTO_OPEN",
  CHECK_BACKOFF_INITIAL_MS: "GD_UPDATE_CHECK_BACKOFF_INITIAL_MS",
  CHECK_BACKOFF_MAX_MS: "GD_UPDATE_CHECK_BACKOFF_MAX_MS",
  CHECK_INITIAL_DELAY_MS: "GD_UPDATE_CHECK_INITIAL_DELAY_MS",
  CHECK_INTERVAL_MS: "GD_UPDATE_CHECK_INTERVAL_MS",
  CHANNEL: "GD_UPDATE_CHANNEL",
  CURRENT_VERSION: "GD_UPDATE_CURRENT_VERSION",
  DOWNLOAD_ROOT: "GD_UPDATE_DOWNLOAD_ROOT",
  ENABLED: "GD_UPDATE_ENABLED",
  METADATA_URL: "GD_UPDATE_METADATA_URL",
  MODE: "GD_UPDATE_MODE",
  OPEN_DRY_RUN: "GD_UPDATE_OPEN_DRY_RUN",
  PLATFORM: "GD_UPDATE_PLATFORM",
} as const);

const DEFAULT_RELEASE_ORIGIN = "https://releases.galyarder-design.ai";
const OWNERSHIP_SENTINEL = ".galyarder-design-updater-root.json";
...
function logStoreError(logger: DesktopUpdaterLogger, error: DesktopUpdateErrorSnapshot): void {
  logger.error("[galyarder-design updater] invalid update store", error);
}
      ...
      if (!(await directoryIsEmpty(realRoot))) {
        return {
          ...
          error: createError(
            ...
            `update root is not empty and has no Galyarder Design updater ownership marker: ${realRoot}`,
          ),
        };
      }
      await writeJson(sentinelPath, {
        createdAt: new Date().toISOString(),
        owner: "galyarder-design-updater",
...
async function cleanupBackDirectory(root: string, logger: DesktopUpdaterLogger): Promise<void> {
  ...
  if (!entry.isDirectory() || entry.isSymbolicLink()) {
    logger.warn("[galyarder-design updater] skipped invalid update backup directory", backDir);
    return;
  }
  const realBackDir = await realpath(backDir).catch(() => null);
  if (realBackDir == null || !containsPath(root, realBackDir)) {
    logger.warn("[galyarder-design updater] skipped escaped update backup directory", backDir);
    return;
  }
  ...
  await Promise.all(entries.map(async (entry) => {
    ...
    await rm(resolved, { force: true, recursive: true }).catch((error: unknown) => {
      logger.warn("[galyarder-design updater] failed to clean update backup entry", error);
    });
  }));
}
...
function scheduleBackCleanup(root: string, logger: DesktopUpdaterLogger): void {
  void cleanupBackDirectory(root, logger).catch((error: unknown) => {
    logger.warn("[galyarder-design updater] failed to clean update backup directory", error);
  });
}
...
async function clearInterruptedIncomingDownload(
  ...
  if (containsPath(stagingRoot, stagingDir)) {
    await rm(stagingDir, { force: true, recursive: true }).catch((error: unknown) => {
      logger.warn("[galyarder-design updater] failed to clean interrupted update staging directory", error);
    });
  } else {
    logger.warn("[galyarder-design updater] skipped escaped interrupted update staging directory", {
  ...
  logger.warn("[galyarder-design updater] cleared interrupted update download", {
    cycleId: incoming.cycleId,
...
export function createDesktopUpdater(
  ...
  async function writeInstallObservation(attemptedAt: string): Promise<InstallerObservationHandle | null> {
    ...
    try {
      ...
      logger.warn("[galyarder-design updater] failed to write installer observation", observationError);
      return null;
    }
  }
  ...
  async function markInstallObservationOpenFailed(
    ...
    try {
      ...
      logger.warn("[galyarder-design updater] failed to update installer observation", observationError);
    }
  }
...
export function createDesktopUpdaterScheduler(
  ...
  const tick = async () => {
    ...
    try {
      ...
      logger.warn("[galyarder-design updater] scheduled update check failed", error);
    } finally {
      tickRunning = false;
    }
