...
| D | shipped (MVP) | `apps/daemon/src/plugins/atoms/registry.ts` + `built-ins.ts` introduce an atom worker registry; `firePipelineForRun` now drives stages through `runStageWithRegistry`. Set `GD_PIPELINE_RUNNER=stub` to fall back to the v1 canned-signal runner. Real workers only ship for `critique-theater` today (reads `run_devloop_iterations.critique_summary` for the latest parseable score); every other FIRST_PARTY_ATOM registers a permissive worker so happy-path convergence matches v1. |
...
- `apps/daemon/src/server.ts#firePipelineForRun` now branches on `GD_PIPELINE_RUNNER`: default (`registry`) drives stages through `runStageWithRegistry`; `GD_PIPELINE_RUNNER=stub` falls back to the canned signal pump for diagnostic bisection.
...
- `plugins/_official/scenarios/od-media-generation/{galyarder-design.json,SKILL.md}`
- `plugins/_official/atoms/media-generate/{galyarder-design.json,SKILL.md}`
...
- R3 — Stage D changes runtime behaviour. Keep `GD_BUNDLED_ATOM_PROMPTS=0` and an explicit `GD_PIPELINE_RUNNER=stub` escape hatch until e2e stabilises.
