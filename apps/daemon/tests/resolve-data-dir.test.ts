/**
 * Unit tests for resolveDataDir, the GD_DATA_DIR path resolver. Covers the
...
describe('resolveDataDir', () => {
  ...
  it('returns <projectRoot>/.gd when GD_DATA_DIR is unset', () => {
    expect(resolveDataDir(undefined, projectRoot)).toBe(path.join(projectRoot, '.gd'));
    expect(resolveDataDir('', projectRoot)).toBe(path.join(projectRoot, '.gd'));
  });

  it('expands a leading ~/ against the user home directory', () => {
    const out = resolveDataDir('~/gd-test', projectRoot);
    expect(out).toBe(path.join(fakeHome, 'gd-test'));
  });
  ...
  it('expands $HOME/ against the user home directory', () => {
    const out = resolveDataDir('$HOME/gd-test', projectRoot);
    expect(out).toBe(path.join(fakeHome, 'gd-test'));
  });
  ...
  it('resolves relative paths against projectRoot', () => {
    const out = resolveDataDir('rel-gd', projectRoot);
    expect(out).toBe(path.join(projectRoot, 'rel-gd'));
  });
});
