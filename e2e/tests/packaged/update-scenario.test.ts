...
describe('packaged updater release scenario', () => {
  ...
  test('does not override current version for release-channel smoke', () => {
    ...
    expect(env.GD_UPDATE_CURRENT_VERSION).toBeUndefined();
    expect(env.GD_UPDATE_METADATA_URL).toBe('http://127.0.0.1:1234/nightly/latest/metadata.json');
    expect(env.GD_UPDATE_AUTO_CHECK).toBe('1');
  });
});
