// French (fr) localized display content for the gallery / picker / catalog
// surfaces. This file mirrors the role of `apps/web/src/i18n/locales/fr.ts`
// for content metadata that is too large to inline in the main locale dict.
// Non-English literals below are the canonical French translations of
// gallery copy and are treated functionally identical to dictionary
// content under `apps/web/src/i18n/locales/`.
...
export const FR_SKILL_COPY: Record<string, { description?: string; examplePrompt?: string }> = {
  ...
  'agent-browser': {
    examplePrompt:
      'Vérifiez la preview locale Galyarder Design avec agent-browser : démarrez ou connectez Chrome CDP, ouvrez http://127.0.0.1:17573/, puis rapportez le titre, l’URL, le texte visible et enregistrez un screenshot.',
    description:
      'Automatisation navigateur pour valider la preview locale Galyarder Design. Se connecte à un endpoint Chrome CDP vérifié, lit l’état rendu de la page, peut cliquer/saisir si nécessaire et enregistre un screenshot.',
  },
  'blog-post': {
    examplePrompt:
  ...
  'galyarder-design-landing': {
    examplePrompt:
      'Concevez la landing page marketing Galyarder Design dans le style Atelier Zero / Monocle — canvas papier chaud, collage surréaliste plâtre + architecture, grande typographie display serif italique mixée, chiffres romains comme marqueurs de sections et un seul accent corail.',
  },
  'galyarder-design-landing-deck': {
    examplePrompt:
      'Créez le pitch deck Galyarder Design dans le style Atelier Zero — cover avec hero plate, séparateurs de section en chiffres romains, slide stats (31 Skills · 72 systèmes · 12 CLIs), citation client, CTA et end-card mega italic-serif. Pagination horizontal-swipe comme un magazine imprimé.',
    description:
      'Crée un slide deck single-file dans le style Atelier Zero (papier chaud, spans accent en serif italique, points finaux corail, plaques de collage surréalistes). Pagination magazine horizontale avec navigation par flèches et espace, HUD live avec compteur de slides et progress bar ; partage le stylesheet et la bibliothèque d’images à 16 slots avec le Skill frère `galyarder-design-landing`.',
  },
  'email-marketing': {
    examplePrompt:
  ...
  'kami-deck': {
    examplePrompt:
      'Créez un deck de conférence en six slides dans le style kami ("paper") — parchemin chaud, encre bleue sur la cover, une seule graisse de serif, swipe magazine horizontal.',
    description:
  ...
  'kami-landing': {
    ...
    description:
      'Génère un one-pager prêt à imprimer dans le style kami ("paper") : parchemin chaud, accent encre bleue, serif dans une seule graisse, sans italique, sans gris froids. Se lit comme un whitepaper ou un one-pager studio, pas comme une UI d’app. Multilingue (EN · id · ja). Un seul fichier HTML sans dépendances.',
  },
  'kanban-board': {
    examplePrompt:
