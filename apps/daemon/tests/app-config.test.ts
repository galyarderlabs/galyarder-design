...
describe('app-config', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'gd-appconfig-'));
  });
...
describe('app-config disabled lists', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'gd-disabled-'));
  });
...
describe('app-config telemetry prefs', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'gd-telemetry-'));
  });
