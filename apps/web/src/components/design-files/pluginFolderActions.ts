...
const INSTALL_NOTE =
  'Prefer the supported `gd plugin install --source` flow after confirming the manifest.';
...
export function buildPluginFolderAgentActionPrompt(
  ...
  return [
    ...
    `Manifest: \`${folderPath}/galyarder-design.json\``,
    '',
    'Please do this through the `gd` CLI from the current project workspace, not through hidden UI APIs.',
    INSTALL_NOTE,
...
// `contribute` opens a draft PR against the `galyarderlabs/galyarder-design` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Galyarder Design community catalog at `galyarderlabs/galyarder-design`.',
    ...
    `Manifest: \`${folderPath}/galyarder-design.json\``,
    '',
    'Run this deterministic Galyarder Design CLI workflow from the current project workspace:',
    '',
    `\`"$GD_NODE_BIN" "$GD_BIN" plugin galyarder-design-pr ${folderPath}\``,
    '',
    ...
    '- Do NOT call the legacy `gd plugin publish --to galyarder-design` CLI — that flow produces an issue URL, which is the old path we are replacing.',
  ].join('\n');
}
...
// submission path — `gd plugin publish --to galyarder-design` produces an
// Galyarder Design issue URL and belongs to the "Galyarder Design PR" button. Before
// this rewrite the prompt said "Use the supported `gd plugin publish` or
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/galyarder-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `gd plugin publish --to galyarder-design`. That command emits an Galyarder Design issue URL and belongs to the "Galyarder Design PR" button.',
    '',
    'Run this deterministic Galyarder Design CLI workflow from the current project workspace:',
    '',
    `\`"$GD_NODE_BIN" "$GD_BIN" plugin publish-repo ${folderPath}\``,
    '',
    ...
    '',
    '**Hard constraints.** Treat these as inviolable:',
    '- Do NOT call `gd plugin publish --to galyarder-design` (or any `--to <catalog>` variant). That is the registry-submission flow, not the repository-publish flow.',
