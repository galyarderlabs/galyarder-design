---
name: html-ppt
description: HTML PPT Studio — author professional static HTML presentations in many styles, layouts, and animations, all driven by templates. Use when the user asks for a presentation, PPT, slides, keynote, deck, slideshow, a reveal-style HTML deck, a RedNote-style image post, or any kind of multi-slide pitch / report / sharing document that should look tasteful and be usable with keyboard navigation. Triggers include keywords like "presentation", "ppt", "slides", "deck", "keynote", "reveal", "slideshow", "talk slides", "sharing slides", "RedNote post", "pitch deck", "tech sharing", "technical presentation".
triggers:
  - "ppt"
  - "deck"
  - "slides"
...
od:
  ...
  speaker_notes: true
  animations: true
  example_prompt: "Use html-ppt to build a 12-slide HTML deck. Confirm three things first: content / page count / audience, theme (recommend 2-3 from the 36 available), and starting full-deck template (one of pitch-deck / tech-sharing / weekly-report / xhs-post / presenter-mode-reveal). Once aligned, start writing slides."
---
...
- **15 full-deck templates** (`templates/full-decks/<name>/`) — complete multi-slide decks with scoped `.tpl-<name>` CSS. 8 extracted from real-world decks (xhs-white-editorial, graphify-dark-graph, knowledge-arch-blueprint, hermes-cyber-terminal, obsidian-claude-gradient, testing-safety-alert, xhs-pastel-card, dir-key-nav-minimal), 7 scenario scaffolds (pitch-deck, product-launch, tech-sharing, weekly-report, xhs-post 3:4, course-module, **presenter-mode-reveal** — built for presenter mode)
...
### 🎤 Presenter Mode (presenter view + verbatim script)

If the user mentions any of: **public talk / sharing session / speaker script / verbatim script / speaker notes / presenter view / teleprompter**, or says things like "I need to give a team talk on X", "I'm running a tech-sharing session", "I'm worried about losing my flow", or "I want a deck with a verbatim script attached" — **use the `presenter-mode-reveal` full-deck template** and write 150–300 words of verbatim script in each slide's `<aside class="notes">`.
...
1. **Cue, not script.** Bold the core words and break transitions out into their own paragraphs.
2. **150–300 words per slide.** That's the 2–3 minute-per-slide cadence.
3. **Use spoken language, not written prose.** Replace formal connectives ("therefore" → "so", "this proposal" → "this one") with the way you'd actually speak.
...
- 🟠 **SPEAKER SCRIPT** — large-font verbatim script (scrollable)
...
Only `presenter-mode-reveal` is designed from the ground up around the feature with proper example verbatim scripts on every slide.
...
1. **Content & audience.** What's the deck about, how many slides, who's
   watching (engineers / execs / RedNote readers / students / VC)?
2. **Style / theme.** Which of the 36 themes fits? If unsure, recommend 2-3
   ...
   - RedNote post → `xiaohongshu-white`, `soft-pastel`, `rainbow-gradient`,
     `magazine-bold`
...
3. **Starting point.** One of the 14 full-deck templates, or scratch? Point
   ...
   user's content suggests something obvious (e.g. "I'm running a product launch" →
   `product-launch`), propose it confidently instead of asking blindly.

A good opening message looks like:

> Happy to put this PPT together! Three quick questions:
> 1. Rough content / page count / who's the audience?
> 2. Style preference? I'd suggest one of these three themes: `tokyo-night` (default-pretty for tech sharing), `xiaohongshu-white` (RedNote feel), or `corporate-clean` (formal report).
> 3. Want me to start from the `tech-sharing` full-deck template as a base?
...
- **NEVER put presenter-only text on the slide itself.** Descriptive text like
  "This slide shows…" or "Speaker: feel free to add…" or small explanatory captions
...
- [references/presenter-mode.md](references/presenter-mode.md) — **presenter mode + verbatim-script authoring guide (must-read for tech sharing / public talks)**.
