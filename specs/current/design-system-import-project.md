...
Galyarder Design needs imported design systems to satisfy four stakeholders at
once:
...
`tokens.css` is the normalized GD token contract. It must use the standard
...
- `hybrid`: normalized GD files plus source/evidence files. This is the
