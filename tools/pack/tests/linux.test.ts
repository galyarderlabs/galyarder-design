...
import { requestJsonIpc, resolveAppIpcPath } from "@galyarder-design/sidecar";
import {
  APP_KEYS,
...
vi.mock("@galyarder-design/sidecar", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@galyarder-design/sidecar")>();
  return {
    ...actual,
...
describe("buildDockerArgs", () => {
  ...
  it("passes the telemetry relay URL into containerized builds when configured", () => {
    ...
    expect(args).toContain("GALYARDER_DESIGN_TELEMETRY_RELAY_URL=https://telemetry.galyarder-design.ai/api/langfuse");
  });
  ...
  it("exports GD_TOOLS_PACK_PNPM_BIN=/tmp/pnpm so the inner build's production install skips npm", () => {
    const args = buildDockerArgs(makeConfig(), { uid: 1000, gid: 1000 });
    const envFlagIndex = args.findIndex(
      (arg, i) => arg === "-e" && args[i + 1] === "GD_TOOLS_PACK_PNPM_BIN=/tmp/pnpm",
    );
...
describe("stopPackedLinuxHeadless", () => {
  it("ignores the desktop AppImage identity marker and reads only the headless marker", async () => {
    const root = await mkdtemp(join(tmpdir(), "gd-linux-headless-marker-"));
    ...
    try {
      ...
      await writeFile(
        markerPath,
        `${JSON.stringify({
          appPath: "/tmp/Galyarder-Design.AppImage",
  ...
  it("removes stale desktop AppImage markers during headless cleanup", async () => {
    const root = await mkdtemp(join(tmpdir(), "gd-linux-headless-cleanup-"));
  ...
  it("skips headless cleanup while the desktop marker PID is live in the snapshot table", async () => {
    const root = await mkdtemp(join(tmpdir(), "gd-linux-headless-cleanup-live-"));
...
describe("resolveProductionInstallCommand", () => {
  it("defaults to npm install --omit=dev --no-package-lock when GD_TOOLS_PACK_PNPM_BIN is unset", () => {
  ...
  it("treats an empty GD_TOOLS_PACK_PNPM_BIN as unset and keeps the npm host default", () => {
    expect(resolveProductionInstallCommand({ GD_TOOLS_PACK_PNPM_BIN: "" })).toEqual({
      command: "npm",
      args: ["install", "--omit=dev", "--no-package-lock"],
    });
  });

  it("uses GD_TOOLS_PACK_PNPM_BIN with hoisted-layout pnpm flags when set", () => {
    ...
    expect(
      resolveProductionInstallCommand({ GD_TOOLS_PACK_PNPM_BIN: "/tmp/pnpm" }),
    ).toEqual({
      command: "/tmp/pnpm",
  ...
  it("chains end-to-end with buildDockerArgs: docker exports GD_TOOLS_PACK_PNPM_BIN and the resolver returns the standalone pnpm install for that value", () => {
    const dockerArgs = buildDockerArgs(makeConfig(), { uid: 1000, gid: 1000 });
    const envFlagIndex = dockerArgs.findIndex(
      (arg, i) => arg === "-e" && dockerArgs[i + 1]?.startsWith("GD_TOOLS_PACK_PNPM_BIN="),
    );
    expect(envFlagIndex).toBeGreaterThan(-1);
    const envValue = dockerArgs[envFlagIndex + 1]?.split("=")[1];
    expect(envValue).toBe("/tmp/pnpm");

    const resolved = resolveProductionInstallCommand({ GD_TOOLS_PACK_PNPM_BIN: envValue });
...
Type=Application
Name=Galyarder Design (@@NAMESPACE@@)
Exec=env GD_PACKAGED_NAMESPACE=@@NAMESPACE@@ @@EXEC_PATH@@ --appimage-extract-and-run %U
Icon=@@ICON_PATH@@
MimeType=x-scheme-handler/gd;
`;

  it("substitutes all @@TOKEN@@ placeholders", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "default",
      execPath: "/home/u/.local/bin/Galyarder-Design.default.AppImage",
      iconName: "galyarder-design-default",
    });
    expect(out).toContain("Name=Galyarder Design (default)");
    expect(out).toContain(
      "Exec=env GD_PACKAGED_NAMESPACE=default /home/u/.local/bin/Galyarder-Design.default.AppImage --appimage-extract-and-run %U",
    );
    expect(out).toContain("Icon=galyarder-design-default");
  });

  it("uses GD_PACKAGED_NAMESPACE (not GD_NAMESPACE) so apps/packaged actually picks up the namespace override", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "ns",
      execPath: "/x",
      iconName: "galyarder-design-ns",
    });
    expect(out).toMatch(/^Exec=env GD_PACKAGED_NAMESPACE=ns /m);
    expect(out).not.toMatch(/GD_NAMESPACE=/);
  });
  ...
  it("preserves the MimeType=x-scheme-handler/gd; line", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "ns",
      execPath: "/x",
      iconName: "galyarder-design-ns",
    });
    expect(out).toContain("MimeType=x-scheme-handler/gd;");
  });
});
...
describe("shouldRejectLinuxHeadlessInspectOptions", () => {
  ...
  it("rejects headless eval and screenshot requests", () => {
    ...
    expect(shouldRejectLinuxHeadlessInspectOptions({ path: "/tmp/galyarder-design-linux.png" })).toBe(true);
    expect(
      shouldRejectLinuxHeadlessInspectOptions({
        expr: "document.title",
        path: "/tmp/galyarder-design-linux.png",
      }),
    ).toBe(true);
  });
});
...
describe("inspectPackedLinuxApp", () => {
  ...
  it("allows desktop inspect eval and screenshot options when headless is omitted", async () => {
    ...
    requestJsonIpcMock
      .mockResolvedValueOnce({ state: "running", url: "gd://app/" })
      .mockResolvedValueOnce({ ok: true, value: "Galyarder Design" })
      .mockResolvedValueOnce({ path: "/tmp/galyarder-design-linux.png" });
    ...
    expect(result).toEqual({
      eval: { ok: true, value: "Galyarder Design" },
      screenshot: { path: "/tmp/galyarder-design-linux.png" },
      status: { state: "running", url: "gd://app/" },
    });
...
describe("matchesAppImageProcess", () => {
  const installPath = "/home/u/.local/bin/Galyarder-Design.default.AppImage";
  ...
  it("matches --appimage-extract-and-run mode (executable in /tmp/appimage_extracted_*/<binary>)", () => {
    const ok = matchesAppImageProcess(
      {
        pid: 1234,
        executable: "/tmp/appimage_extracted_fe548e54/Galyarder Design",
