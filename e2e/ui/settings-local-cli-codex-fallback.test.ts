...
// Locale-aware label regexes (English + zh-CN + zh-TW); CJK literals are
// kept verbatim as functional UI-locale detection patterns.
const OPEN_SETTINGS_LABEL = /Open settings|打开设置|開啟設定/i;
const LOCAL_CLI_LABEL = /Local CLI|本机 CLI|本地 CLI/i;
// Locale-aware label regex matching the Codex executable path field
// (English + zh-CN); CJK kept verbatim as a UI detection pattern.
const CODEX_EXECUTABLE_PATH_LABEL = /Codex executable path|Codex 可执行文件路径/i;
...
async function openLocalCliSettings(
  ...
  await expect(
    dialog.getByLabel(CODEX_EXECUTABLE_PATH_LABEL),
  ).toBeVisible();
  return dialog;
}
