...
const GENERATION_JOB_STORAGE_PREFIX = 'gd:design-system-generation-job:';
          ...
          <button
            ...
            onClick={() => {
              if (!state.company.trim()) {
                setError('Tell Galyarder Design about the company or design system first.');
                return;
              }
              setStep('confirm');
            }}
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask Galyarder Design to update this design system.');
        return;
      }
      ...
      const wasOnboardingHandoff =
        ...
        || sessionStorage.getItem(`gd:auto-send-first:${projectId}`) === '1';
          ...
          <div className="ds-review-column">
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'revision'
                    ? 'Galyarder Design is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                    : 'Galyarder Design is still working, but you can start giving feedback on the work so far.'
                  : 'Galyarder Design is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
            ...
            <div className="ds-warning-card">
              ...
              <span>
                ...
                Galyarder Design is rendering typography with substitute web fonts.
              </span>
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'Galyarder Design is updating this system'
              : status === 'failed'
      ...
      <div>
        ...
        <span>
          <strong>
            {active
              ? job.kind === 'revision'
                ? 'Galyarder Design is revising'
                : 'Galyarder Design is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      id: 'native-oauth',
      icon: 'link',
      title: 'Galyarder Design account',
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by Galyarder Design; this build does not use an GD-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
    ...
    >
      <div className="ds-github-access-header">
        <span>
          ...
          <p>Paste a GitHub URL. Galyarder Design will use the first working access method.</p>
        </span>
        <button
          type="button"
...
async function prepareCreatedDesignSystemProject({
  ...
  try {
    ...
    try {
      window.sessionStorage.setItem(`gd:auto-send-first:${project.id}`, '1');
    } catch {
...
function buildCreationAgentPrompt(
  ...
  const title = inferDesignSystemTitle(state);
  return [
    'Create this project as a complete Galyarder Design design system workspace.',
    '',
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that Galyarder Design attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real Galyarder Design design-system package.',
    ...
    '- Before marking the design system ready, run `"$GD_NODE_BIN" "$GD_BIN" tools connectors design-system-package-audit --path . --fail-on-warnings` and fix every reported error or warning.',
...
function buildLocalFolderRunbook(folders: string[]): string {
  ...
  const intakeCommands = folders
    .map((folder, index) => `   - \`"$GD_NODE_BIN" "$GD_BIN" tools connectors local-design-context --path ${shellQuote(folder)} --output context/local-code/${localEvidenceFileName(folder, index)}\``)
    .join('\n');
  return [
...
function buildGithubConnectorRunbook(githubUrls: string[]): string {
  ...
  const intakeCommands = githubUrls
    .map((url) => `   - \`"$GD_NODE_BIN" "$GD_BIN" tools connectors github-design-context --repo ${shellQuote(url)} --output context/github/${githubEvidenceFileName(url)}\``)
    .join('\n');
  return [
...
// ─── DesignSystemFlow — gallery + Sheet preview rail (Req 15) ────────────────
//
// Card grid of design systems. Activating a card opens a Sheet (side="right")
// preview rail while keeping the grid fully visible behind it (Req 15.2).
// EmptyState on zero entries (Req 15.5 / Req 23). Error indicator with retry
// on metadata failure (Req 15.4 / Req 24). Exported component name and prop
// signature preserved per Req 15.3.

// DesignSystemSummary is already imported at the top of this file via ../types.

export interface DesignSystemFlowProps {
  /** Currently selected design system id (highlights the active card). */
  selectedId?: string | null;
  /** Called when the user selects a design system as the default. */
  onSelect?: (id: string) => void;
  /** Called when the user wants to open the full detail view for a system. */
  onOpenSystem?: (id: string) => void;
  /** Called when the user wants to create a new design system. */
  onCreate?: () => void;
  /** Called after a mutation so the parent can refresh its own list. */
  onSystemsRefresh?: () => Promise<void> | void;
}
...
/**
 * Design System Gallery — card grid with a `Sheet` (side="right") preview
 * rail. The grid stays visible while the preview is open (Req 15.2).
 *
 * @example
 *   <DesignSystemFlow
 *     selectedId={config.designSystemId}
 *     onSelect={(id) => handleChangeDefault(id)}
 *     onOpenSystem={(id) => navigate({ kind: 'design-system-detail', designSystemId: id })}
 *     onCreate={() => navigate({ kind: 'design-system-create' })}
 *   />
 */
export function DesignSystemFlow({
  selectedId,
  onSelect,
  onOpenSystem,
  onCreate,
  onSystemsRefresh: _onSystemsRefresh,
}: DesignSystemFlowProps) {
  const [systems, setSystems] = useState<DesignSystemSummary[]>([]);
  const [loadState, setLoadState] = useState<LoadState>('loading');
  const [previewId, setPreviewId] = useState<string | null>(null);
  const [previewHtml, setPreviewHtml] = useState<string | null>(null);
  const [previewLoading, setPreviewLoading] = useState(false);
  // Track which card triggered the sheet so focus can return on close.
  ...
  // Fetch showcase HTML whenever the preview sheet opens.
  useEffect(() => {
    if (!previewId) {
      setPreviewHtml(null);
      return undefined;
  ...
  function handleClosePreview() {
    ...
    if (id) {
      ...
      if (btn) {
        // Defer so Radix's focus-return doesn't race.
        setTimeout(() => btn.focus(), 0);
      }
    }
  }

  const previewSystem = systems.find((s) => s.id === previewId) ?? null;

  // ── Loading skeleton ──────────────────────────────────────────────────────
  if (loadState === 'loading') {
    return (
      <div className="dsf-gallery" aria-busy="true" aria-label="Loading design systems">
        <div className="dsf-grid">
          {Array.from({ length: 6 }).map((_, i) => (
            // eslint-disable-next-line react/no-array-index-key
            <div key={i} className="dsf-card-skeleton">
  ...
  if (loadState === 'error') {
    return (
      <div className="dsf-gallery">
        <div className="dsf-error-state" role="alert">
          <Banner
            variant="danger"
            data-testid="dsf-error-banner"
          >
            Could not load design systems. Check that the daemon is running.
          </Banner>
          <Button
            variant="secondary"
            size="sm"
            onClick={() => void load()}
            data-testid="dsf-retry-button"
  ...
  if (systems.length === 0) {
    return (
      <div className="dsf-gallery">
        <EmptyState
          icon={<DsIcon name="Blocks" size={24} />}
          title="No design systems yet"
          description="Create your first design system to define the visual style for your projects."
          action={
            onCreate ? (
              <Button variant="primary" onClick={onCreate} data-testid="dsf-create-button">
                Create design system
              </Button>
            ) : undefined
          }
          data-testid="dsf-empty-state"
        />
      </div>
    );
  }
  ...
  return (
    <div className="dsf-gallery">
      {/* Card grid — stays visible while the Sheet is open (Req 15.2) */}
      <div
        className="dsf-grid"
        role="list"
        aria-label="Design systems"
        data-testid="dsf-grid"
      >
        {systems.map((system) => {
          const isSelected = system.id === selectedId;
          const isActive = system.id === previewId;
          return (
            <div key={system.id} role="listitem" className="dsf-grid-item">
              <Card
                elevation={isActive ? 'raised' : 'resting'}
                className={[
                  'dsf-card',
                  isSelected ? 'dsf-card--selected' : '',
                  isActive ? 'dsf-card--active' : '',
                ]
                  .filter(Boolean)
                  .join(' ')}
                data-testid={`dsf-card-${system.id}`}
              >
                {/* Swatch strip — preview rail thumbnail (Req 15.1) */}
                {system.swatches && system.swatches.length > 0 ? (
                  <div className="dsf-card-swatches" aria-hidden="true">
                    {system.swatches.slice(0, 5).map((swatch: string, idx: number) => (
                      // eslint-disable-next-line react/no-array-index-key
                      <span
                        key={idx}
                        className="dsf-card-swatch"
                        style={{ background: swatch }}
                      />
                    ))}
                  </div>
                ) : (
                  <div className="dsf-card-swatch-placeholder" aria-hidden="true" />
                )}

                {/* Card body */}
                <div className="dsf-card-body">
                  <div className="dsf-card-meta">
                    <span className="dsf-card-title">{system.title}</span>
                    {system.status === 'draft' ? (
                  ...
                  {system.summary ? (
                    <p className="dsf-card-summary">{system.summary}</p>
                  ) : null}
                  {system.category ? (
                    <span className="dsf-card-category">{system.category}</span>
                  ) : null}
                </div>

                {/* Card actions — ds/* Button and IconButton primitives */}
                <div className="dsf-card-actions">
                  <Button
                    ref={(el) => {
                      triggerRefs.current.set(system.id, el);
                    }}
                    variant="ghost"
                    size="sm"
                    ...
                    data-testid={`dsf-preview-btn-${system.id}`}
                  >
                  ...
                  {onSelect ? (
                    <Button
                      ...
                      size="sm"
                      aria-label={isSelected ? `${system.title} is active` : `Use ${system.title}`}
                      aria-pressed={isSelected}
                      onClick={() => onSelect(system.id)}
                      data-testid={`dsf-select-btn-${system.id}`}
                    >
                      {isSelected ? (
                        <>
                  ...
                  {onOpenSystem ? (
                    <IconButton
                      variant="ghost"
                      size="sm"
                      aria-label={`Open ${system.title} detail`}
                      onClick={() => onOpenSystem(system.id)}
                      data-testid={`dsf-open-btn-${system.id}`}
                      className="dsf-card-open-btn"
                    >
      ...
      {/* Create CTA */}
      {onCreate ? (
        <div className="dsf-create-row">
          <Button
            variant="secondary"
            size="sm"
            onClick={onCreate}
            data-testid="dsf-create-cta"
          >
      ...
      {/* Sheet preview rail — grid stays mounted and visible (Req 15.2) */}
      <Sheet
        open={previewId !== null}
        onOpenChange={(open) => {
      ...
      >
        <SheetContent
          side="right"
          size="lg"
          // Prevent the sheet from covering the entire grid on wide viewports.
          className="dsf-preview-sheet"
          aria-label={previewSystem ? `Preview: ${previewSystem.title}` : 'Design system preview'}
          data-testid="dsf-preview-sheet"
        >
          <div className="dsf-preview-header">
            <SheetTitle className="dsf-preview-title">
              {previewSystem?.title ?? 'Preview'}
            </SheetTitle>
            {previewSystem?.status === 'draft' ? (
              <Badge variant="warning" size="sm">Draft</Badge>
            ) : null}
            <SheetClose asChild>
              <IconButton
                ...
                data-testid="dsf-preview-close"
                className="dsf-preview-close"
              >
          ...
          {previewSystem ? (
            <div className="dsf-preview-meta">
              {previewSystem.category ? (
                <span className="dsf-preview-category">{previewSystem.category}</span>
              ) : null}
              {previewSystem.summary ? (
                <p className="dsf-preview-summary">{previewSystem.summary}</p>
              ) : null}
              {previewSystem.swatches && previewSystem.swatches.length > 0 ? (
                <div className="dsf-preview-swatches" aria-label="Color swatches">
                  {previewSystem.swatches.map((swatch: string, idx: number) => (
                    // eslint-disable-next-line react/no-array-index-key
                    <span
                      key={idx}
                      className="dsf-preview-swatch"
                      style={{ background: swatch }}
                      aria-hidden="true"
                    />
                  ))}
          ...
          <ScrollArea className="dsf-preview-scroll" aria-label="Design system preview content">
            {previewLoading ? (
              <div className="dsf-preview-loading" aria-label="Loading preview">
                <Spinner size="md" aria-label="Loading design system preview" />
              </div>
            ) : previewHtml ? (
              <iframe
                className="dsf-preview-iframe"
                title={previewSystem ? `${previewSystem.title} showcase` : 'Design system showcase'}
                srcDoc={previewHtml}
                sandbox="allow-scripts"
                loading="lazy"
                aria-label={previewSystem ? `${previewSystem.title} showcase preview` : 'Design system showcase preview'}
              />
            ) : (
              <div className="dsf-preview-empty" aria-label="No preview available">
                <EmptyState
                  icon={<DsIcon name="ImageOff" size={24} />}
                  title="No preview available"
                  description="This design system does not have a showcase preview yet."
                />
              </div>
            )}
          </ScrollArea>
          ...
          <div className="dsf-preview-footer">
            {onSelect && previewSystem ? (
              <Button
                variant="primary"
                size="sm"
                onClick={() => {
                  if (previewSystem) onSelect(previewSystem.id);
                }}
                aria-pressed={previewSystem?.id === selectedId}
                data-testid="dsf-preview-use-btn"
              >
                {previewSystem?.id === selectedId ? (
                  <>
                    <DsIcon name="Check" size={16} />
                    Active
                  </>
            ...
            {onOpenSystem && previewSystem ? (
              <Button
                variant="secondary"
                size="sm"
                onClick={() => {
                  if (previewSystem) onOpenSystem(previewSystem.id);
                }}
                data-testid="dsf-preview-open-btn"
              >
                Open detail
                <DsIcon name="ArrowRight" size={16} />
              </Button>
