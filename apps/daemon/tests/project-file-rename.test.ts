...
describe('project file rename route', () => {
  ...
  it('renames a text file and preserves non-ASCII target names', async () => {
    ...
    // UTF-8 / CJK filename fixture; verifies the rename API preserves non-ASCII bytes.
    ...
    // UTF-8 / CJK fixture; verifies the rename API preserves non-ASCII bytes.
    expect(body.newName).toBe('需求说明.txt');
    expect(body.file.name).toBe('需求说明.txt');

    // UTF-8 / CJK fixture; verifies the raw-fetch URL round-trips encoded non-ASCII.
  ...
  it('does not overwrite a target file created during rename', async () => {
    const folder = mkdtempSync(path.join(tmpdir(), 'gd-rename-race-'));
  ...
  it('renames files in imported folders on disk', async () => {
    const folder = mkdtempSync(path.join(tmpdir(), 'gd-rename-import-'));
    tempDirs.push(folder);
  ...
  it('renames imported folder files whose existing names contain spaces', async () => {
    const folder = mkdtempSync(path.join(tmpdir(), 'gd-rename-import-spaces-'));
    tempDirs.push(folder);
  ...
  it('rejects source paths that escape through a symlinked directory', async () => {
    const folder = mkdtempSync(path.join(tmpdir(), 'gd-rename-symlink-source-'));
    const outside = mkdtempSync(path.join(tmpdir(), 'gd-rename-outside-source-'));
  ...
  it('rejects target paths that escape through a symlinked directory', async () => {
    const folder = mkdtempSync(path.join(tmpdir(), 'gd-rename-symlink-target-'));
    const outside = mkdtempSync(path.join(tmpdir(), 'gd-rename-outside-target-'));
    tempDirs.push(folder, outside);
