...
// is byte-equal to what `gd plugin install --source <https://...>`
// would download.
//
// What we put in the archive:
//   - galyarder-design.json (required; this is what the installer
//     resolves first)
...
export interface PackPluginInput {
  // Path to the plugin folder. Must contain galyarder-design.json.
  folder: string;
...
export async function packPlugin(input: PackPluginInput): Promise<PackPluginResult> {
  ...
  // Confirm the folder shape — galyarder-design.json must exist + parse.
  let manifestRaw: string;
  try {
    manifestRaw = await fsp.readFile(path.join(folder, 'galyarder-design.json'), 'utf8');
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new PackPluginError(`folder ${folder} does not contain galyarder-design.json`);
    }
    throw new PackPluginError(`failed to read galyarder-design.json: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    throw new PackPluginError(`galyarder-design.json failed to parse as JSON: ${(err as Error).message}`);
  }
