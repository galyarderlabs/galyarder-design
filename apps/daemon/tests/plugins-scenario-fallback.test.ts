...
import { resolveAppliedPipeline, type ScenarioRegistryEntry } from '@galyarder-design/plugin-runtime';

let tmpRoot: string;

beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-scenario-fallback-'));
});
...
const consumerPlugin = (gd: NonNullable<PluginManifest['gd']>): InstalledPluginRecord => {
...
describe('apply: bundled-scenario pipeline fallback (spec §23.3.3)', () => {
  it("uses the consumer's declared pipeline when present", () => {
    ...
    const out = applyPlugin({
      ...
      registry: baseRegistry([
        { id: 'gd-code-migration', taskKind: 'code-migration', pipeline: codeMigrationScenarioPipeline },
      ]),
    });
  ...
  it('falls back to the matching scenario pipeline when the consumer omits it', () => {
    ...
    const out = applyPlugin({
      ...
      inputs: {},
      registry: baseRegistry([
        { id: 'gd-new-generation',  taskKind: 'new-generation',  pipeline: newGenerationScenarioPipeline },
        { id: 'gd-code-migration',  taskKind: 'code-migration',  pipeline: codeMigrationScenarioPipeline },
      ]),
    });
...
describe('daemon scenarios collector (registry view source)', () => {
  ...
  it('reads bundled-scenario rows from installed_plugins', async () => {
    ...
    expect(resolveAppliedPipeline({
      manifest: { $schema: '', name: 'consumer', version: '0.1.0', gd: { taskKind: 'code-migration' } } as PluginManifest,
      scenarios: [{ id: 'gd-code-migration', taskKind: 'code-migration', pipeline: manifest.gd!.pipeline! }],
