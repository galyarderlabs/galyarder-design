...
// Used by:
//
//   - `gd plugin stats` — operator at-a-glance inventory check,
//   - the desktop / web 'Plugins' settings panel (via the JSON
//     mode of the same CLI),
//   - the `gd doctor` summary (a future patch can fold this in
...
import type { InstalledPluginRecord, PluginSourceKind } from '@galyarder-design/contracts';
...
export function pluginInventoryStats(plugins: ReadonlyArray<InstalledPluginRecord>): PluginInventoryStats {
  ...
  for (const plugin of plugins) {
    ...
    const taskKind = plugin.manifest.gd?.taskKind ?? 'unknown';
    stats.byTaskKind[taskKind] = (stats.byTaskKind[taskKind] ?? 0) + 1;

    const declared = plugin.manifest.gd?.capabilities ?? [];
...
// `gd plugin sources` CLI; lives next to the other stats
