...
//
//   (a) Apply path: resolve `gd.connectors.required[]` against the
...
// Resolve `gd.connectors.required[] + optional[]` into apply-time bindings.
...
export function resolveConnectorBindings(
  ...
  const required: PluginConnectorRef[] = [
    ...(manifest.gd?.connectors?.required ?? []).map((r) => ({ ...r, required: true })),
    ...(manifest.gd?.connectors?.optional ?? []).map((r) => ({ ...r, required: false })),
  ];
...
// Validate `gd.connectors.required[].tools[]` against the catalog.
// Returns issues grouped by connector. Used by `gd plugin doctor` (F7).
export function validateConnectorRefs(
  ...
  const all = [
    ...(manifest.gd?.connectors?.required ?? []),
    ...(manifest.gd?.connectors?.optional ?? []),
  ];
  const required = manifest.gd?.connectors?.required ?? [];
  const declaredCaps = new Set(manifest.gd?.capabilities ?? []);
