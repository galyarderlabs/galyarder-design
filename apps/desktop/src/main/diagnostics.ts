...
import {
  ...
  diagnosticsFileName,
  type LogSource,
} from "@galyarder-design/diagnostics";
...
export async function exportDiagnosticsToFile(
  ...
  const dialogOptions = {
    title: "Export Galyarder Design diagnostics",
    defaultPath,
  ...
  try {
    const result = await buildDiagnosticsZip({
      context: {
        app: { name: "galyarder-design", version: app.getVersion(), packaged: app.isPackaged },
        source: "desktop-ipc",
        namespace: runtime.namespace,
        extra: {
      ...
      crashReports: {
        // Restrict to Galyarder Design's own process names. A generic "Electron"
        ...
        // the support bundle.
        matchSubstrings: ["Galyarder Design", "galyarder-design"],
        withinDays: 7,
        maxReports: 10,
        homeDir: homedir(),
      },
