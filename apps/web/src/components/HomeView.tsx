...
import { DEFAULT_UNSELECTED_SCENARIO_PLUGIN_ID } from '@galyarder-design/contracts';
import { projectKindToTracking } from '@galyarder-design/contracts/analytics';
...
const AUTHORING_DEFAULT_SCENARIO_INPUTS = {
  artifactKind: 'Galyarder Design plugin',
  audience: 'Galyarder Design plugin authors',
  topic: 'packaging a reusable workflow as an Galyarder Design plugin',
};
...
// Legacy CJK title used by older versions of the picker; kept verbatim
// as a detection pattern so projects saved with the previous Chinese
// label still migrate to the new auto-design-system option id.
...
export function HomeView({
  ...
  useEffect(() => {
    ...
    load();
    window.addEventListener('galyarder-design:plugins-changed', load);
    return () => {
      cancelled = true;
      window.removeEventListener('galyarder-design:plugins-changed', load);
    };
  }, []);
  ...
  // chips share gd-new-generation, so surfacing the raw plugin title
  ...
  async function usePlugin(
    ...
    const inputFields = options?.inputFields ?? record.manifest?.gd?.inputs ?? [];
    ...
    const queryTemplate =
      options?.queryTemplate !== undefined
        ...
        ? null
        : resolvePluginQueryFallback(record.manifest?.gd?.useCase?.query, locale) || null;
    ...
    if (!suppressPromptUpdate && (nextPrompt === undefined || nextPrompt === null)) {
      const reconciledQuery =
        options?.queryTemplate !== undefined
          ? options.queryTemplate
          : result.query || resolvePluginQueryFallback(record.manifest?.gd?.useCase?.query, locale);
      if (reconciledQuery) {
  ...
  function requestActivePlugin(
    ...
    const replacement = previewPluginReplacement(record, nextPrompt, {
      inputs: withHomeDesignSystemDefault(options?.inputs, options?.inputFields ?? record.manifest?.gd?.inputs ?? [], designSystemOptions),
  ...
  function previewPluginReplacement(
    ...
    const query =
      options?.queryTemplate !== undefined
        ? options.queryTemplate
        : resolvePluginQueryFallback(record.manifest?.gd?.useCase?.query, locale);
    if (!query) return null;
    const fields = options?.inputFields ?? record.manifest?.gd?.inputs ?? [];
  ...
  function renderPluginContextPrompt(
    ...
    const query = resolvePluginQueryFallback(record.manifest?.gd?.useCase?.query, locale);
    if (!query) return null;
    return renderPluginBriefTemplate(
      query,
      hydratePluginInputs(record.manifest?.gd?.inputs ?? [], inputs),
    );
  }
  ...
  function queuePluginAuthoring(chipId: string | null, goal?: string) {
    ...
    runWithReplacementConfirmation('Plugin authoring', nextPrompt, async () => {
      ...
      before: active?.record.id ?? null,
      after: 'gd-plugin-authoring',
    });
  }

  useEffect(() => {
    if (!pendingAuthoringChipId || pluginsLoading) return;
    const authoringRecord = plugins.find((plugin) => plugin.id === 'gd-plugin-authoring');
    const record = authoringRecord ?? plugins.find((plugin) => plugin.id === 'gd-new-generation');
...
const AUTO_DESIGN_SYSTEM_STOP_WORDS = new Set([
  ...
  'system',
  'style',
  // CJK stop words. Functional patterns matched against user briefs in
  // Chinese; kept verbatim so the auto-design-system picker filters
  // common filler words ("a", "this", "use", "generate", "design",
  // "page", "site", "app") in non-English briefs as well.
  '一个',
  '这个',
  '使用',
  '生成',
...
function estimatePluginContextItemCount(
  ...
  const context = record.manifest?.gd?.context;
  if (!context) return 0;
