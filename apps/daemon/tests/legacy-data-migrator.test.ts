/**
 * Unit tests for the GD_LEGACY_DATA_DIR one-shot migrator. Hermetic:
 * each test runs against a fresh pair of mkdtemp() directories so no
 * test ever touches a real GD install.
 *
 * The migrator's contract is "loud or correct": when GD_LEGACY_DATA_DIR
...
function seedLegacyDir(legacyDir: string): void {
  // A typical 0.3.x .gd/ payload: SQLite, a project's CWD, an artifact,
...
describe('migrateLegacyDataDirSync', () => {
  ...
  beforeEach(() => {
    legacyDir = mkdtempSync(path.join(os.tmpdir(), 'gd-legacy-'));
    dataDir = mkdtempSync(path.join(os.tmpdir(), 'gd-data-'));
  });
  ...
  it('returns noop when GD_LEGACY_DATA_DIR is not set', () => {
  ...
  it('returns noop when GD_LEGACY_DATA_DIR is the empty string', () => {
    expect(
  ...
  it('marker beats a missing legacyDir on the next boot', async () => {
    ...
    // GD_LEGACY_DATA_DIR set after success and then deletes or moves
    // the old `.gd/` (which is the documented launchctl setenv path),
    ...
    // Simulate the user removing the old repo `.gd/` after the
    // successful migration but leaving GD_LEGACY_DATA_DIR set.
    await rm(legacyDir, { recursive: true, force: true });
  ...
  it('refuses to migrate when a symlink is anywhere inside the legacy payload', () => {
    ...
    // data root via a crafted .gd/ tree.
    seedLegacyDir(legacyDir);

    const escapeTarget = mkdtempSync(path.join(os.tmpdir(), 'gd-escape-'));
  ...
  it('cleans up the staging dir on staging failure (no half-copied state in dataDir)', () => {
    ...
    const escapeTarget = mkdtempSync(path.join(os.tmpdir(), 'gd-stage-cleanup-'));
    ...
    // No .gd-migrate-* sibling left behind.
    const parent = path.dirname(dataDir);
    const base = path.basename(dataDir);
    const leftovers = fs
      .readdirSync(parent)
      .filter((entry) => entry.startsWith(`${base}.gd-migrate-`));
  ...
  it('rolls back already-promoted entries when promotion fails mid-loop', () => {
    ...
    const stagingDir = path.join(
      ...
      `${path.basename(dataDir)}.gd-migrate-rollback-test`,
    );
...
describe('dataDirIsEmptyOrFresh', () => {
  let dataDir: string;
  beforeEach(() => {
    dataDir = mkdtempSync(path.join(os.tmpdir(), 'gd-fresh-'));
  });
  afterEach(async () => {
    await rm(dataDir, { recursive: true, force: true });
  });
...
describe('legacyDirHasPayload', () => {
  let legacyDir: string;
  beforeEach(() => {
    legacyDir = mkdtempSync(path.join(os.tmpdir(), 'gd-legacy-prove-'));
  });
  afterEach(async () => {
    await rm(legacyDir, { recursive: true, force: true });
  });
...
describe('dataDirHasExistingPayload', () => {
  let dataDir: string;
  beforeEach(() => {
    dataDir = mkdtempSync(path.join(os.tmpdir(), 'gd-payload-probe-'));
  });
  afterEach(async () => {
    await rm(dataDir, { recursive: true, force: true });
  });
