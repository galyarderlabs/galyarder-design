...
const GD_SCHEME = "gd";
const GD_ENTRY_URL = `${GD_SCHEME}://app/`;
...
function buildProxyErrorResponse(error: unknown, target: string): Response {
  ...
  return new Response(
    JSON.stringify({
      error: "GD_PROTOCOL_PROXY_FAILED",
      message,
      ...(code === null ? {} : { code }),
      target,
    }),
...
/**
 * Inner request handler for the `gd://` Electron protocol — every
...
export function registerOdProtocol(webRuntimeUrl: string): void {
  protocol.handle(GD_SCHEME, async (request) => {
