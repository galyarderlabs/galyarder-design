...
// disabled when `GD_SNAPSHOT_GC_INTERVAL_MS` is set to `0`.
//
// Operator escape hatch: `gd plugin snapshots prune --before <ts>` calls
...
export function startSnapshotGc(opts: SnapshotGcOptions): SnapshotGcHandle {
  ...
  const tick = () => {
    try {
      // Plan §3.M1 / spec PB2 — feed GD_SNAPSHOT_RETENTION_DAYS into
