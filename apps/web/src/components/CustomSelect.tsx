...
export function CustomSelect({
  ...
  const menu = (
    ...
    >
      {options.map((item) => {
        if (isGroup(item)) {
          return (
            <div className="gd-select-group" key={`group:${item.label}`}>
              <div className="gd-select-group-label">{item.label}</div>
  ...
  return (
    <div className={['gd-select', className].filter(Boolean).join(' ')}>
      <button
        ref={buttonRef}
        type="button"
        className={['gd-select-trigger', triggerClassName].filter(Boolean).join(' ')}
        role="combobox"
        value={value}
        ...
        onFocus={onFocus}
      >
        <span id={`${idBase}-value`} className="gd-select-value">
          {selectedLabel}
        </span>
...
function SelectOptionButton({
  ...
  return (
    ...
    >
      <span className="gd-select-option-label">{option.label}</span>
      <span className="gd-select-option-check" aria-hidden>
