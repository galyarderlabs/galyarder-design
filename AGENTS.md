...
- `apps/daemon` is the local privileged daemon and `gd` bin. It owns `/api/*`, agent spawning, skills, design systems, artifacts, and static serving.
...
- `packages/sidecar-proto` owns the Galyarder Design sidecar business protocol; `packages/sidecar` owns the generic sidecar runtime; `packages/platform` owns generic OS process primitives.
...
- `tools-dev` exports `GD_PORT` for the web proxy target and `GD_WEB_PORT` for the web listener; do not use `NEXT_PORT`.
...
- Public packaged app identity must stay channel-distinct: stable uses `Galyarder Design`, beta uses `Galyarder Design Beta`, and preview uses `Galyarder Design Preview`. Do not ship beta or preview mac DMGs whose drag-install app bundle is `Galyarder Design.app`.
- Windows beta updater validation must use the real beta namespace `release-beta-win`; otherwise a local beta-like namespace can create a separate uninstall registry key while looking like the same `Galyarder Design Beta` app. See `tools/pack/AGENTS.md` for the architecture map and high-confidence acceptance harness.
...
- Default runtime files live under `<project-root>/.tmp/<source>/<namespace>/...`; POSIX IPC sockets are fixed at `/tmp/galyarder-design/ipc/<namespace>/<app>.sock`.

## Capability exposure (UI/CLI dual-track)

Every user-facing capability must be reachable through both the web UI **and** the `gd` CLI (`apps/daemon/src/cli.ts`). Shipping a feature with only one of the two surfaces is a regression.

- The CLI is the embeddability contract. External agents (hermes-agent, openclaw, custom Slack/Discord bots, packaged runtimes invoked from another shell) drive Galyarder Design through `gd` subcommands — they do not render the web UI. If a capability is UI-only, it cannot be composed into those external agents.
...
- Existing reference points: `gd automation …` mirrors the Automations tab against `/api/routines`; `gd plugin …`, `gd ui …`, `gd project …`, `gd media …`, `gd mcp …`, `gd research …` follow the same shape. Copy that pattern for new capabilities.
...
- `apps/web/src/i18n/types.ts` is the typed `Dict`; every key must be defined in all 17 locale files under `apps/web/src/i18n/locales/*.ts` (`ar`, `de`, `en`, `es-ES`, `fa`, `fr`, `hu`, `id`, `it`, `ja`, `ko`, `pl`, `pt-BR`, `ru`, `th`, `tr`, `uk`). Add the key to `types.ts` first; missing translations produce a typecheck error.
...
pnpm tools-dev inspect desktop screenshot --path /tmp/galyarder-design.png
pnpm tools-dev stop
...
```bash
pnpm --filter @galyarder-design/web typecheck
pnpm --filter @galyarder-design/web test
pnpm --filter @galyarder-design/web build
pnpm --filter @galyarder-design/daemon test
pnpm --filter @galyarder-design/daemon build
pnpm --filter @galyarder-design/desktop build
pnpm --filter @galyarder-design/tools-dev build
pnpm --filter @galyarder-design/tools-pack build
pnpm --filter @galyarder-design/tools-pr build
pnpm --filter @galyarder-design/tools-serve build
```
...
`@galyarder-design/sidecar-proto` owns Galyarder Design app/mode/source constants, namespace validation, stamp fields/flags, IPC message schema, status shapes, and error semantics. `@galyarder-design/sidecar` provides only generic bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime files. `@galyarder-design/platform` provides only generic OS process stamp serialization, command parsing, and process matching/search primitives, consuming the proto descriptor.
...
1. `GD_DATA_DIR=<dir>` — relocates *all* daemon runtime data to `<dir>` (used by Playwright for test isolation, and by the packaged daemon and the Home Manager / NixOS modules to point the daemon at a writable directory when the install root is read-only). The path is resolved with `~/` expansion and relative paths anchored to `<projectRoot>`.
2. `GD_MEDIA_CONFIG_DIR=<dir>` — narrower override that relocates *only* `media-config.json`. Same resolution semantics. Most installs do not need this; it exists for setups that want to keep API credentials in a different location from the rest of the runtime data.

Default precedence is GD_MEDIA_CONFIG_DIR > GD_DATA_DIR > `<projectRoot>/.od`.
