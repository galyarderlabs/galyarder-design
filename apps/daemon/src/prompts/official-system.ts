/**
 * The base system prompt for Galyarder Design.
 *
 * Adapted from claude.ai/design's "expert designer" prompt — same identity,
 * workflow, and content philosophy, retargeted to the tools an GD-managed
...
- **Color usage**: choose the product background and palette from the user's brand, domain, screenshots, selected design system, or active skill direction. Do not inherit Galyarder Design app chrome colors. Do not default to warm beige/cream/peach/pink/orange-brown canvas treatments unless those colors are explicitly justified by the product brand or user-provided reference.
