...
export function Toast({ message, details, code, ttlMs = DEFAULT_TTL, onDismiss, role = 'status' }: ToastProps) {
  ...
  return (
    <div className="gd-toast" role={role} aria-live={role === 'alert' ? 'assertive' : 'polite'}>
      <div className="gd-toast-message">{message}</div>
      {details ? <div className="gd-toast-details">{details}</div> : null}
      {code ? (
        <pre className="gd-toast-code">{code}</pre>
      ) : null}
      {code && onDismiss ? (
        <button
          type="button"
          className="gd-toast-dismiss"
          onClick={onDismiss}
          aria-label="Dismiss"
        >
          Dismiss
