...
import type { GalyarderDesignHostActionResult, GalyarderDesignHostUpdaterActionOptions } from "@galyarder-design/host";
...
/**
 * Pure-function HMAC mint for the `X-GD-Desktop-Import-Token` header.
...
export type DesktopRuntimeOptions = {
   ...
   * renderer from `gd://app/`, which only resolves through Electron's
...
const DESKTOP_IMPORT_TOKEN_HEADER = "X-GD-Desktop-Import-Token";
...
export type PickAndImportFolderDeps = {
   ...
   * renderer-only `gd://app/` webUrl. Renamed from `webUrl` to make the
...
export function isAllowedChildWindowUrl(url: string): boolean {
  try {
    ...
    // `gd:` here lets Electron open the link in a child BrowserWindow
...
function osLocaleAdditionalArguments(osLocale: string | undefined): string[] | undefined {
  return osLocale ? [`--gd-os-locale=${encodeURIComponent(osLocale)}`] : undefined;
}
...
function checkOptionsFromHost(options: unknown): { autoDownload?: boolean } | undefined {
  const input = options as GalyarderDesignHostUpdaterActionOptions | null | undefined;
...
async function reportRendererCrash(
  ...
  try {
    ...
    // mandatory because the renderer-only `gd://app/` scheme isn't
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  ipcMain.handle(
    ...
    async (_event, init?: { name?: string; skillId?: string | null; designSystemId?: string | null }) => {
      ...
      // (`gd://app/`) over `discoverUrl`, but Node-side fetch can't
  ...
  ipcMain.handle("shell:open-path", async (_event, projectId: string) => {
    // Round-7 (lefarcen P2): same packaged gd:// → daemon URL pivot as
  ...
  const window = new BrowserWindow({
    ...
    // At widths above --breakpoint-md (768px) the secondary NavRail
    // stays expanded; at or below it collapses with no horizontal
    // clipping. 1024×720 is the minimum that keeps both panes ≥320px
    // and the NavRail in its expanded state (Req 33.1–33.4).
    minHeight: 720,
    minWidth: 1024,
    show: true,
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("updater IPC is only available to the main Galyarder Design window");
    }
  };
  ipcMain.handle("gd:update:status", async (event) => {
    requireMainWindowSender(event);
    const status = await (options.updater?.status() ?? unavailableUpdaterStatus());
    sendUpdaterStatus(status);
    return status;
  });
  ipcMain.handle("gd:update:check", async (event, updaterOptions: unknown) => {
    requireMainWindowSender(event);
    const status = await (options.updater?.checkForUpdates(checkOptionsFromHost(updaterOptions)) ?? unavailableUpdaterStatus());
    sendUpdaterStatus(status);
    return status;
  });
  ipcMain.handle("gd:update:download", async (event) => {
    requireMainWindowSender(event);
    const status = await (options.updater?.downloadUpdate() ?? unavailableUpdaterStatus());
    sendUpdaterStatus(status);
    return status;
  });
  ipcMain.handle("gd:update:install", async (event) => {
    requireMainWindowSender(event);
    const status = await (options.updater?.installUpdate() ?? unavailableUpdaterStatus());
    sendUpdaterStatus(status);
    return status;
  });
  ipcMain.handle("gd:update:quit", async (event): Promise<GalyarderDesignHostActionResult> => {
    requireMainWindowSender(event);
    const status = await (options.updater?.status() ?? unavailableUpdaterStatus());
  ...
  ipcMain.removeHandler('gd:print-pdf');
  ipcMain.handle('gd:print-pdf', async (_event, html: unknown, nonce: unknown, options: unknown): Promise<void> => {
