...
beforeEach(async () => {
  tmp = await mkdtemp(path.join(os.tmpdir(), 'gd-storage-'));
});
...
describe('S3ProjectStorage', () => {
  ...
  it('PUT signs the request + reports back ProjectFileMeta on success', async () => {
    ...
    const storage = new S3ProjectStorage({
      bucket: 'gd-bucket', region: 'us-east-1', credentials,
    ...
    expect(seen[0]?.url).toBe('https://gd-bucket.s3.us-east-1.amazonaws.com/p1/hello.txt');
  ...
  it('GET returns the body bytes as a Buffer', async () => {
    ...
    const storage = new S3ProjectStorage({
      bucket: 'gd-bucket', region: 'us-east-1', credentials, fetchFn, now: fixedNow,
    });
    const buf = await storage.readFile('p1', 'hello.txt');
  ...
  it('GET 404 surfaces StorageError with code=NOT_FOUND', async () => {
    ...
    const storage = new S3ProjectStorage({
      bucket: 'gd-bucket', region: 'us-east-1', credentials, fetchFn,
    });
...
describe('resolveProjectStorage', () => {
  ...
  it('returns S3ProjectStorage when GD_PROJECT_STORAGE=s3', () => {
    const storage = resolveProjectStorage({
      projectsRoot: tmp,
      env: {
        GD_PROJECT_STORAGE:        's3',
        GD_S3_BUCKET:              'my-bucket',
        GD_S3_REGION:              'us-east-1',
        GD_S3_PREFIX:              'tenant',
        GD_S3_ENDPOINT:            'https://oss.aliyuncs.com',
        GD_S3_ACCESS_KEY_ID:       'AKIA-FIXTURE',
        GD_S3_SECRET_ACCESS_KEY:   'shhh',
      },
    });
...
describe('resolveDaemonDbConfig', () => {
  ...
  it('parses postgres env vars when GD_DAEMON_DB=postgres', () => {
    const cfg = resolveDaemonDbConfig({
      GD_DAEMON_DB: 'postgres',
      GD_PG_HOST:   'pg.local',
      GD_PG_PORT:   '6543',
      GD_PG_DATABASE: 'galyarder_design',
      GD_PG_USER:   'gd',
      GD_PG_SSL_MODE: 'disable',
    });
    expect(cfg.kind).toBe('postgres');
    expect(cfg.postgres).toEqual({
      host:     'pg.local',
      port:     6543,
      database: 'galyarder_design',
      user:     'gd',
      sslMode:  'disable',
    });
  });
  ...
  it('throws when postgres env vars are incomplete', () => {
    expect(() =>
      resolveDaemonDbConfig({ GD_DAEMON_DB: 'postgres', GD_PG_HOST: 'pg.local' }),
    ).toThrow(DaemonDbConfigError);
  });

  it('throws on an unknown GD_DAEMON_DB value', () => {
    expect(() => resolveDaemonDbConfig({ GD_DAEMON_DB: 'mongo' })).toThrow(DaemonDbConfigError);
  });
});
