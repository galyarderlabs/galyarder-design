...
async function runElectronBuilderRaw(config: ToolPackConfig, paths: WinPaths, projectDir: string): Promise<void> {
  ...
  const builderConfig = {
    ...
    publish: [{ provider: "generic", url: "https://updates.invalid/galyarder-design" }],
    win: {
...
export async function materializeCachedUnpackedForInstaller(
  ...
  await cp(paths.packagedConfigPath, join(paths.unpackedRoot, "resources", "galyarder-design-config.json"));
