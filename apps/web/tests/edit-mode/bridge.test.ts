...
describe('manual edit bridge target normalization', () => {
  it('prefers explicit data-gd-id over generated ids', () => {
    const dom = new JSDOM('<main><h1 data-gd-id="hero">Title</h1></main>');
    const target = dom.window.document.querySelector('h1')!;

    expect(manualEditStableIdForElement(target)).toBe('hero');
    expect(target.getAttribute('data-gd-runtime-id')).toBeNull();
  });
  ...
  it('generates stable DOM path ids for unannotated elements', () => {
    ...
    expect(manualEditStableIdForElement(target)).toBe('path-0-0-1');
    expect(target.getAttribute('data-gd-runtime-id')).toBe('path-0-0-1');
  });

  it('generates DOM path ids against source-shaped children, ignoring host shim nodes', () => {
    const dom = new JSDOM(
      '<script data-gd-sandbox-shim></script><main><section><p>First</p><p>Second</p></section></main><script data-gd-edit-bridge></script>',
    );
    const target = dom.window.document.querySelectorAll('p')[1]!;

    expect(isManualEditHostNode(dom.window.document.querySelector('[data-gd-sandbox-shim]')!)).toBe(true);
    expect(manualEditDomPathForElement(target)).toBe('path-0-0-1');
  });

  it('discovers meaningful elements and ignores tiny or irrelevant elements', () => {
    const dom = new JSDOM('<main><h1 data-gd-source-path="path-0-0">Title</h1><script>1</script></main>');
  ...
  it('does not expose path targets unless they carry a source path marker', () => {
    const dom = new JSDOM('<main><h1>Runtime title</h1><p data-gd-source-path="path-0-1">Source text</p></main>');
  ...
  it('acks live preview style patches by id and version', () => {
    ...
    expect(bridge).toContain("type: 'gd-edit-preview-style-applied'");
  ...
  it('moves the runtime selected marker between selected targets', () => {
    const dom = new JSDOM(
      `<main>
        <h1 data-gd-id="title">Title</h1>
        <p data-gd-id="body">Body</p>
      </main>${buildManualEditBridge(true)}`,
      { runScripts: 'dangerously', url: 'http://localhost' },
    );
    const title = dom.window.document.querySelector('[data-gd-id="title"]')!;
    const body = dom.window.document.querySelector('[data-gd-id="body"]')!;

    dom.window.dispatchEvent(new dom.window.MessageEvent('message', {
      data: { type: 'gd-edit-selected-target', id: 'title' },
    }));
    expect(title.getAttribute('data-gd-edit-selected')).toBe('true');
    expect(body.hasAttribute('data-gd-edit-selected')).toBe(false);

    dom.window.dispatchEvent(new dom.window.MessageEvent('message', {
      data: { type: 'gd-edit-selected-target', id: 'body' },
    }));
    expect(title.hasAttribute('data-gd-edit-selected')).toBe(false);
    expect(body.getAttribute('data-gd-edit-selected')).toBe('true');
  ...
  it('clears runtime selected markers for null selection and edit-mode exit', () => {
    const dom = new JSDOM(
      `<main>
        <h1 data-gd-id="title">Title</h1>
        <p data-gd-id="body" data-gd-edit-selected="true">Body</p>
      </main>${buildManualEditBridge(true)}`,
      { runScripts: 'dangerously', url: 'http://localhost' },
    );
    const body = dom.window.document.querySelector('[data-gd-id="body"]')!;
    ...
    dom.window.dispatchEvent(new dom.window.MessageEvent('message', {
      data: { type: 'gd-edit-selected-target', id: null },
    }));
    expect(body.hasAttribute('data-gd-edit-selected')).toBe(false);
    ...
    dom.window.dispatchEvent(new dom.window.MessageEvent('message', {
      data: { type: 'gd-edit-mode', enabled: false },
    }));
    expect(body.hasAttribute('data-gd-edit-selected')).toBe(false);
  ...
  it('keeps runtime selection marker out of source-shaped target data', () => {
    ...
    expect(bridge).toContain("attr.name === 'data-gd-edit-selected'");
    expect(bridge).toContain('replace(/\\sdata-gd-edit-selected="[^"]*"/g, \'\')');
    expect(bridge).toContain('[data-gd-edit-selected]');
  });
