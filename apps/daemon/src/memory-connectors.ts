...
// Functional CJK keyword set: these phrases ("design thinking",
// "design preferences", "visual style", "brand logo", "design
// system", etc., in Chinese) are query strings used against connector
// search APIs (Notion, Linear, etc.). They live in the user's source
// language so we surface notes they actually wrote — translating them
// would defeat the keyword match.
const DEFAULT_CONNECTOR_MEMORY_QUERY =
  '设计思路 设计偏好 UI UX 视觉风格 品牌 logo 设计系统 GalyarderDesign';
const CONNECTOR_MEMORY_QUERY_VARIANTS = [
  // Chinese keyword variants (kept verbatim — see comment above):
  // 设计思路 = design approach, 设计偏好 = design preference,
  // 视觉风格 = visual style, 品牌 logo = brand logo,
  // 黑色 logo 深色主题 = black logo / dark theme,
  // 设计系统 = design system, 组件 布局 交互 = components / layout / interaction,
  // 用户路径 工作流 = user paths / workflow, 产品设计 = product design.
  '设计思路',
  '设计偏好',
  'UI UX',
  '视觉风格',
...
const CONNECTOR_MEMORY_SYSTEM_PROMPT = `You are a design-memory extractor for GalyarderDesign connected-app context.

You will receive compacted read-only data from apps such as Notion, Figma, Linear, Google Drive, GitHub, and Slack. Propose only durable memories that would improve future GalyarderDesign design work.
...
- It would change how GalyarderDesign creates, critiques, edits, or explains future design artifacts.
...
- Meta language about extraction or saving, including phrases like "GalyarderDesign read X", "Found N readable items", "Summary from X", "context summary", or "Save this if it should be reused".
...
- feedback: preferences about how GalyarderDesign should work or answer
...
const CONNECTOR_DESIGN_MEMORY_PATTERNS = [
  ...
  /\bhandoff\b/i,
  /\bdesign system\b/i,
  // Chinese-language equivalents of the design vocabulary above
  // (设计 = design, 界面 = interface, 视觉 = visual, 品牌 = brand,
  // 字体 = typography, 颜色/配色 = colour/palette, 布局 = layout,
  // 组件 = component, 交互 = interaction, 原型 = prototype,
  // 画布 = canvas, 素材 = asset, 图标 = icon, 页面 = page,
  // 网站 = website, 信息密度 = information density,
  // 深色 = dark, 主题 = theme, 用户路径 = user path,
  // 工作流 = workflow, 风格 = style, 参考 = reference,
  // 设计系统 = design system). Kept literal so connector content
  // written in Chinese still passes the design-relevance gate.
  /设计/,
  /界面/,
  /视觉/,
  /品牌/,
...
export async function suggestMemoryFromConnectors(
  ...
  const drafts = await suggestWithLLM(
    dataDir,
    {
      userMessage: `Suggest durable GalyarderDesign memories from connected apps. Search hint: ${options.query?.trim().slice(0, 240) || DEFAULT_CONNECTOR_MEMORY_QUERY}`,
...
export async function extractMemoryFromConnectors(
  ...
  const changed = await extractWithLLM(
    dataDir,
    {
      userMessage: `Extract durable GalyarderDesign memory from connected apps. Search hint: ${options.query?.trim().slice(0, 240) || DEFAULT_CONNECTOR_MEMORY_QUERY}`,
      assistantMessage: context.connectorContext,
    },
    {
      projectRoot: options.projectRoot,
