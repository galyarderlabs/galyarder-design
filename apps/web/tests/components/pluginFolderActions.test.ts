...
// `galyarderlabs/galyarder-design`; `publish` drives `gh repo create / push` against the
...
describe('buildPluginFolderAgentActionPrompt', () => {
  describe('install', () => {
    it('mentions the folder path and the supported install CLI', () => {
      ...
      expect(prompt).toContain('gd plugin install --source');
    });
  });
  ...
  describe('publish (repo-publish flow)', () => {
    ...
    it('delegates repo publishing to the deterministic plugin CLI helper', () => {
      expect(prompt).toContain(`Plugin folder: \`${FOLDER}\``);
      expect(prompt).toContain(`"$GD_NODE_BIN" "$GD_BIN" plugin publish-repo ${FOLDER}`);
    ...
    it('bans the registry-submission CLI explicitly', () => {
      ...
      expect(prompt).toMatch(
        /Do NOT (call|route through) `?gd plugin publish --to galyarder-design`?/i,
      );
      expect(prompt).toMatch(
        /registry[- ]submission|registry-submission flow|Galyarder Design PR/i,
      );
    });
  ...
  describe('contribute (PR-based flow)', () => {
    ...
    it('delegates Galyarder Design PR creation to the deterministic plugin CLI helper', () => {
      expect(prompt).toContain('galyarderlabs/galyarder-design');
      expect(prompt).toContain(`"$GD_NODE_BIN" "$GD_BIN" plugin galyarder-design-pr ${FOLDER}`);
    });
    ...
    it('states the CLI-owned PR workflow instead of re-listing shell steps', () => {
      ...
      // ban ("Do NOT call the legacy `gd plugin publish --to galyarder-design`")
      // — verify the ban is in place, not the bare command.
      expect(prompt).toMatch(/do not call the legacy `gd plugin publish --to galyarder-design`/i);
    });
    ...
    it('interpolates the actual folder path into manifest and copy steps', () => {
      ...
      expect(prompt).toContain(`plugin galyarder-design-pr ${FOLDER}`);
      expect(prompt).not.toContain('${folderPath}');
    });
