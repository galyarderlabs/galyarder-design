...
beforeEach(async () => {
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'gd-validate-'));
...
async function writeManifest(body: Record<string, unknown>) {
  await writeFile(path.join(folder, 'galyarder-design.json'), JSON.stringify(body, null, 2));
}
...
describe('validatePluginFolder', () => {
  ...
  it('rejects malformed galyarder-design.json with a parse error', async () => {
    await writeFile(path.join(folder, 'galyarder-design.json'), '{ this is not json');
    const result = await validatePluginFolder({ folder });
    expect(result.ok).toBe(false);
    expect(result.resolveErrors.some((e) => e.includes('galyarder-design.json'))).toBe(true);
  });

  it('flags an unknown atom id in gd.pipeline', async () => {
    await writeManifest({
      name: 'pipe',
      version: '0.1.0',
      gd: {
