...
<html>
  ...
  <body>
    <main>
      <h1 data-gd-id="hero-title">Original title</h1>
      <a data-gd-id="cta" href="/start">Start</a>
      <button data-gd-id="button-cta">Start button</button>
      <a data-gd-id="nested-cta" href="/nested"><span>Buy now</span><svg viewBox="0 0 1 1"></svg></a>
      <img data-gd-id="hero-image" src="/old.png" alt="Old image">
      <section data-gd-id="card" class="hero" style="color: red; padding: 8px;" data-keep="yes">Card</section>
      <p data-gd-id="nested"><strong>Nested</strong> copy</p>
...
describe('manual edit source patches', () => {
  ...
  it('applies attributes additively and preserves class/style unless explicitly updated', () => {
    const result = applyManualEditPatch(baseSource, {
      kind: 'set-attributes',
      id: 'card',
      attributes: { 'aria-label': 'Hero card', 'data-empty': '', 'data-gd-id': 'blocked' },
    });
    ...
    expect(attrs['data-gd-id']).toBe('card');
    expect(attrs['data-empty']).toBeUndefined();
  });

  it('preserves data-gd-id when selected outerHTML omits it', () => {
    const result = applyManualEditPatch(baseSource, {
      kind: 'set-outer-html',
      id: 'card',
    ...
    expect(html).toContain('data-gd-id="card"');
    expect(html).toContain('class="replacement"');
  });

  it('replaces full source for snapshot-based undo history', () => {
    const source = '<!doctype html><html><body><h1 data-gd-id="hero-title">Snapshot</h1></body></html>';
  ...
  it('preserves fragment-shaped HTML when saving patches', () => {
    const source = '<main><h1 data-gd-id="hero-title">Original title</h1></main>';
    const result = applyManualEditPatch(source, { kind: 'set-text', id: 'hero-title', value: 'Edited title' });

    expect(result.ok).toBe(true);
    expect(result.source).toBe('<main><h1 data-gd-id="hero-title">Edited title</h1></main>');
  ...
  it('preserves full documents with leading comments when saving patches', () => {
    const source = [
      '<!-- generated by galyarder design -->',
      '<!doctype html><html><head><style>:root { --brand: #111; }</style></head>',
      '<body><main><h1 data-gd-id="hero-title">Original title</h1></main></body></html>',
    ].join('\n');
    const result = applyManualEditPatch(source, { kind: 'set-text', id: 'hero-title', value: 'Edited title' });
    ...
    expect(result.source).toContain('<h1 data-gd-id="hero-title">Edited title</h1>');
  });
