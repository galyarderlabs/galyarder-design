...
// identifier is empty (e.g. all-non-ASCII identifiers like "测试"
// (Chinese for "test") strip to nothing). Without matching against
// this, sibling discovery would miss the entire "artifact*.html"
// family that such identifiers produce.
...
// (e.g. "测试" vs "首页" — Chinese for "test" vs "homepage", which both
// slugify to empty) is also not treated as a match for the same
...
export async function findPriorArtifactSiblings(
  ...
  for (const entry of entries) {
    ...
    // where 测试 and 首页 — Chinese for "test" and "homepage" — both
    // land in artifact*.html) would falsely
...
export function readArtifactStubGuardConfigFromEnv(env: NodeJS.ProcessEnv = process.env): ArtifactStubGuardConfig {
  const rawMode = (env.GD_ARTIFACT_STUB_GUARD ?? '').toLowerCase();
  const mode: ArtifactStubGuardMode =
    rawMode === 'reject' || rawMode === 'warn' || rawMode === 'off'
      ? rawMode
      : DEFAULT_ARTIFACT_STUB_GUARD_CONFIG.mode;

  const ratioRaw = Number(env.GD_ARTIFACT_STUB_GUARD_MIN_RATIO);
  ...
  const minPriorBytesRaw = Number(env.GD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES);
  const minPriorBytes =
...
function buildWarning(
  ...
  return {
    ...
    message:
      ...
      'Set GD_ARTIFACT_STUB_GUARD=warn to record the warning without rejecting, or =off to disable the guard entirely.',
    identifier,
    newSize,
