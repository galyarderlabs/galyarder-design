 ...
 * `@galyarder-design/desktop/main`. The helpers are part of the security
 ...
 *
 * @see https://github.com/galyarderlabs/galyarder-design/issues/911
 */
...
} from '@galyarder-design/desktop/main';
...
describe('isHttpUrl', () => {
  ...
  it('rejects non-http schemes', () => {
    expect(isHttpUrl('gd://app/foo')).toBe(false);
...
describe('isAllowedChildWindowUrl (issue #911)', () => {
  it('allows the packaged gd:// scheme so live artifact previews open in a child BrowserWindow', () => {
    ...
    // In packaged builds the renderer lives at `gd://app/`, so that
    // relative href resolves to `gd://app/api/live-artifacts/.../preview?projectId=...`
    // by the time `setWindowOpenHandler` sees it.
    expect(isAllowedChildWindowUrl('gd://app/api/live-artifacts/abc/preview?projectId=p1')).toBe(true);
    expect(isAllowedChildWindowUrl('gd://app/')).toBe(true);
  });
...
describe('resolveDesktopStatusUrl', () => {
  it('reports the pending URL while navigation is in flight', () => {
    expect(resolveDesktopStatusUrl(null, 'gd://app/')).toBe('gd://app/');
    expect(resolveDesktopStatusUrl('http://127.0.0.1:3000/', 'gd://app/')).toBe('gd://app/');
  });

  it('falls back to the last successful URL when no navigation is pending', () => {
    expect(resolveDesktopStatusUrl('gd://app/', null)).toBe('gd://app/');
