...
// claude-plugin folder installable through GD without crashing.
...
export function adaptClaudePlugin(
  ...
  if (safeName !== name) {
    warnings.push(`claude-plugin name '${name}' was sanitized to '${safeName}' to fit the GD plugin id pattern`);
  }
  ...
  if (commands > 0) {
    warnings.push(`claude-plugin declares ${commands} command(s); v1 GD apply does not auto-register hooks. Add them via gd.context.claudePlugins[].`);
  }
...
function synthesizeFallback(folderId: string, compatPath: string): PluginManifest {
  return {
    ...
    version: '0.0.0',
    compat: { claudePlugins: [{ path: compatPath }] },
    gd: { kind: 'skill', taskKind: 'new-generation' },
  };
}
