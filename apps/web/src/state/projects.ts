...
export type { PluginInstallOutcome } from '@galyarder-design/contracts';
export type { PluginShareAction } from '@galyarder-design/contracts';
...
export async function createProject(input: {
  ...
  try {
    ...
    // when `crypto.randomUUID` is unavailable. Galyarder Design served over
...
export function isVisiblePlugin(plugin: InstalledPluginRecord): boolean {
  const gd = (plugin.manifest?.gd ?? {}) as Record<string, unknown>;
  return gd.hidden !== true;
}
...
export async function installGeneratedPluginFolder(
  ...
  try {
    ...
    if (outcome.ok && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('galyarder-design:plugins-changed'));
    }
    return outcome;
  } catch (err) {
    return {
...
export interface PluginShareTaskStart {
  taskId: string;
  action: 'publish-github' | 'contribute-galyarder-design';
  path: string;
...
export async function contributeGeneratedPluginToGalyarderDesign(
  projectId: string,
  relativePath: string,
): Promise<PluginShareOutcome> {
  return postGeneratedPluginShareAction(projectId, relativePath, 'contribute-galyarder-design');
}
