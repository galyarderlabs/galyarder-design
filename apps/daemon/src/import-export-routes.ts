...
export function registerImportRoutes(app: Express, ctx: RegisterImportRoutesDeps) {
  ...
  // and GD works inside it directly: every write goes to metadata.baseDir.
  ...
  app.post('/api/projects/:id/working-dir', async (req, res) => {
    try {
      ...
      if (isDesktopAuthGateActive()) {
        ...
        const headerValue = req.get('x-gd-desktop-import-token');
      ...
      /** @type {import('@galyarder-design/contracts').ReplaceProjectWorkingDirResponse} */
  ...
  app.post('/api/import/folder', async (req, res) => {
    try {
      ...
      /** @type {import('@galyarder-design/contracts').ImportFolderResponse} */
...
export function registerProjectExportRoutes(app: Express, ctx: RegisterProjectExportRoutesDeps) {
  ...
  //
  // See galyarderlabs/galyarder-design#368 and the architecture lock at
  // https://github.com/galyarderlabs/galyarder-design/issues/368#issuecomment-4366243218.
