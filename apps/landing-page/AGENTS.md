...
the Galyarder Design marketing surface in the **Atelier Zero** style and
...
- Design template: `design-templates/galyarder-design-landing/` — agent workflow + the source-of-truth
  `example.html` known-good rendering for the homepage hero.
- Design system: `design-systems/atelier-zero/DESIGN.md` — token spec.
- Image assets: `design-templates/galyarder-design-landing/assets/*.png` are uploaded to
  Cloudflare R2 (`galyarder-design-static`) and served through
  `static.galyarder-design.ai` with Image Resizing (`format=auto`). Do not
...
- Astro static output. The site has multiple route groups:
  ...
  - `/templates/` + `/templates/<slug>/` — Live Artifacts in
    `templates/live-artifacts/` plus skills with `gd.mode: template`.
...
- All styles split between `app/globals.css` (homepage, kept in
  lockstep with `design-templates/galyarder-design-landing/example.html`) and
...
- Must not import from `@galyarder-design/web`, `@galyarder-design/daemon`,
  `@galyarder-design/desktop`, `@galyarder-design/sidecar*`, or
  `@galyarder-design/contracts`. Those are product runtime concerns.
...
- When the canonical `design-templates/galyarder-design-landing/example.html`
...
- Content-collection schemas in `app/content.config.ts` stay loose
  ...
  when an author uses a slightly different `gd:` key.
...
- `apps/landing-page/**`
- `design-templates/galyarder-design-landing/**`
- `skills/**`
- `design-systems/**`
- `craft/**`
- `templates/**`
...
```bash
pnpm --filter @galyarder-design/landing-page dev          # http://127.0.0.1:17574
pnpm --filter @galyarder-design/landing-page typecheck
pnpm --filter @galyarder-design/landing-page previews     # render thumbnails
pnpm --filter @galyarder-design/landing-page build        # static export → out/
```
...
- New section added to the canonical landing page → port it into
  ...
  `design-templates/galyarder-design-landing/example.html`.
- Brand re-keying for a non-Galyarder-Design tenant → fork the app, update
