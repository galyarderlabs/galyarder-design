 ...
 *   { type: 'gd:slide', action: 'next' | 'prev' | 'first' | 'last' | 'go', index?: number }
 * and the iframe responds with:
 *   { type: 'gd:slide-state', active: number, count: number }
...
<html>
  ...
  // [data-gd-id] / [data-screen-label] node and route the host's reply
  ...
  // srcdoc rebuild where the host's `gd:*-mode` postMessage races the
 ...
 *   1. Register a listener for `gd:srcdoc-transport-activate` that replaces
 *      its own document with the real artifact HTML.
 *   2. Post `gd:srcdoc-transport-ready` to the parent as soon as the listener
...
<html>
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <script data-gd-lazy-srcdoc-transport>(function(){
      window.addEventListener('message', function(ev){
        var data = ev && ev.data;
        if (!data || data.type !== 'gd:srcdoc-transport-activate' || typeof data.html !== 'string') return;
        document.open();
      ...
      try {
        if (window.parent && window.parent !== window) {
          window.parent.postMessage({ type: 'gd:srcdoc-transport-ready' }, '*');
        }
...
export interface SrcDocActivationInputs {
  ...
  /** The shell document has loaded AND posted `gd:srcdoc-transport-ready`. */
  shellReady: boolean;
 ...
 * `gd:srcdoc-transport-activate` to the shell iframe.
 *
...
function injectSrcdocTransportActivationBridge(doc: string): string {
  const script = `<script data-gd-srcdoc-transport-activation>(function(){
  window.addEventListener('message', function(ev){
    var data = ev && ev.data;
    if (!data || data.type !== 'gd:srcdoc-transport-activate' || typeof data.html !== 'string') return;
    document.open();
...
function injectSnapshotBridge(doc: string): string {
  const script = `<script data-gd-snapshot-bridge>(function(){
  ...
  function renderSnapshot(id){
    ...
    img.onload = function(){
      try {
        ...
        window.parent.postMessage({ type: 'gd:snapshot:result', id: id, dataUrl: canvas.toDataURL('image/png'), w: canvas.width, h: canvas.height }, '*');
      } catch (err) {
        window.parent.postMessage({ type: 'gd:snapshot:result', id: id, error: String(err && err.message || err) }, '*');
      }
    };
    img.onerror = function(){
      window.parent.postMessage({ type: 'gd:snapshot:result', id: id, error: 'snapshot image failed' }, '*');
    };
    img.src = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
  }
  window.addEventListener('message', function(ev){
    var data = ev && ev.data;
    if (!data || data.type !== 'gd:snapshot' || !data.id) return;
...
function injectPaletteBridge(
  ...
  const initial = options.initialPalette
    ...
    : 'null';
  const script = `<script data-gd-palette-bridge>(function(){
  var PALETTES = {
  ...
  window.addEventListener('message', function(ev){
    var data = ev && ev.data;
    if (!data || data.type !== 'gd:palette') return;
...
/**
 * Auto-annotate structural HTML elements that lack `data-gd-id` or
 ...
 * generated outside of Galyarder Design and therefore carries no GD-specific
 * annotations.
 */
function annotateMissingOdIds(doc: string): string {
  ...
  try {
    ...
    parsed.body.querySelectorAll(selector).forEach((el) => {
      if (el.hasAttribute('data-gd-id') || el.hasAttribute('data-screen-label')) return;
      const tag = el.tagName.toLowerCase();
      if (skipTags.has(tag)) return;
      const path = sourcePathForElement(el);
      el.setAttribute('data-gd-id', path || `gd-${tag}-${fallbackIndex++}`);
    });
    return serializeHtmlDocument(parsed);
  } catch {
    return doc;
...
function injectSandboxShim(doc: string): string {
  const shim = `<script data-gd-sandbox-shim>(function(){
  function makeStore(){
    var data = {};
    var api = {
...
// Both modes pick a [data-gd-id] / [data-screen-label] element on click;
...
//   in:  { type: 'gd:inspect-set', elementId, selector, prop, value }
//        Apply (or unset, when value === '') a per-element CSS override.
//   in:  { type: 'gd:inspect-reset', elementId? } Clear overrides for one
//        element, or all if elementId is omitted.
//   in:  { type: 'gd:inspect-extract' } Reply with the cumulative
//        override map so the host can persist to source.
//   in:  { type: 'gd:inspect-replay', overrides } Replace the in-memory
...
//        same allow-list / value sanitizer applied to gd:inspect-set.
//   out: { type: 'gd:inspect-overrides', overrides } The current snapshot,
...
//        same-source gd:inspect-overrides containing a hostile `css`.
//
// Overrides are written into a single <style data-gd-inspect-overrides>
...
function injectSelectionBridge(
  ...
  const script = `<script data-gd-selection-bridge>(function(){
  ...
  // overrides[elementId] = { selector: '[data-gd-id="x"]', props: { color: '#fff', ... } }
  ...
  // through gd:inspect-set. Keep this in sync with the InspectPanel UI.
  var ALLOWED_PROPS = {
    'color': true,
  ...
  // decide which attribute kind (data-gd-id vs data-screen-label) was the
  ...
  function safeSelectorFor(elementId, hint){
    ...
    if (typeof hint === 'string') {
      if (hint.indexOf('[data-gd-id=') === 0) kind = 'data-gd-id';
    ...
    if (kind === 'data-gd-id' && document.querySelector('[data-gd-id="' + esc(id) + '"]')) {
      return '[data-gd-id="' + esc(id) + '"]';
    }
    if (document.querySelector('[data-gd-id="' + esc(id) + '"]')) {
      return '[data-gd-id="' + esc(id) + '"]';
    }
  ...
  function ensureStyleEl(){
    ...
    styleEl = document.querySelector('style[data-gd-inspect-overrides]');
    if (!styleEl) {
      styleEl = document.createElement('style');
      styleEl.setAttribute('data-gd-inspect-overrides', '');
      (document.head || document.documentElement).appendChild(styleEl);
    }
    return styleEl;
  }
  // Hydrate the in-memory override map from any persisted
  // <style data-gd-inspect-overrides> block already in the document.
  // Without this, the first gd:inspect-set rebuilds the sheet from an
  ...
  function hydrateOverridesFromDom(){
    var existing = document.querySelector('style[data-gd-inspect-overrides]');
    if (!existing) return;
    var text = existing.textContent || '';
    var ruleRe = /(\\[data-(?:gd-id|screen-label)="[^"]*"\\])\\s*\\{\\s*([^}]*)\\}/g;
    var match;
  ...
  function postOverrides(){
    ...
    // gd:inspect-overrides with a hostile css (e.g. </style><script>...).
    ...
    try { window.parent.postMessage({ type: 'gd:inspect-overrides', overrides: clean }, '*'); } catch (_) {}
  }
  ...
  function annotatedSelectorFor(el){
    var id = el.getAttribute('data-gd-id') || el.getAttribute('data-screen-label');
    if (!id) return null;
    return el.hasAttribute('data-gd-id') ? '[data-gd-id="' + esc(id) + '"]' : '[data-screen-label="' + esc(id) + '"]';
  }
...
function meaningfulDomFallbackTarget(el) {
  ...
  function allTargets(){
    var annotatedNodes = document.querySelectorAll('[data-gd-id], [data-screen-label]');
  ...
  function requestPreviewScrollRestore(){
    window.parent.postMessage({ type: 'gd:preview-scroll-request' }, '*');
  }
  function postTargets(){
    if (!active()) return;
    window.parent.postMessage({ type: 'gd:comment-targets', targets: allTargets() }, '*');
  }
  ...
  function canUseDomFallback(){
    return commentEnabled && !inspectEnabled && document.querySelectorAll('[data-gd-id], [data-screen-label]').length === 0;
  }
  function closestTarget(event){
    ...
    while (el && el !== document.documentElement) {
      if (el.getAttribute && (el.hasAttribute('data-gd-id') || el.hasAttribute('data-screen-label'))) {
  ...
  window.addEventListener('message', function(ev){
    ...
    if (data.type === 'gd:comment-mode') {
      commentEnabled = !!data.enabled;
      mode = data.mode === 'pod' ? 'pod' : 'picker';
      document.documentElement.toggleAttribute('data-gd-comment-mode', commentEnabled);
      document.documentElement.setAttribute('data-gd-comment-mode-kind', mode);
      if (active()) setTimeout(postTargets, 0);
      else hoveredId = null;
      if (!commentEnabled || mode !== 'pod') {
        drawing = false;
        stroke = [];
        try { window.parent.postMessage({ type: 'gd:pod-clear' }, '*'); } catch (_) {}
      }
      return;
    }
    if (data.type === 'gd:preview-scroll-restore') {
      var frame = document.scrollingElement || document.documentElement;
      var el = previewScrollElement();
    ...
    if (data.type === 'gd:inspect-mode') {
      inspectEnabled = !!data.enabled;
      document.documentElement.toggleAttribute('data-gd-inspect-mode', inspectEnabled);
      if (active()) setTimeout(postTargets, 0);
      else hoveredId = null;
      return;
    }
    if (data.type === 'gd:inspect-set') {
      applyOverride(data.elementId, data.selector, data.prop, data.value);
      return;
    }
    if (data.type === 'gd:inspect-reset') {
      resetOverrides(data.elementId);
      return;
    }
    if (data.type === 'gd:inspect-extract') {
      postOverrides();
      return;
    }
    if (data.type === 'gd:inspect-replay') {
  ...
  document.addEventListener('mouseover', function(ev){
    ...
    window.parent.postMessage(Object.assign({}, payload, { type: 'gd:comment-hover' }), '*');
  }, true);
  document.addEventListener('mouseout', function(ev){
    ...
    hoveredId = null;
    window.parent.postMessage({ type: 'gd:comment-leave' }, '*');
  }, true);
  document.addEventListener('click', function(ev){
    ...
    // at a click location even when the artifact has no data-gd-id
  ...
  if (commentEnabled) document.documentElement.toggleAttribute('data-gd-comment-mode', true);
  if (inspectEnabled) document.documentElement.toggleAttribute('data-gd-inspect-mode', true);
  document.documentElement.setAttribute('data-gd-comment-mode-kind', mode);
  ...
  const style = `<style data-gd-selection-bridge-style>
html[data-gd-comment-mode] body * { cursor: crosshair !important; }
html[data-gd-inspect-mode] body * { cursor: crosshair !important; }
html[data-gd-comment-mode][data-gd-comment-mode-kind="pod"] body * { cursor: cell !important; }
...
html[data-gd-comment-mode] body iframe,
html[data-gd-inspect-mode] body iframe { pointer-events: none !important; }
</style>`;
...
function injectDeckBridge(doc: string, initialSlideIndex = 0): string {
  ...
  const styleFix = isFrameworkDeck
    ? ''
    : `<style data-gd-deck-fix>
.stage, .deck-stage, .deck-shell { place-content: center !important; }
</style>`;
  const script = `<script data-gd-deck-bridge>(function(){
  ...
  window.addEventListener('message', function(ev){
    var data = ev && ev.data;
    if (!data || data.type !== 'gd:slide') return;
...
// native tweaks panel. Bidirectional: host posts `gd:tweaks-panel-visible` to
// drive panel visibility; bridge posts `gd:tweaks-panel-state` back whenever the
// artifact's own `× close` button or `T` shortcut flips the `.tw-hidden` class,
// so the toolbar toggle stays in sync. Also reports `gd:tweaks-available` so the
...
function injectTweaksBridge(doc: string): string {
  ...
  const style = `<style data-gd-tweaks-bridge-style>
[data-gd-tweaks-hidden] .tw-panel {
...
.tw-restore { display: none !important; }
  ...
  const script = `<script data-gd-tweaks-bridge>(function(){
  ...
  document.documentElement.setAttribute('data-gd-tweaks-hidden', '');
  ...
  function setPanelVisible(visible){
    suppressEcho = true;
    document.documentElement.toggleAttribute('data-gd-tweaks-hidden', !visible);
  ...
  function onReady(){
    ...
    // attribute. The bridge sets data-gd-tweaks-hidden synchronously in head
    ...
    document.documentElement.toggleAttribute('data-gd-tweaks-hidden', !initialVisible);
  ...
  window.addEventListener('message', function(ev){
    if (!ev.data || ev.data.type !== 'gd:tweaks-panel-visible') return;
