...
async function withFakeAgent<T>(
  ...
  const dir = await fsp.mkdtemp(join(tmpdir(), 'gd-chat-route-bin-'));
...
describe('/api/chat', () => {
  ...
  const originalAgentHome = process.env.GD_AGENT_HOME;
  const tempDirs: string[] = [];

  beforeAll(async () => {
    if (process.env.GD_DATA_DIR) {
      originalMemoryConfig = await readMemoryConfig(process.env.GD_DATA_DIR);
      await writeMemoryConfig(process.env.GD_DATA_DIR, {
        enabled: false,
        extraction: null,
      });
    }
  ...
  afterEach(() => {
    ...
    if (originalAgentHome == null) {
      delete process.env.GD_AGENT_HOME;
    } else {
      process.env.GD_AGENT_HOME = originalAgentHome;
    }
  });
  ...
  afterAll(async () => {
    ...
    if (process.env.GD_DATA_DIR && originalMemoryConfig) {
      await writeMemoryConfig(process.env.GD_DATA_DIR, {
  ...
  it('does not reference an out-of-scope response while starting a run', async () => {
    process.env.PATH = '';
    const emptyAgentHome = mkdtempSync(join(tmpdir(), 'gd-empty-agent-home-'));
    tempDirs.push(emptyAgentHome);
    process.env.GD_AGENT_HOME = emptyAgentHome;
...
`,
  ...
  it('fails stalled json-stream runs after the inactivity timeout elapses', async () => {
    const previous = process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '500';
    try {
      await withFakeAgent(
        'opencode',
        `
...
`,
      ...
      if (previous == null) {
        delete process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
      } else {
        process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = previous;
      }
    }
  });
  ...
  it('keeps Claude stream runs alive while structured output is still flowing', async () => {
    const previous = process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '3000';
    try {
      await withFakeAgent(
        'claude',
        `
...
`,
  ...
  it('caps oversized inactivity overrides so Node does not fire the timer immediately', async () => {
    const previous = process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.GD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '10000000000';
    try {
      await withFakeAgent(
        'opencode',
        `
...
`,
  ...
  it('marks submitted discovery form answers as the active turn before the transcript', async () => {
    const captureDir = mkdtempSync(join(tmpdir(), 'gd-form-answer-prompt-'));
    tempDirs.push(captureDir);
    const capturePath = join(captureDir, 'prompt.txt');
    const previousCapturePath = process.env.GD_CAPTURE_PROMPT_PATH;
    process.env.GD_CAPTURE_PROMPT_PATH = capturePath;
    try {
      await withFakeAgent(
        'opencode',
        `
...
process.stdin.on('end', () => {
  fs.writeFileSync(process.env.GD_CAPTURE_PROMPT_PATH, input, 'utf8');
...
`,
      ...
      if (previousCapturePath == null) {
        delete process.env.GD_CAPTURE_PROMPT_PATH;
      } else {
        process.env.GD_CAPTURE_PROMPT_PATH = previousCapturePath;
      }
    }
  });
});
...
describe('daemon run creation during shutdown', () => {
  it('rejects new run creation while shutdown cleanup is still in flight', async () => {
    const previousGrace = process.env.GD_CHAT_RUN_SHUTDOWN_GRACE_MS;
    process.env.GD_CHAT_RUN_SHUTDOWN_GRACE_MS = '100';
    const started = await startServer({ port: 0, returnServer: true }) as {
      url: string;
      server: http.Server;
...
`,
      ...
      if (previousGrace == null) {
        delete process.env.GD_CHAT_RUN_SHUTDOWN_GRACE_MS;
      } else {
        process.env.GD_CHAT_RUN_SHUTDOWN_GRACE_MS = previousGrace;
      }
...
describe('chat prompt helpers', () => {
  ...
  it('rejects a generated_images final-component symlink', () => {
    const root = mkdtempSync(join(tmpdir(), 'gd-codex-generated-symlink-'));
    try {
  ...
  it('rejects a generated_images dir whose canonical path is inside a protected root', () => {
    const root = mkdtempSync(join(tmpdir(), 'gd-codex-generated-protected-'));
    try {
  ...
  it('grants Codex the canonical validated generated_images dir', () => {
    const root = mkdtempSync(join(tmpdir(), 'gd-codex-generated-canonical-'));
    try {
  ...
  it('limits Codex extra allowed dirs to the generated_images output dir', () => {
    ...
    const codex = getAgentDef('codex');
    if (!codex) throw new Error('Codex agent definition missing');
    const args = codex.buildArgs('', [], dirs, {}, { cwd: '/tmp/gd-project' });
    expect(
      args.filter(
        (arg, index) =>
          arg === '--add-dir' || args[index - 1] === '--add-dir',
  ...
  it('omits the Codex override when validation fails or the dir is not granted', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'gd-codex-generated-prompt-'));
    try {
      const codexHome = join(root, 'codex-home');
      const symlinkTarget = join(root, 'actual-generated-images');
      mkdirSync(codexHome, { recursive: true });
