...
export const PLUGIN_AUTHORING_PROMPT_TEMPLATE = [
  `Create an Galyarder Design plugin for: {{${PLUGIN_AUTHORING_GOAL_INPUT}}}.`,
  '',
  ...
  '- galyarder-design.json with valid metadata: specVersion, name, version, description, mode, task kind, inputs, plus any pipeline / context references the workflow needs',
  '- plugin.repo is optional during scaffolding, but do not silently omit it: check `gh --version` and `gh auth status`, then prefer the local account login printed by auth status. Only use `gh api user --jq .login` as a fallback when auth status does not expose a login. If `gh` is missing, not logged in, rate-limited, or cannot resolve a real owner, omit plugin.repo instead of inventing an owner and explicitly report the auth problem with `gh auth refresh -h github.com -s repo,workflow`, `gh auth login -h github.com -s repo,workflow`, or `gd plugin publish-repo generated-plugin --owner <github-login-or-org>` as recovery commands. Never write placeholder owners such as `galyarder-design-user`, `<vendor>`, `example-user`, `your-org`, or `your-username` into the final manifest.',
  '- optional examples/ and assets/ when useful',
  '',
  'Validate the plugin locally before reporting: run `gd plugin validate` on the folder, then `gd plugin pack` for a tarball, then `gd plugin install --source <absolute-folder-path>` to confirm the install path works.',
  '',
  'When the work above is done, write a single summary turn covering: files created, `gd plugin validate` status, local install / run status, and `gd plugin pack` output. Then STOP.',
  '',
  '**Do NOT** suggest follow-up CLI commands such as `gd plugin publish`, `gd plugin publish --to galyarder-design`, `gh repo create`, `git init` / `git remote add` / `git push`, or any other publish / repo wiring. The plugin-folder card under Design Files already exposes three buttons whose prompts drive those flows end-to-end with the right auth gates, fallbacks, and retry rules baked in:',
  '- **Add to My plugins** — already satisfied by this turn\'s `gd plugin install --source` step.',
  ...
  '- **Galyarder Design PR** — opens a draft PR against `galyarderlabs/galyarder-design` for the community catalog.',
  '',
  ...
  '',
  '**Do NOT** assume the standalone `jq` binary is installed (it is not part of the GD agent runtime baseline and is missing from default macOS / Windows shells). When you need to read the manifest, prefer your built-in file-reading tool, then `cat generated-plugin/galyarder-design.json` followed by manual JSON parsing, then `node -e \'console.log(JSON.parse(require("fs").readFileSync("generated-plugin/galyarder-design.json","utf8")))\'`. The `gh ... --jq` flag is fine because gh ships its own embedded library; the brew-installed standalone `jq` is NOT.',
].join('\n');
