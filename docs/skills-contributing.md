...
```bash
# 1. Fork & clone
git clone git@github.com:<your-username>/galyarder-design.git
cd galyarder-design
...
If you're not sure your idea fits, **open a discussion first** ([github.com/galyarderlabs/galyarder-design/discussions](https://github.com/galyarderlabs/galyarder-design/discussions)) — we'd rather spend 5 minutes redirecting than have you build the wrong thing for a week.
...
triggers:
  - "your trigger phrase"
  - "another phrase"
  - "a CJK trigger phrase (kept as a literal in non-English user briefs)"
...
- [ ] **Triggers are concrete.** "design something cool" is not a trigger. "investor pitch deck", "saas landing page", or a literal non-English brief phrase like "dating app" in the user's source language are.
...
- [ ] **Slug is ASCII, kebab-case.** `your-skill-name`, not `YourSkillName`, `your_skill_name`, or any non-ASCII variant.
...
- [ ] **Run `pnpm --filter @galyarder-design/web test` and `pnpm --filter @galyarder-design/e2e test tests/localized-content.test.ts`** locally before pushing. These suites catch undisplayable discovered resources and verify localized fallback behavior.
...
- [ ] Ran `pnpm --filter @galyarder-design/web test` and `pnpm --filter @galyarder-design/e2e test tests/localized-content.test.ts`; localized-content coverage is green
