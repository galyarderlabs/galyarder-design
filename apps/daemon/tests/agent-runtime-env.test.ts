...
describe('agent runtime tool environment', () => {
  it('injects daemon URL and run-scoped tool token into agent sessions', () => {
    const env = createAgentRuntimeEnv(
      { PATH: '/bin', GD_TOOL_TOKEN: 'stale-token' },
      'http://127.0.0.1:7456',
      { token: 'fresh-token' },
      '/opt/galyarder-design/bin/node',
    );

    expect(env).toMatchObject({
      PATH: `/opt/galyarder-design/bin${path.delimiter}/bin`,
      GD_DAEMON_URL: 'http://127.0.0.1:7456',
      GD_NODE_BIN: '/opt/galyarder-design/bin/node',
      GD_TOOL_TOKEN: 'fresh-token',
    });
  });
  ...
  it('does not leak stale inherited tool tokens when no run token was minted', () => {
    ...
    expect(env.GD_DAEMON_URL).toBe('http://127.0.0.1:7456');
    expect(env.GD_NODE_BIN).toBe('/opt/galyarder-design/bin/node');
    expect(env.GD_TOOL_TOKEN).toBeUndefined();
  });
  ...
  it('pins the daemon runtime data dir into agent sessions', () => {
    ...
    expect(env.GD_DATA_DIR).toBe(process.env.GD_DATA_DIR);
  });
  ...
  it('describes daemon URL and token availability without exposing the token', () => {
    ...
    expect(prompt).toContain('`GD_DAEMON_URL`');
    expect(prompt).toContain('`GD_NODE_BIN`');
    expect(prompt).toContain('`"$GD_NODE_BIN" "$GD_BIN" tools ...`');
    expect(prompt).toContain('& $env:GD_NODE_BIN $env:GD_BIN tools ...');
    expect(prompt).toContain('`GD_TOOL_TOKEN` is available');
  ...
  it('describes missing token availability without exposing stale internals', () => {
    ...
    expect(prompt).toContain('Daemon URL: `http://127.0.0.1:7456`');
    expect(prompt).toContain('`GD_TOOL_TOKEN` is not available');
