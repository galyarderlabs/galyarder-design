# Galyarder Design Telemetry Relay

Cloudflare Worker relay for opt-in Galyarder Design telemetry. The shipped desktop
...
release config should use only `GALYARDER_DESIGN_TELEMETRY_RELAY_URL`.

## Abuse controls

The Worker requires the Galyarder Design telemetry marker header, validates the
...
```bash
pnpm --filter @galyarder-design/telemetry-worker deploy
```

After deploy, set the repository variable `GALYARDER_DESIGN_TELEMETRY_RELAY_URL` to
