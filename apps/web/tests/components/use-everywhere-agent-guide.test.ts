...
describe('buildAgentGuideMarkdown', () => {
  it('emits a top-level header and the setup checklist by default', () => {
    ...
    expect(md).toMatch(/^# Galyarder Design — agent setup guide/);
  ...
  it('surfaces version and CLI hints in the checklist when supplied', () => {
    const md = buildAgentGuideMarkdown({
      versionHint: '0.42.0',
      cliHint: '/usr/local/bin/gd',
    });
    expect(md).toContain('Reported Galyarder Design version: `0.42.0`');
    expect(md).toContain('The user reported `gd` at: `/usr/local/bin/gd`');
  });

  it('omits hint sentences when the corresponding option is not provided', () => {
    const md = buildAgentGuideMarkdown();
    expect(md).not.toContain('Reported Galyarder Design version');
    expect(md).not.toContain('The user reported `gd` at');
  });
