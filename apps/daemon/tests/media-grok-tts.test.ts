...
describe('xAI Grok TTS media generation', () => {
  ...
  const originalGrokKey = process.env.GD_GROK_API_KEY;
  const originalXaiKey = process.env.XAI_API_KEY;

  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), 'gd-xai-tts-'));
    ...
    delete process.env.GD_GROK_API_KEY;
    delete process.env.XAI_API_KEY;
  });

  afterEach(async () => {
    globalThis.fetch = realFetch;
    if (originalMediaConfigDir == null) delete process.env.GD_MEDIA_CONFIG_DIR;
    else process.env.GD_MEDIA_CONFIG_DIR = originalMediaConfigDir;
    if (originalDataDir == null) delete process.env.GD_DATA_DIR;
    else process.env.GD_DATA_DIR = originalDataDir;
    if (originalGrokKey == null) delete process.env.GD_GROK_API_KEY;
    else process.env.GD_GROK_API_KEY = originalGrokKey;
