 ...
 * fake-protocol prose. The CJK `读取` (Chinese for "read") is preserved
 * because the system prompt names it as a forbidden literal output
 * pattern; this test asserts the override actually forbids that exact
 * literal. The old `streamFormat: 'plain'` rule was appended at
...
describe('composeSystemPrompt — API mode (#313)', () => {
  ...
  describe('API mode (streamFormat: plain)', () => {
    ...
    it('pins the override at the top so it overrides the discovery layer', () => {
      ...
      // string `# GD core directives`. The API-mode override must appear
      ...
      const discoveryIdx = prompt.indexOf('# GD core directives');
    ...
    it('forbids the pseudo-tool markup observed in #313 (`<todo-list>` and `[读取 ...]` — `读取` is Chinese for "read", kept as a literal detection pattern)', () => {
      const prompt = composeSystemPrompt({ streamFormat: 'plain' });
      expect(prompt).toMatch(/<todo-list>/);
      // CJK literal: matches the forbidden pseudo-tool prose the system
      // prompt explicitly bans for #313.
