...
import { pickAndImportFolder } from "@galyarder-design/desktop/main";
...
describe("pickAndImportFolder lazy retry on DESKTOP_AUTH_PENDING", () => {
  ...
  // renderer from `gd://app/`, which the main-process Node fetch cannot
  ...
  it("composes the import URL from apiBaseUrl, not from a renderer protocol scheme", async () => {
    ...
    expect(url as string).not.toMatch(/^gd:\/\//);
  });
