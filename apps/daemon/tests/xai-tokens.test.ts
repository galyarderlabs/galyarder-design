...
describe('xai-tokens persistence', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'gd-xai-tokens-'));
  });
...
describe('readTokensFile', () => {
  it('returns empty when ENOENT', async () => {
    const dir = await mkdtemp(path.join(tmpdir(), 'gd-xai-read-'));
    try {
  ...
  it.skipIf(!isPosix)('rethrows non-ENOENT read errors', async () => {
    const dir = await mkdtemp(path.join(tmpdir(), 'gd-xai-perm-'));
