...
describe('HomeHero intent rail', () => {
  ...
  it('disables every visible chip while a plugin apply is in flight', () => {
    renderHero({ pendingPluginId: 'gd-figma-migration', pendingChipId: 'figma' });
  ...
  it('media chips route to gd-media-generation with the matching project kind', () => {
    expect(findChip('image')?.action).toMatchObject({
      kind: 'apply-scenario',
      pluginId: 'gd-media-generation',
      projectKind: 'image',
    });
    expect(findChip('video')?.action).toMatchObject({ pluginId: 'gd-media-generation', projectKind: 'video' });
    expect(findChip('audio')?.action).toMatchObject({ pluginId: 'gd-media-generation', projectKind: 'audio' });
  });
  ...
  it('prototype and slide-deck chips route to their specialised bundled scenario plugin', () => {
    ...
    // the generic gd-new-generation router. Same for Slide deck →
