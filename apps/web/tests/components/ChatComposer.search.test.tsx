...
describe('ChatComposer /search command', () => {
    ...
    window.dispatchEvent(new CustomEvent(ANNOTATION_EVENT, {
      detail: {
        ...
        target: {
          ...
          selector: '[data-gd-id="metric-card"]',
          label: 'Metric card',
          text: '3 important emails',
          position: { x: 10, y: 20, width: 300, height: 120 },
          htmlHint: '<div data-gd-id="metric-card">',
        },
      },
    }));
  ...
  it('expands /search into a first-action research command prompt', () => {
    ...
    expect(prompt).toContain(
      'If the GD command fails because Tavily is not configured or unavailable',
    );
    expect(prompt).toContain(
  ...
  it('keeps shell metacharacters out of the concrete GD command examples', () => {
    const onSend = vi.fn();
