...
// `GD_ACP_STAGE_TIMEOUT_MS` from the environment.
...
// `GD_ACP_STAGE_TIMEOUT_MS=0` would call `setTimeout(..., 0)` and fail every
...
function resolveAcpTimeoutMs(env: NodeJS.ProcessEnv, fallbackMs: number): number {
  const raw = Number(env.GD_ACP_TIMEOUT_MS);
...
export async function detectAcpModels({
  ...
  clientName = 'galyarder-design-detect',
...
export function attachAcpSession({
  ...
  const resetStageTimer = (label: string) => {
    ...
    // Without this, an operator setting `GD_ACP_STAGE_TIMEOUT_MS=0` would
