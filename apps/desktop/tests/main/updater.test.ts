...
async function createUpdaterFixture(options: {
  ...
  const artifactName = platform === "win"
    ? `galyarder-design-${version}-win-x64-setup.exe`
    : `galyarder-design-${version}-mac-arm64.dmg`;
  const artifactPath = `/artifact.${artifactExt}`;
  const artifactBody = Buffer.from(options.artifactBody ?? "galyarder design updater fixture");
...
function makeRoot(): string {
  return mkdtempSync(join(tmpdir(), "gd-updater-test-"));
}
  ...
  return new Response(JSON.stringify({
    ...
    platforms: {
      mac: {
        ...
        artifacts: {
          dmg: {
            name: `galyarder-design-${version}-mac-arm64.dmg`,
            sha256: "0".repeat(64),
            size: 1,
            url: `https://example.invalid/galyarder-design-${version}-mac-arm64.dmg`,
          },
        },
      },
    },
...
describe("desktop updater", () => {
  ...
  it("downloads, verifies, persists, and dry-runs opening a mac package", async () => {
    ...
    try {
      ...
      expect(await readFile(checked.downloadPath ?? "", "utf8")).toBe("galyarder design updater fixture");
  ...
  it("resumes an interrupted artifact download before surfacing an error", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "galyarder design updater fixture with retry",
  ...
  it("does not expose raw terminated transport errors when update download retries are exhausted", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "galyarder design updater fixture that keeps failing",
  ...
  it("reports old flat updater stores as protocol errors without repairing them", async () => {
    ...
    const fixture = await createUpdaterFixture();
    try {
      await writeFile(join(root, ".galyarder-design-updater-root.json"), JSON.stringify({
    ...
    try {
      ...
      await writeFile(join(root, "metadata.json"), JSON.stringify({
        incoming: {
          arch: "x64",
          artifact: {
            name: "galyarder-design-1.0.1-win-x64-setup.exe",
            platformKey: "win",
            type: "installer",
            url: "https://fixture.test/galyarder-design-1.0.1-win-x64-setup.exe",
          },
          channel: "stable",
          cycleId,
          metadata: {},
  ...
  symlinkIt("refuses to use a symlinked updater root", async () => {
    ...
    try {
      ...
      expect(checked.state).toBe(DESKTOP_UPDATE_STATES.ERROR);
      expect(checked.error?.code).toBe("update-root-not-owned");
      expect(existsSync(join(realRoot, ".galyarder-design-updater-root.json"))).toBe(false);
    } finally {
      await fixture.close();
