...
const GITHUB_STARS_STORAGE_KEY = 'galyarder-design:gh-stars';
const VISUAL_STYLE_ID = 'gd-visual-stability-style';
// Keep this exact-route mock narrow so unrelated GitHub UI still behaves normally.
const VISUAL_GITHUB_REPO_API = 'https://api.github.com/repos/galyarderlabs/galyarder-design';
...
const VISUAL_PLUGINS = [
  ...
  makeVisualPlugin({
    ...
    description: 'Migrate a Figma frame into an editable Galyarder Design project.',
    mode: 'prototype',
...
export async function waitForVisualReady(page: Page): Promise<void> {
  await page.getByText('Loading Galyarder Design…').waitFor({ state: 'detached', timeout: 10_000 }).catch(() => {});
...
export async function captureVisual(page: Page, name: string): Promise<string> {
  const outputDir = path.resolve(process.env.GD_VISUAL_OUTPUT_DIR || 'ui/reports/visual-screenshots');
