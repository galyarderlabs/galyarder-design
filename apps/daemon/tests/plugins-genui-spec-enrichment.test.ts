...
// the AppliedPluginSnapshot. This is the wire that lets `gd ui show`
...
beforeEach(async () => {
  pluginRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-genui-spec-'));
  ...
  await writeFile(
    path.join(pluginFolder, 'galyarder-design.json'),
    JSON.stringify({
...
afterEach(async () => {
  ...
  // The user's real `.gd/app.sqlite` is what the daemon talks to, so we
