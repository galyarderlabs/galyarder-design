...
import type { GalyarderDesignHostUpdaterStatusListener, GalyarderDesignHostUpdaterStatusSnapshot } from '@galyarder-design/host';
...
function idleStatus(): GalyarderDesignHostUpdaterStatusSnapshot {
  return {
    arch: 'arm64',
    capabilities: {
...
function downloadedStatus(overrides: Partial<GalyarderDesignHostUpdaterStatusSnapshot> = {}): GalyarderDesignHostUpdaterStatusSnapshot {
  return {
    ...idleStatus(),
    availableVersion: '1.2.3-beta.4',
    downloadPath: '/tmp/galyarder-design-updater/Galyarder Design Beta.dmg',
    state: 'downloaded',
    ...overrides,
  };
}
...
describe('UpdaterPopup', () => {
  ...
  it('stays hidden for non-installable updater states', async () => {
    for (const status of [
      ...
      restoreHost = installMockGalyarderDesignHost({
        host: {
          updater: {
  ...
  it('shows only the ready indicator until the user opens the install prompt', async () => {
    ...
    expect(screen.getByText('Galyarder Design 1.2.3-beta.4 is ready. Galyarder Design will close and open the installer.')).toBeTruthy();
  ...
  it('keeps the prompt in handoff loading after opening the installer', async () => {
    ...
    let resolveInstall: (status: GalyarderDesignHostUpdaterStatusSnapshot) => void = () => undefined;
    const install = vi.fn(() => new Promise<GalyarderDesignHostUpdaterStatusSnapshot>((resolve) => {
    ...
    await act(async () => {
      status = downloadedStatus({
        installResult: {
          ...
          path: '/tmp/galyarder-design-updater/Galyarder Design Beta.dmg',
        },
      });
  ...
  it('reacts to updater subscription events by showing the ready indicator only', async () => {
    const listeners = new Set<GalyarderDesignHostUpdaterStatusListener>();
    restoreHost = installMockGalyarderDesignHost({
      host: {
        updater: {
