...
fsTest(
  ...
  () => {
    const dir = mkdtempSync(join(tmpdir(), 'gd-agents-resolve-'));
    try {
...
fsTest(
  ...
  () => {
    const home = mkdtempSync(join(tmpdir(), 'gd-agents-home-'));
    try {
      const dir = join(
        home,
        '.local',
...
fsTest(
  ...
  () => {
    const home = mkdtempSync(join(tmpdir(), 'gd-agents-npm-global-'));
    try {
...
fsTest(
  ...
  () => {
    const home = mkdtempSync(join(tmpdir(), 'gd-agents-npm-packages-'));
    try {
...
fsTest(
  ...
  () => {
    const home = mkdtempSync(join(tmpdir(), 'gd-agents-vp-home-'));
    try {
...
fsTest(
  ...
  () => {
    const vpHome = mkdtempSync(join(tmpdir(), 'gd-agents-vp-custom-'));
    try {
...
// Test isolation: when GD_AGENT_HOME points at a sandbox, an exported
...
fsTest(
  'GD_AGENT_HOME isolates resolution from $NPM_CONFIG_PREFIX leakage',
  () => {
    const sandbox = mkdtempSync(join(tmpdir(), 'gd-agents-sandbox-'));
    const realPrefix = mkdtempSync(join(tmpdir(), 'gd-agents-real-prefix-'));
    const realPrefixBin = join(realPrefix, 'bin');
    try {
      // Sandbox is empty — gemini does not exist under GD_AGENT_HOME.
      ...
      process.env.GD_AGENT_HOME = sandbox;
      process.env.PATH = '/usr/bin:/bin';
      ...
      assert.equal(
        resolved,
        null,
        `GD_AGENT_HOME sandbox must not see the real $NPM_CONFIG_PREFIX bin; ` +
          `got ${resolved}`,
      );
    } finally {
...
fsTest(
  'GD_AGENT_HOME isolates resolution from $VP_HOME leakage',
  () => {
    const sandbox = mkdtempSync(join(tmpdir(), 'gd-agents-vp-sandbox-'));
    const realVpHome = mkdtempSync(join(tmpdir(), 'gd-agents-vp-real-home-'));
    ...
    try {
      ...
      assert.equal(
        resolved,
        null,
        `GD_AGENT_HOME sandbox must not see the real $VP_HOME bin; got ${resolved}`,
      );
    } finally {
      rmSync(sandbox, { recursive: true, force: true });
