...
function popupBlockedMessage(): string {
  return 'Popup blocked. Allow popups for Galyarder Design and try again.';
}

export async function openExternalUrl(url: string): Promise<boolean> {
  if (isGalyarderDesignHostAvailable()) {
...
export async function connectConnector(connectorId: string): Promise<ConnectorActionResult> {
  ...
  const useExternalBrowser = isGalyarderDesignHostAvailable();
  try {
    ...
    authWindow.document.body.innerHTML = `
      <main style="min-height:100vh;display:grid;place-items:center;margin:0;background:#0f1115;color:#f6f7fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
        <div style="display:grid;gap:14px;justify-items:center;text-align:center;padding:32px;">
          <div aria-hidden="true" style="width:28px;height:28px;border-radius:999px;border:3px solid rgba(255,255,255,.22);border-top-color:#fff;animation:gd-spin .8s linear infinite;"></div>
...
export type SkillExampleResult =
  ...
  // is the raw `gd.preview.type` from SKILL.md so future preview kinds
...
//
// `previewType` is the skill's `gd.preview.type` (defaults to `'html'`
...
// Project files — all paths are scoped under .gd/projects/<id>/ on disk.
...
export async function replaceProjectWorkingDir(
  ...
  if (desktopImportToken) {
    headers['x-gd-desktop-import-token'] = desktopImportToken;
  }
  const resp = await fetch(
...
export async function fetchHostEditors(): Promise<
  import('@galyarder-design/contracts').HostEditorsResponse
> {
  const resp = await fetch('/api/editors');
  if (!resp.ok) throw new Error(`GET /api/editors failed: ${resp.status}`);
  return (await resp.json()) as import('@galyarder-design/contracts').HostEditorsResponse;
}

export async function openProjectInEditor(
  projectId: string,
  editorId: import('@galyarder-design/contracts').HostEditorId,
): Promise<import('@galyarder-design/contracts').OpenProjectInEditorResponse> {
  const resp = await fetch(
  ...
  return (await resp.json()) as import('@galyarder-design/contracts').OpenProjectInEditorResponse;
}
...
// `gd.useCase.exampleOutputs[].path` minus its extension).
