 ...
 * resolveDataDir (server.ts, drives GD_DATA_DIR) and resolveOverrideDir
 * (media-config.ts, drives GD_MEDIA_CONFIG_DIR + the GD_DATA_DIR fallback)
 * use this so the two resolvers cannot split state — a launcher passing
 * $HOME/.galyarder-design lands every daemon write at the same expanded path.
 *
 ...
 * prefix so a Windows launcher passing $HOME\.galyarder-design behaves the same
 * as a Unix launcher passing $HOME/.galyarder-design; the result is rebuilt via
