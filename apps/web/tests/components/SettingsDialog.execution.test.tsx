...
describe('SettingsDialog MCP server interactions', () => {
  const installInfo = {
    command: '/Applications/Galyarder Design.app/Contents/Resources/galyarder-design/bin/node',
    args: [
      '/Applications/Galyarder Design.app/Contents/Resources/app/node_modules/@galyarder-design/daemon/dist/cli.js',
      'mcp',
      '--daemon-url',
  ...
  it('renders the default Claude Code install snippet after fetching daemon install info', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/claude mcp add-json --scope user galyarder-design/i)).toBeTruthy();
    });
    expect(screen.getByText(/Restart your client to pick up the new server/i)).toBeTruthy();
    expect(screen.getByText(/Galyarder Design must be running for MCP tool calls to succeed/i)).toBeTruthy();
  });
  ...
  it('switches client instructions and snippet content when a different MCP client is selected', async () => {
    ...
    expect(screen.getByText(/\[mcp_servers\.galyarder-design\]/i)).toBeTruthy();
  ...
  it('copies the currently selected MCP snippet to the clipboard', async () => {
    ...
    await waitFor(() => {
      expect(writeTextMock).toHaveBeenCalledWith(
        expect.stringContaining("claude mcp add-json --scope user galyarder-design"),
      );
    });
...
describe('SettingsDialog language interactions', () => {
  afterEach(() => {
    cleanup();
    window.localStorage.removeItem('galyarder-design:locale');
...
describe('SettingsDialog appearance interactions', () => {
  ...
  it('localizes the accent color controls in Indonesian', () => {
    render(
    ...
    expect(screen.getByText('Aksen')).toBeTruthy();
    expect(screen.getByRole('radiogroup', { name: 'Aksen' })).toBeTruthy();
    expect(screen.getByRole('radio', { name: 'Warna aksen default' })).toBeTruthy();
    expect(screen.getByLabelText('Kustom')).toBeTruthy();
  });
});
