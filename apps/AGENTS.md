...
- `apps/web`: Next.js 16 App Router + React 18 web runtime. Entrypoints live in `apps/web/app/`; the main client shell is `apps/web/src/App.tsx`. During local `tools-dev` web runs, `apps/web/next.config.ts` rewrites `/api/*`, `/artifacts/*`, and `/frames/*` to `GD_PORT`.
- `apps/daemon`: Express + SQLite local daemon and `gd` bin. It owns REST/SSE APIs, agent CLI spawning, skills, design systems, artifact persistence, static serving, and local data under `.gd/`.
...
- `apps/packaged`: Thin packaged Electron runtime entry. It starts packaged daemon/web sidecars, registers the `gd://` entry protocol, and delegates desktop host behavior to `apps/desktop`.
...
- Packaged web uses Next.js SSR through the web sidecar; do not put Next output under daemon `GD_RESOURCE_ROOT`.
- Packaged `GD_RESOURCE_ROOT` is only for daemon non-Next read-only resources: `skills/`, `design-systems/`, and `frames/`.
...
pnpm --filter @galyarder-design/daemon typecheck
...
pnpm --filter @galyarder-design/packaged typecheck
pnpm --filter @galyarder-design/packaged build
```
