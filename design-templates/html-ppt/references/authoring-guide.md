...
4. **Format** — on-screen live, PDF export, RedNote (xhs) image post?
...
for RedNote (xhs) image posts (1242×1660).
...
- **Don't put presenter-only text on the slide.** Any descriptive text,
  ...
  on this page is…", "Note: mention X here", small grey captions explaining
  the slide's purpose) MUST go inside `<div class="notes">`, not as visible
  elements. The `.notes` div is hidden (`display:none`) and only shown via
  the S overlay. Slides should contain ONLY audience-facing content.
