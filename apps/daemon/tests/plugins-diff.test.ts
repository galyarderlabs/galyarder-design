...
const make = (
  id: string,
  manifest: NonNullable<PluginManifest['gd']> | undefined = undefined,
...
describe('diffPlugins — manifest body', () => {
  it('detects gd.taskKind changes', () => {
    const a = make('p', { taskKind: 'new-generation' });
    const b = make('p', { taskKind: 'code-migration' });
    const r = diffPlugins({ a, b });
    const e = r.entries.find((x) => x.field === 'gd.taskKind');
  ...
  it('detects added / removed inputs[] by name', () => {
    ...
    const r = diffPlugins({ a, b });
    const e = r.entries.find((x) => x.field === 'gd.inputs[]');
  ...
  it('detects pipeline added / removed / per-stage atoms changes', () => {
    ...
    expect(r.entries.find((e) => e.field === 'gd.pipeline.stages')?.kind).toBe('changed');
    // Per-stage atoms diff for the surviving stage 'plan'.
    const planAtoms = r.entries.find((e) => e.field === 'gd.pipeline.stages[plan].atoms');
    expect(planAtoms?.summary).toMatch(/1 added/);
  });

  it('emits gd.pipeline=added when only the rhs has a pipeline', () => {
    const a = make('p', { taskKind: 'new-generation' });
    const b = make('p', { taskKind: 'new-generation', pipeline: { stages: [{ id: 'x', atoms: ['todo-write'] }] } });
    const r = diffPlugins({ a, b });
    const e = r.entries.find((x) => x.field === 'gd.pipeline');
  ...
  it('detects connectors required-list churn', () => {
    ...
    const r = diffPlugins({ a, b });
    const e = r.entries.find((x) => x.field === 'gd.connectors.required');
    expect(e?.summary).toMatch(/1 added/);
  });
});
