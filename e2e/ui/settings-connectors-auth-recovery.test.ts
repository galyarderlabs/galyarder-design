...
// Locale-aware label regex (English + zh-CN + zh-TW); CJK kept verbatim
// as a functional UI-locale detection pattern.
...
async function openConnectorsSettings(
    ...
    ({ key, value, pendingAuthorization, blockPopup }) => {
      ...
      if (pendingAuthorization) {
        window.sessionStorage.setItem(
          'gd-connectors-authorization-pending',
  ...
  // Locale-aware tab label (English + zh-CN); CJK kept verbatim as a UI
  // detection pattern.
...
test.describe('Settings connectors auth recovery', () => {
  test('keeps a pending authorization visible when the connector enters authorization-pending state', async ({ page }) => {
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => {
          const raw = window.sessionStorage.getItem('gd-connectors-authorization-pending');
          if (!raw) return false;
  ...
  test('settles a pending authorization into Disconnect when status polling reports the connector as connected', async ({ page }) => {
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => window.sessionStorage.getItem('gd-connectors-authorization-pending')),
      )
      .toBe(null);
  });
