...
const dataDir = path.join(process.env.GD_DATA_DIR ?? process.cwd(), 'memory-connectors-test');
  ...
  return {
    ...
    execute: async (request: { connectorId: string; toolName: string; input: unknown }) => {
      ...
      return {
        ...
        outputSummary: 'Found GalyarderDesign design memory notes in Notion.',
        output: {
          pages: [
            {
              title: 'GalyarderDesign memory plan',
              text: 'GalyarderDesign connector memory should collect design preferences, UI decisions, and visual references from Notion.',
            },
          ],
        },
      };
...
describe('connector memory extraction', () => {
              ...
              entries: [
                {
                  ...
                  body: 'GalyarderDesign connector memories should focus on design preferences, UI decisions, and visual references rather than generic app activity.',
                },
              ],
            }),
          },
  ...
  it('tries additional connector queries when the first read is empty', async () => {
            ...
            pages: [
              {
                ...
                text: 'GalyarderDesign should summarize connector findings when no durable memory is obvious.',
              },
            ],
          },
        };
    ...
    // Detection patterns; CJK matches the DEFAULT_CONNECTOR_MEMORY_QUERY and
    // CONNECTOR_MEMORY_QUERY_VARIANTS constants in src/memory-connectors.ts.
    expect(executeCalls[0]).toEqual(expect.objectContaining({
      input: expect.objectContaining({
        query: '设计思路 设计偏好 UI UX 视觉风格 品牌 logo 设计系统 GalyarderDesign',
      }),
    }));
    expect(executeCalls[1]).toEqual(expect.objectContaining({
      // Detection pattern; CJK '设计思路' matches CONNECTOR_MEMORY_QUERY_VARIANTS.
  ...
  it('continues past readable but off-topic results to find Chinese design notes', async () => {
    // Detection pattern; the CJK page title '设计思路' (design approach) and body
    // text are what the connector memory keyword detector in src/memory-connectors.ts
    // (e.g. /设计思路/) is meant to find — the test asserts the second call is the
    // variant query that surfaces them.
    const executeCalls: Array<{ connectorId: string; toolName: string; input: unknown }> = [];
    const service = {
      ...createNotionService(executeCalls),
      execute: async (request: { connectorId: string; toolName: string; input: unknown }) => {
        ...
        return {
          ...
          output: {
            // CJK fixture; matches the connector keyword detector in src/memory-connectors.ts.
            pages: [
              {
                title: '设计思路',
    ...
    expect(executeCalls.length).toBeGreaterThan(1);
    // Detection pattern; '设计思路' matches the CONNECTOR_MEMORY_QUERY_VARIANTS constant.
  ...
  it('uses the Notion empty-query fallback when specific title searches miss', async () => {
    // Detection pattern; CJK page title '设计思路' and CJK body text exercise the
    // connector keyword detector — see src/memory-connectors.ts (/设计思路/ score boost).
  ...
  it('opens relevant Notion pages after search so page body reaches the memory model', async () => {
    // Detection pattern; the CJK page title '设计思路' (design approach) and CJK
    // markdown body verify the connector keyword detector picks up the page and
    // forwards its CJK contents into the memory model prompt.
            ...
            results: [
              {
                ...
                properties: {
                  Name: {
                    // CJK fixture; matches the connector keyword detector.
    ...
    expect(result.connectors).toEqual([
      expect.objectContaining({
        status: 'succeeded',
        // CJK fixture; verifies the page-content summary surfaces the CJK title verbatim.
    ...
    // CJK fixture; verifies the page body's CJK content reaches the model prompt.
  ...
  it('tries another search tool when the first tool only finds off-topic pages', async () => {
    // Detection pattern; CJK page title '设计思路' and CJK body exercise the
    // connector keyword detector — the test verifies the broad-search tool is
    // tried when the first tool's results miss the design-memory keywords.
            ...
            pages: [
              {
                ...
                text: 'GalyarderDesign should read the currently available Notion search tool.',
              },
            ],
          },
        };
      ...
      execute: async (request: { connectorId: string; toolName: string; input: unknown }) => ({
        ...
        output: {
          pages: [
            {
              title: 'Memory source notes',
              text: 'GalyarderDesign should summarize connector findings before saving them as memory.',
            },
          ],
        },
      }),
              ...
              entries: [
                {
                  ...
                  body: 'GalyarderDesign read GitHub via List notifications. Summary: Found 5 readable items from GitHub. Save this if it should be reused as context in future chats.',
                },
                {
                  type: 'feedback',
                  name: 'UI density preference',
                  description: 'The user prefers denser design interfaces',
                  body: 'The user prefers GalyarderDesign UI to use higher information density with clear hierarchy instead of spacious marketing-style cards.',
                },
              ],
            }),
          },
      ...
      entries: [
        {
          ...
          body: 'GalyarderDesign connector memory extraction should use the same Claude Code Local CLI selected for chat when the memory model is set to same as chat.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'GalyarderDesign connector memory extraction should use the same OpenCode Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'GalyarderDesign connector memory extraction should use the same Codex Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
  ...
  it('runs Codex Local CLI through JSON event stream with stdin prompt', async () => {
    await writeMemoryConfig(dataDir, { extraction: null });
    const tempDir = await fsp.mkdtemp(path.join(tmpdir(), 'gd-codex-memory-'));
    const binPath = path.join(tempDir, 'codex');
    const capturePath = path.join(tempDir, 'capture.json');
    const previousPath = process.env.PATH;
    const previousCapture = process.env.GD_MEMORY_CODEX_ARGS_OUT;
...
fs.writeFileSync(process.env.GD_MEMORY_CODEX_ARGS_OUT, JSON.stringify({ args, stdin }));
...
`,
    ...
    try {
      ...
      process.env.GD_MEMORY_CODEX_ARGS_OUT = capturePath;
      ...
      expect(captured.stdin).toContain('GalyarderDesign connector memory should collect design preferences');
    } finally {
      ...
      if (previousCapture == null) {
        delete process.env.GD_MEMORY_CODEX_ARGS_OUT;
      } else {
        process.env.GD_MEMORY_CODEX_ARGS_OUT = previousCapture;
      }
  ...
  it('runs OpenCode Local CLI with a message argument and attached prompt file', async () => {
    await writeMemoryConfig(dataDir, { extraction: null });
    const tempDir = await fsp.mkdtemp(path.join(tmpdir(), 'gd-opencode-memory-'));
    const binPath = path.join(tempDir, 'opencode-cli');
    const capturePath = path.join(tempDir, 'capture.json');
    const previousPath = process.env.PATH;
    const previousCapture = process.env.GD_MEMORY_OPENCODE_ARGS_OUT;
...
const stdin = fs.readFileSync(0, 'utf8');
fs.writeFileSync(process.env.GD_MEMORY_OPENCODE_ARGS_OUT, JSON.stringify({ args, attachedFile, prompt, stdin }));
process.stdout.write(JSON.stringify({
  type: 'text',
  part: {
    type: 'text',
...
`,
    ...
    try {
      process.env.PATH = `${tempDir}${path.delimiter}${previousPath ?? ''}`;
      process.env.GD_MEMORY_OPENCODE_ARGS_OUT = capturePath;
      ...
      expect(captured.args).toEqual(expect.arrayContaining([
        ...
        'json',
        '--file',
        'Read the attached GalyarderDesign memory extraction prompt and return strict JSON only.',
      ]));
      expect(captured.args).toContain('openai/gpt-5');
      expect(captured.prompt).toContain('You are a design-memory extractor');
      expect(captured.prompt).toContain('GalyarderDesign connector memory should collect design preferences');
      ...
      if (previousCapture == null) {
        delete process.env.GD_MEMORY_OPENCODE_ARGS_OUT;
      } else {
        process.env.GD_MEMORY_OPENCODE_ARGS_OUT = previousCapture;
      }
      await fsp.rm(tempDir, { recursive: true, force: true });
    }
  });
