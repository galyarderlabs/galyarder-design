...
Run all adapters' `detect()` in parallel on daemon start, then cache results in `~/.galyarder-design/agents.json` with a 24h TTL. Re-detect on daemon `SIGHUP`.
...
- Tool use: we register `Read/Write/Edit` as tools, implement them in the daemon against the artifact cwd, and run the loop ourselves. This is the one place GD does own the loop — because the user has no agent at all. Keep it as dumb as possible.
...
- **Why ship this at all?** Topology C requires it (no daemon available in a pure-Vercel deploy). Also, users trying GD for the first time without a CLI installed still get a working experience.
...
If the user's preferred agent fails (crash, auth, timeout), GD offers a one-click fallback in this order:
...
- **Claude Code** respects its own `--allowed-tools` and `--permission-mode` flags. GD passes through user preferences.
- **Codex / Cursor** sandbox by workspace; GD always sets cwd to the artifact dir so nothing outside is visible by default.
...
- **Billing awareness.** Some agents bill per message, some per token. GD doesn't track cost in MVP; v1 adds an optional "usage" event from adapters that expose it.
