...
export const MANUAL_EDIT_SOURCE_PATH_ATTR = 'data-gd-source-path';
export const MANUAL_EDIT_HOST_NODE_SELECTOR = [
  '[data-gd-sandbox-shim]',
  '[data-gd-deck-bridge]',
  '[data-gd-comment-bridge]',
  '[data-gd-edit-bridge]',
  '[data-gd-comment-bridge-style]',
  '[data-gd-edit-bridge-style]',
  '[data-gd-deck-fix]',
].join(',');
...
export function manualEditStableIdForElement(el: Element): string {
  const explicit = el.getAttribute('data-gd-id');
  if (explicit) return explicit;
  const generated = el.getAttribute(MANUAL_EDIT_SOURCE_PATH_ATTR) || el.getAttribute('data-gd-runtime-id') || manualEditDomPathForElement(el);
  if (generated) el.setAttribute('data-gd-runtime-id', generated);
...
export function isSourceMappableManualEditElement(el: Element): boolean {
  return el.hasAttribute('data-gd-id') || el.hasAttribute(MANUAL_EDIT_SOURCE_PATH_ATTR);
}

export function buildManualEditBridge(enabled: boolean): string {
  return `<script data-gd-edit-bridge>(function(){
  ...
  function stableId(el){
    var explicit = el.getAttribute('data-gd-id');
    if (explicit) return explicit;
    var generated = el.getAttribute(sourcePathAttr) || el.getAttribute('data-gd-runtime-id') || domPath(el);
    if (generated) el.setAttribute('data-gd-runtime-id', generated);
    return generated || 'unknown';
  }
  function isSourceMappable(el){
    return !!(el && el.hasAttribute && (el.hasAttribute('data-gd-id') || el.hasAttribute(sourcePathAttr)));
  }
  ...
  function isPrimaryTarget(el){
    ...
    if (el.hasAttribute('data-gd-id') || el.hasAttribute('data-gd-edit')) return true;
    var tag = el.tagName ? el.tagName.toLowerCase() : '';
    return tag === 'a' || tag === 'button';
  }
  function inferKind(el){
    var explicit = el.getAttribute('data-gd-edit');
    if (explicit) return explicit;
    var tag = el.tagName ? el.tagName.toLowerCase() : '';
  ...
  function labelFor(el, id, kind){
    var explicit = el.getAttribute('data-gd-label');
    if (explicit) return explicit;
  ...
  function attrsFor(el){
    ...
    for (var i = 0; i < el.attributes.length; i++) {
      ...
      if (!attr || attr.name.indexOf('data-gd-runtime') === 0 || attr.name === 'data-gd-edit-selected') continue;
  ...
  function targetFrom(el, includeOuterHtml){
    ...
    return {
      ...
      outerHtml: includeOuterHtml ? (el.outerHTML || '').replace(/\\sdata-gd-runtime-id="[^"]*"/g, '').replace(/\\sdata-gd-source-path="[^"]*"/g, '').replace(/\\sdata-gd-edit-selected="[^"]*"/g, '') : ''
    };
  }
  function allTargets(){
  ...
  function postTargets(){
    if (!enabled) return;
    window.parent.postMessage({ type: 'gd-edit-targets', targets: allTargets() }, '*');
  }
  function clearSelectedTarget(){
    var selected = document.querySelectorAll('[data-gd-edit-selected]');
    for (var i = 0; i < selected.length; i++) selected[i].removeAttribute('data-gd-edit-selected');
  }
  function setSelectedTarget(id){
    clearSelectedTarget();
    if (!id) return;
    var el = findById(id);
    if (el) el.setAttribute('data-gd-edit-selected', 'true');
  }
  ...
  function findById(id){
    ...
    var el = document.querySelector('[data-gd-id="' + cssEscapeId(id) + '"]')
          || document.querySelector('[data-gd-runtime-id="' + cssEscapeId(id) + '"]')
  ...
  function applyPreviewStyles(id, styles, version){
    var el = findById(id);
    if (!el) {
      window.parent.postMessage({ type: 'gd-edit-preview-style-applied', id: id || '', version: Number(version) || 0, ok: false, error: 'Target not found' }, '*');
      return;
    }
    ...
    try {
      ...
      window.parent.postMessage({ type: 'gd-edit-preview-style-applied', id: id, version: Number(version) || 0, ok: true }, '*');
    } catch (e) {
      window.parent.postMessage({ type: 'gd-edit-preview-style-applied', id: id, version: Number(version) || 0, ok: false, error: e && e.message ? String(e.message) : 'Could not apply preview styles' }, '*');
    }
  }
  window.addEventListener('message', function(ev){
    if (!ev.data) return;
    if (ev.data.type === 'gd-edit-mode') {
      enabled = !!ev.data.enabled;
      document.documentElement.toggleAttribute('data-gd-edit-mode', enabled);
      if (!enabled) clearSelectedTarget();
      if (enabled) setTimeout(postTargets, 0);
      return;
    }
    if (ev.data.type === 'gd-edit-selected-target') {
      setSelectedTarget(ev.data.id || null);
      return;
    }
    if (ev.data.type === 'gd-edit-preview-style') {
  ...
  document.addEventListener('click', function(ev){
    ...
    ev.preventDefault();
    ev.stopPropagation();
    window.parent.postMessage({ type: 'gd-edit-select', target: targetFrom(el, true) }, '*');
  }, true);
...
export function buildManualEditBridgeStyle(): string {
  return `<style data-gd-edit-bridge-style>
html[data-gd-edit-mode] body * { cursor: pointer !important; }
html[data-gd-edit-mode] [data-gd-id],
html[data-gd-edit-mode] [data-gd-runtime-id] { outline: 1px dashed rgba(37, 99, 235, 0.35); outline-offset: 3px; }
html[data-gd-edit-mode] [data-gd-id]:hover,
html[data-gd-edit-mode] [data-gd-runtime-id]:hover { outline: 2px solid #2563eb; }
html[data-gd-edit-mode] [data-gd-edit-selected] {
