...
Create an Galyarder Design live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
1. **Resolve scope and data source without blocking on connected connectors**
   ...
   - If connector data is needed, first list connectors with `"$GD_NODE_BIN" "$GD_BIN" tools connectors list --format compact`. If the named connector is present with `status: "connected"`, choose an appropriate read-only `auto` tool from its catalog and execute it through the connector wrapper.
...
5. **Register or update through daemon wrappers**
   - Use the Galyarder Design daemon wrapper commands via `"$GD_NODE_BIN" "$GD_BIN"` instead of raw `curl`, bare `node`, or bare `gd`:

     ```bash
     "$GD_NODE_BIN" "$GD_BIN" tools live-artifacts create --input artifact.json
     "$GD_NODE_BIN" "$GD_BIN" tools live-artifacts list --format compact
     "$GD_NODE_BIN" "$GD_BIN" tools live-artifacts update --artifact-id "$ARTIFACT_ID" --input artifact.json
     ```

   - The wrapper reads injected `GD_NODE_BIN`, `GD_BIN`, `GD_DAEMON_URL`, and `GD_TOOL_TOKEN`; do not print, persist, or override token values.
...
6. **Use connector wrappers for connector data**
   - Discover available connectors and tools:
     ...
     ```bash
     "$GD_NODE_BIN" "$GD_BIN" tools connectors list --format compact
     ```

   - Execute a read-only connector tool with a JSON object input file:

     ```bash
     "$GD_NODE_BIN" "$GD_BIN" tools connectors execute --connector "$CONNECTOR_ID" --tool "$TOOL_NAME" --input input.json
     ```
