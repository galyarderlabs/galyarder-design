# Galyarder Design Plugin & Marketplace Spec (v1)

> **In one sentence:** Galyarder Design plugins turn portable `SKILL.md` capabilities into marketplace-ready, one-click design workflows while preserving compatibility with existing agent skill catalogs, headless CLI use, and self-hosted deployment.
...
A **Plugin** is the unit of distribution for Galyarder Design. Where a [Skill](skills-protocol.md) describes a single capability that an agent can run, a Plugin is the shippable bundle around it: one or more skills, an optional design system reference, optional craft rules, optional Claude-plugin assets, a preview, a use-case query, an asset folder, and a small machine-readable sidecar that powers OD's marketplace surface. A plugin is always anchored to a portable `SKILL.md` so it is publishable to every existing skill catalog without modification.

> **Compatibility promise (extends [`skills-protocol.md`](skills-protocol.md)):** Any plugin folder that ships a `SKILL.md` works as a plain agent skill in Claude Code, Cursor, Codex, Gemini CLI, OpenClaw, Hermes, etc. Adding `galyarder-design.json` is purely additive — it unlocks OD's marketplace card, preview, one-click "use" flow, and typed context-chip strip, but it never changes how the underlying skill runs. **One repo, two consumption modes.**

## Executive map for readers

This is an **agent-era plugin system**, not a Figma-era UI extension system. A plugin does not directly mount inside the canvas, own a local panel lifecycle, or talk to the app through bespoke `postMessage` / RPC channels. Instead, the plugin is a packaged intent and context layer that the user, UI, CLI, or another agent can select; GD resolves it into a query, context chips, assets, design-system references, MCP/tool capabilities, and run metadata; then the selected code agent consumes that package through the same project/run pipeline as any other GD task.

The shortest mental model:

1. **Plugin author ships portable capability.** `SKILL.md` remains the executable agent contract; `galyarder-design.json` adds GD marketplace metadata, input fields, defaults, previews, and context wiring.
...
3. **GD applies the plugin without making the plugin a UI process.** Apply returns a hydrated brief, typed context chips, assets, and capability requirements. It does not start a hidden plugin runtime.
...
5. **UI is a collaboration surface.** The web/desktop UI can show forms, previews, direction pickers, critique panels, and live artifacts, but the same flow must work headlessly through `gd`.

### Figma-era vs agent-era boundary

| Question | Figma-era plugin assumption | Galyarder Design v1 answer |
| --- | --- | --- |
...
| Is there a plugin-to-app RPC protocol? | Often yes. | Not as the primary contract. GD uses HTTP internally, CLI for agents, MCP where useful, and SSE/ND-JSON for run events. |
...
| What is persisted? | Host document mutations. | GD project metadata, artifacts, conversation/run history, and plugin provenance. |
...
sequenceDiagram
  ...
  participant S as GD Surface<br/>(Web, Desktop, CLI, CI)
  participant D as GD Daemon
  participant P as Plugin Manifest<br/>(SKILL.md + galyarder-design.json)
...
| Headless CLI / code agent | Claude Code, Cursor, Codex, CI, or script shells out to `gd` | `gd run create --plugin make-a-deck --input audience=investors --input topic='AI design ops'` | Same manifest resolution, same context chips, same run events without opening desktop. |
...
- **Atom (§10):** a named capability exposed by the GD daemon and first-party tools (discovery-question-form, direction-picker, todo-write, file-read/write, research-search, media-image, live-artifact, critique-theater, etc.).
...
**Current implementation clarification:** `discovery -> plan -> generate -> critique` is a reference pipeline shape, not a fixed hard-coded wizard. A plugin snapshot can carry `od.pipeline.stages[].atoms[]`; the daemon resolves that snapshot, injects the active plugin block plus active stage atom blocks into the system prompt, emits stage events, and lets the agent work through the pipeline. When the user has not explicitly selected a plugin, GD still does **not** launch a generic naked agent: the base Galyarder Design designer prompt and discovery rules are always present. Product entry points bind sensible defaults on top of that base: Home free-form input routes through the bundled, hidden `od-default` scenario, while typed New Project flows choose the default bundled scenario for the project kind. `od-default` is a router and task-shaper; it should guide the run into the normal design pipeline, not be treated as a standalone "make it beautiful" aesthetic engine.
...
| `tune-collab` | An existing GD project + artifact | Critique-tune, brand swap, A/B variants, stakeholder review on top of an existing artifact | direction-picker → patch-edit → critique → handoff |
...
5. [`galyarder-design.json` schema](#5-galyarder-designjson--schema-v1)
6. [`galyarder-design-marketplace.json` schema](#6-galyarder-design-marketplacejson--federated-catalog)
...
10. [First-party atoms](#10-first-party-atoms--galyarder-designs-atomic-capabilities)
...
13. [Public web surface](#13-public-web-surface-galyarder-designaimarketplace)
...
19. [Why this is a meaningful step for Galyarder Design](#19-why-this-is-a-meaningful-step-for-galyarder-design)
...
- Defaults locked from the planning round (overridable in review):
  - **Compatibility = wrap-then-extend.** Existing `SKILL.md` and `.claude-plugin/plugin.json` repos run as-is; `galyarder-design.json` is an additive sidecar.
...
6. The GD core engine, atomic capabilities, and plugin runtime are all reachable from CLI so any code agent can drive Galyarder Design headlessly.
...
- Replacing SKILL.md / claude-plugin spec — GD never forks.
- Hosting plugin binaries — GD points to GitHub / CDN URLs; storage is the publisher's responsibility.
- A signing/PKI ecosystem — capability gating relies on user consent, not signatures.
- A web-hosted SaaS marketplace running GD agents on behalf of users — local-first only in v1.

## 3. Compatibility matrix — what makes a folder a valid plugin for whom

| File present                                                       | GD installs    | Claude Code / Cursor / Codex / Gemini CLI | OpenClaw / Hermes | awesome-agent-skills | clawhub | skills.sh |
| ------------------------------------------------------------------ | -------------- | ----------------------------------------- | ----------------- | -------------------- | ------- | --------- |
| `SKILL.md` only                                                    | yes            | yes                                       | yes               | yes                  | yes     | yes       |
| `.claude-plugin/plugin.json` only                                  | yes            | yes (claude)                              | partial           | listable             | listable| listable  |
| `galyarder-design.json` only                                            | metadata-only  | no                                        | no                | no                   | no      | no        |
| `SKILL.md` + `galyarder-design.json`                                    | enriched       | yes                                       | yes               | yes                  | yes     | yes       |
| `.claude-plugin/...` + `galyarder-design.json`                          | enriched       | yes (claude)                              | partial           | listable             | listable| listable  |
| `SKILL.md` + `.claude-plugin/...` + `galyarder-design.json`             | fully enriched | yes                                       | yes               | yes                  | yes     | yes       |

The takeaway: **`SKILL.md` is the lowest common denominator**. Every plugin recommended for distribution should ship a `SKILL.md` so it lands cleanly in every major catalog, then add `galyarder-design.json` to gain OD's product surface.

A folder that contains only `galyarder-design.json` is not a runnable plugin in v1; it is a **metadata-only preset**. GD may read it to show a marketplace card, aggregate remote references, or act as a future install stub, but it cannot trigger an agent run and cannot be listed in cross-agent catalogs. `gd plugin doctor` must mark this shape as `metadata-only` and prompt the author to add `SKILL.md` or `.claude-plugin/plugin.json` before publishing it as a runnable plugin.
...
│   └── plugin.json
├── galyarder-design.json                  # optional sidecar — unlocks GD product surface
├── README.md                         # standard catalog readme
├── preview/                          # GD preview assets
│   ├── index.html
│   ├── poster.png
│   └── demo.mp4
...
- `galyarder-design.json` only ever **points** at SKILL.md / DESIGN.md / craft files; it never duplicates their bodies.
- Existing OD-specific frontmatter on SKILL.md (the `od:` namespace already documented in [`skills-protocol.md`](skills-protocol.md) and used in [`skills/blog-post/SKILL.md`](../skills/blog-post/SKILL.md)) is honored as a fallback for plugins without `galyarder-design.json`. We do not deprecate it; we layer over it.
- A runnable v1 plugin must contain at least one of `SKILL.md` or `.claude-plugin/plugin.json`. `galyarder-design.json` does not define agent behavior by itself; it only tells GD how to display, resolve, and apply that behavior.

## 5. `galyarder-design.json` — schema v1

```json
{
  "$schema": "https://galyarder-design.dev/schemas/plugin.v1.json",
  "specVersion": "1.0.0",
  "name": "make-a-deck",
  "title": "Make a deck",
  "title_i18n": { "en": "Make a deck", "zh-CN": "Make a deck (zh-CN translation)" },
  "version": "1.0.0",
  ...
  "description_i18n": {
    ...
    "zh-CN": "Generate a 12-slide investor deck from a one-line brief. (zh-CN translation)"
  },
  "author":   { "name": "Galyarder Design", "url": "https://galyarder-design.dev" },
  "license":  "MIT",
  "homepage": "https://github.com/galyarder-design/plugins/make-a-deck",
  "icon":     "./icon.svg",
  ...
  "gd": {
    ...
    "useCase": {
      "query": {
        ...
        "zh-CN": "Make a 12-slide investor deck for a Series A SaaS startup targeting {{audience}} on {{topic}}. (zh-CN translation)"
      },
      "exampleOutputs": [
...
- `compat.*` — relative paths to inherited files. The loader concatenates their content into the GD prompt stack assembled by [`composeSystemPrompt()`](../apps/daemon/src/prompts/system.ts).
- `specVersion` — the Galyarder Design plugin spec version used to interpret the manifest. This is distinct from plugin `version` and is frozen into apply snapshots for replay.
...
- `od.useCase.query` — the exact text that lands in the brief field on click-to-use. It may be a legacy string or a locale map keyed by BCP-47-style locale tags (for example `{ "en": "...": "..." }`). Apply-time resolution tries the requested locale, base language, `en`, then the first available value. `{{var}}` placeholders bind to `od.inputs`.
...
- Declaring `mcp` with any command that is not an GD built-in stdio tool implies `subprocess`.
...
| `od.capabilities_required` | `od.capabilities` | Map only capabilities that can be expressed; unknown capabilities are kept in `compatWarnings[]`, and `gd plugin doctor` must surface them |

If `galyarder-design.json` and `SKILL.md` frontmatter both exist, `galyarder-design.json` wins, but the loader must preserve adapter warnings. Authors can migrate incrementally: first keep the old skill runnable as-is, then add GD marketplace metadata.

## 6. `galyarder-design-marketplace.json` — federated catalog
...
```json
{
  "$schema": "https://galyarder-design.dev/schemas/marketplace.v1.json",
  "specVersion": "1.0.0",
  "name": "galyarder-design-official",
  "version": "1.0.0",
  "owner":    { "name": "Galyarder Design", "url": "https://galyarder-design.dev" },
  "metadata": { "description": "First-party plugins", "version": "1.0.0" },
  "plugins": [
    { "name": "make-a-deck", "version": "1.0.0", "source": "github:galyarder-design/plugins/make-a-deck", "tags": ["deck"] },
    { "name": "tweet-card",  "version": "1.0.0", "source": "https://files.../tweet-card-1.0.0.tgz",  "tags": ["marketing"] }
  ]
}
```

The marketplace top-level `version` is the catalog snapshot version; every `plugins[]` entry also declares the listed plugin version. Installers still verify the target folder's own `galyarder-design.json` after fetching, but registry search, audit logs, and marketplace refresh events can now reason about catalog and plugin versions before install.
...
| -------- | ------------------------------------------------ | ------------------ | ---------------------------------------------------------------------- |
| 1        | `<projectCwd>/.galyarder-design/plugins/<id>/`        | plugin bundle      | New; explicitly installed into the project and committed with user code |
...
| 3        | `<daemonDataDir>/plugins/<id>/`                  | plugin bundle      | New; written by `gd plugin install` under the daemon data root          |
| 4        | `~/.galyarder-design/skills/<id>/`                    | legacy `SKILL.md`  | GD canonical skill install path; may symlink into other agents          |
...
Conflict resolution uses normalized `name` / plugin id; lower numeric priority wins. Legacy `SKILL.md` locations are synthesized into plugin records by the adapter, but are not copied into `<daemonDataDir>/plugins/` unless the user explicitly runs `gd plugin install`. This keeps existing Claude skills zero-config while giving plugin bundles a clear install root.

### 7.2 Install sources

```
gd plugin install ./folder
gd plugin install github:owner/repo
gd plugin install github:owner/repo@v1.2.0
gd plugin install github:owner/repo/path/to/subfolder
gd plugin install https://example.com/plugin.tar.gz
gd plugin install make-a-deck                   # via configured marketplaces
od marketplace add https://.../galyarder-design-marketplace.json
```
...
- A plugin can be upgraded between two runs via `gd plugin update`.
...
3. **Replay** — `gd run replay <runId>` and `gd plugin export <runId>` must reconstruct prompt and assets from the snapshot rather than the live manifest, so old runs remain reproducible after plugin upgrades.
...
Only the daemon writes `AppliedPluginSnapshot`; CLI/UI clients are read-only. Plugin upgrades or marketplace ref drift cause `gd plugin doctor` to mark affected historical snapshots as `stale`, but **never** to rewrite them: reproducibility wins over freshness.
...
A `restricted` plugin can never reach P3/P4/P5 unless the user grants the capability — either through `gd plugin trust <id>` or "Grant capabilities" on the detail page. Only two sources are trusted by default: repo-bundled first-party plugins and the official GD marketplace. User-added third-party marketplaces are discovery sources; plugins from them still install as `restricted` unless the marketplace itself is explicitly trusted, or an individual plugin is granted capabilities by id + version + capability.
...
1. **Pre-trust** (recommended for hosted / CI).
   ...
   ```bash
   gd plugin trust make-a-deck   --caps fs:read,mcp,subprocess
   gd plugin trust make-a-digest --caps fs:read,connector:slack,connector:notion
   gd plugin trust make-a-deck   --caps all          # equivalent to all capabilities the manifest declares
   ```
...
2. **Per-call temporary grant**.
   ...
   ```bash
   gd plugin apply make-a-deck   --project p_abc --grant-caps fs:read,mcp --json
   gd plugin apply make-a-digest --project p_abc --grant-caps fs:read,connector:slack --json
   gd plugin run   make-a-deck   --project p_abc --grant-caps fs:read --follow
   ```
   ...
   ```json
   {
     "error": {
       ...
       "data": {
         ...
         "remediation": [
           "gd plugin trust make-a-deck --caps mcp,subprocess",
...
Plugin previews may come from untrusted GitHub repos or archives, so they cannot run with the same privileges as the GD app. `od.preview.entry` HTML previews must follow these constraints:

- Preview iframes start with `sandbox="allow-scripts"` only. They do not get `allow-same-origin`, `allow-forms`, `allow-popups`, or `allow-downloads` by default. If a first-party preview needs an extra flag, it must declare that in the manifest and `gd plugin doctor` must mark it as an elevated preview.
...
| `research-search` | `gd research search` ([`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts)) | Tavily web research | new-generation |
| `media-image` / `media-video` / `media-audio` | `gd media generate` | Media generation with provider config | new-generation, tune-collab |
...
`(planned)` atoms are **not** implemented in v1, but their IDs are reserved here and in §5 schema to avoid a future name churn. `GET /api/atoms` returns only implemented atoms in v1; planned atoms emit a clear "not yet implemented" warning from `gd plugin doctor` rather than a generic "unknown atom" error.
...
- `until` is a lightweight expression evaluated by the daemon (only comparisons and known signal variables: `critique.score`, `iterations`, `user.confirmed`, `preview.ok`); it is **not** arbitrary JS. `gd plugin doctor` validates syntax.
...
4. The user can break out anytime via `gd run respond <runId> --json '{"action":"break-loop"}'` or the UI "Stop refining" button.

Two hard constraints on devloop:

- **`until` is required.** Pipelines with `repeat: true` but no `until` fail `gd plugin doctor` and the daemon refuses to execute them.
- **Iteration ceiling.** The daemon enforces `iterations <= 10` (configurable via `GD_MAX_DEVLOOP_ITERATIONS`) to keep a buggy plugin from burning provider quota in an infinite loop.
...
`gd plugin doctor` enforces at install / apply time that: (1) when `oauth.route === 'connector'`, `oauth.connectorId` is present in the plugin's own `od.connectors.required[]` or `od.connectors.optional[]`; (2) when `oauth.route === 'mcp'`, `oauth.mcpServerId` matches a name in the plugin's MCP server set.
...
5. **User revoke:** UI / CLI provide `gd ui revoke <surface-id>` to flip a row to `invalidated`. Common case: OAuth logout.
...
The ND-JSON stream from `gd run watch` / `gd run start --follow` includes `genui_surface_request` events. A third-party code agent has three response paths:

```bash
# Inspect pending surfaces on a run
gd ui list --run <runId> --json

# Read a single surface (kind / schema / prompt) for rendering or auto-fill
gd ui show <runId> <surface-id> --json

# Respond from any process; daemon writes to genui_surfaces, the run continues
gd ui respond <runId> <surface-id> --value-json '{"audience":"VC"}'
gd ui respond <runId> <surface-id> --skip          # triggers onTimeout='skip'
gd ui revoke  <projectId> <surface-id>             # cross-conversation revoke
```
...
```bash
gd ui prefill <projectId> --plugin <pluginId> --json '{"figma-oauth":"<token>","direction-pick":"editorial"}'
```
...
| Shared state | `genui_surfaces` table + `genui_state_synced` event | Map persisted GD state onto AG-UI's `state` channel for external consumers |
| Frontend SDK compatibility | GD desktop / web with built-in renderer | `@galyarder-design/agui-adapter` lets CopilotKit / other AG-UI clients consume an GD run unchanged |

The adapter is an interoperability surface, not the internal UI source of truth. GD should not add CopilotKit as a required product dependency unless a separate external embed/demo/client explicitly needs it. v1 plugins need no change to be consumable inside the AG-UI ecosystem because the adapter is a projection of OD's own events.
...
- `parsers/manifest.ts` — read `galyarder-design.json` → `PluginManifest` (Zod-validated).
...
- `merge.ts` — merge sidecar + adapters with `galyarder-design.json` winning; foreign content lands in `compat.*`.
- `resolve.ts` — resolve `od.context.*` refs against the registry → `ResolvedContext`.
- `validate.ts` — JSON Schema (drives both runtime checks and `gd plugin doctor`).
...
| New `apps/daemon/src/plugins/snapshots.ts` | §8.2.1 immutable snapshot read/write; `status='stale'` flips driven by `gd plugin doctor`; provides the replay helper backing `POST /api/runs/:runId/replay`. |
| New `apps/daemon/src/plugins/pipeline.ts` | Parses `od.pipeline` (including the `until` expression evaluator), schedules stages, and drives §10.2 devloop (with `GD_MAX_DEVLOOP_ITERATIONS` ceiling and break signaling). |
...
CREATE TABLE installed_plugins (
  ...
  manifest_json        TEXT NOT NULL,    -- cached galyarder-design.json (or synthesized)
...
CREATE TABLE applied_plugin_snapshots (
  ...
  status                  TEXT NOT NULL DEFAULT 'fresh', -- fresh | stale (set by `gd plugin doctor` after upgrade)
  applied_at              INTEGER NOT NULL,
  expires_at              INTEGER,                       -- NULL when referenced by any run / conversation / project (pinned forever); otherwise applied_at + GD_SNAPSHOT_UNREFERENCED_TTL_DAYS at insert time. GC worker (Phase 5) deletes rows where expires_at <= now()
  FOREIGN KEY (project_id)      REFERENCES projects(id)      ON DELETE CASCADE,
  FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE SET NULL,
  FOREIGN KEY (run_id)          REFERENCES runs(id)          ON DELETE SET NULL
);
...
-- GD primary keys unchanged; backwards compatible.
...
Migrations are additive only; existing `projects` / `runs` / `conversations` column semantics are untouched. The daemon writes per the schema at install / apply / run start / stage end. `gd plugin doctor` flips affected snapshots to `status='stale'` after a plugin upgrade by comparing `manifest_source_digest`, but **never** deletes or rewrites a snapshot row — historical reproducibility wins over storage cost.
...
| Method | Path | Purpose |
| --- | --- | --- |
| GET | `/api/applied-plugins/:snapshotId` | read an immutable snapshot; used for audit, replay, `gd plugin export` |
...
- Every `gd plugin export` / `od files upload --to <target>` / `od deploy ...` appends an `exportTargets` / `deployTargets` row but **never** mutates `sourcePluginSnapshotId`.
...
GD runs in three operating modes that share **one** daemon, **one** CLI, and **one** plugin runtime. The differences are purely presentational. This is the same shape Cursor uses: `cursor-agent` (CLI) is sufficient on its own; the IDE is sugar.
...
| **Desktop**         | Daemon + web bundle + electron shell                   | Full product experience (today's default)            | `gd` (current default, unchanged)                |
...
- **Marketplace browsing** — UI: grid + filters + previews. CLI: `gd plugin list/info`, `od marketplace search`, `gd plugin info <id> --json` returns the same manifest the UI renders.
- **Plugin apply** — UI: click card, chips and inputs hydrate in place. CLI: `gd plugin apply <id> --json` returns the identical `ApplyResult`.
- **Run streaming** — UI: chat bubbles, todo list, progress chrome. CLI: `gd run start --follow` emits ND-JSON events from the same `PersistedAgentEvent` discriminated union the UI consumes.
- **Direction picker / question form** — UI: rendered as inline cards. CLI: emitted as structured events on stdout; agent or scripted wrapper picks an option by writing to stdin or via `gd run respond <runId> --json '{...}'`.
...
- A user with **only Claude Code** (or any code agent) plus `npm i -g @galyarder-design/cli` plus a running headless daemon can do the entire user journey: install plugin → create project → run → consume artifacts. No GD desktop required.
- The GD desktop UI installs the same daemon and the same CLI; it just adds a window. Users who later install the desktop find the same projects, plugins, and history that the headless flow produced — there is no "headless project format" vs. "desktop project format". Same `.od/projects/<id>/`, same SQLite db.
- CI is a first-class citizen: a GitHub Action can `npm i -g @galyarder-design/cli && od daemon start --headless && gd plugin install … && gd run start --project … --follow`. No display, no electron, no per-step UI scripting.
- External products can embed GD by spawning a headless daemon and shelling out — `gd` is the public surface, internals are free to evolve.
...
GD already has **two** `composeSystemPrompt()` implementations:
...
The consequence: of the three consumption modes in §14.2 (skill-only / headless GD / full OD), only the latter two ever carry plugin context in v1 — consistent with §1, where a plugin is a wrapper around a long-running task that needs the daemon (or its headless equivalent) to assemble. Phase 4 closes that gap if and when fallback-mode plugin support is wanted.

This also answers the "no plugin selected" path: a run without an applied plugin still receives the base GD prompt stack (`DISCOVERY_AND_PHILOSOPHY`, the official designer instructions, project metadata, active design system/craft when present, and any default scenario routing chosen by the entry point). Plugin context is additive. Selecting a plugin adds the snapshot-derived `## Active plugin`, `## Plugin inputs`, and active-stage atom blocks; it does not turn a non-design agent into a design agent from scratch.

## 12. CLI surface

The CLI (`od …`) is **the canonical agent-facing API** for Galyarder Design. Plugin verbs are one slice of it; the rest of the CLI wraps the daemon's core capabilities — projects, conversations, runs, file operations, design library introspection, daemon control — so that any code agent can drive GD end-to-end through shell calls. This is the "natural-language project + task creation through CLI" path: a code agent reads a user's request, then issues a sequence of `od …` calls instead of speaking HTTP.
...
| MCP stdio             | MCP-aware agents (Claude Code, Cursor, etc.)          | `gd mcp` and `gd mcp live-artifacts` (existing)                  |

When a new capability ships, the CLI subcommand is the primary contract. The HTTP route exists to back the CLI; the MCP server exposes a curated subset of CLI subcommands as tools. Versioning: subcommand names, argument names, and `--json` schemas are governed by `packages/contracts` and tested in CI; breaking changes follow a major-version bump of the `gd` bin.
...
```
gd project create [--name "<title>"] [--skill <id>] [--design-system <id>]
                  [--plugin <id>] [--input k=v ...] [--brief "<text>"]
                  [--metadata-json <path|->] [--json]
gd project list   [--json]
gd project info   <id> [--json]
gd project delete <id>
gd project import <path> [--name "<title>"] [--json]   # wraps existing /api/import/folder
gd project open   <id>                                 # opens browser at /projects/<id>
```

Result of `gd project create --json`:
...
```
gd run start --project <projectId> [--conversation <conversationId>]
             [--message "<text>"] [--plugin <pluginId>] [--input k=v ...]
             [--agent claude|codex|gemini] [--model <id>] [--reasoning <level>]
             [--attachments <relpath,...>] [--follow] [--json]

gd run watch  <runId>                # ND-JSON SSE-equivalent events on stdout
gd run cancel <runId>
gd run list   [--project <id>] [--status running|done|failed] [--json]
gd run logs   <runId>                # historical tail; --since for incremental
```
...
```
gd plugin install   <source>                            # github: | https://… | ./folder | <name from marketplace>
gd plugin uninstall <id>
gd plugin list      [--kind skill|scenario|atom|bundle] [--trust trusted|restricted] [--json]
gd plugin info      <id> [--json]
gd plugin update    [<id>]
gd plugin trust     <id> [--caps fs:write,mcp,bash,hooks]   # permanent grant; persisted on installed_plugins
gd plugin apply     <id> --project <projectId> [--input k=v ...] [--grant-caps fs:read,mcp ...] [--json]
                                                            # returns ApplyResult; pure (no run)
                                                            # --grant-caps: scoped to this apply's snapshot only
gd plugin run       <id> --project <projectId> [--input k=v ...] [--grant-caps ...] [--follow] [--json]
                                                            # shorthand: apply + run start --follow
gd plugin replay    <runId> [--follow] [--json]             # rerun the long-task using the run's applied_plugin_snapshot
gd plugin export    <projectId> --as od|claude-plugin|agent-skill --out <dir>
gd plugin doctor    <id>
gd plugin scaffold
```
...
```
gd ui list      [--run <runId> | --project <projectId>] [--status pending|resolved|timeout|invalidated] [--json]
gd ui show      <runId> <surface-id> [--json]                 # surface kind / schema / prompt
gd ui respond   <runId> <surface-id> --value-json '{...}'     # write answer, unblock the run
gd ui respond   <runId> <surface-id> --skip                   # trigger onTimeout='skip'
gd ui revoke    <projectId> <surface-id>                      # invalidate persisted answer (e.g. OAuth logout)
gd ui prefill   <projectId> --plugin <pluginId> --json '{ "<surface-id>": <value>, ... }'
...
od daemon start  [--headless] [--serve-web] [--port <n>] [--host <h>]
                                                           ...
                                                           # default `gd` (no args) keeps current behavior
od daemon stop   [--daemon-url <url>]
od daemon status [--json]                                   # alias of `gd status`
gd status        [--json]                                   # daemon up? port? installed plugins count
od doctor                                                   # diagnostics: skills/DS/craft/plugins, providers, MCP
od version       [--json]
gd config get|set|list|unset  [--key ...] [--value ...]     # backed by media-config.json + db
```

`od daemon start --headless` is the entry for the headless mode in §11.7 (no web bundle, no electron). `od daemon start --serve-web` adds the local web UI without electron. Both keep using the existing tools-dev port conventions ([`GD_PORT`, `GD_WEB_PORT`](../AGENTS.md)).
...
od tools connectors  ...
gd mcp                       # stdio MCP server
gd mcp live-artifacts        # specialized MCP server
```
...
| --- | --- | --- | --- |
| 64 | Daemon not running | `gd status`, then start daemon | `{ host, port }` |
| 65 | Plugin not found / not installed | `gd plugin list` then `gd plugin install <source>` | `{ pluginId, candidateSources[] }` |
| 66 | Plugin restricted, capability required | `gd plugin trust <id> --caps …` or retry with `--grant-caps …` | `{ pluginId, pluginVersion, required[], granted[], remediation[] }` |
| 67 | Required input missing on apply | re-run with `--input k=v` for each missing field | `{ pluginId, missing[], schema }` |
| 68 | Project not found | `gd project list` | `{ projectId }` |
| 69 | Run not found / already terminal | `gd run list --project <id>` | `{ runId, status }` |
| 70 | Provider not configured | `gd config set ...` for the provider key | `{ provider, requiredKeys[] }` |
| 71 | Plugin requires daemon mode | start daemon or switch to desktop / headless | `{ pluginId, mode: 'api-fallback' }` (§11.8) |
| 72 | Applied plugin snapshot stale | `gd plugin replay <runId>` or `gd plugin upgrade <id>` then re-apply | `{ snapshotId, pluginId, currentVersion, snapshotVersion }` |
| 73 | GenUI surface awaiting response | inspect via `gd ui show <runId> <surface-id>` then answer with `gd ui respond`, or `gd ui prefill` before the run | `{ runId, surfaceId, kind, schema, prompt, persist, timeoutAt }` (§10.3) |
...
A code agent driving Galyarder Design through the CLI typically does:
...
PID=$(gd project create --skill blog-post --design-system linear-clone --json | jq -r .projectId)

# 3. Apply a plugin to preview the brief and context (pure; no run yet).
gd plugin apply make-a-deck --project "$PID" --input topic="agentic design" --input audience=VC --json

# 4. Start the run, follow events live (ND-JSON on stdout).
gd run start --project "$PID" --plugin make-a-deck \
...
Every group above is additive to [`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts). The current default `gd` (start daemon + open web UI) remains unchanged. Existing `gd media`, `gd research`, `od tools`, `gd mcp` commands keep their exact contracts. The new groups are wrappers around endpoints that already exist in `apps/daemon/src/server.ts` for the ones the desktop UI uses today (project create/list, run start/watch, file upload/list), plus the new endpoints from §11.5 for plugins/marketplace/atoms.
...
## 13. Public web surface (galyarder-design.dev/marketplace)

The product site already lives at [galyarder-design.dev](https://galyarder-design.dev). The public marketplace ships as a path on that same site — `galyarder-design.dev/marketplace` (canonical) with `galyarder-design.dev/plugins` as an alias — not as a separate domain. It is a static-rendered catalog rendered from the official `galyarder-design-marketplace.json` index, with plugin detail pages backed by the same `galyarder-design.json` files inside each listed repo. Visually it mirrors what [`skills.sh`](https://skills.sh/) does for skills, but its detail pages render OD-specific previews (the `od.preview.entry` HTML, sample outputs, the use-case query, the chip preview).

The site shares one source of truth with the in-app marketplace:

- Same JSON Schemas (`https://galyarder-design.dev/schemas/plugin.v1.json`, `https://galyarder-design.dev/schemas/marketplace.v1.json`).
- Same federated listing format (`galyarder-design-marketplace.json`).
- Same plugin manifests (`galyarder-design.json` inside each repo).

Two consumption surfaces, one substrate:

| Surface                                                | Audience                       | Primary CTA                                                                                                       |
| ------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| In-app marketplace (`/marketplace`, §11.6)             | Logged-in GD users             | "Use this plugin" → applies in place                                                                              |
| Public marketplace (`galyarder-design.dev/marketplace`)      | Anonymous visitors, SEO, share | Deep-link `od://plugins/<id>?apply=1` (auto-installs and applies in the desktop app), plus "Copy install command" |
...
The desktop app registers the `od://` URL scheme; clicking a button on `galyarder-design.dev/marketplace` either launches the desktop or, if it is not installed, falls back to a "How to install Galyarder Design" flow.
...
| `galyarder-design-marketplace.json`                                                                   | entry referencing `github:owner/repo`                                      | `galyarder-design.json` enriches the listing                |

### 14.1 Author tooling

- `gd plugin scaffold` — writes a starter folder containing both `SKILL.md` (industry-standard, with `od:` frontmatter for backward compat) and `galyarder-design.json` (GD enrichment with `compat.agentSkills` pointing at the SKILL.md).
- `gd plugin doctor` — runs the JSON Schema, the SKILL.md frontmatter parser, and a "does this look listable on awesome-agent-skills / clawhub / skills.sh?" lint that checks for README presence, license file, and frontmatter completeness.
- `gd plugin publish --to <catalog>` (Phase 4) — opens a browser to the catalog's PR template with a pre-filled row.

### 14.2 Cross-agent consumption

Any code agent that consumes a folder via `SKILL.md` works without GD installed. The plugin is one repo with three valid consumption modes:

1. **Skill-only consumption (no OD).** A Cursor user runs `npx skills add galyarder-design/make-a-deck`. Cursor reads `SKILL.md` and runs the workflow. No GD CLI, no GD daemon. The plugin's marketplace polish (`galyarder-design.json`) is ignored — Cursor sees a vanilla skill.
2. **Headless GD (CLI + code agent, no GD UI).** A power user keeps using their preferred code agent — Claude Code, Cursor, Codex, etc. — but adds GD as a side service to gain plugin context resolution, project bookkeeping, design library injection, and artifact tracking. No browser, no electron. See §14.3 below for the concrete pipeline.
3. **Full GD (CLI + code agent + GD UI).** Same as (2) plus the desktop or web UI for live preview, marketplace browsing, chat/canvas split-view, etc.

The plugin author writes the SKILL.md once. All three modes consume it.

### 14.3 Concrete headless pipeline (Claude Code + `gd` CLI, no GD UI)

This mirrors what cursor-agent + scripts can do for Cursor — code agent does the thinking, GD CLI provides the project / plugin / artifact substrate.

```bash
# One-time setup: install the GD CLI as an npm global (publishable as @galyarder-design/cli).
npm install -g @galyarder-design/cli

# Start the daemon in headless mode — no web bundle, no electron, no browser.
od daemon start --headless --port 17456

# Install the GD plugin you want to drive (or an upstream agent skill — both work).
gd plugin install github:galyarder-design/plugins/make-a-deck

# Create a project bound to the plugin. Inputs are templated into the brief.
PID=$(gd project create \
        --plugin make-a-deck \
...
# Path A — let `gd` orchestrate Claude Code as the run's agent:
gd run start --project "$PID" --plugin make-a-deck \
             --agent claude --follow

# Path B — drive Claude Code directly inside the project cwd; GD only provides
# context resolution and artifact bookkeeping. Useful when the user's existing
# code-agent setup is opinionated.
CWD=$(gd project info "$PID" --json | jq -r .cwd)
cd "$CWD"
# GD has already staged the merged SKILL.md / DESIGN.md / craft / atoms into
...
- The GD daemon does not need to render anything; it acts as a project + plugin + artifact server.
- The same project, when later opened in the GD desktop UI, shows the full conversation history, files, and artifacts produced by the headless run — because there is exactly one storage layer (§4.6 in [`spec.md`](spec.md), `.od/projects/<id>/` + SQLite).

### 14.4 Analogy: Cursor vs `cursor-agent`, GD desktop vs `gd` CLI

The mental model:

| Layer                 | Cursor                                       | Galyarder Design                                          |
| --------------------- | -------------------------------------------- | ---------------------------------------------------- |
| Headless agent CLI    | `cursor-agent` (drives the agent loop)       | `gd run start --agent claude --follow` + `gd plugin run` |
| Local services / db   | Cursor's background indexing / state         | GD daemon, SQLite, `.od/projects/<id>/`              |
| GUI productivity layer| Cursor IDE                                   | GD desktop / web UI (`apps/web` + `apps/desktop`)    |
| Plugin / skill format | `.cursor/rules/`, MCP servers                | `SKILL.md` + `galyarder-design.json` + atoms              |
...
GD ships as a single multi-arch Docker image so the full plugin/marketplace system can be brought up with one command and run unchanged on every major cloud. This is the substrate for the ecosystem and commercial story: a partner self-hosts inside their VPC; an enterprise runs a private marketplace; a CI job spins up a per-job GD daemon. The image is the headless mode of §11.7 packaged for ops, optionally serving the web UI from §11.6 when `--serve-web` is set.

### 15.1 Image shape

- **Tag**: `ghcr.io/galyarder-design/od:<version>` plus moving `:latest` and `:edge`.
...
- **Contents**:
  ...
  - Bundled code-agent CLIs that GD supports as agent backends: Claude Code, Codex CLI, Gemini CLI. Selectable per run; default is `GD_AGENT_BACKEND`.
...
Three paths the operator should mount as volumes; they map onto existing GD env vars from the root [`AGENTS.md`](../AGENTS.md), so no daemon code change is needed.

| Mount path        | Env var                  | Purpose                                                |
| ----------------- | ------------------------ | ------------------------------------------------------ |
| `/data/od`        | `GD_DATA_DIR`            | Projects, SQLite, artifacts, installed plugins (`<GD_DATA_DIR>/plugins`) |
| `/data/config`    | `GD_MEDIA_CONFIG_DIR`    | Provider credentials (`media-config.json`)             |
| `/data/marketplaces` | (under `GD_DATA_DIR`)  | Cached marketplace indexes                             |
...
```env
GD_PORT=17456
GD_BIND_HOST=0.0.0.0                 # the variable the daemon already reads ([`apps/daemon/src/server.ts`](../apps/daemon/src/server.ts))
GD_DATA_DIR=/data/od
GD_MEDIA_CONFIG_DIR=/data/config
GD_TRUST_DEFAULT=restricted          # safe default for hosted (§9) — introduced in Phase 5
GD_AGENT_BACKEND=claude              # default code agent backend
GD_API_TOKEN=<random>                # required when GD_BIND_HOST != 127.0.0.1 — Phase 5 introduces the bearer middleware
GD_SNAPSHOT_UNREFERENCED_TTL_DAYS=30 # see §11.4: unreferenced applied_plugin_snapshots expire after this window; set to 0 to keep forever
GD_SNAPSHOT_RETENTION_DAYS=          # opt-in (default unset): also retire referenced snapshots once their run/conversation/project is terminal and applied_at is older than the window
GD_SNAPSHOT_GC_INTERVAL_MS=21600000  # snapshot GC worker tick (Phase 5)
ANTHROPIC_API_KEY=...                # provider keys; also storable via `gd config set`
TAVILY_API_KEY=...
```

> **Current implementation vs. spec (migration note):**
>
> - `GD_BIND_HOST` already exists in the daemon ([`apps/daemon/src/server.ts`](../apps/daemon/src/server.ts), [`apps/daemon/src/origin-validation.ts`](../apps/daemon/src/origin-validation.ts)). Earlier draft text referred to the same variable as `GD_HOST`; the correct name is `GD_BIND_HOST`, and this spec uses it everywhere. **No `GD_HOST` alias is introduced** — that would invite double-name drift.
> - `GD_TRUST_DEFAULT`, `GD_API_TOKEN`, and the corresponding bearer-token middleware are **not yet implemented**; they are part of Phase 5 "Cloud deployment + pluggable storage" (§15.7, §16 Phase 5). Until landed, hosted deployments must rely on a reverse proxy / network ACL for access control; §15.7 explicitly calls out this prerequisite.

Anything settable via the desktop UI is also settable via `docker exec od gd config set ...` or by mounting a pre-baked `media-config.json` into `/data/config`.
...
```bash
docker run --rm -p 17456:17456 ghcr.io/galyarder-design/od:latest
...
docker run -d --name od \
  ...
  -e GD_DATA_DIR=/data/od \
  -e GD_MEDIA_CONFIG_DIR=/data/config \
  -e GD_BIND_HOST=0.0.0.0 \
  -e GD_API_TOKEN="$(openssl rand -hex 32)" \
  -e ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" \
  ghcr.io/galyarder-design/od:latest
```

Reach the same surfaces inside the container:

```bash
docker exec od gd plugin install github:galyarder-design/plugins/make-a-deck
docker exec od gd project create --plugin make-a-deck --json
docker exec od gd status --json
```
...
- `GD_TRUST_DEFAULT=restricted` is the recommended default. Capabilities (`mcp`, `subprocess`, `bash`, `network`) require explicit operator opt-in via `gd plugin trust <id>` or a `GD_TRUSTED_PLUGINS` allow-list env var.
- The image runs as a non-root user; plugin sandboxes inherit this.
- The HTTP API listens on `GD_BIND_HOST`; when set to `0.0.0.0`, **once Phase 5 lands** `GD_API_TOKEN` is required and is checked on every request via `Authorization: Bearer <token>`. When unset, the daemon refuses to bind to a public interface and exits with an error. Before Phase 5 (i.e., today's implementation), hosted deployments must isolate the daemon port behind a reverse proxy / network ACL.
...
- **Authentication scope (v1):** single-tenant only — one shared `GD_API_TOKEN`. **Multi-tenant auth** (per-user OAuth, RBAC, project ownership, billing) is **explicitly out of scope for v1** and tracked as an open item in §18.

### 15.8 What this unlocks (ecosystem motions)

1. **Self-hosted enterprise.** A company hosts a private GD instance, registers an internal `galyarder-design-marketplace.json` (`od marketplace add https://internal/...`), restricting plugins to internally vetted ones. Their designers and PMs use the desktop client locally; their CI uses `docker exec od od …`.
2. **Partner integrations.** Vendors (CMS, design tools, BI platforms, SaaS dashboards) embed GD inside their stack to add design generation. One image, no per-vendor port.
3. **Cloud-native CI.** "Generate slides for the daily report" becomes a GitHub Action / GitLab pipeline / Tekton task that spins up an ephemeral GD container, applies a plugin, drops artifacts to S3 / OSS / COS / OBS.
4. **Sovereign-cloud reach.** GD runs unchanged on Aliyun / Tencent / Huawei for customers in regulated regions — no rewrite, no separate distribution channel.
...
- JSON Schemas at `docs/schemas/galyarder-design.plugin.v1.json` and `galyarder-design.marketplace.v1.json`.
...
Validation: `pnpm guard`, `pnpm typecheck`, `pnpm --filter @galyarder-design/contracts test`.
...
- **Plugin CLI verbs:** `gd plugin install/list/info/uninstall/apply/doctor`. `gd plugin apply --json` is required by Phase 2's inline rail and by external code agents, and must already return an `ApplyResult` containing `appliedPlugin: AppliedPluginSnapshot`.
- **Headless MVP CLI loop (newly pulled forward):** `gd project create/list/info`, `gd run start/watch/cancel` (with `--follow` and ND-JSON streaming), `od files list/read`. These wrap endpoints already used by the desktop UI today (`POST /api/projects`, `POST /api/runs`, `GET /api/runs/:id/events`, project list/read endpoints) — no new HTTP surface, only CLI surface.
- `<daemonDataDir>/plugins/<id>/` write path with safe extraction (path-traversal guard, size cap, symlink rejection).

Validation:

- `pnpm --filter @galyarder-design/plugin-runtime test` (parser fixtures: pure SKILL.md, pure claude plugin, metadata-only galyarder-design.json, all three combined, SKILL frontmatter mapping).
- `pnpm --filter @galyarder-design/daemon test`. `pnpm guard`, `pnpm typecheck`.
- **End-to-end headless smoke** (equivalent to the §12.5 walkthrough): `gd plugin install ./fixtures/sample-plugin` → `gd project create --plugin <id> --json` → `gd run start --project <pid> --plugin <id> --follow` → `od files read <pid> <artifact>`. The produced artifact bytes must match exactly what the same plugin produces under the Phase 2A UI flow.
- **Apply purity smoke:** after `gd plugin apply <id>` followed by cancel-before-send, the project cwd is empty of staged assets, no `.mcp.json` is generated, but the `applied_plugin_snapshots` row exists (unreferenced from any run/project).
...
- **§10.3 GenUI minimum surface set lands:**
  ...
  - CLI: `gd ui list/show/respond/revoke/prefill`; `gd run watch` ND-JSON includes `genui_*` events; exit code 73 wired in.
...
- **§9 connector capability gate lands:**
  ...
  - `gd plugin doctor` enforces: (a) every `od.connectors.required[].id` exists in `connectorService` catalog; (b) every `tools[]` is a subset of that connector's `allowedToolNames`; (c) when an `oauth-prompt` surface has `oauth.route='connector'`, its `oauth.connectorId` is one of the plugin's declared connectors.
  - Exit code 66 / `409 capabilities-required` `data.required` includes `connector:<id>` entries.
  - `gd plugin trust` and `gd plugin apply --grant-caps` accept the `connector:<id>` form (§9.1).
...
(c) `gd plugin replay <runId>` reproduces the **exact same prompt** even after the source plugin has been upgraded via `gd plugin update` (snapshot is immutable).

(d) In web API-fallback mode (GD daemon stopped, browser talking provider directly), the inline rail still renders plugin cards but clicking "Use" pops a daemon-required notice; resuming the daemon restores normal behavior.

(e) A plugin declares both `oauth-prompt` and `confirmation` surfaces: after conversation A completes them, conversation B (same project) re-applies the plugin. In the new run the `oauth-prompt` (`persist=project`) is served from cache; the `confirmation` (`persist=run`) re-asks. After `gd ui revoke`, the next run re-asks the `oauth-prompt`.
...
(f) **Connector trust gate.** A local plugin declares `od.connectors.required = [{ id: 'slack', tools: ['channels.list'] }]` but **does not** explicitly grant `connector:slack`:
  ...
  - `apply` (no `--grant-caps`) → exit 66 / 409 with `data.required` including `connector:slack` and `data.remediation` listing `gd plugin trust <id> --caps connector:slack`.
...
- `gd project delete/import`, `gd run list/logs --since`, `od files write/upload/delete/diff`, `od conversation list/new/info`.
- These wrap existing endpoints; no new HTTP surface.

Validation: extend the §12.5 walkthrough — `gd project import` an external folder → `gd plugin apply` → `gd run replay <runId>` reruns on top of the imported project.

### Phase 3 — Federated marketplace + tiered trust (3–5 days)

- `od marketplace add/remove/trust/untrust/list/refresh`; `gd plugin install <name>` resolves through marketplaces.
...
- `gd plugin doctor <id>` runs full validation.
...
- `gd plugin export <projectId> --as od|claude-plugin|agent-skill` — generates a publish-ready folder from an existing project.
- `gd plugin run <id> --input k=v --follow` — shorthand wrapper for apply + run start + watch.
- `gd plugin scaffold` interactive starter.
- `gd plugin publish --to anthropics-skills|awesome-agent-skills|clawhub` opens a PR template.
- **Remaining CLI parity:** `od conversation list/new/info`, `od skills/design-systems/craft/atoms list/show`, `gd status/doctor/version`, `gd config get/set/list`, `od marketplace search`. All purely CLI work — endpoints exist or are trivial.
- Optional: extract atoms into `skills/_official/<atom>/SKILL.md`. Only after Phases 1–3 are stable.
- **§10.3.5 full AG-UI alignment:**
  - New package `@galyarder-design/agui-adapter` — bidirectionally maps OD's `PersistedAgentEvent` + `GenUIEvent` onto AG-UI canonical events (`agent.message`, `tool_call`, `state_update`, `ui.surface_requested`, `ui.surface_responded`).
  - Daemon adds an optional `/api/runs/:runId/agui` SSE endpoint that emits AG-UI canonical events so CopilotKit / other AG-UI clients can consume an GD run unchanged.
...
- **Bound-API-token guard (new in Phase 5):** daemon refuses to bind `GD_BIND_HOST=0.0.0.0` without `GD_API_TOKEN`; bearer-token middleware on `/api/*` (skipped only when host is loopback).
...
- **`AppliedPluginSnapshot` retention enforcement worker:** the `expires_at` column added in Phase 1 is now enforced. A daemon background job (default every 6 h, knob `GD_SNAPSHOT_GC_INTERVAL_MS`) deletes rows where `expires_at IS NOT NULL AND expires_at <= now()`. Unreferenced snapshots get `expires_at = applied_at + GD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default `30`; `0` disables) at insert time; referenced snapshots stay `NULL` (pinned per §8.2.1). Operators may set `GD_SNAPSHOT_RETENTION_DAYS` to additionally retire referenced rows once their referencing run/conversation/project is terminal. Each deletion writes an audit log entry. CLI escape hatch: `gd plugin snapshots prune --before <ts>` for forced cleanup.
- **Per-cloud one-click templates** in a separate `galyarder-design/deploy` repo (CloudFormation, Deployment Manager, ARM, Aliyun ROS, Tencent TIC, Huawei RFS) — non-blocking; track separately.

Validation:

- `docker run` smoke: image starts, web UI renders, `gd plugin install` works inside the container.
...
GD reads it as a plugin via the existing `od:` frontmatter loader documented in [`skills-protocol.md`](skills-protocol.md). No `galyarder-design.json` needed — the plugin lacks marketplace polish but is fully runnable.
...
### 17.2 Enriched plugin (cross-catalog publishable, full GD product surface)
...
`SKILL.md` stays portable — Cursor / Codex / OpenClaw read it directly. `galyarder-design.json` adds preview, query, chip strip, capabilities. The repo lists cleanly on every catalog in §14 without modification.
...
├── galyarder-design.json                  # kind: 'bundle'; lists nested skills
├── skills/
...
| Risk                                                        | Mitigation                                                                                          |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Schema drift between GD plugin and the broader skill spec   | `galyarder-design.json` is sidecar-only; it never modifies SKILL.md. CI tests run against the public anthropics/skills repo. |
...
| Atoms-as-plugins is large                                   | Entry slice shipped: bundled atom SKILL.md bodies + `renderActiveStageBlock()` exist, while the base GD designer/discovery prompt remains in daemon code until the remaining §23 migration is complete. |
| Project-local plugins committed to user repos | Discovery only at `<projectCwd>/.galyarder-design/plugins/`; opt-in via `gd plugin install --project`. |
...
| Plugins shipping their own MCP servers may fail to start | `gd plugin doctor` runs a dry-launch of declared MCP commands; failures surfaced before "Use". |
| Unbounded growth of `applied_plugin_snapshots` | Per PB2 (resolved): unreferenced snapshots auto-expire at `applied_at + GD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default 30 d); referenced snapshots stay pinned forever (reproducibility wins); GC worker lands in Phase 5 (§16). `gd plugin snapshots prune --before <ts>` remains as a forced-cleanup escape hatch; rows with `status='stale'` can be archived to external storage in batch. |
...
| `od.pipeline` devloop infinite loop burning quota | `until` is required and uses a restricted syntax; `GD_MAX_DEVLOOP_ITERATIONS` ceiling (default 10); both UI and CLI expose a "Stop refining" break action. |
| `GD_HOST` / `GD_BIND_HOST` naming drift | Spec uses the variable the daemon already reads, `GD_BIND_HOST`; no `GD_HOST` alias is introduced; §15.3 explicitly notes the deviation from earlier draft text. |
| Hosted deployments without the bound-API-token guard could leak the API publicly (pre-Phase 5 must rely on a reverse proxy) | Once Phase 5 lands, daemon refuses to bind `GD_BIND_HOST=0.0.0.0` without `GD_API_TOKEN`; bearer-token middleware enforced on `/api/*`; §15.3 / §15.7 record the current vs. target gap. |
...
| Multi-cloud testing matrix is large                         | Phase 5 ships a single canonical compose smoke (one cloud), then adds clouds incrementally; per-cloud one-click templates live in `galyarder-design/deploy` and can move at their own cadence (§15.5). |
| Malicious plugins phishing the user via GenUI surfaces      | `od.genui.surfaces[]` must be declared in the manifest and pass `gd plugin doctor`; runtime rejects undeclared surface kinds / surface ids; `oauth-prompt` and `confirmation` always show "from plugin <id>, vetted by marketplace <id>"; restricted plugins must explicitly grant `network` before raising an `oauth-prompt` (§9). |
| AG-UI ecosystem may evolve, drifting OD's wire format from canonical AG-UI | OD-native `GenUIEvent` remains the internal source of truth. `@galyarder-design/agui-adapter` is an external projection layer, so upstream protocol revs do not couple to the daemon or web renderer release cadence. |
| Cross-conversation reuse via `genui_surfaces` may make users "forget what they authorized" | The web `GenUIInbox` and `gd ui list --project <id>` must enumerate every `persist=project` resolved row with revoke entry points; hosted mode can default-expire via `GD_GENUI_PROJECT_TTL_DAYS`; revoke writes an audit log entry. |
...
- **`gd plugin run` headless contract** — sufficient as-is, or also expose an HTTP POST endpoint for non-CLI agents? (Default: CLI only in v1; HTTP added in Phase 4 if needed.)
- **Multi-tenant auth (per-user OAuth, RBAC, project ownership, billing)** is explicitly out of scope for v1. The Docker image is single-tenant by design (one `GD_API_TOKEN`). Multi-tenancy is a post-v1 story that needs its own spec — confirm this scoping is acceptable for the first ecosystem release.
- **Trust propagation in hosted mode** — current spec locks arbitrary GitHub / URL / local plugins to `restricted` by default, and third-party marketplaces do not propagate trust by default. Confirm whether hosted deployments may trust individual plugins through `GD_TRUSTED_PLUGINS`, or whether operators must first trust the source marketplace.
- **Discovery-time hot reload** — should the daemon watch `<daemonDataDir>/plugins/` for filesystem changes (developer ergonomics), or only reload after `gd plugin install/update/uninstall` (stability)? (Default: watch, with a 500ms debounce.)
- **Versioning policy** — pin to a tag/SHA on install, or always track the default branch with an opt-in pin? (Default: pin to the resolved ref at install time; `gd plugin update` re-resolves.)
...
- ~~**`AppliedPluginSnapshot` retention**~~ — **resolved (PB2, see `docs/plans/plugins-implementation.md` §7).** Snapshots referenced by any run / conversation / project stay pinned forever (`expires_at = NULL`); unreferenced snapshots get `expires_at = applied_at + GD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default `30`, `0` disables). The "expire even referenced rows" knob `GD_SNAPSHOT_RETENTION_DAYS` is operator-opt-in only (default unset), and applies only when the referencing row is terminal. The `expires_at` column lands in Phase 1 (§11.4); the GC worker lands in Phase 5 (§16). The `gd plugin snapshots prune` CLI remains as a forced-cleanup escape hatch.
...
- ~~**Should `od.genui.surfaces[].component` ship in v1?**~~ — **resolved as a gated extension path.** The manifest schema accepts the field and `gd plugin doctor` enforces `genui:custom-component` plus traversal guards. The built-in product renderer remains the default for `form` / `choice` / `confirmation` / `oauth-prompt`; custom components are sandboxed add-ons, not a replacement for core collaboration UI.
- **Coupling between GenUI persisted state and `AppliedPluginSnapshot`** — when a plugin upgrades and `surface.schema` changes, old rows auto-`invalidate`; should we additionally **force a re-apply** (generating a new `AppliedPluginSnapshot`) or allow the surface to invalidate while leaving the snapshot untouched? (Default: surface only; `gd plugin doctor` flags schema drift; replay still uses the old snapshot.)
- ~~**Timing of AG-UI protocol adoption**~~ — **resolved.** `@galyarder-design/agui-adapter` and `GET /api/runs/:runId/agui` have shipped as optional interoperability. OD-native GenUI remains the internal renderer and CopilotKit is not a required product dependency.

## 19. Why this is a meaningful step for Galyarder Design

- **Inherited supply.** Every public agent skill on `anthropics/skills`, `awesome-agent-skills`, `clawhub`, and `skills.sh` is one optional `galyarder-design.json` away from being an GD plugin — and reciprocally, every GD plugin is publishable to all four catalogs without modification.
...
- **CLI from day 1.** Every new endpoint has a matching `gd plugin …` subcommand, so the same surface is reachable from any code agent without the desktop app.
- **Marketplace-first product narrative.** From Phase 2 onward, the home screen becomes "input + chip strip + deep marketplace" — exactly the inversion described in the brief: the primary interaction is "input box + plugin community".
...
| `renderKind` | How GD previews or opens it | `html`, `jsx`, `pptx`, `markdown`, `video`, `image`, `diff`, `repo` |
...
The four "core agent-native design problems" the GD product targets, restated to make this section navigable:
...
| 4 | Design → deliverable production code | `tune-collab` (handoff side) | partial (§20.3 explicitly post-v1) | **not in v1** — v1 caps at `handoffKind: 'design-only' \| 'implementation-plan' \| 'patch'`; `'deployable-app'` is post-v1 | Either land §20.3 in full, or formalize the §14.3 GD ↔ external code-agent handoff as the v1 production-code path |
...
- Repo-aware multi-file diff orchestration with build evidence inside GD (scenario 4 native path).
...
- `token-map` atom — map the extracted tokens onto the active GD design system (`design-systems/<id>/DESIGN.md`). Spec reserves the id.
...
**v1 fallback (also documented in §21.5):** instead of running this end-to-end inside OD, point users at the §14.3 headless pipeline — GD generates a `html-prototype` or `implementation-plan` artifact; Cursor / Claude Code applies the patch in the user's repo cwd and runs build/test. v1 does not block this workflow; it just does not own the patch-application step.
...
- Plus repo-aware multi-file patch orchestration with build evidence captured inside GD (a meaningful step beyond §20.3 itself).
...
### 21.5 GD ↔ code-agent handoff as the v1 production-code path
...
1. **GD stages the design substrate into a project cwd.** Per §14.3, the daemon writes SKILL.md / DESIGN.md / craft into `.od-skills/` and generated artifacts into the project cwd via `od files`. The cwd is discoverable via `gd project info <id> --json | jq -r .cwd`.
2. **The user's code agent operates in that cwd or in their own repo cwd.** GD does not run inside the IDE; it runs as a daemon next to the IDE. Cursor / Claude Code / Codex / Gemini CLI are the patch-applying surface.
3. **Bookkeeping stays in OD.** `ArtifactManifest` (§11.5.1) records `sourcePluginSnapshotId`, `sourceTaskKind: 'tune-collab'` (or `'code-migration'` once Phase 7 lands), and `handoffKind: 'patch'`; `od files` tracks every artifact byte. Even when the code agent does the patch, GD remains the audit log.
4. **Re-entry into GD is single-step.** The user can reapply any plugin (or a different plugin) on top of the same project at any time via the inline rail (§8) or `gd plugin apply ... --project <id>`. `parentArtifactId` chaining (§11.5.1) preserves the lineage across the GD ↔ code-agent boundary.
...
| --- | --- | --- |
| Call a tool GD does not provide (Figma REST, AST parsing, SVG conversion, etc.) | Bundle an MCP server in `od.context.mcp[]` | §5 / §5.3 (`mcp` + `subprocess` + `network`) |
...
| Operate on the user's real repo | `gd project import <path>` brings the repo into OD's project model; `od files` and agent file ops then work in-place | §12 / §11.7 / §14.3 |
...
| Reproducible replay months later | `AppliedPluginSnapshot` immutable + `gd plugin replay` | §8.2.1 / §12 |
...
Pre-step the user runs: `gd project import /path/to/old-repo`. Project cwd is now the real repo.
...
#### 22.3.3 Design → production code via GD ↔ code-agent handoff
...
Three categories cover everything GD does today:
...
- `@galyarder-design/agui-adapter` and `/api/runs/:runId/agui` provide external AG-UI event projection without changing OD's internal GenUI renderer.

This is why §22 holds: the substrate is already self-hosting for plugin artifacts, snapshots, GenUI declarations, pipeline declarations, bundled scenarios, and the first atom-body injection path. The remaining hard-coded parts are narrower and more product-shaped: the base GD designer/discovery prompt, some stage-entry selection logic, Home's curated scenario rail, and the closed signal / surface vocabularies listed in §22.4.
...
- An atom plugin is a folder with `galyarder-design.json` (`od.kind: 'atom'`) plus `SKILL.md`.
...
- `plugins/_official/scenarios/od-new-generation/galyarder-design.json`
- `plugins/_official/scenarios/od-figma-migration/galyarder-design.json` (after Phase 6)
- `plugins/_official/scenarios/od-code-migration/galyarder-design.json` (after Phase 7)
- `plugins/_official/scenarios/od-tune-collab/galyarder-design.json`
...
| `trusted` | First-party / explicitly-trusted marketplace | No — auto-granted on install | Yes — `gd plugin update` may pull a newer version | `marketplace` |
...
3. `gd plugin uninstall` refuses to uninstall a `bundled` plugin (would break the daemon); `gd plugin update` is a no-op for bundled.
...
1. **Audit surface convergence.** Every byte that enters the agent system prompt is reachable from a plugin manifest + SKILL.md. Reading `plugins/_official/...` gives a complete answer to "what is GD telling the agent to do today?"; no codebase grep needed.
...
| --- | --- | --- |
| §21 | What does GD ship natively in v1? Which scenarios are covered, partial, or post-v1? | Maintainers planning the §16 phased rollout |
| §22 | Without waiting for GD to ship more native atoms, how far can a third-party plugin author go on the v1 substrate? | Plugin authors and integrators evaluating the platform |
