...
export function PluginMetaSections({ record, omit, compact, heading }: Props) {
  ...
  const query = resolvePluginQueryFallback(gd.useCase?.query);
  const inputs = (gd.inputs ?? []) as InputField[];
  const ctx = gd.context ?? {};
  const stages = gd.pipeline?.stages ?? [];
  const surfaces = gd.genui?.surfaces ?? [];
  const required = (gd.connectors?.required ?? []) as PluginConnectorRef[];
  const optional = (gd.connectors?.optional ?? []) as PluginConnectorRef[];
