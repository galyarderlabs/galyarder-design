...
describe('media-config OpenAI OAuth fallback', () => {
  ...
  const originalMediaConfigDir = process.env.GD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.GD_DATA_DIR;
  let homedirSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(async () => {
    homeDir = await mkdtemp(path.join(tmpdir(), 'gd-media-home-'));
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-media-project-'));
    process.env.HOME = homeDir;
    homedirSpy = vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
    for (const key of OPENAI_ENV_KEYS) {
      delete process.env[key];
    }
    delete process.env.GD_MEDIA_CONFIG_DIR;
    delete process.env.GD_DATA_DIR;
  });
  ...
  afterEach(async () => {
    ...
    if (originalMediaConfigDir == null) {
      ...
      process.env.GD_MEDIA_CONFIG_DIR = originalMediaConfigDir;
    }
    if (originalDataDir == null) {
      delete process.env.GD_DATA_DIR;
    } else {
      process.env.GD_DATA_DIR = originalDataDir;
    }
  ...
  async function writeStoredMediaConfig(data: unknown) {
    const file = path.join(projectRoot, '.gd', 'media-config.json');
    await mkdir(path.dirname(file), { recursive: true });
    await writeFile(file, JSON.stringify(data), 'utf8');
  }
  ...
  it('resolves Nano Banana env and stored model overrides', async () => {
    process.env.GD_NANOBANANA_API_KEY = 'env-nano-key';
    await writeStoredMediaConfig({
      providers: {
        nanobanana: {
    ...
    delete process.env.GD_NANOBANANA_API_KEY;
  });
  ...
  describe('GD_MEDIA_CONFIG_DIR / GD_DATA_DIR storage routing', () => {
    let overrideRoot: string;
    let originalMediaConfigDir: string | undefined;
    let originalDataDir: string | undefined;

    beforeEach(async () => {
      overrideRoot = await mkdtemp(path.join(tmpdir(), 'gd-media-override-'));
      originalMediaConfigDir = process.env.GD_MEDIA_CONFIG_DIR;
      originalDataDir = process.env.GD_DATA_DIR;
      delete process.env.GD_MEDIA_CONFIG_DIR;
      delete process.env.GD_DATA_DIR;
    });
    ...
    it('reads media-config.json from an absolute GD_MEDIA_CONFIG_DIR', async () => {
      process.env.GD_MEDIA_CONFIG_DIR = overrideRoot;
    ...
    it('expands a leading ~/ against the user home directory', async () => {
      ...
      const subdir = '.gd-test';
      process.env.GD_MEDIA_CONFIG_DIR = `~/${subdir}`;
    ...
    it('resolves a relative override against projectRoot, not process.cwd', async () => {
      ...
      // server.ts anchors GD_DATA_DIR.
      const relative = 'config/media';
      process.env.GD_MEDIA_CONFIG_DIR = relative;
    ...
    it('falls back to GD_DATA_DIR when GD_MEDIA_CONFIG_DIR is unset', async () => {
      // Packaged daemon (apps/packaged/src/sidecars.ts) and the
      // Home Manager / NixOS modules already set GD_DATA_DIR for the
      // rest of the daemon's runtime state. media-config should
      // co-locate there without needing a second env var.
      process.env.GD_DATA_DIR = overrideRoot;
      await writeProvidersAt(overrideRoot, {
        providers: {
          openai: {
            apiKey: 'datadir-key',
    ...
    it('GD_MEDIA_CONFIG_DIR takes precedence over GD_DATA_DIR', async () => {
      const dataDir = await mkdtemp(path.join(tmpdir(), 'gd-media-data-'));
      try {
        process.env.GD_DATA_DIR = dataDir;
        process.env.GD_MEDIA_CONFIG_DIR = overrideRoot;
        // Two competing files; only the GD_MEDIA_CONFIG_DIR one should
        // be read.
    ...
    it('writeConfig creates the override directory tree on first write', async () => {
      ...
      process.env.GD_MEDIA_CONFIG_DIR = target;
    ...
    // GD_DATA_DIR=$HOME/.galyarder-design (and ${HOME}/.galyarder-design) routes
    // both daemon runtime data AND media credentials to the same expanded
    // path. Without this, media-config.json was written under
    // <projectRoot>/$HOME/.galyarder-design and stored provider keys appeared
    // missing on the next read.
    it('expands $HOME/... in GD_DATA_DIR fallback so media-config co-locates with daemon data', async () => {
      const subdir = '.gd-test-home';
      process.env.GD_DATA_DIR = `$HOME/${subdir}`;
      const expandedDir = path.join(homeDir, subdir);
      await writeProvidersAt(expandedDir, {
        providers: {
          openai: {
    ...
    it('expands ${HOME}/... in GD_DATA_DIR fallback', async () => {
      const subdir = '.gd-test-braced';
      process.env.GD_DATA_DIR = `\${HOME}/${subdir}`;
      const expandedDir = path.join(homeDir, subdir);
      await writeProvidersAt(expandedDir, {
        providers: {
    ...
    it('expands $HOME/... in GD_MEDIA_CONFIG_DIR (explicit override path)', async () => {
      const subdir = '.gd-media-home';
      process.env.GD_MEDIA_CONFIG_DIR = `$HOME/${subdir}`;
      const expandedDir = path.join(homeDir, subdir);
      await writeProvidersAt(expandedDir, {
        providers: {
          openai: {
...
const GROK_ENV_KEYS = ['GD_GROK_API_KEY', 'XAI_API_KEY'];
...
describe('media-config Grok / xAI OAuth fallback', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(path.join(tmpdir(), 'gd-media-grok-home-'));
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-media-grok-project-'));
    process.env.HOME = homeDir;
    homedirSpy = vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
  ...
  async function writeOdXaiTokens(token: {
    ...
    const file = path.join(projectRoot, '.gd', 'xai-tokens.json');
    await mkdir(path.dirname(file), { recursive: true });
    await writeFile(
      file,
      JSON.stringify({
  ...
  it('uses GD-native xai-tokens.json when one is stored', async () => {
    await writeOdXaiTokens({
      accessToken: 'gd-bearer-1',
    ...
    expect(resolved.apiKey).toBe('gd-bearer-1');
  ...
  it('borrows the Hermes-side xai-oauth token when GD has no native creds', async () => {
    await writeHomeJson('.hermes/auth.json', {
      providers: {
        'xai-oauth': {
  ...
  it('prefers GD-native xai-tokens over Hermes borrowing', async () => {
    await writeOdXaiTokens({
      accessToken: 'gd-bearer-2',
      expiresAt: Date.now() + 3_600_000,
    });
    await writeHomeJson('.hermes/auth.json', {
      providers: {
  ...
  it('skips an GD-native token within the expiry skew when no refresh_token is stored', async () => {
...
describe('media-config model alias resolution (issue #1277)', () => {
  let projectRoot: string;
  const originalEnvAliases = process.env.GD_MEDIA_MODEL_ALIASES;
  const originalMediaConfigDir = process.env.GD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.GD_DATA_DIR;

  beforeEach(async () => {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-media-alias-'));
    delete process.env.GD_MEDIA_MODEL_ALIASES;
    delete process.env.GD_MEDIA_CONFIG_DIR;
    delete process.env.GD_DATA_DIR;
  });
  ...
  afterEach(async () => {
    if (originalEnvAliases == null) {
      ...
      process.env.GD_MEDIA_MODEL_ALIASES = originalEnvAliases;
    }
    if (originalMediaConfigDir == null) {
      delete process.env.GD_MEDIA_CONFIG_DIR;
    } else {
  ...
  it('redirects via the GD_MEDIA_MODEL_ALIASES env var', async () => {
    process.env.GD_MEDIA_MODEL_ALIASES = JSON.stringify({
  ...
  it('tolerates malformed env JSON and falls through to the stored map', async () => {
    // A user with a half-typed env var (`GD_MEDIA_MODEL_ALIASES='{'`)
    // should still get their on-disk aliases, not a hard error mid-
    // generation.
    process.env.GD_MEDIA_MODEL_ALIASES = '{not valid json';
    await writeStoredMediaConfig({
      providers: {},
      aliases: { 'doubao-seedream-3-0-t2i-250415': 'doubao-seedream-5-0' },
    });
  ...
  it('writeConfig preserves aliases when a Settings-style provider PUT lands', async () => {
    ...
    const onDisk = JSON.parse(
      await readFile(
        path.join(projectRoot, '.gd', 'media-config.json'),
        'utf8',
      ),
    );
...
describe('seedProviderIfMissing', () => {
  let projectRoot: string;
  const SENSEAUDIO_ENV_KEYS = ['GD_SENSEAUDIO_API_KEY', 'SENSEAUDIO_API_KEY'];
  const originalEnv = Object.fromEntries(
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-media-seed-'));
  ...
  it('no-ops when an env var resolves a key for the provider', async () => {
    process.env.GD_SENSEAUDIO_API_KEY = 'env-key';
    const wrote = await seedProviderIfMissing(projectRoot, 'senseaudio', {
      apiKey: 'sa-byok-key',
      baseUrl: 'https://api.senseaudio.cn',
    });
