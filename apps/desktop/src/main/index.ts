...
import { readProcessStamp } from "@galyarder-design/platform";
...
export const OS_LOCALE_PRELOAD_ARG_PREFIX = "--gd-os-locale=";
...
export type DesktopMainOptions = {
   ...
   * renderer URL (`gd://app/`) over `discoverWebUrl`, but Node-side
  ...
  const rebuild = () => {
    const template: MenuItemConstructorOptions[] = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: "Galyarder Design",
            click() {
              void shell.openExternal("https://github.com/galyarderlabs/galyarder-design");
            },
          },
          { type: "separator" },
...
export async function runDesktopMain(
  ...
  if (!registered) {
    console.warn(
      "[galyarder-design desktop] initial import-token handshake with daemon did not complete; " +
...
if (isDirectEntry()) {
  const stamp = readProcessStamp(process.argv.slice(2), GALYARDER_DESIGN_SIDECAR_CONTRACT);
