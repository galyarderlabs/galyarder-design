...
describe('xai-routes', () => {
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-xai-routes-'));
  ...
  it('POST /api/xai/search forwards bearer + x_search options to xAI Responses API and parses the response', async () => {
    ...
    const cfgPath = pathMod.join(projectRoot, '.gd', 'media-config.json');
...
describe('xai-routes — cross-origin guard', () => {
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-xai-routes-co-'));
    onCallbackHolder.current = null;
    startMock.mockClear();
    stopMock.mockClear();
