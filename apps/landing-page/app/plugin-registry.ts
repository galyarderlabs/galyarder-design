...
const RAW_REPO = 'https://raw.githubusercontent.com/galyarderlabs/galyarder-design/main';
const findRepoRoot = () => {
  const candidates = [
    process.cwd(),
...
const registryUrlFor = (registryId: string) =>
  `${RAW_REPO}/plugins/registry/${registryId}/galyarder-design-marketplace.json`;
...
const findManifestFiles = (dir: string): string[] => {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.isFile() && entry.name === 'galyarder-design.json') {
...
const entryFromMarketplace = (
  ...
  const description =
    ...
    'Agent-native Galyarder Design workflow packaged as a portable plugin.';
  ...
  return {
    ...
    detailHref,
    installCommand: `gd plugin install ${id}`,
    directInstallCommand: `gd plugin install ${source}`,
    tags,
    capabilities,
    publisher,
...
const loadRegistryEntries = (): PublicPluginEntry[] => {
  ...
  for (const dirent of readdirSync(REGISTRY_ROOT, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(REGISTRY_ROOT, registryId, 'galyarder-design-marketplace.json');
...
const officialEntryFromManifest = (manifestPath: string): PublicPluginEntry | undefined => {
  ...
  const id = `galyarder-design/${pluginName}`;
  const pluginDir = path.dirname(manifestPath);
  const repoPath = toPosix(path.relative(REPO_ROOT, pluginDir));
  const source = `github:galyarderlabs/galyarder-design@main/${repoPath}`;
  const gd = asRecord(manifest?.gd) as RawOdMetadata | undefined;
  const capabilities = asStringArray(gd?.capabilities);
  ...
  const description =
    ...
    'First-party Galyarder Design workflow packaged as a portable plugin.';
  const detailHref = detailHrefFor(id);
  const mode = asString(gd?.mode);
  const taskKind = asString(gd?.taskKind);
  const surface = asString(gd?.surface);
  const preview = previewFrom(pluginDir, id, gd?.preview);
  const exampleQuery = useCaseQuery(gd?.useCase);
