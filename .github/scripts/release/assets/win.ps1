...
$sourceInstaller = Join-Path $env:RUNNER_TEMP "tools-pack/out/win/namespaces/${env:TOOLS_PACK_NAMESPACE}/builder/Galyarder Design-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
if (!(Test-Path $sourceInstaller)) {
  throw "expected installer not found at $sourceInstaller"
}

$versionedInstaller = "galyarder-design-${env:RELEASE_VERSION}$assetSuffix-win-x64-setup.exe"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Galyarder Design ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
@(
