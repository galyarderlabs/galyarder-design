...
// instead of routing through the generic gd-new-generation router and
...
//
// Generic / catch-all kinds (template, other) keep gd-new-generation,
// which runs discovery → plan → generate → critique without a
// surface-specific seed. Media kinds keep gd-media-generation, which
...
//   1. `gd-*` scenarios (under `plugins/_official/scenarios/`) — generic
...
export const DEFAULT_UNSELECTED_SCENARIO_PLUGIN_ID =
  'gd-default' satisfies DefaultScenarioPluginId;
...
export const DEFAULT_SCENARIO_PLUGIN_BY_TASK_KIND: Record<TaskKind, DefaultScenarioPluginId> = {
  'new-generation':  'gd-new-generation',
  'figma-migration': 'gd-figma-migration',
  'code-migration':  'gd-code-migration',
  'tune-collab':     'gd-tune-collab',
};
