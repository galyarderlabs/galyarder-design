...
//   ---
//   name: User role
//   description: User is a senior FE engineer working on Galyarder Design.
//   type: user
//   ---
//
//   {markdown body}
...
// aim is to make `/记住 X` ("remember X" in Chinese) and `remember: X`
...
// gets a stable, human-readable label like `User identity` with body
// `- Identity / role: X` instead of three identical fields all set to
// the raw captured phrase ("a software engineer"). The regex captures
// `$1` once;
...
// other under one shared `User identity` slot.
...
const REMEMBER_PATTERNS: ExtractionPattern[] = [
  ...
  // Chinese-language patterns. Regex bodies stay in CJK (functional —
  // they match what users actually type in Chinese). The display
  // fields (`name`, `descriptionTemplate`, `bodyTemplate`) are
  // English so the rendered memory entry reads natively in the
  // settings UI.
  // Pattern: "记住:..." → "remember: ..." imperative.
  {
    re: /记住[:：\s]+([^\n。]{2,200})/,
    type: 'feedback',
    name: 'Remembered note',
  ...
  // Pattern: "我是 X" → "I am X" (role / identity).
  {
    re: /我是\s*([^\n。，]{2,80})/,
    type: 'user',
    name: 'User identity',
    descriptionTemplate: 'User identity/role: $1',
    bodyTemplate:
      '- Identity / role: $1\n\nWhen to apply: factor this background into examples, phrasing, and depth across every chat.',
  },
  // Pattern: "我喜欢 X" → "I like X" (preference).
  {
    re: /我喜欢\s*([^\n。，]{2,200})/,
    type: 'feedback',
    name: 'User preference',
    descriptionTemplate: 'User likes: $1',
    bodyTemplate:
      '- Preference: $1\n\nWhen to apply: prefer this when a relevant choice comes up.',
  },
  // Pattern: "我偏好 X" → "I prefer X" (preference, formal register).
  {
    re: /我偏好\s*([^\n。，]{2,200})/,
    type: 'feedback',
    name: 'User preference',
  ...
  // Pattern: "我在 / 我再 X" → "I'm in X" (location). [在再] tolerates a
  // common Chinese IME typo where "在" is mistyped as "再"; without
  // this branch the original "我再德国, ..." misses every pattern and
  // falls through to the LLM extractor.
  {
    re: /我[在再]\s*([^\n。，！？!?,]{2,80})/,
    type: 'user',
    name: 'User location',
  ...
  // Pattern: "我住在 X" → "I live in X" (location, residential).
  {
    re: /我住在\s*([^\n。，！？!?,]{2,80})/,
    type: 'user',
    name: 'User location',
  ...
  // Pattern: "我想 / 我希望 / 我打算 / 我计划 X" → long-running goals.
  // Unlike the imperative "记住", these phrases describe ongoing
  // intent that's tied to the project itself, so we bucket them as
  // `project`.
  {
    re: /我(?:想|希望|打算|计划)\s*([^\n。！？!?]{4,200})/,
    type: 'project',
  ...
  // Pattern: "备忘:..." → "memo: ..." short-form note.
  {
    re: /备忘[:：\s]+([^\n]{2,200})/,
    type: 'reference',
    name: 'Quick note',
