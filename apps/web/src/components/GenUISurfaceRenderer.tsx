...
import type { GenUISurfaceSpec } from '@galyarder-design/contracts';

export interface PendingSurface {
  // The surface descriptor as declared in `gd.genui.surfaces[]`.
...
export function GenUISurfaceRenderer(props: Props) {
  ...
  //     install time by `gd plugin doctor`; the renderer trusts the
  ...
  return (
    <div className="genui-surface genui-surface--component" role="dialog" aria-label={surfaceId}>
      <iframe
        ...
        style={{ width: '100%', minHeight: 320, border: '1px solid var(--gd-border, #ddd)' }}
      />
      {onSkip ? (
        <div className="genui-surface__actions">
          <button
