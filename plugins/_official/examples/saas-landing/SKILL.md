...
- Each editable element tagged with `data-gd-id="<unique-slug>"` so the host app's comment mode can target it.
...
- `data-gd-id` tagging is how we wire elements to comment mode. Skills that want comment-mode compatibility must annotate their output.
