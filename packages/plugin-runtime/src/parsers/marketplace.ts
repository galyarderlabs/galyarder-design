import { MarketplaceManifestSchema, type MarketplaceManifest } from '@galyarder-design/contracts';
...
export function parseMarketplace(raw: string): MarketplaceParseResult {
  ...
  try {
    ...
    return { ok: false, errors: [`galyarder-design-marketplace.json is not valid JSON: ${(err as Error).message}`] };
  }
