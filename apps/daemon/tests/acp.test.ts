...
test('ACP session params do not require MCP servers by default', () => {
  assert.deepEqual(buildAcpSessionNewParams('/tmp/gd-project'), {
    cwd: path.resolve('/tmp/gd-project'),
    mcpServers: [],
  });
});

test('ACP session params do not request global MCP config mutation', () => {
  const params = buildAcpSessionNewParams('/tmp/gd-project');
...
test('ACP session params normalize explicit MCP servers to ACP stdio shape', () => {
  const mcpServers = [{ name: 'galyarder-design-live-artifacts', command: 'gd', args: ['mcp', 'live-artifacts'] }];

  assert.deepEqual(buildAcpSessionNewParams('/tmp/gd-project', { mcpServers }), {
    cwd: path.resolve('/tmp/gd-project'),
    mcpServers: [
      {
        type: 'stdio',
...
test('ACP session params preserve caller-provided type and env fields', () => {
  ...
  const result = buildAcpSessionNewParams('/tmp/gd-project', { mcpServers });
...
test('attachAcpSession treats stageTimeoutMs <= 0 as a watchdog disable, not an immediate-failure schedule', async () => {
  ...
  try {
    ...
    attachAcpSession({
      ...
      mcpServers: [],
      send: (event, payload) => events.push({ event, payload }),
      // GD_ACP_STAGE_TIMEOUT_MS=0 escape hatch: operator wants to disable the
