...
A **Skill** is the atomic unit of design capability in OD. We adopt Claude Code's `SKILL.md` convention verbatim as the base format, then add optional fields for design-specific features (preview type, input schema, slider parameters). A skill written for plain Claude Code runs in OD. An GD skill that doesn't use our extensions runs in plain Claude Code.

> **Compatibility promise:** A skill like [`guizang-ppt-skill`](https://github.com/op7418/guizang-ppt-skill) works in GD **without modification**. It just drops into `~/.claude/skills/` and GD discovers it.
...
description: |
  ...
  Trigger keywords: magazine deck, swipe slides, magazine ppt.
en_description: |
  Magazine-style horizontal-swipe web deck.
  Trigger keywords: magazine deck, swipe slides.
triggers:
  - "magazine deck"
  - "magazine ppt"
...
> Localized variants such as `zh_name`, `zh_description`, and locale entries
> in `triggers` are also supported — the source of truth in this repo stays
> English, but skill authors can add locale-specific copy as additional
> optional fields and the picker will use them for matching display locales.
...
**GD reads all of this as-is.** No changes required.

## 2. GD extensions (optional)
...
od:
  ...
  example_prompt_i18n:
    # Locale-specific overrides; values use the target locale's source language.
    # The English `example_prompt` above is the fallback for any unlisted locale.
    zh-CN: "Create a magazine-style web deck from my content. (zh-CN translation)"
  design_system:
...
### 2.1 What GD uses each field for
...
`cc-switch` maintains a central skill dir at `~/.cc-switch/skills/` and symlinks it into each agent's expected location (`~/.claude/skills/`, `~/.codex/skills/`, etc.). GD can opt into the same model:

```
~/.galyarder-design/skills/
    magazine-web-ppt/      (canonical location)
~/.claude/skills/
    magazine-web-ppt → ~/.galyarder-design/skills/magazine-web-ppt
~/.codex/skills/
    magazine-web-ppt → ~/.galyarder-design/skills/magazine-web-ppt
```
...
- **Post-run:** GD prompts the user to set this DESIGN.md as the project's active design system.

## 5. The DESIGN.md as skill context

Every non–design-system skill (modes 1–3) can consume the active `DESIGN.md`. GD injects it as:
...
GD ships these as a third axis at `<projectRoot>/craft/`:
...
# → clones into ~/.galyarder-design/skills/magazine-web-ppt
...
4. User types a brief like "Make me an 8-page magazine-style investor PPT" (or the same brief in another source language; the skill's `triggers` will still match).
...
7. GD streams the agent's tool calls as UI events; artifact dir grows.
8. Agent signals done; daemon sets preview iframe to `index.html`.
9. User clicks "Export PPTX" — export pipeline notices the skill has no `slides.json` output (the upstream skill doesn't produce one). GD falls back to "print to PDF then page-to-slide PPTX," which is uglier but works. This is a known limitation documented per-skill.
...
A skill ships with optional test inputs that GD uses for CI:
