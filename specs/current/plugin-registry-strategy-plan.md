# Galyarder Design Plugin Registry Strategy Plan
...
Turn Galyarder Design plugins from "installed local workflows plus a light marketplace index" into a registry-shaped ecosystem:

- Galyarder Design hosts the official catalog on the main site and ships first-party plugins there.
- Community authors publish by opening GitHub PRs into the Galyarder Design registry repository.
- `gd` remains the canonical headless API for CRUD, search, install, update, trust, pack, doctor, and publish.
- Third parties can self-host the same `galyarder-design-marketplace.json` shape as their own plugin source.
...
- `docs/plugins-spec.zh-CN.md` names CLI as the canonical API, supports headless OD, and reserves federated `galyarder-design-marketplace.json`.
...
- `apps/landing-page` now has a static public `/plugins/` registry renderer and per-plugin detail routes generated from `plugins/registry/*/galyarder-design-marketplace.json` plus bundled official manifests.
...
- `apps/daemon/src/plugins/publish.ts` now builds submission links for external catalogs and the Galyarder Design registry target. Full GitHub fork/branch/PR mutation is still future backend work.
...
1. **Official web registry**
   - Hosted on the main Galyarder Design site.
   - Renders official and community-approved plugins from the Galyarder Design registry repo.
...
3. **Headless CLI registry**
   ...
   - Uses `gh` as the v1 backend driver and auth provider, without inventing an GD account system.
...
Plugin source repo
  ...
  galyarder-design.json must include plugin.repo.
        |
        | gd plugin validate / pack / publish
        v
Plugin artifact
  GitHub ref, GitHub Release .tgz, HTTPS archive, or local folder.
        ...
        v
Registry backend
  v1: GitHub repo + generated galyarder-design-marketplace.json.
...
- **galyarder-design.dev/plugins** is the public presentation of the official and community registry sources. It is equivalent to a polished static renderer over repo-owned catalog data, not a separate source of truth. `galyarder-design.dev/marketplace` can remain an alias later if needed.
- **`gd` CLI** remains the canonical client. Every UI action must map to a CLI operation or daemon API that the CLI can also drive.
- **Galyarder Design GitHub registry repo** is the v1 storage backend. It can later be swapped for a database backend without changing user-facing nouns.
...
Galyarder Design packaged runtime
...
Default community registry
  ...
  -> plugin is copied to ~/.galyarder-design/plugins/<plugin-id>
  -> Installed becomes part of agent context/runtime consumption
```
...
`~/.galyarder-design/plugins/<plugin-id>` by default. The daemon reloads those
...
Create plugin
  ...
  -> gd plugin scaffold / validate / local install / pack
  -> gd plugin login/whoami through gh
  -> gd plugin publish opens GitHub registry PR
  -> merge regenerates galyarder-design-marketplace.json
  -> users refresh Sources, see Available, install into Installed
```

The `Create plugin` product entry should therefore start an agent workflow, not a bare JSON form. The agent can gather intent, generate `SKILL.md`, `galyarder-design.json`, examples, previews, and capability metadata, run local validation, install a local test copy, pack it, and drive the `gh`-backed publish flow.

High-level architecture relationship:

```text
                    galyarder-design.dev/plugins
                 public registry pages and docs
                               |
                               v
+--------------------------------------------------+
| Galyarder Design GitHub registry repo                 |
|                                                  |
| plugins/registry/official/galyarder-design-marketplace.json |
| plugins/registry/community/galyarder-design-marketplace.json |
| plugins/community/<vendor>/<plugin-name>/galyarder-design.json |
| generated galyarder-design-marketplace.json           |
+-----------------------------+--------------------+
                              |
...
- The generated `galyarder-design-marketplace.json` is the machine-readable artifact fetched by daemon/CLI/UI.
- The per-plugin `community/**/galyarder-design.json` or `entry.json` files are the human-reviewable source data used for GitHub PR governance.
- The file can initially live in the main Galyarder Design repo if that lowers setup cost, but the product abstraction should still be `RegistryBackend`, not "read this monorepo path". Moving to `galyarder-design/plugin-registry` later should be a data relocation, not a product rewrite.
...
```text
galyarder-design-plugin-registry/
├── galyarder-design-marketplace.json
├── community/
│   └── official/
│       ├── galyarder-design-marketplace.json
│       └── plugins/
...
`galyarder-design-marketplace.json` remains the portable file that third-party hosts can copy. The per-plugin `entry.json` files make PRs reviewable, reduce merge conflicts, and let the build step generate the final marketplace index deterministically.

Current repo-friendly data placement:

- First-party runtime plugins that ship inside GD still live under `plugins/_official/**` and are installed as `bundled`, but they carry official marketplace provenance so product/audit treat them as preinstalled official registry entries.
...
- Community submissions can land as plugin source folders under `plugins/community/<vendor-or-plugin>` and be referenced by `plugins/registry/community/galyarder-design-marketplace.json`; trust tier stays encoded per source/entry.
...
- The source-of-truth repo is declared in `galyarder-design.json` as `plugin.repo`. Published plugins can be "anything that packs": no special repo layout is required beyond passing `gd plugin validate` and `gd plugin pack`, shipping a runnable skill anchor, and declaring `plugin.repo`.
- `plugin.repo` points at the canonical source repository or subdirectory, for example `https://github.com/galyarder-design/plugins/tree/main/make-a-deck`. Registry entries may mirror it as `sourceRepository` for search and review without replacing the manifest field.

Minimum entry shape:

```json
{
  "name": "galyarder-design/make-a-deck",
  "title": "Make a Deck",
  "version": "1.2.0",
  "distTags": {
    "latest": "1.2.0",
  ...
  "source": "github:galyarder-design/plugins@3f4c2d1/make-a-deck",
  "ref": "3f4c2d1",
  "dist": {
    "type": "github",
    "archive": "https://github.com/galyarder-design/plugins/archive/3f4c2d1.tar.gz",
    "integrity": "sha512-...",
    "manifestDigest": "sha256-..."
  },
  "publisher": {
    "id": "galyarder-design",
    "github": "galyarder-design"
  },
  "sourceRepository": "https://github.com/galyarder-design/plugins/tree/main/make-a-deck",
  "homepage": "https://galyarder-design.dev/plugins/galyarder-design/make-a-deck/",
  "license": "MIT",
...
Required `galyarder-design.json` fields for registry publish:
...
```json
{
  ...
  "version": "1.2.0",
  "plugin": {
    "repo": "https://github.com/galyarder-design/plugins/tree/main/make-a-deck"
  }
}
```
...
- `LocalMarketplaceJsonBackend`: reads cached `galyarder-design-marketplace.json` rows from SQLite.
...
- A company can deploy GD plus `DatabaseRegistryBackend` inside its own VPC.
...
- Fully managed commercial GD can use the same database backend with multi-tenant org boundaries, billing entitlements, SSO/SAML/OIDC, SCIM later, audit exports, and private object storage.
...
- CLI commands must not expose GitHub-specific nouns except where the source explicitly is GitHub. `gd plugin publish --to galyarder-design` may use `gh` internally, but the command contract should survive a later database backend.
...
- Installing the `gd` CLI should ensure `gh` is available. If the host has no `gh`, the installer bootstraps it using the platform package path available to that distribution channel, or fails with a precise remediation when auto-install is impossible.
- `gd plugin login` wraps `gh auth login` with Galyarder Design copy and required scopes/host guidance.
- `gd plugin whoami` wraps `gh auth status` plus `gh api user` and prints the active account, host, scopes, and whether it can publish to the configured registry repo.
- `gd plugin logout` can wrap `gh auth logout` only after an explicit confirmation, because it affects the user's global GitHub CLI session.
...
Keep existing `od marketplace` and `gd plugin` naming. Avoid adding a second `registry` noun until there is a real non-marketplace backend exposed to users.
...
- [x] `gd plugin install <source-or-name>`
- [ ] `gd plugin install <name>@<version-or-tag> [--from <marketplace-id>] [--save-lock]`
- [ ] `gd plugin upgrade [id] [--policy latest|pinned|range] [--dry-run]`
- [ ] `gd plugin outdated [--json]`
- [x] `gd plugin list/info/search/apply/run/trust/uninstall/doctor`
- [ ] `gd plugin view <name> [--from <marketplace-id>] [--versions] [--json]`
- [ ] `gd plugin lock write|verify|diff`

### Plugin Author Commands

- [x] `gd plugin login [--host <github-host>]`
- [x] `gd plugin whoami [--host <github-host>] [--json]`
- [x] `gd plugin scaffold`
- [x] `gd plugin validate`
- [x] `gd plugin pack`
- [ ] `gd plugin publish --to galyarder-design [--dry-run]`
- [ ] `gd plugin publish --to marketplace-json --catalog <path-or-url> [--branch <name>]`
- [ ] `gd plugin publish --to github --repo <owner/repo> [--public|--private]`
- [ ] `gd plugin deprecate <name>@<version> --reason <text>`
- [ ] `gd plugin yank <name>@<version> --reason <text>`

Publish v1 should use `gh`:

- `gd plugin login` wraps `gh auth login`; `gd plugin whoami` wraps `gh auth status` and `gh api user`.
- `gd plugin publish --to galyarder-design` validates, packs if needed, renders the registry entry, creates a branch, opens a PR, and prints the PR URL.
- `gd plugin publish` uses `gh api`, `gh repo fork`, and `gh pr create` through a testable `GhClient`; it does not implement a parallel GitHub OAuth flow.
...
- Show install command, `gd plugin view`, `gd plugin run` example, version selector, dist-tags, changelog, source repo, publisher, license, integrity, capabilities, connector requirements, examples, and security notes.
...
- "Contribute to Galyarder Design registry" flow with required files, checklist, CLI command, and GitHub PR path.
...
- Credentials live in `gh`, never inside `galyarder-design-marketplace.json` and never inside the daemon database.
...
- [ ] Add formal `plugin.repo` schema field to `galyarder-design.json` and require it for registry publish.
...
- [x] Add `gd plugin install <name>@<version-or-tag>`.
...
- [ ] Add `gd plugin lock verify`.
- [ ] Add `gd plugin outdated`.
...
Goal: make Galyarder Design contributions feel like npm publish, while actually opening a GitHub PR.

- [ ] Define official registry repo layout and generated index build step.
- [ ] Make `gh` an explicit `gd` CLI dependency and installer prerequisite/bootstrap step.
- [x] Add `gd plugin login` and `gd plugin whoami` as wrappers over `gh auth login/status` and `gh api user`.
- [ ] Add `gd plugin publish --to galyarder-design --dry-run --json`.
...
- [ ] Run `gd plugin validate`, `pack`, `doctor`, and integrity calculation before PR creation.
...
- [x] Add `gd plugin publish --to marketplace-json --catalog <path>` for self-hosted static catalogs.
...
- [x] Add install/use/upgrade card states for available entries. Current install uses the existing bare-name `gd plugin install <name>` path and now preserves provenance; explicit `--from <marketplace-id>` remains a P1 follow-up.
...
- [x] Make `Create plugin` launch an agent-assisted authoring flow backed by `gd plugin scaffold/validate/pack/publish`, including local install/run validation before publish and `gh` login/whoami checks before opening a registry PR. Current slice updates the agent prompt and CLI wrapper; full GitHub PR mutation remains in P2.
- [x] Add public `/plugins/` route on `apps/landing-page` for galyarder-design.dev: searchable official/community registry listing, static plugin detail pages, canonical/OG/Twitter metadata, JSON-LD item/detail data, and homepage/header entry points.
...
- [ ] Add migration/import from static `galyarder-design-marketplace.json` and GitHub registry repo into database rows.
- [ ] Add export back to `galyarder-design-marketplace.json` so enterprises can mirror or air-gap catalogs.
...
6. **GitHub CLI auth wrapper**
   ...
   - Outcome: `gd plugin login/whoami` provide the product auth surface while reusing the user's `gh` session.
...
- Should the official registry live in this monorepo or a separate `galyarder-design/plugin-registry` repo? Separate is cleaner for community PR review and static-site generation.
- Should `official` marketplace trust be allowed for user-added URLs, or only for built-in source ids shipped by Galyarder Design? Recommendation: only built-in sources can be `official`; user-added sources can be `trusted` or `restricted`.
...
- Should `gd plugin publish --to marketplace-json` mutate a local catalog file directly or create a branch/PR when the catalog URL maps to GitHub? Recommendation: support both, but default to PR when a GitHub remote is detectable.
...
- Source repo shape: accept anything that passes `gd plugin validate` and `gd plugin pack`.
- Source pointer: registry-published `galyarder-design.json` must include `plugin.repo`.
...
The architecture is already pointed in the right direction: Galyarder Design can become a "multi-source plugin registry" without giving up local-first/headless operation. The next move should not be a bigger gallery. It should be provenance, trust vocabulary, and exact resolution first; then the UI can safely graduate from "official plugins plus import" to "installed and available plugins across sources."
