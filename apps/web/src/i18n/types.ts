...
export type Locale = 'en' | 'id' | 'de' | 'pt-BR' | 'es-ES' | 'ru' | 'fa' | 'ar' | 'ja' | 'ko' | 'pl' | 'hu' | 'fr' | 'uk' | 'tr' | 'th' | 'it';

export const LOCALES: Locale[] = ['en', 'id', 'de', 'pt-BR', 'es-ES', 'ru', 'fa', 'ar', 'ja', 'ko', 'pl', 'hu', 'fr', 'uk', 'tr', 'th', 'it'];

// Native-name labels for the language picker. Each value is the
// self-name of the locale in its own script (this is i18n display data:
// the locale picker shows users their own language's name in their
// language). Treated functionally identical to dictionary content under
// `apps/web/src/i18n/locales/`; non-English literals here are the
// canonical native names of the supported UI locales.
...
export interface Dict {
  ...
  'settings.telemetry': string;
  'settings.telemetryHint': string;
  ...
  // `gd.preview.type` is not `html` (image / markdown / …) so the
  ...
  // Friendly placeholder copy for skills whose `gd.preview.type` is not
  ...
  // Body copy uses the `{kind}` placeholder (raw `gd.preview.type`
  ...
  'previewChrome.toolbarAria': string;
  'previewChrome.zoomLabel': string;
  'previewChrome.zoomAria': string;
  'previewChrome.deviceFrameAria': string;
  'previewChrome.deviceDesktop': string;
  'previewChrome.deviceTablet': string;
  'previewChrome.deviceMobile': string;
  'previewChrome.renderModeAria': string;
  'previewChrome.renderModeUrl': string;
  'previewChrome.renderModeSrcdoc': string;
  'previewChrome.commentPanelAria': string;
  'previewChrome.tweaksPanelAria': string;
}
