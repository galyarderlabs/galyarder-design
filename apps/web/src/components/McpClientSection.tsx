...
//
// Galyarder Design connects to the configured servers as a CLIENT and surfaces
...
// so the most useful categories for Galyarder Design (visual generation, then
  ...
  return (
    <section className="settings-section">
      ...
      <div className="mcp-foot">
        ...
        <span className="hint">
          {t('mcpClient.storedAt')} <code>.gd/mcp-config.json</code>
        </span>
      </div>
    </section>
  );
      ...
      {expanded ? (
        <>
          ...
          {isHttpLike && !row._isNew && row.id ? (
            usesManagedOAuth ? (
              ...
              <div className="mcp-oauth-hint hint">
                <strong>No managed OAuth.</strong> Galyarder Design will use this
          ...
          {isHttpLike && row._isNew && usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Galyarder Design
              access via the provider's OAuth flow.
            </div>
          ) : null}
          {isHttpLike && row._isNew && !usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              <strong>No managed OAuth.</strong> Save this server and Galyarder Design
...
function McpOAuthControl({ serverId }: { serverId: string }) {
  ...
  useEffect(() => {
    ...
    if (typeof BroadcastChannel !== 'undefined') {
      bc = new BroadcastChannel('galyarder-design-mcp-oauth');
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Galyarder Design access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
