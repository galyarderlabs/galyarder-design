...
// `gd.category` so the Settings → Skills filter row groups them visibly.
//
...
interface CuratedSkill {
  ...
  triggers: string[];
  // Maps to the existing `gd.mode` filter. Pick the closest of the seven
  ...
  // whole catalogue.
  category: string;
  // Upstream URL recorded under `gd.upstream`. The Skills detail panel
...
const CATALOGUE: CuratedSkill[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    triggers: ['canvas design', 'visual art', 'poster design', 'create poster', 'illustration'],
    mode: 'image',
  ...
  },
  {
    ...
    triggers: ['algorithmic art', 'generative art', 'p5js', 'procedural art', 'seeded randomness'],
    mode: 'image',
    category: 'image-generation',
  ...
  },
  {
    ...
    triggers: ['gif sticker', 'funko sticker', 'animated sticker', 'pop mart', 'emoji sticker'],
    mode: 'image',
    category: 'image-generation',
  ...
  },
  {
    id: 'ui-ux-pro-max',
    description:
      'Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in Galyarder Design.',
    ...
    catalogueOnlyNote:
      'Galyarder Design ships this entry as discovery metadata only. The upstream UI/UX Pro Max data CSVs, scripts/search.py helper, templates, references, and related skill instructions are not bundled here; if those files are absent, disclose the limitation before falling back to Galyarder Design defaults.',
  },
  {
    id: 'taste-skill',
    description:
  ...
  },
  {
    id: 'swiftui-design',
    description:
      'SwiftUI front-end design skill — anti AI-slop rules, design direction advisor, brand asset protocol, and five-dimension review. Works with Claude Code, Cursor, Codex, and OpenCode.',
...
function buildBody(s: CuratedSkill): string {
  ...
  if (s.catalogueOnlyNote) {
    lines.push('## Current Galyarder Design scope');
    lines.push('');
  ...
  lines.push('');
  lines.push(
    'This catalogue entry advertises the skill in Galyarder Design so the agent',
  );
  lines.push(
