import { SIDECAR_DEFAULTS } from "@galyarder-design/sidecar-proto";
...
function appIdForChannel(channel: ReleaseChannelIdentity): string {
  if (channel === "beta") return "io.galyarder-design.desktop.beta";
  if (channel === "nightly") return "io.galyarder-design.desktop.nightly";
  if (channel === "preview") return "io.galyarder-design.desktop.preview";
  return "io.galyarder-design.desktop";
}
...
export function resolveMacInstallIdentity(config: Pick<ToolPackConfig, "namespace" | "appVersion">): MacInstallIdentity {
  ...
  const channelIdentity = channel == null
    ? { appId: "io.galyarder-design.desktop", productName: PRODUCT_NAME }
