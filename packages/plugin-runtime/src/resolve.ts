...
// `gd.context.*` refs into the typed ContextItem chips the UI / CLI render.
//
...
export interface RegistryView {
  ...
  // `gd.design_system.requires: true` without a concrete ref. Daemon
  ...
  // omits `gd.pipeline`, apply consults this list and uses the
...
export interface ScenarioRegistryEntry {
  // The scenario plugin's id (e.g. 'gd-code-migration'). Used by tests
  // and audits to attribute the fallback choice.
  id: string;
  // The taskKind enum value this scenario claims to default for. Apply
  // matches against `manifest.gd.taskKind` (or 'new-generation' when
  // absent).
  taskKind: 'new-generation' | 'figma-migration' | 'code-migration' | 'tune-collab';
  // The scenario plugin's `gd.pipeline`. Copied verbatim into the
