...
export function App() {
  ...
  // because the dynamic-import loading shell (`<div class="gd-loading-shell">
  // Loading Galyarder Design…</div>`) is itself >MIN_VISIBLE_TEXT and would
  ...
  useEffect(() => {
    if (typeof document !== 'undefined') {
      document.documentElement.setAttribute('data-gd-app-mounted', '1');
    }
  }, []);
      ...
      if (
        ...
        try {
          window.sessionStorage.setItem(
            `gd:auto-send-first:${result.project.id}`,
            '1',
          );
          if (firstMessageAttachments.length > 0) {
            window.sessionStorage.setItem(
              `gd:auto-send-attachments:${result.project.id}`,
  ...
  const handleCreatePluginShareProject = useCallback(
    async (
      ...
      try {
        window.sessionStorage.setItem(
          `gd:auto-send-first:${outcome.project.id}`,
          '1',
        );
      } catch {
  ...
  const handleImportFolderResponse = useCallback(async (result: GalyarderDesignHostProjectImportSuccess) => {
  ...
  if (route.kind === 'marketplace') {
    ...
    appMain = (
      <EntryView
        ...
        onGalyarderDesignSystem={(id: string) => navigate({ kind: 'design-system-detail', designSystemId: id })}
