...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  const { GD_BIN, RUNTIME_DATA_DIR, PROJECTS_DIR } = ctx.paths;
  ...
  // even when `gd` isn't on the user's PATH (the common case for source clones
  // - and macOS/Linux ship a /usr/bin/gd octal-dump tool that shadows ours
  ...
  app.get('/api/mcp/install-info', (req, res) => {
    ...
    const cliPath = GD_BIN;
    // The daemon was bootstrapped as a sidecar (tools-dev, packaged) iff
    // bootstrapSidecarRuntime stamped GD_SIDECAR_IPC_PATH into the env.
    // In sidecar mode the snippet omits --daemon-url and the spawned
    // `gd mcp` discovers the live URL via the concrete IPC endpoint on
    // every spawn, so the client config survives ephemeral-port
    // restarts. For direct `gd` / `gd --port X` launches there is no
  ...
  // External MCP server configuration. Galyarder Design connects to these as a
  ...
  // where GD_PUBLIC_BASE_URL pins the externally-routable URL.
  // ─────────────────────────────────────────────────────────────────
