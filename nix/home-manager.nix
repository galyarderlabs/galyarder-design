# Home Manager module for Galyarder Design — primary interface for individual
...
#
# Usage:
#   imports = [ inputs.galyarder-design.homeManagerModules.default ];
...
#   };
{
  ...
  cfg = config.services.galyarder-design;
  ...
  # `services.galyarder-design.webFrontend.package` and bring their own
  ...
  # `GD_DAEMON_URL=""`, so the bundled JS calls relative URLs like
  ...
  caddyfile = pkgs.writeText "galyarder-design-web.Caddyfile" ''
    {
      auto_https off
      admin off
      persist_config off
  ...
  daemonEnv =
    {
      GD_PORT = toString cfg.port;
      GD_DATA_DIR = toString cfg.dataDir;
    ...
    // lib.optionalAttrs cfg.webFrontend.enable {
      ...
      GD_WEB_PORT = toString cfg.webFrontend.port;
    }
    // lib.optionalAttrs (cfg.webFrontend.allowedOrigins != []) {
      ...
      GD_ALLOWED_ORIGINS = lib.concatStringsSep "," cfg.webFrontend.allowedOrigins;
    }
    // cfg.extraEnv;

  envToList = e: lib.mapAttrsToList (k: v: "${k}=${v}") e;
in {
  options.services.galyarder-design = commonOpts;
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    {
      ...
      home.activation.galyarderDesignDataDir = lib.hm.dag.entryAfter ["writeBoundary"] ''
      ...
      assertions = [
        {
          ...
          message = ''
            services.galyarder-design.webFrontend.host = "${cfg.webFrontend.host}" exposes the
            bundled web frontend on a non-loopback interface, but
            services.galyarder-design.webFrontend.allowedOrigins is empty.
            ...
            the SPA will be loaded from, e.g.
              ...
              services.galyarder-design.webFrontend.allowedOrigins = [
    ...
    (lib.mkIf (pkgs.stdenv.isLinux && cfg.autoStart) {
      systemd.user.services.galyarder-design = {
        Unit = {
          Description = "Galyarder Design daemon (user service)";
    ...
    (lib.mkIf (pkgs.stdenv.isLinux && cfg.webFrontend.enable) {
      systemd.user.services.galyarder-design-web = {
        Unit = {
          Description = "Galyarder Design web frontend (static file server)";
          After = ["network-online.target"];
          Wants = ["network-online.target"];
        };
        Install.WantedBy = ["default.target"];
    ...
    (lib.mkIf (pkgs.stdenv.isDarwin && cfg.autoStart) (let
      ...
      daemonLaunchScript = pkgs.writeShellScript "galyarder-design-daemon-launch" ''
        set -a
    ...
    in {
      launchd.agents.galyarder-design = {
        enable = true;
        config = {
          Label = "io.nexu.galyarder-design";
          ProgramArguments = programArguments;
          RunAtLoad = true;
          KeepAlive = true;
          EnvironmentVariables = daemonEnv;
          StandardOutPath = "${cfg.dataDir}/galyarder-design.out.log";
          StandardErrorPath = "${cfg.dataDir}/galyarder-design.err.log";
        };
      };
    }))

    (lib.mkIf (pkgs.stdenv.isDarwin && cfg.webFrontend.enable) {
      launchd.agents.galyarder-design-web = {
        enable = true;
        config = {
          Label = "io.nexu.galyarder-design-web";
          ProgramArguments = [
            (lib.getExe caddy)
            "run"
            "--config"
