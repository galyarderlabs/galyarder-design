...
- Local packaging orchestration for packaged Galyarder Design artifacts.
...
- Consuming sidecar/process/path primitives from `@galyarder-design/sidecar-proto`, `@galyarder-design/sidecar`, and `@galyarder-design/platform`.
...
- Do not hand-build `--gd-stamp-*` args; use `createProcessStampArgs` with `GALYARDER_DESIGN_SIDECAR_CONTRACT`.
...
- Public release artifacts must use channel-specific app identity: stable uses `Galyarder Design`, beta uses `Galyarder Design Beta`, and preview uses `Galyarder Design Preview`. Local tools-pack installs may still use namespace-scoped install paths only as a developer multi-instance validation convention.
...
- `apps/desktop/src/main/runtime.ts` exposes updater IPC to the renderer through `gd:update:status|check|download|install|quit` and emits `gd:update:status-changed`. Keep installer launch separate from process shutdown; quit is an explicit post-installer action.
...
The runtime updater reads `https://releases.galyarder-design.ai/<channel>/latest/metadata.json` unless `GD_UPDATE_METADATA_URL` overrides it. For package-launcher updates:
...
- `GD_UPDATE_CURRENT_VERSION` may override the packaged version for tests, but user-flow package validation should prefer building the package with the intended `--app-version`.
...
- Stable: `Galyarder Design`, namespace `default` or stable release namespace.
- Beta Windows: `Galyarder Design Beta`, namespace `release-beta-win`, uninstall key `Galyarder Design-release-beta-win`.
- Preview Windows: `Galyarder Design Preview`, namespace `release-preview-win`, uninstall key `Galyarder Design-release-preview-win`.
...
```bash
GD_UPDATE_ENABLED=1
GD_UPDATE_METADATA_URL=<fixture metadataUrl>
GD_UPDATE_CURRENT_VERSION=99.0.0-beta.0
GD_UPDATE_OPEN_DRY_RUN=1
GD_UPDATE_AUTO_CHECK=1
```
...
```bash
curl.exe --ssl-no-revoke -fsSL https://releases.galyarder-design.ai/beta/latest/metadata.json
```
...
```text
C:\odtp-beta-release-fixed\out\win\namespaces\release-beta-win\builder\Galyarder Design-release-beta-win-setup.exe
```

4. Expected user flow:

- User installs `0.8.0-beta.5` through the NSIS UI.
- User launches `Galyarder Design Beta`.
...
- Clicking `Open installer` opens the real downloaded beta installer. Installing it should overwrite the same `Galyarder Design-release-beta-win` registry key, not create a second beta key.

5. Registry sanity check after beta.6 install:

```powershell
Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue |
  Where-Object { $_.DisplayName -like 'Galyarder Design*' } |
  Select-Object PSChildName,DisplayName,DisplayVersion,InstallLocation
```

For a clean beta channel result, expect one beta entry with `PSChildName` `Galyarder Design-release-beta-win` and the latest `DisplayVersion`.
...
```bash
pnpm --filter @galyarder-design/desktop test -- tests/main/updater.test.ts tests/main/updater-host-boundary.test.ts tests/main/preload-host-boundary.test.ts
pnpm --filter @galyarder-design/web test -- tests/components/UpdaterPopup.test.tsx tests/lib/updater.test.ts
pnpm --filter @galyarder-design/tools-serve test
pnpm --filter @galyarder-design/tools-pack test -- tests/win-identity.test.ts tests/win-app.test.ts tests/win-builder.test.ts
pnpm --filter @galyarder-design/desktop typecheck
pnpm --filter @galyarder-design/web typecheck
pnpm --filter @galyarder-design/tools-pack typecheck
pnpm --filter @galyarder-design/tools-serve typecheck
git diff --check
pnpm guard
pnpm typecheck
```
