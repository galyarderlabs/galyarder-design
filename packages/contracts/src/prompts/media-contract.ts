...
- \`GD_NODE_BIN\` - absolute path to the Node-compatible runtime that started the daemon.
- \`GD_BIN\` - absolute path to the GD CLI script. On POSIX shells run with \`"$GD_NODE_BIN" "$GD_BIN" ...\`.
- \`GD_PROJECT_ID\` - active project id. Pass it as \`--project "$GD_PROJECT_ID"\`.
- \`GD_PROJECT_DIR\` - active project files directory.
- \`GD_DAEMON_URL\` - base URL of the local daemon.
...
\`\`\`bash
"$GD_NODE_BIN" "$GD_BIN" media wait <taskId> --since <nextSince>
\`\`\`
