...
// adapter — that's an opt-in flag away (`GD_PROJECT_STORAGE=s3`).
...
/**
 * v1 default — backed by the daemon's existing `<dataDir>/.gd/projects/`
...
export interface S3ProjectStorageOptions {
  bucket:    string;
  region:    string;
  // Optional path prefix inside the bucket. Lets multiple GD
  // deployments share one bucket.
  prefix?:   string;
  // S3-compatible endpoint URL (Aliyun OSS, Tencent COS, Huawei OBS,
  // MinIO). Omit for AWS S3.
  endpoint?: string;
  // AWS access credentials. Read from GD_S3_ACCESS_KEY_ID /
  // GD_S3_SECRET_ACCESS_KEY by resolveProjectStorage(); the test
 ...
 * Default is local-disk; setting GD_PROJECT_STORAGE=s3 pulls the
...
export function resolveProjectStorage(opts: {
  ...
  const kind = (env.GD_PROJECT_STORAGE ?? 'local').trim().toLowerCase();
  if (kind === 's3') {
    // Read AWS creds from the GD-specific knobs first, then fall
    // back to the standard AWS_* env vars so existing AWS toolchain
    // setups (`aws configure` exporters, IAM-role pods) drop in
    // without renaming.
    const accessKeyId     = env.GD_S3_ACCESS_KEY_ID     ?? env.AWS_ACCESS_KEY_ID     ?? '';
    const secretAccessKey = env.GD_S3_SECRET_ACCESS_KEY ?? env.AWS_SECRET_ACCESS_KEY ?? '';
    const sessionToken    = env.GD_S3_SESSION_TOKEN     ?? env.AWS_SESSION_TOKEN;
    ...
    return new S3ProjectStorage({
      bucket:   env.GD_S3_BUCKET ?? '',
      region:   env.GD_S3_REGION ?? env.AWS_REGION ?? '',
      ...(env.GD_S3_PREFIX   ? { prefix:   env.GD_S3_PREFIX }   : {}),
      ...(env.GD_S3_ENDPOINT ? { endpoint: env.GD_S3_ENDPOINT } : {}),
      credentials,
    });
  }
