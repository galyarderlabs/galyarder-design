...
async function createWorkspaceFixture(workspaceRoot: string): Promise<void> {
  ...
  await writeFile(
    join(workspaceRoot, "plugins", "_official", "sample", "galyarder-design.json"),
    "{\"id\":\"sample\"}\n",
    "utf8",
  );
  ...
  await writeFile(
    join(workspaceRoot, "plugins", "registry", "community", "galyarder-design-marketplace.json"),
    "{\"plugins\":[]}\n",
    "utf8",
  );
...
describe("prepareResourceTree", () => {
  it("invalidates the Windows resource tree cache when design templates change", async () => {
    const root = await mkdtemp(join(tmpdir(), "galyarder-design-win-resources-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "galyarder-design");
