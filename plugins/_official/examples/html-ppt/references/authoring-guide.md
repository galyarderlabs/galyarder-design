...
3. **Language** — English-only, bilingual, or mixed-script? (Noto Sans SC is preloaded if you need CJK glyphs.)
4. **Format** — on-screen live, PDF export, RedNote image-text post?
...
- Fonts are already imported in `fonts.css` (Noto Sans SC + Noto Serif SC) so CJK glyphs render correctly when needed.
- If a section is primarily CJK, set `lang="zh-CN"` on `<html>`.
- For bilingual titles, stack lines: `<h1 class="h1">Primary headline<br><span class="dim">English subtitle</span></h1>`.
- Keep secondary-language subtitles in a lighter weight (300) and dim color to avoid
  visual competition.
...
for a RedNote image-text post (1242×1660).
