# Contributing to Galyarder Design

Thanks for thinking about contributing. GD is small on purpose — most of the value lives in **files** (skills, design systems, prompt fragments) rather than framework code. That means the highest-leverage contributions are usually one folder, one Markdown file, or one PR-sized adapter.
...
|---|---|---|---|
| Make GD render a new kind of artifact (an invoice, an iOS Settings screen, a one-pager…) | a **Skill** | [`skills/<your-skill>/`](skills/) | one folder, ~2 files |
| Make GD speak a new brand's visual language | a **Design System** | [`design-systems/<brand>/DESIGN.md`](design-systems/) | one Markdown file |
...
If you're not sure which bucket your idea is in, [open a discussion / issue first](https://github.com/galyarderlabs/galyarder-design/issues/new) and we'll point you at the right surface.
...
pnpm --filter @galyarder-design/web build  # web package build when needed
```
...
Run Galyarder Design without installing Node.js or pnpm.
...
2. **Comments in English.** Even if the PR is translating something into Deutsch or another language, code comments stay in English so we can keep one set of greppable references.
...
- Architecture question, design question, "is this a bug or a misuse" → [GitHub Discussions](https://github.com/galyarderlabs/galyarder-design/discussions) (preferred — searchable for the next person).
...
- **Add telemetry / analytics / phone-home.** GD is local-first. The only outbound calls are to providers the user explicitly configured.
...
[discussions]: https://github.com/galyarderlabs/galyarder-design/discussions
[discord]: https://github.com/galyarderlabs/galyarder-design/discussions
