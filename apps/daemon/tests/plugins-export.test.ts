// Phase 4 / spec §14 — `gd plugin export` unit test.
//
...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'gd-export-'));
...
describe('exportPlugin', () => {
  it('target=gd writes SKILL.md + galyarder-design.json + README.md', async () => {
    const snap = persistSampleSnapshot();
    const result = await exportPlugin({ db, snapshotId: snap.snapshotId, target: 'gd', outDir: tmpDir });
    ...
    const manifest = JSON.parse(
      await readFile(path.join(result.folder, 'galyarder-design.json'), 'utf8'),
    );
  ...
  it('falls back to the most recent snapshot for a given projectId', async () => {
    ...
    db.prepare('UPDATE applied_plugin_snapshots SET applied_at = applied_at + 100 WHERE id = ?')
      .run(b.snapshotId);
    const result = await exportPlugin({ db, projectId: 'project-1', target: 'gd', outDir: tmpDir });
    expect(result.snapshotId).toBe(b.snapshotId);
  });

  it('throws ExportError when neither snapshot nor project resolves', async () => {
    await expect(
      exportPlugin({ db, snapshotId: 'missing', target: 'gd', outDir: tmpDir }),
    ).rejects.toBeInstanceOf(ExportError);
    await expect(
      exportPlugin({ db, projectId: 'no-such-project', target: 'gd', outDir: tmpDir }),
    ).rejects.toBeInstanceOf(ExportError);
  });
});
