...
import {
  applyPlugin,
  contributeGeneratedPluginToGalyarderDesign,
...
describe('applyPlugin', () => {
  ...
  it('passes the current locale to the daemon apply endpoint', async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response(
      JSON.stringify({
        // i18n locale fixture; CJK is the zh-CN translation that must
        // round-trip through the apply endpoint when the locale arg
        // selects Chinese.
        query: '生成一份简报。',
        contextItems: [],
        inputs: [],
    ...
    expect(JSON.parse(String(init?.body))).toMatchObject({
      inputs: {},
      grantCaps: [],
      // i18n locale fixture; the BCP-47 tag must round-trip verbatim
      // because the daemon apply endpoint keys translation lookups on it.
      locale: 'zh-CN',
    });
  });
});
...
describe('listPlugins', () => {
  ...
  it('hides plugins marked gd.hidden from UI-facing lists', async () => {
    const visible = {
      id: 'gd-new-generation',
      title: 'New generation',
      manifest: { gd: { kind: 'scenario' } },
    };
    const hidden = {
      id: 'gd-default',
      title: 'Default design router',
      manifest: { gd: { kind: 'scenario', hidden: true } },
    };
    ...
    expect(rows.map((row) => row.id)).toEqual(['gd-new-generation']);
  });
  ...
  it('can include hidden plugins for installed-entry matching', async () => {
    ...
    expect(rows.map((row) => row.id)).toEqual(['gd-default', 'gd-new-generation']);
  });
});
...
describe('installGeneratedPluginFolder', () => {
  ...
  it('preserves install diagnostics from non-2xx project folder responses', async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response(
      JSON.stringify({
        ok: false,
        warnings: ['Missing galyarder-design.json'],
...
describe('generated plugin share actions', () => {
  ...
  it('posts publish and contribute actions for project-relative plugin folders', async () => {
    ...
    const contribute = await contributeGeneratedPluginToGalyarderDesign('project-1', 'generated-plugin');
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/projects/project-1/plugins/contribute-galyarder-design',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ path: 'generated-plugin' }),
      }),
