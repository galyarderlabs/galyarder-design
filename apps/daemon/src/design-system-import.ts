...
import { extractComponentsManifest } from '@galyarder-design/contracts';
...
function renderUsageMd(name: string, scan: ProjectScan): string {
  ...
  return [
    `# ${name} Usage`,
    '',
    '> Auto-generated by Galyarder Design importer. Review and edit before treating it as a canonical brand guide.',
    '',
    '## Read Order',
    '',
    ...
    '## Design Highlights',
    '',
    ...(highlights.length > 0 ? highlights.map((line) => `- ${line}`) : ['- Imported web design system with generated GD tokens.']),
    '',
    '## Do',
    '',
    '- Preserve semantic roles from the source project before inventing new values.',
    '- Use `tokens.css` as the normalized GD token contract.',
    ...
    '- Do not rename GD standard tokens in generated artifacts.',
    '',
  ].join('\n');
}
