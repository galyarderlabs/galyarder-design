...
interface Props {
  ...
  // plugin (the user-bug-report quoted in CJK said: "I picked
  // new-generation but the composer ended up showing multiple plugins" —
  // CJK kept verbatim because it's the original user-report text).
  // The active plugin still appears as an
    ...
    function expandSearchCommand(input: string): { prompt: string; query: string } | null {
      ...
      return {
        ...
        prompt: [
          `Search for: ${query}`,
          '',
          'Before answering, your first tool action must be the GD research command for your shell.',
          'POSIX: "$GD_NODE_BIN" "$GD_BIN" research search --query "<search query>" --max-sources 5',
          'PowerShell: & $env:GD_NODE_BIN $env:GD_BIN research search --query "<search query>" --max-sources 5',
          'cmd.exe: "%GD_NODE_BIN%" "%GD_BIN%" research search --query "<search query>" --max-sources 5',
          ...
          query.replace(/```/g, '`\u200b`\u200b`'),
          '```',
          'If the GD command fails because Tavily is not configured or unavailable, report that error, then use your own search capability as fallback and label the fallback clearly.',
        ...
        <div className="composer-tools-empty">
          {plugins.length === 0 ? (
            <>
              ...
              <code>gd plugin install &lt;source&gt;</code>.
            </>
          ) : query ? (
