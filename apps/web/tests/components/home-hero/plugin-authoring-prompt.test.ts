...
// summary turn freeform-recommended `gd plugin publish --to galyarder-design`
...
describe('PLUGIN_AUTHORING_PROMPT_TEMPLATE', () => {
  ...
  it('still asks the agent to scaffold generated-plugin with SKILL.md + manifest', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('galyarder-design.json');
  ...
  it('forbids placeholder plugin.repo owners in generated manifests', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('gd plugin publish-repo generated-plugin --owner <github-login-or-org>');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toMatch(/Never write placeholder owners/i);
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('galyarder-design-user');
  ...
  it('still drives the local validation chain', () => {
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('gd plugin validate');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('gd plugin pack');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('gd plugin install --source');
  });

  it('bans freeform publish / repo CLI suggestions in the summary turn', () => {
    // The agent transcript in #2332 had the agent recommending
    // `gd plugin publish --to galyarder-design`, `gh repo create
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('gd plugin publish --to galyarder-design');
  ...
  it('points the user at the plugin-folder card buttons instead', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('Galyarder Design PR');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toMatch(
  ...
  it('warns against assuming standalone jq is installed', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toMatch(/cat .*galyarder-design\.json/);
