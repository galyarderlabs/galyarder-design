...
const residualAllowedExactPaths = new Set([
  ...
  "apps/web/public/gd-notifications-sw.js",
...
const testLayoutSkippedDirectories = new Set([".next", ".gd-data", "dist", "node_modules", "out", "reports", "test-results"]);
...
const e2eSkippedDirectories = new Set([".gd-data", "node_modules", "reports", "test-results"]);
...
const webTestSkippedDirectories = new Set([".gd-data", "reports", "test-results"]);
  ...
  if (isStylePolicySource(repositoryPath)) {
    for (const match of source.matchAll(defaultTailwindPaletteClassPattern)) {
      violations.push({
        ...
        match: match[0],
        reason: "default Tailwind palette classes must use Galyarder Design token utilities instead",
      });
    }
  }
    ...
    if (repositoryPath.endsWith(".css") && isHardcodedColorEnforcedPath(repositoryPath)) {
      for (const match of collectCssHardcodedColorMatches(source)) {
        ...
        addStylePolicyViolation(
          ...
          match.index,
          value,
          "unregistered hardcoded UI colors must use Galyarder Design tokens or an explicit allowlist entry",
        );
      }
    } else {
...
async function checkStylePolicy(): Promise<boolean> {
  ...
  if (violations.length > 0) {
    ...
    console.error("Use Galyarder Design token utilities/CSS variables or add a narrow allowlist entry with a reason.");
    return false;
  }
