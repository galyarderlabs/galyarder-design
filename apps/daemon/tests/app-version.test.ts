...
describe('app version helpers', () => {
  ...
  it('prefers packaged app version metadata from the environment', () => {
    expect(resolveAppVersionInfo({
      ...
      env: { GD_APP_VERSION: '0.3.1-beta.1' },
      resourcesPath: '/Applications/Galyarder Design.app/Contents/Resources',
      execPath: '/Applications/Galyarder Design.app/Contents/Resources/galyarder-design/bin/node',
      platform: 'darwin',
      arch: 'arm64',
    })).toEqual({
  ...
  it('detects packaged runtimes without sidecar protocol knowledge', () => {
    expect(isPackagedRuntime({ resourcesPath: '/Applications/Galyarder Design.app/Contents/Resources' })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/Applications/Galyarder Design.app/Contents/Resources/galyarder-design/bin/node',
      platform: 'darwin',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Galyarder Design\\resources\\galyarder-design\\bin\\node.exe',
      platform: 'win32',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/opt/Galyarder Design/resources/galyarder-design/bin/node',
      platform: 'linux',
    })).toBe(true);
