...
beforeEach(() => {
  ...
  mockedListMarketplaces.mockResolvedValue([
    {
      id: 'catalog-1',
      url: 'https://example.com/galyarder-design-marketplace.json',
      trust: 'official',
      manifest: {
...
describe('PluginsView', () => {
  ...
  it('shows registry provenance, permissions, versions, and copyable install command in available details', async () => {
    ...
    expect(within(dialog).getByTestId('plugins-available-install-command').textContent)
      .toContain('gd plugin install remote-plugin@1.1.0');
    expect(within(dialog).getByTestId('plugins-available-provenance').textContent)
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith('gd plugin install remote-plugin@1.1.0'),
    );
  ...
  it('filters available marketplace entries by source and search', async () => {
    mockedListMarketplaces.mockResolvedValue([
      ...
      {
        id: 'catalog-2',
        url: 'https://team.example.com/galyarder-design-marketplace.json',
        trust: 'restricted',
        manifest: {
          name: 'Team Catalog',
          version: '1.0.0',
  ...
  it('keeps installed registry entries out of Available', async () => {
    ...
    official.sourceMarketplaceEntryName = 'galyarder-design/official-plugin';
    official.sourceMarketplaceEntryVersion = '1.0.0';
    official.marketplaceTrust = 'official';
    official.manifest.gd = { ...official.manifest.gd, hidden: true };
    ...
    mockedListMarketplaces.mockResolvedValue([
      {
        ...
        manifest: {
          ...
          version: '0.1.0',
          plugins: [{
            name: 'galyarder-design/official-plugin',
            title: 'Official Plugin',
            source: 'github:galyarderlabs/galyarder-design@main/plugins/_official/scenarios/official-plugin',
            version: '1.0.0',
  ...
  it('manages registry sources from the Sources tab', async () => {
    ...
    const sourceUrl =
      'https://raw.githubusercontent.com/galyarderlabs/galyarder-design/main/plugins/registry/community/galyarder-design-marketplace.json';
  ...
  it('uploads zip and folder plugins from the import dialog', async () => {
    ...
    const folderFile = new File(['{}'], 'galyarder-design.json', { type: 'application/json' });
