...
function fresh(): string {
  return mkdtempSync(path.join(tmpdir(), 'gd-skill-stage-'));
}
...
describe('stageActiveSkill', () => {
  it('exposes the documented alias name so the skill preamble stays in sync', () => {
    expect(SKILLS_CWD_ALIAS).toBe('.gd-skills');
  });

  it('stages a per-project copy under <cwd>/.gd-skills/<folder>/', async () => {
    const fs = fresh();
  ...
  it('REGRESSION: writes through the staged copy do not mutate the source', async () => {
    ...
    // when `.gd-skills` was a directory junction, an agent could
