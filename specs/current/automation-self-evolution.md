...
Automations should become the product-wide loop that lets Galyarder Design improve
...
|---|---|---|
| [OpenHuman](https://github.com/tinyhumansai/openhuman) | Connector auto-fetch into a memory tree, local editable knowledge, chunking, and optional token compression before LLM use. | Keep GD design-focused and local-first; do not copy its subscription/model-router product shape. |
| [Hermes Agent](https://github.com/nousresearch/hermes-agent) | Closed learning loop: persistent memory, agent-created skills from experience, skill improvement during use, scheduled automations, and explicit compression commands. | Do not replace the user's agent loop or turn GD into a general messaging gateway. |
| [Multica](https://github.com/multica-ai/multica) | Managed agents, agent-scoped skills, workspace/local skill imports, and skill attachment as a team-level capability. | Do not turn GD into an issue board; use the skill attachment pattern for design work. |
| [GenericAgent](https://github.com/lsdefine/GenericAgent) | After solving a new task, crystallize the execution path into a reusable skill tree and recall it for similar tasks with a smaller context window. | Do not grant broad uncontrolled desktop authority; GD keeps daemon and connector capability gates. |

## Product thesis

Galyarder Design becomes a self-evolving design agent when every useful run can
...
The packet should be inspectable in UI and available to `gd automation`.
...
This keeps GD positioned as a self-evolving design agent rather than a generic
personal assistant.
...
|---|---|---|
| List templates | Automations template picker | `gd automation template list --json` |
| Run ingestion | Automations "Ingest source" panel | `gd automation source ingest --source-kind <kind> --body-file <path|-> --json` |
| Inspect source packets | Recent ingestions list | `gd automation source list/get --json` |
| Review proposals | Review drawer with diff and apply/reject | `gd automation proposal list/apply/reject --json` |
| Edit memory tree | Settings -> Memory tree | `od memory tree ... --json` |
| Manage token compression | Ingestion compression selector | `gd automation source ingest --compression off|balanced|aggressive --json` |
| Promote design system | Evolution proposal apply action | `gd automation proposal apply <proposal> --json` |
| Crystallize skill | Evolution proposal apply action | `gd automation proposal apply <proposal> --json` |
...
- Update the existing Automations page and `gd automation` commands to present
...
- `gd automation proposal list/get/apply/reject` mirrors the UI review path.
...
- `gd automation source ingest/list/get` mirrors the Automations source panel.
