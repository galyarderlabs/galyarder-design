...
const GALYARDER_DESIGN_REPO_URL = 'https://github.com/galyarderlabs/galyarder-design';
const GALYARDER_DESIGN_REPO_LABEL = 'galyarderlabs/galyarder-design';
...
export function derivePluginSourceLinks(
  ...
  const authorName = typeof author.name === 'string' && author.name.trim().length > 0
    ? author.name.trim()
    : null;
  const authorProfileUrl = officialBundled ? GALYARDER_DESIGN_REPO_URL : safeHttpUrl(author.url);
  const homepageUrl = officialBundled ? GALYARDER_DESIGN_REPO_URL : safeHttpUrl(homepageRaw);
  ...
  if (record.sourceKind === 'github') {
    ...
    sourceUrl = GALYARDER_DESIGN_REPO_URL;
    sourceLabel = GALYARDER_DESIGN_REPO_LABEL;
  } else {
