...
function buildOpportunities(input: {
  ...
  const doorwayQueries = queries
    ...
    .map((row) =>
      opportunity(
        'Doorway page query',
        row.keys[0] ?? '',
        `Impressions ${number(row.impressions)} · avg rank ${row.position.toFixed(1)} · CTR ${percent(row.ctr)}`,
        'Strengthen the matching page title, H1, FAQ, and internal links to push page-2 queries into the top 10.',
      ),
    );
  ...
  const lowCtrQueries = queries
    ...
    .map((row) =>
      opportunity(
        'High-impression low-CTR query',
        row.keys[0] ?? '',
        `Impressions ${number(row.impressions)} · rank ${row.position.toFixed(1)} · CTR ${percent(row.ctr)}`,
        'Rewrite the target page title / meta description so the snippet matches search intent and differentiating value.',
      ),
    );
  ...
  const lowCtrPages = pages
    ...
    .map((row) =>
      opportunity(
        'High-rank low-CTR page',
        row.keys[0] ?? '',
        `Impressions ${number(row.impressions)} · rank ${row.position.toFixed(1)} · CTR ${percent(row.ctr)}`,
        'Audit the SERP title, description, and above-the-fold promise so a strong rank does not leak clicks.',
      ),
    );
...
function deviceCtrGapOpportunities(
  ...
  return [
    opportunity(
      'Device CTR gap',
      `${worse} CTR is lagging`,
      `mobile ${percent(mobile.ctr)} / desktop ${percent(desktop.ctr)} · gap ${(relativeGap * 100).toFixed(0)}%`,
      `Audit the ${worse} SERP promise and landing-page experience — check whether the hero, page speed, and CTA are dragging click-through down.`,
    ),
  ];
}
...
function buildFeishuCard(report: DailyReport) {
  return {
    ...
    header: {
      ...
      title: {
        tag: 'plain_text',
        content: `SEO daily report · ${report.reportDate}`,
      },
    },
    elements: [
      markdown(summaryMarkdown(report)),
      { tag: 'hr' },
      markdown(dimensionMarkdown('Device split (last 7 days)', report.devices)),
      markdown(dimensionMarkdown('Top 5 countries / regions (last 7 days)', report.countries)),
      ...(report.searchAppearances.length > 0
        ? [markdown(dimensionMarkdown('Search appearance (last 7 days)', report.searchAppearances))]
        : []),
      { tag: 'hr' },
      markdown(moversMarkdown('Top 5 page risers', report.pageRisers)),
      markdown(moversMarkdown('Top 5 page fallers', report.pageFallers)),
      markdown(moversMarkdown('Top 5 query risers', report.queryRisers)),
      { tag: 'hr' },
      markdown(opportunitiesMarkdown(report)),
      { tag: 'hr' },
      markdown(
        `Coverage: single day ${report.reportDate} vs ${report.comparisonDate}; dimensions / opportunities ${report.rollingStartDate}..${report.rollingEndDate} · ${GSC_SITE_URL} · GSC API`,
      ),
    ],
  };
}
...
function summaryMarkdown(report: DailyReport): string {
  return [
    `**Site**: \`${GSC_SITE_URL}\``,
    '',
    `- Clicks: **${number(report.metrics.clicks)}** ${percentDeltaText(report.delta.clicks)}`,
    `- Impressions: **${number(report.metrics.impressions)}** ${percentDeltaText(report.delta.impressions)}`,
    `- CTR: **${percent(report.metrics.ctr)}** ${pointsDeltaText(report.delta.ctrPoints)}`,
    `- Avg rank: **${report.metrics.position.toFixed(1)}** ${positionDeltaText(report.delta.position)}`,
  ].join('\n');
}
...
function moversMarkdown(title: string, movers: Mover[]): string {
  ...
  return [
    `**${title}**`,
    '',
    '| Item | Day clicks | Δ vs last week | Day impressions |',
    '| --- | ---: | ---: | ---: |',
    ...movers.map(
      (mover) =>
...
function opportunitiesMarkdown(report: DailyReport): string {
  const sections = [
    opportunitySection('Doorway page query', report.opportunities.doorwayQueries),
    opportunitySection('High-impression low-CTR query', report.opportunities.lowCtrQueries),
    opportunitySection('High-rank low-CTR page', report.opportunities.lowCtrPages),
    opportunitySection('Device CTR gap', report.opportunities.deviceCtrGaps),
  ].filter(Boolean);

  const thresholdLine = `Thresholds: impressions ≥ ${number(report.thresholds.minImpressions)} · low CTR < ${percent(report.thresholds.lowCtr)} · relative device CTR gap > ${(report.thresholds.mobileDesktopCtrGap * 100).toFixed(0)}%`;

  if (sections.length === 0) {
    return `**Optimization opportunities (last 7 days)**\n\nNo clear candidates in the last 7 days.\n\n${thresholdLine}`;
  }

  return ['**Optimization opportunities (last 7 days)**', '', thresholdLine, '', ...sections].join('\n');
}
...
function opportunitySection(title: string, opportunities: Opportunity[]): string {
  ...
  return [
    ...
    ...opportunities.map(
      (item) =>
        `- ${formatKey(item.item)} — ${item.evidence}; suggestion: ${item.action}`,
    ),
  ].join('\n');
}
