...
describe('adaptAgentSkill', () => {
  it('synthesizes a v1-valid manifest from gd: frontmatter', () => {
    ...
    expect(result.manifest.gd?.mode).toBe('prototype');
    expect(result.manifest.gd?.preview?.entry).toBe('index.html');
    expect(result.manifest.gd?.context?.craft).toEqual(['typography', 'typography-hierarchy']);
    expect(result.manifest.gd?.inputs?.[0]?.type).toBe('select');
    expect(result.manifest.gd?.inputs?.[0]?.options).toEqual(['editorial', 'casual']);
