...
export function buildDesignManifestContent(opts: {
  ...
  const title = opts.title || 'Galyarder Design artifact';
  ...
  return JSON.stringify({
    schema: 'galyarder-design.design-manifest.v1',
    title,
    kind: opts.kind ?? 'html',
    entryFile,
    sourceFiles: {
...
- Keep generated product UI free of Galyarder Design chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or Galyarder Design chrome in the production UI.
...
export function requestPreviewSnapshot(
  ...
  return new Promise((resolve) => {
    ...
    function onMsg(ev: MessageEvent) {
      ...
      if (!d || d.type !== 'gd:snapshot:result' || d.id !== id) return;
      if (done) return;
      done = true;
    ...
    try {
      win.postMessage({ type: 'gd:snapshot', id }, '*');
    } catch {
      /* sandboxed */
    }
    setTimeout(() => {
...
function injectParentPrintReadyCache(doc: string, nonce: string): string {
  const script = `<script>window.__odPrintReady=false;window.addEventListener('message',function(e){if(e.data&&e.data.type==='GD_PRINT_READY'&&e.data.nonce==='${nonce}'&&(e.source===window||(window.frames&&e.source===window.frames[0])))window.__odPrintReady=true});<\/script>`;
