...
// Locale-aware label regex for the "Open Settings" button — matches the
// English source as well as the Simplified and Traditional Chinese
// translations from apps/web/src/i18n/locales/. CJK literals are kept
// verbatim because they're functional UI-locale detection patterns.
const OPEN_SETTINGS_LABEL = /Open settings|打开设置|開啟設定/i;
// Locale-aware label regex for the "Local CLI" tab; CJK literals match the
// zh-CN / zh-TW translations and are kept verbatim as detection patterns.
const LOCAL_CLI_LABEL = /Local CLI|本机 CLI|本地 CLI/i;

test.describe.configure({ timeout: 30_000 });

async function waitForLoadingToClear(page: Page) {
  await expect(page.getByText('Loading Galyarder Design…')).toHaveCount(0, { timeout: 15_000 });
}
