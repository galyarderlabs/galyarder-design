...
describe('resolveProjectDir', () => {
  const projectsRoot = '/var/gd/projects';
...
describe('detectEntryFile', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(path.join(tmpdir(), 'gd-detect-entry-'));
  });
...
describe('listFiles with metadata.baseDir', () => {
  ...
  beforeEach(async () => {
    baseDir = mkdtempSync(path.join(tmpdir(), 'gd-list-'));
  ...
  // standard .gd/projects/<id>/ instead of the imported folder. Codex
  ...
  it('does not skip those dirs for non-baseDir projects (back-compat)', async () => {
    ...
    const standardDir = mkdtempSync(path.join(tmpdir(), 'gd-list-std-'));
    try {
