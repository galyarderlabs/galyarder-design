...
import { resolveNamespace } from "@galyarder-design/sidecar";
...
export type ToolPackConfig = {
   ...
   * at packaging time. Baked into galyarder-design-config.json so the packaged
  ...
  /**
   * PostHog project ID (e.g. `420348` for the official Galyarder Design project)
...
function resolveToolPackWebOutputMode(platform: ToolPackPlatform, value: string | undefined): ToolPackWebOutputMode {
  ...
  throw new Error(`unsupported GD_WEB_OUTPUT_MODE value: ${value}`);
}
...
function resolveToolPackTelemetryRelayUrl(value: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new Error(`GALYARDER_DESIGN_TELEMETRY_RELAY_URL must be an absolute https URL: ${value}`);
  }
  if (parsed.protocol !== "https:") {
    throw new Error(`GALYARDER_DESIGN_TELEMETRY_RELAY_URL must use https: ${value}`);
  }
  return normalized.replace(/\/+$/, "");
}
...
export function resolveToolPackConfig(
  ...
  return {
    ...
    telemetryRelayUrl: resolveToolPackTelemetryRelayUrl(process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL),
    ...
    webOutputMode: resolveToolPackWebOutputMode(platform, process.env.GD_WEB_OUTPUT_MODE),
