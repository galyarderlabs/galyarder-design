...
const TAVILY_ENV_KEYS = ['GD_TAVILY_API_KEY', 'TAVILY_API_KEY'];
...
describe('research search', () => {
  ...
  async function tempProjectRoot() {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'gd-research-project-'));
    return projectRoot;
  }
  ...
  it('uses shallow Tavily search and normalizes JSON findings', async () => {
    process.env.GD_TAVILY_API_KEY = 'tvly-test';
