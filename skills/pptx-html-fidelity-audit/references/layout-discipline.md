...
def slide_two_col(prs, *, kicker, headline, intro, left, right):
    slide = prs.slides.add_slide(blank_layout)
    paint_bg(slide, COLOR_INK)
    add_chrome(slide, "First-Time Caveats", "05 / 14", theme="dark")
    ...
    add_footer(slide, "Page 05 · First-time caveats", "Ceremony ≠ Leadership Day", theme="dark")
```
...
def slide_obs_grid(prs, *, kicker, headline, intro, cards):
    ...
    add_footer(slide, "Page 09 · Six observation indicators", "For records · Don't grade in the moment", theme="light")
```
