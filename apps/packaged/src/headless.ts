...
import { bootstrapSidecarRuntime, createJsonIpcServer, resolveAppIpcPath } from "@galyarder-design/sidecar";
...
function resolveHeadlessNamespaceBaseRoot(): string {
  const odDataDir = process.env.GD_DATA_DIR;
  ...
  return join(dataBase, "galyarder-design", "namespaces");
}

function resolveHeadlessConfig(): PackagedConfig {
  const namespace =
    GALYARDER_DESIGN_SIDECAR_CONTRACT.normalizeNamespace(
      process.env[PACKAGED_NAMESPACE_ENV] ?? SIDECAR_DEFAULTS.namespace,
    );

  const namespaceBaseRoot = resolveHeadlessNamespaceBaseRoot();

  // GD_RESOURCE_ROOT may be set by a launcher script; otherwise default to a
  // sibling galyarder-design/ directory relative to the node_modules that contain
  // this file — the layout written by tools-pack linux headless-install.
  const resourceRoot =
    process.env.GD_RESOURCE_ROOT ??
    join(__dirname, "..", "..", "..", "galyarder-design");
  ...
  return {
    ...
    nodeCommand: null,
    resourceRoot,
    telemetryRelayUrl: process.env.GALYARDER_DESIGN_TELEMETRY_RELAY_URL?.trim() || null,
...
async function main(): Promise<void> {
  ...
  const sidecars = await startPackagedSidecars(runtime, paths, {
    ...
    // Pinning GD_REQUIRE_DESKTOP_AUTH here would arm a gate no client
  ...
  const shutdown = async (): Promise<void> => {
    process.stdout.write("\n Shutting down Galyarder Design...\n");
...
void main().catch((error: unknown) => {
  process.stderr.write(
    `galyarder-design headless failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exit(1);
});
