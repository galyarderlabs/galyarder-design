...
describe('OpenAI-compatible media providers', () => {
  ...
  const originalImageRouterKey = process.env.GD_IMAGEROUTER_API_KEY;
  const originalCustomImageKey = process.env.GD_CUSTOM_IMAGE_API_KEY;
  const originalMediaConfigDir = process.env.GD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.GD_DATA_DIR;

  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), 'gd-openai-compatible-media-'));
    projectRoot = path.join(root, 'project-root');
    projectsRoot = path.join(projectRoot, '.gd', 'projects');
    await mkdir(projectsRoot, { recursive: true });
    delete process.env.GD_IMAGEROUTER_API_KEY;
    delete process.env.GD_CUSTOM_IMAGE_API_KEY;
  ...
  afterEach(async () => {
    ...
    if (originalImageRouterKey == null) {
      ...
      process.env.GD_IMAGEROUTER_API_KEY = originalImageRouterKey;
    }
    if (originalCustomImageKey == null) {
      delete process.env.GD_CUSTOM_IMAGE_API_KEY;
    } else {
      process.env.GD_CUSTOM_IMAGE_API_KEY = originalCustomImageKey;
    }
  ...
  it('renders ImageRouter images through the OpenAI-compatible JSON endpoint', async () => {
    process.env.GD_IMAGEROUTER_API_KEY = 'ir-test-key';
