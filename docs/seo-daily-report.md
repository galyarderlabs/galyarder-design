...
`sc-domain:galyarder-design.dev` to a Feishu group bot.
...
| --- | --- | --- |
| `GSC_SERVICE_ACCOUNT_KEY` | Yes, unless OAuth is configured | Full service-account JSON key. The service-account email must have access to the `galyarder-design.dev` Search Console property. |
...
The current defaults are intentionally low because `galyarder-design.dev` is still
...
```bash
pnpm --filter @galyarder-design/landing-page exec tsx scripts/seo-daily-report.ts --dry-run
```
