...
//   - `gd plugin search <query>` (new in §3.Y1) — searches installed
//     plugins for discovery,
//   - `gd plugin list --filter ...` (new in §3.Y1) — same filter
...
export function searchInstalledPlugins(input: SearchInstalledPluginsInput): SearchInstalledPluginsResult {
  ...
  for (const plugin of input.plugins) {
    ...
    if (taskKind && (manifest.gd?.taskKind ?? '').toLowerCase() !== taskKind) continue;
    if (taskKind) matched.push('taskKind');
    if (mode && (manifest.gd?.mode ?? '').toLowerCase() !== mode) continue;
