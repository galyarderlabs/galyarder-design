...
async function setupProject() {
  const root = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-test-'));
  const projectId = 'p1';
...
describe('deploy config', () => {
  it('stores Vercel credentials in vercel.json and returns only the public mask', async () => {
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-config-test-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
    try {
      ...
      if (priorStateRoot === undefined) delete process.env.GD_USER_STATE_DIR;
      else process.env.GD_USER_STATE_DIR = priorStateRoot;
  ...
  it('requires Cloudflare Pages token and account id while deriving project names automatically', async () => {
    const stateRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-config-required-'));
    const priorStateRoot = process.env.GD_USER_STATE_DIR;
    process.env.GD_USER_STATE_DIR = stateRoot;
    try {
      ...
      // UTF-8 / CJK fixture; verifies safeDnsLabel strips non-ASCII characters.
      expect(cloudflarePagesProjectNameForProject('project-123', 'AI 生图网站')).toBe(
        'gd-ai-project-123',
      );
      // UTF-8 / CJK fixture; an all-CJK name slugifies to empty so we fall back to 'project'.
...
describe('deploy file set', () => {
  ...
  it('injects a closeable deploy hook script from cdn when configured', async () => {
    ...
    const files = await buildDeployFileSet(projectsRoot, projectId, 'page.html', {
      hookScriptUrl: 'https://cdn.example.com/galyarder-design-hook.js',
    });
    const html = files.find((f) => f.file === 'index.html')?.data.toString('utf8') ?? '';

    expect(html).toContain(
      '<script src="https://cdn.example.com/galyarder-design-hook.js" defer data-galyarder-design-deploy-hook="true" data-closeable="true"></script></body>',
    );
  });
...
describe('deploy plan and analyzer', () => {
  async function setupProject() {
    const root = await mkdtemp(path.join(os.tmpdir(), 'gd-deploy-plan-test-'));
    const projectId = 'p1';
    const dir = await ensureProject(path.join(root, 'projects'), projectId);
    return { projectsRoot: path.join(root, 'projects'), projectId, dir };
  }
...
describe('cloudflare pages deploys', () => {
  ...
  it('round-trips typed Cloudflare info while keeping provider metadata internal', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'gd-deployment-db-test-'));
    try {
      const db = openDatabase(root, { dataDir: path.join(root, '.gd') });
      insertProject(db, {
        id: 'project-1',
        name: 'Project 1',
        skillId: null,
      ...
      const saved = upsertDeployment(db, {
        ...
        providerMetadata: {
          ...
          cloudflarePagesCustomDomain: {
            ...
            hostname: 'demo.example.com',
            marker: 'gd:cfp:aaaaaaaaaaaa:bbbbbbbbbbbb',
            dnsRecordId: 'dns-1',
          },
        },
        createdAt: 1,
  ...
  it('creates a Cloudflare DNS CNAME and Pages custom domain while keeping pages.dev primary', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.endsWith('/zones/zone-1/dns_records') && method === 'POST') {
        ...
        expect(body.comment).toMatch(/^gd:cfp:[a-f0-9]{12}:[a-f0-9]{12}$/);
        return new Response(JSON.stringify({ success: true, result: { id: 'dns-1', ...body } }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        });
