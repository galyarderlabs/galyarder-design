...
import { renderPluginBlock } from '@galyarder-design/contracts';
...
async function freshFixture(targetPath: string, version = '1.0.0') {
  ...
  await writeFile(path.join(targetPath, 'galyarder-design.json'), JSON.stringify(manifest, null, 2));
  await writeFile(
...
beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'gd-dod-'));
...
describe('Plan §8 e2e — daemon-side anchors', () => {
  ...
  it('e2e-6 connector trust gate: restricted plugin without connector:<id> blocks apply + tool token', async () => {
    ...
    await writeFile(path.join(folder, 'galyarder-design.json'), JSON.stringify(manifest));
    await writeFile(
