...
**Status:** Draft (for galyarderlabs/galyarder-design Issue, post #597 merge)
**Author:** @infinity-nft
...
plain `npm run dev`). Currently GD opens such projects as a static file
panel — the user has to launch the dev server themselves in another
terminal and then iframe-load it manually.

This RFC proposes letting GD detect a dev-server config from the
...
- Click "Open folder" → GD detects `next dev` script → asks "Launch
...
1. **Permission model** — running `npm install` + `npm run dev` on a
   user folder is more privileged than reading files. Should GD prompt
...
2. **Auto-install missing dependencies** — if `node_modules` is missing,
   should GD offer to run `pnpm install` first? Or fail clearly and let
   the user run it themselves?
3. **Port conflicts** — if 3000 is taken, should GD pick the next free
...
6. **Non-folder projects** — current GD generates HTML files in
...
- HMR support (dev servers handle their own HMR; GD just iframes)
- Production builds (`pnpm build` is the user's concern)
- Custom proxy / rewrites between GD daemon and dev server
