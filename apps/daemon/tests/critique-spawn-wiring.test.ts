...
import type { CritiqueSseEvent } from '@galyarder-design/contracts/critique';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'gd-spawn-wiring-test-'));
  db = freshDb();
});
afterEach(async () => {
  db.close();
...
// ---------------------------------------------------------------------------
// Config gate: GD_CRITIQUE_ENABLED=false (legacy path unchanged)
// ---------------------------------------------------------------------------
...
describe('spawn wiring - cfg.enabled=false (M0 default)', () => {
  ...
  it('loadCritiqueConfigFromEnv with GD_CRITIQUE_ENABLED=false returns enabled=false', () => {
...
// ---------------------------------------------------------------------------
// Config gate: GD_CRITIQUE_ENABLED=true (orchestrator path)
// ---------------------------------------------------------------------------

describe('spawn wiring - cfg.enabled=true (orchestrator path)', () => {
  it('with GD_CRITIQUE_ENABLED=true, runOrchestrator is invoked with the spawn stdout', async () => {
