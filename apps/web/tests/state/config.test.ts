...
describe('loadConfig', () => {
  it('migrates legacy OpenAI-compatible API configs to an explicit apiProtocol', () => {
    ...
    store.set('galyarder-design:config', JSON.stringify(legacyConfig));
  ...
  it('infers protocol for legacy daemon-mode API fields without changing mode', () => {
    ...
    store.set('galyarder-design:config', JSON.stringify(daemonConfig));
  ...
  it('does not overwrite an already explicit apiProtocol', () => {
    ...
    store.set('galyarder-design:config', JSON.stringify(explicitConfig));
  ...
  it('preserves a valid saved accent color', () => {
    ...
    store.set('galyarder-design:config', JSON.stringify(savedConfig));
  ...
  it('returns defaults for malformed localStorage JSON', () => {
    store.set('galyarder-design:config', '{broken-json');
  ...
  it('sets an explicit apiProtocol for new default configs', () => {
    ...
    expect(DEFAULT_CONFIG.accentColor).toBe('#8A00FF');
  });
});
...
describe('saveConfig', () => {
  it('keeps daemon-owned privacy fields out of localStorage', () => {
    ...
    const saved = JSON.parse(store.get('galyarder-design:config') ?? '{}');
