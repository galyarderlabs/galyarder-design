...
const toolsPackDir = resolveFromWorkspace(process.env.GD_PACKAGED_E2E_TOOLS_PACK_DIR ?? '.tmp/tools-pack');
const namespace = process.env.GD_PACKAGED_E2E_NAMESPACE ?? 'release-beta-win';
...
const maxInstallDurationMs = Number.parseInt(process.env.GD_PACKAGED_E2E_WIN_MAX_INSTALL_MS ?? '120000', 10);
const verifyReinstallWhileRunning = process.env.GD_PACKAGED_E2E_WIN_VERIFY_REINSTALL !== '0';
const verifyViolentUpdater = process.env.GD_PACKAGED_E2E_WIN_UPDATER_VIOLENT !== '0';
const verifyRealUpdateInstaller = process.env.GD_PACKAGED_E2E_WIN_REAL_UPDATE_INSTALL === '1';
const releaseChannel = process.env.GD_PACKAGED_E2E_RELEASE_CHANNEL;
const releaseVersion = process.env.GD_PACKAGED_E2E_RELEASE_VERSION;
...
const violentArtifactBytes = Number.parseInt(process.env.GD_PACKAGED_E2E_WIN_UPDATER_BYTES ?? `${16 * 1024 * 1024}`, 10);
const violentChunkDelayMs = Number.parseInt(process.env.GD_PACKAGED_E2E_WIN_UPDATER_CHUNK_DELAY_MS ?? '18', 10);
...
const shouldRunPackagedWinSmoke = process.platform === 'win32' && process.env.GD_PACKAGED_E2E_WIN === '1';
...
winDescribe('packaged windows runtime smoke', () => {
  ...
  test('installs, starts, inspects with eval and screenshot, stops, and uninstalls the built windows artifact', async () => {
    ...
    try {
      ...
      expect(JSON.stringify(install.registryEntries)).toContain(`Galyarder Design-${installIdentity.namespaceToken}`);
      ...
      expect(inspect.status?.url).toBe('gd://app/');

      const value = assertHealthEvalValue(inspect.eval?.value);
      expect(value.href).toBe('gd://app/');
...
async function runRealUpdateInstallerAcceptance(options: {
  ...
  if (releaseChannel == null || releaseChannel === '' || releaseVersion == null || releaseVersion === '') {
    throw new Error('GD_PACKAGED_E2E_WIN_REAL_UPDATE_INSTALL requires GD_PACKAGED_E2E_RELEASE_CHANNEL and GD_PACKAGED_E2E_RELEASE_VERSION');
  }
  ...
  applyManualPackagedUpdateEnv(
    process.env,
    updateScenario,
    `https://releases.galyarder-design.ai/${updateScenario.channel}/latest/metadata.json`,
  );
  ...
  expect(entry.keyPath).toContain(`Galyarder Design-${installIdentity.namespaceToken}`);
...
async function ensureManagedDownloadBase(updateRoot: string): Promise<string> {
  ...
  await writeFile(
    join(downloadsRoot, '.galyarder-design-download-root.json'),
    `${JSON.stringify({
      createdAt: new Date().toISOString(),
      kind: 'galyarder-design-managed-download-root',
      schemaVersion: 1,
    }, null, 2)}\n`,
    'utf8',
  );
...
const UPDATE_ENV_KEYS = [
  'GD_UPDATE_AUTO_CHECK',
  'GD_UPDATE_AUTO_DOWNLOAD',
  'GD_UPDATE_ENABLED',
  'GD_UPDATE_METADATA_URL',
  'GD_UPDATE_CURRENT_VERSION',
  'GD_UPDATE_OPEN_DRY_RUN',
] as const;
...
function applyManualPackagedUpdateEnv(
  ...
  env.GD_UPDATE_AUTO_CHECK = '0';
  env.GD_UPDATE_AUTO_DOWNLOAD = '0';
}
...
async function startViolentUpdaterFixtureProcess(
  ...
  const artifactName = `galyarder-design-${version}-win-x64-setup.exe`;
...
async function resolveExpectedNamespaceRoot(installDir: string): Promise<string> {
  const installedConfig = JSON.parse(
    await readFile(join(installDir, 'resources', 'galyarder-design-config.json'), 'utf8'),
