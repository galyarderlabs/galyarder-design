...
describe('pluginVisualScore', () => {
  it('boosts featured plugins above everything else', () => {
    ...
    const featured = fixture({ id: 'featured', gd: { featured: true } });
  ...
  it('uses numeric featured ranks to order curated picks', () => {
    const lead = fixture({ id: 'lead', gd: { featured: 2 } });
    const later = fixture({ id: 'later', gd: { featured: 19 } });
  ...
  it('ranks media-rich plugins above plain scenarios', () => {
    ...
    const deckHtml = fixture({
      id: 'deck',
      gd: { mode: 'deck', preview: { type: 'html', entry: './index.html' } },
    });
    const image = fixture({
      id: 'image',
      gd: { surface: 'image', mode: 'image', preview: { type: 'image', poster: 'a.png' } },
    });
    const video = fixture({
      id: 'video',
      gd: { surface: 'video', mode: 'video', preview: { type: 'video', video: 'a.mp4' } },
    });
  ...
  it('credits design-system plugins between decks and plain text', () => {
    const text = fixture({ id: 'text' });
    const ds = fixture({ id: 'ds', gd: { mode: 'design-system' } });
  ...
  it('penalises atom kind so they never accidentally lead the grid', () => {
    const atom = fixture({ id: 'a', gd: { kind: 'atom' } });
...
describe('sortByVisualAppeal', () => {
  ...
  it('puts curated home picks before generic featured media', () => {
    const records = [
      ...
      fixture({
        id: 'example-galyarder-design-landing',
        gd: { mode: 'prototype', preview: { type: 'html', entry: './index.html' } },
      }),
    ];
    const sorted = sortByVisualAppeal(records).map((r) => r.id);
    expect(sorted[0]).toBe('example-galyarder-design-landing');
  });
  ...
  it('keeps numeric featured rank ahead of media bonuses', () => {
    const records = [
      ...
      fixture({ id: 'huashu', gd: { mode: 'prototype', featured: 0.03 } }),
      fixture({ id: 'kami', gd: { mode: 'deck', featured: 0.06 } }),
    ];
    const sorted = sortByVisualAppeal(records).map((r) => r.id);
