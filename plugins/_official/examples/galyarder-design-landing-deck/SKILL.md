---
name: galyarder-design-landing-deck
description: >
  ...
  dots, surreal collage plates) — Galyarder Design's brand deck recipe.
  ...
  `galyarder-design-landing` skill.
triggers:
  - slide deck
  - presentation
  - pitch deck
...
inputs:
  ...
  - id: deck_title
    ...
    description: e.g. `'Galyarder Design · Vol. 01 / Issue Nº 26'`.
  - id: slides
  ...
  - id: imagery
    ...
    schema_path: ../galyarder-design-landing/schema.ts#ImageryConfig
parameters:
...
example_prompt: |
  ...
  card. Reuse the galyarder-design-landing image library.
---

# galyarder-design-landing-deck

Sister skill to [`galyarder-design-landing`](../galyarder-design-landing/). Same
...
```text
inputs.json + ../galyarder-design-landing/styles.css
        │
        └──────────► scripts/compose.ts
                            │
                            ▼
...
`galyarder-design-landing` brief, copy `brand` and `imagery` over verbatim.
...
script — it shares the 16-slot library from `galyarder-design-landing`. To
regenerate or stub:

```bash
# generate via gpt-image-2 (fal.ai)
FAL_KEY=... npx tsx ../galyarder-design-landing/scripts/imagegen.ts ../galyarder-design-landing/inputs.example.json --out=../galyarder-design-landing/assets/

# or paper-textured SVG placeholders
npx tsx ../galyarder-design-landing/scripts/placeholder.ts ../galyarder-design-landing/assets/
```

Set your deck's `inputs.imagery.assets_path` to wherever those PNGs
live (default in the example: `../galyarder-design-landing/assets/`).
...
stylesheet from `../galyarder-design-landing/styles.css`, layers
...
- **Reuse the sister skill's stylesheet.** The composer reads
  `../galyarder-design-landing/styles.css` at compile time. Do not
...
- [`galyarder-design-landing`](../galyarder-design-landing/) — landing page sister skill.
