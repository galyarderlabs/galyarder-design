...
export function readManualEditAttributes(source: string, id: string): Record<string, string> {
  ...
  Array.from(el.attributes).forEach((attr) => {
    if (attr.name === 'data-gd-runtime-id') return;
...
function inferKind(el: Element): 'text' | 'link' | 'image' | 'container' {
  const explicit = el.getAttribute('data-gd-edit');
...
function findEditableElement(doc: Document, id: string): Element | null {
  ...
  return (
    doc.querySelector(`[data-gd-id="${cssEscape(id)}"]`) ??
    doc.querySelector(`[data-gd-runtime-id="${cssEscape(id)}"]`) ??
    doc.querySelector(`[data-gd-source-path="${cssEscape(id)}"]`) ??
...
function setAttributes(el: Element, attributes: Record<string, string>): void {
  const protectedAttrs = new Set(['data-gd-id', 'data-gd-edit', 'data-gd-label', 'data-gd-runtime-id']);
...
function replaceOuterHtml(doc: Document, el: Element, html: string): { ok: true } | { ok: false; error: string } {
  ...
  if (el.getAttribute('data-gd-id') && !next.getAttribute('data-gd-id')) {
    next.setAttribute('data-gd-id', el.getAttribute('data-gd-id') ?? '');
  }
  if (el.getAttribute('data-gd-edit') && !next.getAttribute('data-gd-edit')) {
    next.setAttribute('data-gd-edit', el.getAttribute('data-gd-edit') ?? '');
  }
  el.replaceWith(next);
  return { ok: true };
}
