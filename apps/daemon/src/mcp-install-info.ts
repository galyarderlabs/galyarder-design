...
// ELECTRON_RUN_AS_NODE env read, GD_DATA_DIR resolution, sidecar IPC
// detection) all stay in the caller. This module is intentionally pure
// and free of @galyarder-design/sidecar-proto so it can be unit-tested
...
export interface BuildMcpInstallPayloadInputs {
   ...
   *  spawned `gd mcp` should discover the live URL via the IPC
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Galyarder Design CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall Galyarder Design or Node and restart the daemon.`,
    );
  }
  // Pin GD_DATA_DIR to the daemon's resolved data root so the spawned
  ...
  // `gd mcp` falls back to `<cwd>/.gd/...` which is the read-only
  // macOS app bundle for packaged installs and trips EPERM. Issue #848.
  const env: Record<string, string> = {
    GD_DATA_DIR: inputs.dataDir,
    ...inputs.sidecarEnv,
  };
  if (inputs.electronAsNode) {
    env.ELECTRON_RUN_AS_NODE = '1';
  }
  // Sidecar mode: omit --daemon-url so the spawned `gd mcp` discovers
  // the live URL via the IPC status socket on every spawn, surviving
  // ephemeral-port restarts. Direct `gd --port X` launches have no
