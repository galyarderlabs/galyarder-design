...
import { SIDECAR_DEFAULTS, normalizeNamespace } from "@galyarder-design/sidecar-proto";
...
export const PACKAGED_CONFIG_PATH_ENV = "GD_PACKAGED_CONFIG_PATH";
export const PACKAGED_NAMESPACE_ENV = "GD_PACKAGED_NAMESPACE";
export const PACKAGED_WEB_OUTPUT_MODE_OVERRIDE_ENV = "GD_PACKAGED_ALLOW_WEB_OUTPUT_MODE_OVERRIDE";
export const PACKAGED_WEB_STANDALONE_ROOT_ENV = "GD_WEB_STANDALONE_ROOT";
export const PACKAGED_WEB_OUTPUT_MODE_ENV = "GD_WEB_OUTPUT_MODE";
...
export type RawPackagedConfig = {
  ...
  resourceRoot?: string;
  // Baked by tools/pack from GALYARDER_DESIGN_TELEMETRY_RELAY_URL and forwarded to
...
function resolveDefaultConfigPath(): string {
  return join(process.resourcesPath, "galyarder-design-config.json");
}
...
async function readRawPackagedConfig(): Promise<RawPackagedConfig> {
  ...
  return (
    ...
    (await readJsonIfExists(join(electronApp.getAppPath(), "galyarder-design-config.json"))) ??
    {}
  );
}
...
function resolvePackagedWebStandaloneRoot(
  ...
  return join(process.resourcesPath, "galyarder-design-web-standalone");
}
...
export async function readPackagedConfig(): Promise<PackagedConfig> {
  ...
  const resourceRoot = resolveOptionalPath(raw.resourceRoot) ?? join(process.resourcesPath, "galyarder-design");
  const relativeNodeCommand =
    raw.nodeCommandRelative == null || raw.nodeCommandRelative.length === 0
      ? join("galyarder-design", "bin", "node")
