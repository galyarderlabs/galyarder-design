...
npx hyperframes tts "Kyou wa ii tenki desu ne" --voice jf_alpha --output ja.wav
```

(The Japanese example uses romaji so this doc stays in plain Latin script. The
phonemizer accepts native kana/kanji input the same way; substitute the literal
Japanese sentence at the call site if you prefer.)
