...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$GD_NODE_BIN" "$GD_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:GD_NODE_BIN $env:GD_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%GD_NODE_BIN%" "%GD_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
  ...
  if (safeQuery) {
    lines.push(
      ...
      'If the GD command fails because Tavily is not configured or unavailable, report the actual stderr/error, then use your own search capability as fallback and label the fallback clearly.',
