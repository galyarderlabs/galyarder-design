...
// English-only installer; other languages can be re-added via the same
// MUI_LANGUAGE / LangString machinery if multi-language Windows installs
// are reintroduced. The repo policy keeps non-English script out of source.
...
${createNsisLangString("CreateDesktopShortcut", "Create desktop shortcut")}
${createNsisLangString("LaunchApp", `Launch ${productName}`)}
${createNsisLangString("RemoveDesktopShortcut", "Remove desktop shortcut")}
${createNsisLangString("RemoveLocalData", "Delete local data for this installation")}
${createNsisLangString("UninstallOptionsTitle", "Uninstall options")}
${createNsisLangString("UninstallOptionsSubtitle", "Choose which local items to remove.")}
${createNsisLangString("RunningInstancesTitle", `${productName} is still running`)}
${createNsisLangString("RunningInstancesSubtitle", "Close it before continuing installation.")}
${createNsisLangString("RunningInstancesMessage", `${productName} must be closed before installation can continue.`)}
${createNsisLangString("CloseAndContinue", "Close and continue")}
${createNsisLangString("RunningInstancesCloseFailed", `${productName} could not be closed. Close it manually, then try again.`)}
${createNsisLangString("RunningInstancesSilentAbort", `${productName} is still running. Close it before running the installer silently.`)}
${createNsisLangString("ExistingInstallMessage", `${productName} is already installed in the selected folder. Choose OK to overwrite it, or Cancel to stop installation.`)}
${createNsisLangString("ExistingInstallSilentOverwrite", "Existing installation found; silent install will overwrite it.")}
