...
describe('resolveStandaloneServerEntry', () => {
  it('resolves the traced monorepo standalone server entry', async () => {
    const previousDistDir = process.env.GD_WEB_DIST_DIR;
    delete process.env.GD_WEB_DIST_DIR;
    const webRoot = await mkdtemp(join(tmpdir(), 'galyarder-design-web-standalone-'));
    ...
    try {
      ...
      if (previousDistDir == null) {
        ...
        process.env.GD_WEB_DIST_DIR = previousDistDir;
      }
  ...
  it('prefers a copied standalone resource root before package fallback entries', async () => {
    const previousDistDir = process.env.GD_WEB_DIST_DIR;
    delete process.env.GD_WEB_DIST_DIR;
    const webRoot = await mkdtemp(join(tmpdir(), 'galyarder-design-web-package-'));
    const copiedRoot = await mkdtemp(join(tmpdir(), 'galyarder-design-web-copied-'));
  ...
  it('can resolve a copied standalone resource without a web package root', async () => {
    const copiedRoot = await mkdtemp(join(tmpdir(), 'galyarder-design-web-copied-only-'));
    const copiedWebRoot = join(copiedRoot, 'apps', 'web');
...
describe('createStandaloneServerArgs', () => {
  it('preloads a parent monitor before running the standalone server entry', () => {
    const args = createStandaloneServerArgs('/tmp/galyarder-design/server.js');

    expect(args).toHaveLength(3);
    expect(args[0]).toBe('--import');
    expect(args[1]).toBe(createStandaloneParentMonitorImport());
    expect(args[2]).toBe('/tmp/galyarder-design/server.js');
  });

  it('uses a data import that exits when the recorded parent disappears', () => {
    const importSpecifier = createStandaloneParentMonitorImport('GD_TEST_PARENT_PID');
    ...
    expect(source).toContain('process.env["GD_TEST_PARENT_PID"]');
...
describe('standalone backend binding', () => {
  it('keeps the hidden standalone backend on loopback even when the public sidecar host is wider', () => {
    const env = createStandaloneBackendEnv({
      baseEnv: { ...process.env, GD_HOST: '0.0.0.0' },
      parentPid: 1234,
      port: 5876,
    });
    ...
    expect(env.GD_STANDALONE_PARENT_PID).toBe('1234');
  });
});
...
describe('normalizeDaemonProxyOriginHeader', () => {
  ...
  it('normalizes matching wildcard configured dev origins to the daemon origin', () => {
    const previous = process.env.GD_ALLOWED_DEV_ORIGINS;
    process.env.GD_ALLOWED_DEV_ORIGINS = '*.local-origin.dev';
    try {
      expect(
        normalizeDaemonProxyOriginHeader({
          daemonOrigin: 'http://127.0.0.1:7456',
      ...
      if (previous == null) delete process.env.GD_ALLOWED_DEV_ORIGINS;
      else process.env.GD_ALLOWED_DEV_ORIGINS = previous;
    }
  });
