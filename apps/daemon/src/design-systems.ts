...
type DesignSystemProjectManifest = {
  schemaVersion: 'gd-design-system-project/v1';
  id: string;
  name: string;
  category: string;
  description?: string;
...
export function isDesignTokenChannelEnabled(
  ...
  return env.GD_DESIGN_TOKEN_CHANNEL !== '0';
}
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable Galyarder Design design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier Galyarder Design design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
async function writeGeneratedDesignSystemFiles(
  ...
  const summary = input.summary || 'A user-created Galyarder Design design system.';
...
function renderUiKitComponent(name: string, title: string, purpose: string): string {
  ...
  return (
    <section className="gd-ui-kit-${name.toLowerCase()}">
...
function isReplaceableUiKitScaffold(text: string): boolean {
  return Buffer.byteLength(text, 'utf8') < 700 && /gd-ui-kit-[a-z-]+/u.test(text);
}
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future Galyarder Design projects.';
...
A reusable Galyarder Design package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating Galyarder Design artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderReferenceComponent(title: string): string {
  ...
  return (
    <section className="gd-design-system-preview">
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">Galyarder Design system</p>
...
function isProjectManifest(value: unknown, expectedId: string): value is DesignSystemProjectManifest {
  ...
  if (record.schemaVersion !== 'gd-design-system-project/v1') return false;
...
function extractSwatches(raw: string): string[] {
  ...
  // either position around the closing `**`. `[:：]` matches both ASCII
  // `:` and CJK fullwidth `：` (U+FF1A) since IMEs auto-pick the latter
  // when typing in Chinese.
