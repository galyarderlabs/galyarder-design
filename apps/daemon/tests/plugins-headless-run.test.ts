...
//
// Verifies the headless `gd plugin install → project create → run start`
...
async function withFakeAgent<T>(
  ...
  const dir = await mkdtemp(path.join(tmpdir(), 'gd-headless-agent-bin-'));
...
describe('Plan §8 e2e-3 (entry slice) — headless install → project → run', () => {
  ...
  it('creates share projects for publishing and contributing a user plugin', async () => {
    ...
    expect(shareBody.actionPluginId).toBe('gd-plugin-publish-github');
    ...
    expect(shareBody.prompt).toContain('Publish the local Galyarder Design plugin');
    expect(shareBody.prompt).toContain('/api/projects/$GD_PROJECT_ID/plugins/publish-github');
    ...
    expect(fileNames).toContain('plugin-source/sample-plugin/galyarder-design.json');
    ...
    expect(snapshot.pluginId).toBe('gd-plugin-publish-github');
    ...
    const contributeResp = await fetch(`${baseUrl}/api/plugins/sample-plugin/share-project`, {
      method:  'POST',
      headers: { 'content-type': 'application/json' },
      body:    JSON.stringify({ action: 'contribute-galyarder-design', locale: 'en' }),
    });
    ...
    expect(contributeBody.actionPluginId).toBe('gd-plugin-contribute-galyarder-design');
    ...
    expect(contributeBody.prompt).toContain('/api/projects/$GD_PROJECT_ID/plugins/contribute-galyarder-design');
    ...
    const previousRealGit = process.env.GD_REAL_GIT;
    ...
    process.env.GD_REAL_GIT = realGit;
    process.env.GIT_AUTHOR_NAME = 'Galyarder Design Test';
    process.env.GIT_AUTHOR_EMAIL = 'galyarder-design-test@example.com';
    process.env.GIT_COMMITTER_NAME = 'Galyarder Design Test';
    process.env.GIT_COMMITTER_EMAIL = 'galyarder-design-test@example.com';
    try {
      await withFakeAgent(
        'gh',
        `
...
if (args[0] === 'repo' && args[1] === 'fork') ok('forked galyarderlabs/galyarder-design');
if (args[0] === 'repo' && args[1] === 'clone') {
  const dest = args[3] || path.basename(args[2]);
  fs.mkdirSync(dest, { recursive: true });
  const init = spawnSync(process.env.GD_REAL_GIT, ['init'], { cwd: dest, stdio: 'inherit' });
  process.exit(init.status ?? 0);
}
if (args[0] === 'pr' && args[1] === 'create') ok('https://github.com/galyarderlabs/galyarder-design/pull/123');
...
if (args[0] === 'clone') {
  ...
  fs.mkdirSync(dest, { recursive: true });
  const init = spawnSync(process.env.GD_REAL_GIT, ['init', '-b', 'main'], { cwd: dest, encoding: 'utf8' });
  if (init.status !== 0) {
    if (init.stderr) process.stderr.write(init.stderr);
    process.exit(init.status ?? 1);
  }
  const remote = args.find((arg) => String(arg).startsWith('https://')) || 'https://github.com/test-user/galyarder-design.git';
  const remoteAdd = spawnSync(process.env.GD_REAL_GIT, ['remote', 'add', 'origin', remote], { cwd: dest, encoding: 'utf8' });
...
const result = spawnSync(process.env.GD_REAL_GIT, args, {
  cwd: process.cwd(),
  env: process.env,
  encoding: 'utf8',
});
...
`,
            async () => {
              ...
              const contributeEndpointResp = await fetch(
                `${baseUrl}/api/projects/${encodeURIComponent(contributeBody.project.id)}/plugins/contribute-galyarder-design`,
                {
                  method: 'POST',
                  headers: { 'content-type': 'application/json' },
              ...
              expect(contributeEndpointBody.url).toBe('https://github.com/galyarderlabs/galyarder-design/pull/123');
            },
          );
        },
      );
      ...
      if (previousRealGit === undefined) {
        delete process.env.GD_REAL_GIT;
      } else {
        process.env.GD_REAL_GIT = previousRealGit;
      }
  ...
  it('runs the CLI install → project create → plugin run path with query and local SKILL.md in the agent prompt', async () => {
    const pluginRoot = await mkdtemp(path.join(tmpdir(), 'gd-headless-cli-plugin-'));
    const pluginId = `headless-cli-plugin-${randomUUID().slice(0, 8)}`;
    const fixture = path.join(pluginRoot, pluginId);
    await mkdir(fixture, { recursive: true });
    await writeFile(
      path.join(fixture, 'galyarder-design.json'),
      JSON.stringify({
        $schema: 'https://galyarder-design.ai/schemas/plugin.v1.json',
        name: pluginId,
    ...
    try {
      ...
      const captureRoot = await mkdtemp(path.join(tmpdir(), 'gd-headless-cli-capture-'));
      const capturePath = path.join(captureRoot, 'prompt.txt');
      const previousCapture = process.env.GD_PROMPT_CAPTURE;
      process.env.GD_PROMPT_CAPTURE = capturePath;
      try {
        await withFakeAgent(
          'opencode',
          `
...
process.stdin.on('end', () => {
  fs.writeFileSync(process.env.GD_PROMPT_CAPTURE, input);
...
`,
        ...
        if (previousCapture === undefined) {
          delete process.env.GD_PROMPT_CAPTURE;
        } else {
          process.env.GD_PROMPT_CAPTURE = previousCapture;
        }
  ...
  // POST /api/runs so any plugin run with `gd.pipeline.stages[*]`
  // emits the stage timeline before the agent's message_chunk stream.
  it('first SSE event on a plugin run with gd.pipeline is pipeline_stage_started', async () => {
    ...
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'gd-headless-pipeline-'));
    ...
    // The fixture declares gd.pipeline.stages and is installed under
