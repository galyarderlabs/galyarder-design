...
4. Deployed example at `demo.galyarder-design.dev` (Topology C).
...
| SE2 | Automation template registry exposed in both web UI and `gd automation`. |
...
`gd automation proposal list/get/apply/reject` can review memory-node, skill,
...
`/api/automation-source-packets`, and `gd automation source ingest/list/get`.
...
Full design and end-to-end action plan live in
[`plans/cloud-dashboard-local-daemon.md`](plans/cloud-dashboard-local-daemon.md).
The invariant across every tier: the **daemon stays on the user's machine**
(it spawns the local agent CLIs and touches the filesystem — impossible on edge
runtimes); only the UI and an optional relay move off-box. The tiers are
ordered cheapest-first so each one ships value before the next begins.

| Tier | Capability | Reach | State |
|---|---|---|---|
| RA0 | LAN access — `GD_BIND_HOST=0.0.0.0`, open `http://192.168.x.x:7456` from any device on the same WiFi | same network | **works today**; private-LAN IPs already pass `origin-validation.ts`. Needs only docs + a `GD_BIND_HOST` toggle in Settings. |
| RA1 | Tunnel (Topology B) — `gd daemon --expose` prints a public URL; web "Connect daemon" screen takes URL + `GD_API_TOKEN` | anywhere | partially in Phase 2 scope; blocker is the configurable API base + bearer wrapper in `apps/web` (today every `fetch('/api/*')` is hardcoded same-origin). |
| RA2 | Relay + QR pairing (Paseo-style) — daemon dials **out** to a Cloudflare Durable Object relay; web/mobile/desktop pair via a scan code; E2E encrypted; machine never exposed inbound | anywhere, no open ports | new. Needs a relay service + a dial-out client mode in the daemon + a pairing protocol. Largest effort; do only when multi-device/shared use is real. |

Milestones:

| Milestone | Deliverable |
|---|---|
| RA0 | Document LAN flow in `README` / `QUICKSTART`; add a `GD_BIND_HOST` (loopback ↔ LAN) toggle in Settings with a security note for public WiFi. |
| RA1 | Configurable daemon base URL + bearer-auth fetch wrapper in `apps/web`; "Connect daemon" screen with health/agents test; `gd daemon --expose` helper around `cloudflared`. |
| RA2 | Cloudflare relay (Durable Object + WebSocket); daemon dial-out client; pairing handshake (QR / short code) issuing a per-device key; E2E encryption so the relay stays zero-knowledge. |

Exit criteria: RA0 — a phone on the same WiFi drives a real agent run against
the laptop daemon with no code changes. RA1 — the same works from outside the
LAN via a tunnel URL pasted once. RA2 — pairing is a single QR scan with no URL
or token handling, and the machine accepts no inbound connections.
...
- 2026-05-31 — Remote access ships in tiers RA0 (LAN) → RA1 (tunnel) → RA2 (relay + QR), daemon always staying on the user's machine. *Why:* LAN already works with zero code and covers the common "use from my phone on the same WiFi" case; the expensive relay+pairing work (Paseo-style) is deferred until multi-device/shared use is real. See [`plans/cloud-dashboard-local-daemon.md`](plans/cloud-dashboard-local-daemon.md).
