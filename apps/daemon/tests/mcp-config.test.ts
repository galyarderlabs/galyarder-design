...
describe('mcp-config storage', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'gd-mcpconfig-'));
  });
...
describe('isManagedProjectCwd', () => {
  const projectsDir = '/abs/.gd/projects';

  it('accepts a real per-project subdir', () => {
    expect(isManagedProjectCwd('/abs/.gd/projects/abc', projectsDir)).toBe(true);
    expect(
      isManagedProjectCwd('/abs/.gd/projects/abc/sub', projectsDir),
    ).toBe(true);
  });
  ...
  it('rejects path-prefix collisions (different sibling dir)', () => {
    // `/abs/.gd/projects-other` starts with `/abs/.gd/projects` as a string,
    // but is NOT a child of `/abs/.gd/projects/`. Strict-separator check
    // makes sure we don't accidentally write to an unrelated tree.
    expect(
      isManagedProjectCwd('/abs/.gd/projects-other/x', projectsDir),
    ).toBe(false);
  });
});
