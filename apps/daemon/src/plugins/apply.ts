...
//   - Inputs are validated against `manifest.gd.inputs`; missing required
...
export interface ApplyInput {
  ...
  // `gd.context.designSystem.primary: true` without a concrete ref get
  ...
  // apply resolves `gd.connectors.*` against the live catalog and
...
export function applyPlugin(input: ApplyInput): ApplyComputed {
  ...
  const mcpServers = manifest.gd?.context?.mcp?.slice() ?? [];
  ...
  const taskKind = (manifest.gd?.taskKind ?? 'new-generation') as AppliedPluginSnapshot['taskKind'];

  // Spec §23.3.3: when the plugin omits `gd.pipeline`, fall back to
  ...
  // gd.kind='scenario'. Tests + non-daemon callers can pass an empty
  ...
  const declaredSurfaces = manifest.gd?.genui?.surfaces ?? [];
  ...
  if (Array.isArray(manifest.gd?.context?.craft) && manifest.gd!.context!.craft!.length > 0) {
    projectMetadata.craftRequires = manifest.gd!.context!.craft!.slice();
  }

  const queryText = resolveLocalizedText(manifest.gd?.useCase?.query, input.locale);
...
function validateInputs(manifest: PluginManifest, raw: Record<string, unknown>): ValidationResult {
  const fields = manifest.gd?.inputs ?? [];
  ...
  // defined elsewhere (e.g. via `gd.useCase` later). This keeps inputs lossy
...
function buildAssetRefs(manifest: PluginManifest): PluginAssetRef[] {
  ...
  for (const raw of manifest.gd?.context?.assets ?? []) {
...
// Pick a global skill id from gd.context.skills[]. Two ref shapes are
// accepted:
//
...
function pickFirstSkillId(manifest: PluginManifest): string | undefined {
  for (const ref of manifest.gd?.context?.skills ?? []) {
...
function pickDesignSystemId(
  ...
  const ds = manifest.gd?.context?.designSystem;
