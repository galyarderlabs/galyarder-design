import type { MarketplaceManifest } from '@galyarder-design/contracts';
import type {
  RegistryPublishOutcome,
  RegistryPublishRequest,
  RegistryYankOutcome,
} from '@galyarder-design/registry-protocol';
...
export class GithubRegistryBackend extends StaticRegistryBackend {
  ...
  private constructor(options: GithubRegistryBackendOptions & { manifest: MarketplaceManifest }) {
    ...
    this.marketplacePath = options.marketplacePath ?? 'plugins/registry/official/galyarder-design-marketplace.json';
    this.client = options.client;
  }

  static async create(options: GithubRegistryBackendOptions): Promise<GithubRegistryBackend> {
    const ref = options.ref ?? 'main';
    const marketplacePath = options.marketplacePath ?? 'plugins/registry/official/galyarder-design-marketplace.json';
