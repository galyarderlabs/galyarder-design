...
describe('artifact stub guard via /api/projects/:id/files', () => {
  ...
  beforeAll(async () => {
    vi.stubEnv('GD_ARTIFACT_STUB_GUARD', 'reject');
    vi.stubEnv('GD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES', '1024');
    vi.stubEnv('GD_ARTIFACT_STUB_GUARD_MIN_RATIO', '0.2');
  ...
  it('warns instead of rejecting when guard mode is warn', async () => {
    vi.stubEnv('GD_ARTIFACT_STUB_GUARD', 'warn');
  ...
  it('skips the guard entirely when mode is off', async () => {
    vi.stubEnv('GD_ARTIFACT_STUB_GUARD', 'off');
  ...
  it('finds artifact.html siblings when identifier slugifies to empty (lefarcen P2)', async () => {
    ...
    // Detection pattern; CJK identifiers '测试' / '首页' here exercise the empty-slug fallback path.
    const firstResp = await postFile(projectId, {
      name: 'artifact.html',
      content: htmlBody(20_000),
    ...
    const stubResp = await postFile(projectId, {
      ...
      // Detection pattern; CJK identifier '测试' kept verbatim — exercises the empty-slug fallback path.
      artifactManifest: { ...manifestFor('artifact'), metadata: { identifier: '测试' } },
    });
    expect(stubResp.status).toBe(422);
    const body = (await stubResp.json()) as {
  ...
  it('does NOT cross-reject distinct empty-slug identifiers (lefarcen/mrcfps round 4)', async () => {
    ...
    // Detection pattern; CJK identifiers '测试' / '首页' here verify the guard
    // distinguishes two different non-ASCII identifiers that both slugify to empty.
    const firstResp = await postFile(projectId, {
      name: 'artifact.html',
      content: htmlBody(20_000),
      artifactManifest: { ...manifestFor('artifact'), metadata: { identifier: '测试' } },
    ...
    const secondResp = await postFile(projectId, {
      ...
      // Detection pattern; CJK identifier '首页' kept verbatim — distinct empty-slug identifier.
