...
The Galyarder Design landing page automates the parts of search-engine
...
doc is its concrete implementation in `galyarderlabs/galyarder-design`.
...
| Manual `workflow_dispatch` | `blog-indexing-monitor.yml` | Maintainers can dry-run or explicitly publish a token-gated dev.to/Hashnode cross-post with canonical URL pointing back to Galyarder Design. |

The monitor and 3-day digest workflows commit their durable outputs
back via the `galyarder-design-bot` GitHub App. The monitor opens or
...
1. Go to <https://console.cloud.google.com/projectcreate> and create a
   project named `galyarder-design-blog-indexing` (or reuse an existing
...
3. Create an OAuth client under
   ...
   - Name: `galyarder-design-gsc-local`
...
5. Run the local helper:
   ...
   pnpm --filter @galyarder-design/landing-page exec tsx \
     scripts/blog-indexing/authorize-gsc-oauth.ts \
     --out /tmp/galyarder-design-gsc-refresh-token.txt
   ```

6. Open the printed Google URL and authorize with an account that is an
   Owner of the `galyarder-design.dev` Search Console property.
...
1. Open <https://github.com/galyarderlabs/galyarder-design/settings/secrets/actions>.
...
4. Confirm the existing `BOT_APP_ID` and `BOT_APP_PRIVATE_KEY` secrets
   ...
   and `issues:write` for `galyarderlabs/galyarder-design` (already configured).
...
gh workflow run blog-indexing-on-deploy.yml \
  -R galyarderlabs/galyarder-design \
  -f head_sha=<sha>
```
