...
// (the registry only resolves SKILL.md / .claude-plugin / galyarder-design.json
...
export async function runExampleGenerator(opts: ExampleGeneratorOptions): Promise<RunStats> {
  ...
  for (const id of slice) {
    ...
    const mode = fm.gd?.mode ?? 'prototype';
    const surface = fm.gd?.surface ?? inferSurface(mode);
    const scenario = fm.gd?.scenario ?? 'design';
    const platform = fm.gd?.platform;
    const exampleFile = await sideFiles(srcFolder);
    const featured = normaliseFeatured(fm.gd?.featured);
    ...
    const previewEntry = exampleFile.hasExample ? 'example.html' : (fm.gd?.preview?.entry ?? 'example.html');
    ...
    const manifest = buildManifest({
      ...
      homepage: `https://github.com/galyarderlabs/galyarder-design/tree/main/plugins/_official/${TIER_EXAMPLES}/${id}`,
      tags: dedupeTags([
        'example',
        'first-party',
        mode,
      ...
      gd: {
        ...
        preview: { type: fm.gd?.preview?.type ?? 'html', entry: `./${previewEntry}` },
        useCase: {
        ...
        ...(Array.isArray(fm.gd?.inputs) && fm.gd.inputs.length > 0
          ? { inputs: fm.gd.inputs.map(normaliseInput) }
          : {}),
        context: {
          skills: [{ path: './SKILL.md' }],
          ...(fm.gd?.design_system?.requires !== false
            ? { designSystem: { primary: true } }
            : {}),
          ...(Array.isArray(fm.gd?.craft?.requires) && fm.gd.craft.requires.length > 0
            ? { craft: fm.gd.craft.requires }
            : {}),
...
function derivePrompt(fm: SkillFrontmatter): string {
  const explicit = fm.gd?.example_prompt;
